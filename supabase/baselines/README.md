...
while every other digest stayed identical. Two of the twelve also carry fields the
obvious definition omits: `columns` records `attidentity`/`attgenerated`, because
identity semantics live in `pg_attribute` rather than in the type/not-null/default
fields, and `function_security` records `pg_get_function_arguments` alongside the
identity form, because the identity form drops argument defaults. Also require the history
...
result. Three of the required proofs are **negative** tests, because a positive match
cannot show that a guard would have noticed anything. On the restored database: swap
one captured `anon` `EXECUTE` grant for a different function and confirm
`BASELINE_ACL_ANON_EXECUTE_DRIFTED` still raises — revoke it from `PUBLIC` as well as
from `anon`, or the effective set is unchanged and the swap is not really a swap; flip
an identity column from `GENERATED ALWAYS` to `GENERATED BY DEFAULT` and confirm
`columns` moves; and change one RPC argument default and confirm `function_security`
moves. Each of those leaves the naive form of its check untouched — the count stays at
95, and the previous `columns`/`function_security` definitions returned byte-identical
digests across the last two — so a refresh that recorded only the positive match would
have proven nothing. Record each of those as a `true` flag in `disposable_restore_proof`.
