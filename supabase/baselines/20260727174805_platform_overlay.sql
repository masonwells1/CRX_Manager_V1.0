...
BEGIN
  IF EXISTS (
    SELECT 1 FROM "storage"."buckets" WHERE "id" = ANY (ARRAY['blend-ticket-images', 'customer-documents', 'delivery-photos', 'delivery-signatures', 'document-uploads', 'job-attachments', 'receiving-photos', 'supplier-price-evidence', 'team-note-attachments']::text[])
  ) THEN
