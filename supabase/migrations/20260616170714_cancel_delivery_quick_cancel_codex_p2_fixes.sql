-- Nightly-Debug remediation — cancel_delivery quick-cancel: Codex P2 follow-ups
-- ============================================================================
-- Follow-up to 20260616151122 (PARKED-03 quick-delivery prebook release). The
-- independent Codex review (gpt-5.5) of that commit flagged TWO real P2 gaps in
-- the inline quick-order cancel branch — both fixed here by mirroring cancel_order:
--
--   P2-1 (edited orders): update_order_items (live md5 a4fb75dba510710335571cf1fe507e99)
--     edits a 'confirmed' order — including a quick-delivery order — and on add/
--     increase it bumps inventory.quantity_prebooked + order_items but DOES NOT
--     touch delivery_items. v1 released the prebook by looping delivery_items, so
--     an edited quick order left the added/increased prebook stranded after cancel.
--     FIX: release by looping ORDER_ITEMS (GREATEST(total_units_needed -
--     quantity_delivered, 0)) — the booking source of truth, exactly like
--     cancel_order. For an unedited quick delivery this equals the old result.
--
--   P2-2 (paid commissions): v1 only cancelled 'pending' commissions. A quick
--     order can have a 'paid' commission (paid via a batch before the delivery is
--     cancelled). cancel_order counts those and notifies admins; v1 did not.
--     FIX: after zeroing pending commissions, count 'paid' ones and raise a
--     'cancellation_review' notification to every active admin (mirrors cancel_order).
--
-- Reproduced VERBATIM from the live v1 (md5 90ba258797cbac40e80673fba0767369)
-- except: the release loop source (delivery_items -> order_items), the new
-- paid-commission flag block, the new v_paid_commissions DECLARE, and the result
-- jsonb (+paid_commissions_flagged). Reviewed clean by rls-security + migration-
-- drift; rolled-back live smoke covers an edited order + an added item + a paid
-- commission + a sales_rep cancel.
-- ============================================================================
  ...
  IF v_delivery.order_id IS NOT NULL THEN
    ...
    IF v_order_status IS NOT NULL AND v_order_status NOT IN ('cancelled', 'voided') THEN
      IF v_quick_scheduled_exclusive THEN
        -- P2-1: release by ORDER_ITEMS (the booking source of truth) so edited/added
        -- quantities prebooked via update_order_items (which never touches delivery_items)
        -- are released too. Mirrors cancel_order. For an unedited quick delivery this
        -- equals the per-delivery_items result.
