-- Fix Codex BLOCKER 3: save_job actor-forgery.
--
-- save_job (SECURITY DEFINER, authenticated-executable) authorized off the caller-supplied
-- p_performed_by directly (`IF NOT EXISTS (SELECT 1 FROM profiles WHERE id = p_performed_by
-- AND role IN ('admin','sales_rep'))`), never binding to auth.uid(). So any logged-in user
-- could pass p_performed_by = an admin/sales_rep id (readable via profile_public_view) and
-- create/edit jobs. (The earlier H1 sweep missed this because it has no COALESCE(p_performed_by,
-- auth.uid()) — it checks the param directly; Codex caught it by reading.)
--
-- Fix: canonical strict-actor block — bind actor to auth.uid(), reject a mismatched
-- p_performed_by (IS DISTINCT FROM), role-check the REAL caller (v_actor), placed before the
-- idempotency check. p_performed_by (now == v_actor) is still used verbatim for jobs.created_by.
-- Signature unchanged (one overload). Body otherwise verbatim from live. Reversible.
