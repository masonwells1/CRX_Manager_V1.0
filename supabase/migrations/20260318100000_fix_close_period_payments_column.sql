-- ============================================================================
-- Fix close_accounting_period: payments.amount_cents does not exist
--
-- Bug: The 20260317200000 migration references sum(amount_cents) from the
-- payments table, but that table stores amounts as `amount numeric` (dollars).
-- Fix: Replace with (sum(amount) * 100)::bigint to convert to cents.
-- ============================================================================
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'payments_received_cents', COALESCE((
      -- payments.amount is stored as numeric dollars; convert to cents
    ...
    'deliveries_count', (
      ...
      WHERE delivery_date BETWEEN v_period_start AND p_period_end
        AND deleted_at IS NULL
    )
  ) INTO v_summary;
