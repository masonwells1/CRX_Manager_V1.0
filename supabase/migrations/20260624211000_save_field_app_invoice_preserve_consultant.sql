-- 20260624211000_save_field_app_invoice_preserve_consultant.sql
-- Money-lens review of #24 (field-app invoice editor), MED finding:
--   On a NON-admin edit of an existing field-app invoice, the UPDATE branch wrote
--   salesman_id = COALESCE(v_salesman_id, salesman_id). For a non-admin actor
--   v_salesman_id is bound to v_actor (always non-null), so a sales_rep editing ANY
--   field of an admin-assigned invoice silently reassigned the Consultant
--   (salesman_id) to themselves — quietly changing commission/attribution. The #24
--   editor correctly shows the Consultant picker read-only for non-admins, but the
--   server overwrote anyway.
-- FIX (one line, UPDATE branch only): gate the salesman_id overwrite on role —
--   an admin can still set/clear it; a non-admin edit leaves the existing
--   attribution UNTOUCHED.
--     salesman_id = CASE WHEN is_admin() THEN COALESCE(v_salesman_id, salesman_id)
--                        ELSE salesman_id END
-- The INSERT branch is unchanged on purpose: a brand-new invoice created by a
-- non-admin correctly gets their own id (the prior strict-actor bind still holds).
-- Everything else is a byte-faithful copy of the now-live 20260624010000 body
-- (single overload preserved; SECURITY DEFINER + search_path pinned; em-dashes are
-- real U+2014). The ONLY delta vs that body is the salesman_id assignment below.
