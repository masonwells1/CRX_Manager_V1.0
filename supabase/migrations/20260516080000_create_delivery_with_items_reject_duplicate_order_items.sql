...
-- ============================================================================
-- Codex P2 fix (PR #59, 2026-05-16 review of 7bf5f4c) — reject duplicate
-- order_item_id entries within the same create_delivery_with_items call.
-- ============================================================================
-- Audit finding: 20260516050000:125 — the per-item quantity_remaining check
-- uses the locked DB value (e.g. 10 remaining), but doesn't decrement or
-- accumulate the amount already scheduled in the SAME p_items array. A
-- stale/direct caller sending two {order_item_id:X, quantity:6} entries
-- against 10 remaining: both pass the per-item check individually, both
-- get inserted, then complete_delivery increments quantity_delivered by
-- the full 12 while only clamping quantity_remaining to zero. Silent
-- over-fulfillment, exactly the desync the prior 4 layers of validation
-- were supposed to prevent.
--
-- Fix: pre-scan p_items with a GROUP BY + HAVING count > 1 and raise
-- ITEM_DUPLICATE before any inserts. Frontend has no legitimate reason
-- to send the same order_item_id twice in one delivery — a single line
-- item gets one delivery_items row per delivery.
--
-- Body otherwise verbatim from 20260516050000.
-- ============================================================================
...
BEGIN
  ...
  -- Codex P2 fix (2026-05-16): reject duplicate order_item_id entries within
  -- the same p_items array. Without this dedupe, two {order_item_id:X, qty:6}
  -- rows against 10 remaining both pass the per-item check individually but
  -- sum to 12 — silent over-fulfillment on complete_delivery.
  SELECT (item->>'order_item_id')::uuid INTO v_duplicate_oid
  ...
  IF v_duplicate_oid IS NOT NULL THEN
    RAISE EXCEPTION 'ITEM_DUPLICATE: order_item_id % appears more than once in p_items', v_duplicate_oid;
  END IF;
...
DECLARE
  v_has_dedup boolean;
BEGIN
  SELECT prosrc ~ 'ITEM_DUPLICATE' AND prosrc ~ 'HAVING count'
    INTO v_has_dedup
  FROM pg_proc
  WHERE proname = 'create_delivery_with_items' AND pronamespace = 'public'::regnamespace;
  IF NOT COALESCE(v_has_dedup, false) THEN
    RAISE EXCEPTION 'codex-fix verification: missing ITEM_DUPLICATE dedup check';
  END IF;

  RAISE NOTICE 'codex-fix: create_delivery_with_items now rejects duplicate order_item_id entries.';
END
$$;
