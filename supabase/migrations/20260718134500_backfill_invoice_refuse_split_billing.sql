-- H5 (Live Foundation Gauntlet 2026-07-18) — create_invoice_for_unbilled_delivery
-- mono-billed split-billing orders.
--
-- The retroactive backfill builds a SINGLE invoice from the delivery's items. For
-- an order that requires split billing (delivered across multiple fields/payers,
-- orders.needs_split_billing = true), that single invoice mis-attributes all the
-- AR to one payer and defeats the intended per-split invoicing.
--
-- Safe fix (approved 2026-07-18): refuse to auto-backfill a delivery whose order
-- still needs split billing. The admin must create the split invoices through the
-- split-billing flow instead. needs_split_billing is set true when an order needs
-- splitting and cleared false once the split invoices exist, so this guard only
-- fires while a split is genuinely outstanding. Body reproduced verbatim from the
-- live catalog; only the guard block was added (right after the order row lock).
...
BEGIN
  ...
  -- H5 fix: refuse to mono-bill a split-billing order. This single-invoice backfill
  -- would attribute the whole delivery's AR to one payer; an order that still needs
  -- split billing must be invoiced through the split-billing flow (per-split invoices).
  IF (SELECT COALESCE(needs_split_billing, false) FROM orders WHERE id = v_delivery.order_id) THEN
