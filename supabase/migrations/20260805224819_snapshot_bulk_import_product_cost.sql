-- Prevent missing import cost from overstating order profit and commissions.
--
-- A caller may omit unit_cost, but zero is never inferred from absence. The RPC
-- snapshots products.current_cost for missing/blank cost, preserves an explicit
-- zero, rejects malformed or unavailable cost, and uses one normalized item
-- array for every downstream write so validation and commission math cannot
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      v_cost_per_unit := COALESCE(
        NULLIF(btrim(v_item->>'unit_cost'), '')::numeric,
        v_product.current_cost
      );
