-- Forward correction for 20260717110016. Keep PostgreSQL as the sole durable
-- identity authority, but pass its derived key to the atomic writer so the
-- required claim row is created before content-fingerprint finalization.
...
BEGIN
  ...
  IF v_identity_source NOT LIKE '%v_bulk_intent_key := v_expected_intent_key%'
     OR v_identity_source NOT LIKE '%v_payload - ''bulk_import_intent_key'' - ''bulk_import_document''%'
     OR v_identity_source LIKE '%BULK_PO_INTENT_IDENTITY_MISMATCH%' THEN
    RAISE EXCEPTION 'Unexpected pre-correction bulk-PO identity implementation';
  END IF;
END
$guard$;
...
BEGIN
  ...
  IF v_bulk_import_requested THEN
    ...
    -- Replace any legacy browser digest with the server-derived key consumed
    -- by the atomic writer's claim insert. The marker itself is not persisted.
    v_payload := jsonb_set(
      jsonb_set(
        jsonb_set(
...
COMMENT ON FUNCTION public._save_purchase_order_ascii_identity_impl(
  uuid, jsonb, jsonb, uuid, text
) IS 'Internal directly non-executable purchase-order writer. PostgreSQL derives the durable Unicode bulk identity, passes it to the atomic claim insert, and binds replay to reviewed content.';
...
BEGIN
  ...
  END IF;
  IF v_identity_source NOT LIKE '%v_bulk_intent_key := v_expected_intent_key%'
     OR v_identity_source NOT LIKE '%v_payload - ''bulk_import_document''%'
     OR v_identity_source NOT LIKE '%''{bulk_import_intent_key}''%to_jsonb(v_bulk_intent_key)%'
     OR v_identity_source LIKE '%v_payload - ''bulk_import_intent_key'' - ''bulk_import_document''%'
     OR v_identity_source LIKE '%BULK_PO_INTENT_IDENTITY_MISMATCH%'
     OR strpos(v_identity_source, '''{bulk_import_intent_key}''')
        > strpos(v_identity_source, '_save_purchase_order_atomic_number_impl')
     OR v_identity_source NOT LIKE '%BULK_PO_DOCUMENT_CONTENT_CONFLICT%'
     OR v_identity_source NOT LIKE '%public.next_po_number()%' THEN
    RAISE EXCEPTION 'Server-derived bulk-PO claim propagation is incomplete';
  END IF;
  IF has_function_privilege(
    ...
    RAISE EXCEPTION 'Internal purchase-order writer is directly executable';
  END IF;
END
$verify$;
