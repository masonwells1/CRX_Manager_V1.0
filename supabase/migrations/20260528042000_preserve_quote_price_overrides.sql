...
--
-- Fix:
-- 1. Add price_override column to quote_items (NULL = use tier pricing).
-- 2. Replace save_quote() so the INSERT stores the submitted override and the
--    server recalc CTE uses COALESCE(qi.price_override, tier_price) as ppu.
