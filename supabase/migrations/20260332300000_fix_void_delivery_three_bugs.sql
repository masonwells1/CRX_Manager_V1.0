-- ============================================================================
-- FIX: void_delivery — three bugs preventing successful execution
-- ============================================================================
--
-- BUG 1 (column ref): `quantity_remaining < quantity` — `quantity` does not
--   exist in order_items. Correct column is `total_units_needed`.
--
-- BUG 2 (status transition): The function updates orders.status from
--   'fulfilled' → 'confirmed' but doesn't set app.admin_override, so the
--   _enforce_order_status_transition trigger blocks the reverse transition.
--
-- BUG 3 (NOT NULL violation): financial_audit_log INSERT omits actor_user_id.
--   Column defaults to auth.uid() which is NULL inside SECURITY DEFINER.
--   Must explicitly pass v_actor.
--
-- BUG 4 (column refs): idempotency_keys columns are `idempotency_key` and
--   `result`, not `key` and `result_id` as the prior migration had.
-- ============================================================================
...
BEGIN
  ...
  -- Idempotency (BUG 4 FIX: column is `idempotency_key`, not `key`)
  ...
  -- BUG 2 FIX: Enable admin override for reverse status transitions
  ...
  -- BUG 1 FIX: `quantity` → `total_units_needed`
  SELECT NOT EXISTS (
    SELECT 1 FROM order_items
  ...
  -- BUG 3 FIX: Include actor_user_id explicitly
  ...
  -- BUG 4 FIX: column is `idempotency_key` and `result`, not `key` and `result_id`
  IF p_idempotency_key IS NOT NULL THEN
