-- 20260624010000_field_app_invoice_deleted_split_member_stays_deleted.sql
-- Reproduces the now-live save_field_app_invoice (the 20260623140000 B1+B2 version)
-- byte-faithful, then adds ONE behavior (owner decision, Mason 2026-06-24):
--   "Deleted stays deleted." When a split-group invoice member was soft-deleted and
--   then the group is edited, the field still lists that customer as a billing party so
--   derive_customer_shares re-derives them; the live-invoice lookup (deleted_at-aware,
--   from B1.2) finds nothing, and the old code fell through to INSERT a fresh live
--   invoice — re-creating a deleted invoice that could be posted again (Codex final
--   pass, 2026-06-24). Fix: if a customer has a soft-deleted (and no live) invoice in
--   the group, SKIP them entirely — no invoice, no invoice_items/shares, and skip their
--   field_app_location_shares too. The deletion is intentional; honor it.
-- The 3 deltas vs the live body: (1) v_skipped_customer_ids declare; (2) the skip block
-- in the customer loop; (3) the location-shares filter. Nothing else changes.
-- No signature change; single overload preserved. SECURITY DEFINER + search_path pinned.
-- Em-dashes are real U+2014 (byte-identical to live).
