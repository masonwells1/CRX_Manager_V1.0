-- Make the durable bulk-PO document identity case-insensitive for Unicode text
-- without weakening the server-side claim check. JavaScript and PostgreSQL use
-- the same NFKC -> lower -> NFKC sequence, and the apply-time parity vector
-- aborts if this database cannot produce the reviewed browser digest.
...
BEGIN
  IF to_regprocedure(
    ...
    RAISE EXCEPTION 'Expected internal purchase-order identity writer';
  END IF;
  IF to_regprocedure('public.save_purchase_order(uuid,jsonb,jsonb,uuid,text)') IS NULL THEN
    RAISE EXCEPTION 'Expected public purchase-order writer';
  END IF;
  ...
  IF v_claim_count <> 0 THEN
    RAISE EXCEPTION
      'BULK_PO_UNICODE_IDENTITY_TRANSITION_BLOCKED: expected zero claims, found %',
      v_claim_count;
  END IF;
  ...
  IF v_identity_source NOT LIKE '%translate(%ABCDEFGHIJKLMNOPQRSTUVWXYZ%abcdefghijklmnopqrstuvwxyz%'
     OR v_identity_source LIKE '%normalize(%NFKC%' THEN
...
COMMENT ON FUNCTION public._save_purchase_order_ascii_identity_impl(
  uuid, jsonb, jsonb, uuid, text
) IS 'Internal directly non-executable purchase-order writer. Historical function name retained for dependency stability; durable bulk identity uses NFKC/lower/NFKC Unicode canonicalization and content-bound replay.';

COMMENT ON FUNCTION public.save_purchase_order(
  uuid, jsonb, jsonb, uuid, text
) IS 'Saves a purchase order; bulk-import identity is ECMAScript-trimmed then NFKC/lower/NFKC canonicalized before deterministic hashing, replay, and storage.';
...
BEGIN
  ...
  v_vector_hash := encode(
    extensions.digest(
      convert_to(
        normalize(lower(normalize('VENDOR Élan', NFKC)), NFKC)
          || chr(31)
          || normalize(lower(normalize('INV-Ä100', NFKC)), NFKC),
        'UTF8'
      ),
      'sha256'
    ),
  ...
  IF v_identity_source NOT LIKE '%normalize(%lower(%normalize(%v_requested_vendor%NFKC%'
     ...
     OR v_identity_source LIKE '%translate(%ABCDEFGHIJKLMNOPQRSTUVWXYZ%abcdefghijklmnopqrstuvwxyz%'
     OR v_identity_source NOT LIKE '%BULK_PO_INTENT_IDENTITY_MISMATCH%'
    ...
    RAISE EXCEPTION 'Unicode identity or purchase-order invariants are incomplete';
  END IF;
  IF v_vector_hash <> '2fe4b753fbc589fbadd75339ef9af9d8f5d218449439f86254deef769546270c' THEN
    RAISE EXCEPTION 'Browser/PostgreSQL Unicode identity parity failed: %', v_vector_hash;
  END IF;
  ...
  IF has_function_privilege(
    ...
    RAISE EXCEPTION 'Purchase-order Unicode identity grants are incorrect';
  END IF;
END
$verify$;
