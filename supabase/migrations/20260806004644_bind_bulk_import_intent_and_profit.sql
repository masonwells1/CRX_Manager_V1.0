-- Bind bulk-import retries to intent and use canonical stored profit.
--
-- The prior live body could create commissions from its own per-line rounded
-- accumulator after order-item triggers had recalculated a different canonical
-- order profit. It also replayed any receipt with the same key and operation,
-- regardless of actor or payload. Use the trigger-canonical order totals for
-- commissions, reconcile stored line profit to that header, and bind every
-- receipt to a server-derived actor-and-payload fingerprint.
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.order_items (
      ...
      round(v_qty * v_price_per_unit, 2) - (v_qty * v_cost_per_unit),
      CASE
