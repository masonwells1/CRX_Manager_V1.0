-- Migration: restore RPCs — admin_override bracket + restore_quote_version auth
--   (2026-06-08 workflow review, 2 MED findings)
--
-- Problem: three "restore" RPCs write a status transition the status-enforcer
-- triggers forbid, WITHOUT the canonical app.admin_override bracket, so the write
-- is rejected:
--   * restore_quote_version  : UPDATE quotes  SET status='revised'   (enforcer only allows ->revised from 'sent')
--   * restore_cancelled_order: UPDATE orders   SET status='confirmed' (enforcer has no cancelled-> rule)
--   * restore_cancelled_delivery: UPDATE deliveries SET status='scheduled' (enforcer has no cancelled-> rule)
-- restore_quote_version is UI-reachable (QuoteBuilder restore button) and fails on
-- any non-sent/revised quote (e.g. an accepted quote) with a generic toast. The two
-- restore_cancelled_* are dead-in-UI (no src/**.tsx caller) but would fail on every
-- call — landmines if ever wired.
--
-- Fix: bracket ONLY the status write in each with the canonical transaction-local
-- override (exact pattern copied from void_order):
--     PERFORM set_config('app.admin_override','true',true);
--     UPDATE ... SET status=... ;
--     PERFORM set_config('app.admin_override','false',true);
--
-- restore_quote_version additionally:
--   (a) had NO in-function auth check (an ungated SECURITY DEFINER mutator reachable
--       directly via PostgREST). Closed with the canonical strict-actor block
--       (auth.uid() -> AUTH_REQUIRED / ACTOR_MISMATCH / INSUFFICIENT_ROLE on
--       admin|sales_rep), placed BEFORE the idempotency check. Its sole caller
--       (QuoteBuilder.tsx:1167-1172) passes p_performed_by = profile.id (= auth.uid())
--       and the page is admin/sales_rep-gated, so legit callers are unaffected.
--   (b) had a latent invalid-jsonb idempotency save: `result = p_quote_id::text` writes
--       a bare UUID string into the jsonb `idempotency_keys.result` column (invalid
--       JSON, so the keyed-save path errored). Fixed to a valid jsonb object. (The
--       check path doesn't read result, so replay behaviour is unchanged.)
--
-- All three bodies are otherwise reproduced VERBATIM from live. The restore_cancelled_*
-- pre-existing COALESCE(p_performed_by, auth.uid()) actor pattern is left as-is
-- (attribution-only, dead-in-UI) — out of scope for this status-transition fix.
-- Single overload each (verified). idempotency-body-check: exempt.
