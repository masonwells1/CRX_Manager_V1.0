-- 20260707090000_u7_split_gate_allow_predelivery.sql
-- U7 follow-up (Codex push-gate R3 P2): the fully-delivered gate added in
-- 20260707070000 over-reached — it turned the legitimate PRE-delivery whole-order
-- split-invoice flow (existing OrderDetail Create Invoice button -> create_split_
-- invoices_from_order for an allocated order) into a runtime error. Refine it to block
-- ONLY the partial-delivery-in-progress case (a delivery has COMPLETED but product
-- remains), so pre-delivery invoice-on-order works and partial-billing is still
-- prevented. Single-function re-emit; byte-identical to the live definition except the
-- one gate (reconstruction-md5 proven). Same signature -> CREATE OR REPLACE preserves grants.
