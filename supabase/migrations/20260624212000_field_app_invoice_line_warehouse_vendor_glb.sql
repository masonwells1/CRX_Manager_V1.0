-- 20260624212000_field_app_invoice_line_warehouse_vendor_glb.sql
-- Field-app parity #25 (Per-acre pricing from price book — Chemical/Charges tab).
-- Audit gaps closed here (DB half):
--   GAP-1  invoice_items has NO warehouse / vendor columns, so the ChemMan
--          per-line Warehouse + Vendor cannot round-trip. ADD them (text, nullable).
--   GAP-2  the priced chemical lines saved by save_field_app_invoice never wrote
--          the gallon/lb-equivalent of Total Applied (total_applied / *_unit /
--          *_gl_lb / gl_lb_unit) nor epa_registration / product_form, even though
--          those columns exist and the transfer path (transfer_job_to_invoice)
--          already populates them via convert_to_gl_lb(). Populate them on save so
--          the editor and the PDF show the same converted figure whether the
--          invoice was transferred OR hand-built.
--
-- This is ADDITIVE and money-neutral: warehouse/vendor/total_applied/gl_lb/epa/
-- product_form are informational columns. NONE of them touch unit_price_cents,
-- extended_cents, cost_cents, total_amount_cents or any pricing path — the cents
-- math in the two chemical INSERT branches is byte-for-byte unchanged.
--
-- The save_field_app_invoice body below is a faithful copy of the live
-- 20260624211000 body. The ONLY deltas vs that body, all inside the per-customer
-- chemical loop, are marked  -- #25:
--   * a v_total_applied / v_conversion compute before each chemical INSERT
--   * warehouse / vendor / total_applied / total_applied_unit / total_applied_gl_lb /
--     gl_lb_unit / epa_registration / product_form added to BOTH chemical INSERTs
--     (grower-share qty_a branch and priced qty_b branch)
-- The salesman_id role-gate (#24 money-lens), idempotency, strict-actor bind,
-- split-lock, "deleted stays deleted", audit-row, and SECURITY DEFINER +
-- search_path are all preserved verbatim. Single overload (CREATE OR REPLACE on
-- the exact 7-arg signature). Em-dashes below are real U+2014.

-- ── GAP-1: per-line Warehouse + Vendor (free text; products has no warehouse col,
--    so warehouse rides as free text and vendor defaults from products.vendor). ──
ALTER TABLE invoice_items ADD COLUMN IF NOT EXISTS warehouse text;
ALTER TABLE invoice_items ADD COLUMN IF NOT EXISTS vendor    text;

COMMENT ON COLUMN invoice_items.warehouse IS
  'Field-app parity #25: free-text warehouse/location the product is pulled from for this line (ChemMan per-line Warehouse). Informational; never affects pricing.';
COMMENT ON COLUMN invoice_items.vendor IS
  'Field-app parity #25: vendor/supplier for this line, defaulted from products.vendor and editable (ChemMan per-line Vendor). Informational; never affects pricing.';
...
BEGIN
  ...
  LOOP
    ...
    -- [2026-06-24] "Deleted stays deleted": if this customer's only invoice in the group was
    -- soft-deleted (no live one), do NOT recreate it on edit. The field still lists them as a
    -- billing party so they are re-derived, but the deletion is intentional — skip them entirely
    -- (no invoice / invoice_items / invoice_shares; their location shares are skipped below).
    ...
    ELSE
      UPDATE invoices SET
        ...
        -- [2026-06-24 money-lens MED] Only an admin may re-attribute the Consultant on an
        -- edit; a non-admin save must LEAVE the existing salesman_id untouched (the prior
        -- COALESCE(v_salesman_id, salesman_id) silently reassigned commission to the editor
        -- because v_salesman_id = v_actor for a non-admin). The picker is read-only for
        -- non-admins in #24, so the server now matches that intent.
    ...
    LOOP
      DECLARE
        ...
        -- #25: per-line informational fields (warehouse/vendor + gallon/lb-equivalent of
        -- Total Applied). v_wh/v_vendor come from the client (vendor defaults from the
        -- product on the client). v_form/v_epa fall back to the product so the conversion
        -- and label match the transfer path even if the client omits them. The conversion
        -- itself is delegated to convert_to_gl_lb() (the same helper transfer_job_to_invoice
        -- uses) so a transferred and a hand-built invoice show identical figures.
      ...
      BEGIN
        ...
        -- #25: resolve product_form / epa / vendor fallbacks from the product when present.
        ...
        IF v_chem_qty_a > 0 THEN
          ...
          -- #25: gallon/lb-equivalent of this branch's Total Applied (the rounded quantity).
          ...
          INSERT INTO invoice_items (
            ...
            is_application_fee, price_source,
            warehouse, vendor, total_applied, total_applied_unit,        -- #25
            total_applied_gl_lb, gl_lb_unit, epa_registration, product_form  -- #25
          ) VALUES (
            v_invoice_id,
            ...
            false,
            'manual',
            v_wh, v_vendor, ROUND(v_chem_qty_a, 4), v_ta_unit,            -- #25
            v_conv.converted_value, v_conv.converted_unit, v_epa, v_form  -- #25
          );
        ...
        IF v_chem_qty_b > 0 THEN
          ...
          -- #25: gallon/lb-equivalent of this customer's Total Applied (the rounded quantity).
          ...
          INSERT INTO invoice_items (
            ...
            v_wh, v_vendor, ROUND(v_chem_qty_b, 4), v_ta_unit,            -- #25
            v_conv.converted_value, v_conv.converted_unit, v_epa, v_form  -- #25
          );
