...
-- Fix (HIGH, 2026-06-08 workflow review): cancel_return crashes on a *received* return.
--
-- cancel_return explicitly supports cancelling a return in status 'received' and runs
-- the inventory-restock-reversal loop, but then does a plain UPDATE returns SET
-- status='cancelled' with no admin_override. The BEFORE-UPDATE trigger
-- _enforce_return_status_transition only permits received -> credited, so the
-- received -> cancelled UPDATE raises 'Invalid return status transition: received ->
-- cancelled' and the entire RPC (including the inventory reversal) rolls back.
-- The UI (src/pages/Returns.tsx:933) exposes a Cancel button on received returns and
-- promises to reverse the restock, so this is user-reachable; it is latent only
-- because there are currently 0 received returns in production.
--
-- Fix: wrap ONLY the final status UPDATE in a transaction-local app.admin_override
-- (the same _is_admin_override() hatch void_delivery / complete_delivery already use).
-- This keeps the trigger strict against direct PostgREST tampering (a raw
-- received -> cancelled UPDATE without the inventory reversal is still blocked) while
-- letting this vetted, role-gated RPC drive the legitimate transition after it has
-- done the restock reversal.
--
-- Body reproduced verbatim from the live catalog (project rhyzpcqhnizqbxphqdkr,
-- 2026-06-08) with ONLY the two set_config lines added around the final UPDATE.
...
BEGIN
  ...
  -- vetted RPC has already done the reversal, so allow the transition for just
  -- this UPDATE, then immediately clear the override.
