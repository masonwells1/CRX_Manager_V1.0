-- =============================================================================
-- 2026-05-16: Wire canonical idempotency into transfer_job_to_invoice
--
-- Closes the 2026-05-16 ultra-review's P1 #1 finding. The prior session shipped
-- 20260516000000_safe_cents_transfer_job_to_invoice.sql with the exempt marker
-- `-- idempotency-body-check: exempt` because the function declared
-- `p_idempotency_key text DEFAULT NULL` but the body never honored it.
--
-- The ultra reviewer correctly noted that this creates a customer-visible bug:
-- after a successful invoice creation, if the network response is dropped, a
-- retry hits "Job already invoiced" instead of replaying the success. The user
-- sees a failure but the invoice exists.
--
-- This migration wires canonical idempotency:
--   - DECLARE adds v_existing jsonb, v_result jsonb
--   - check_idempotency('transfer_job_to_invoice') at the top (after auth)
--   - save_idempotency at the end before RETURN
--   - Final RETURN changed to use v_result so we can both cache and return it
--
-- All other behavior (safe_cents_qty + ROUND wraps from migration 334, role
-- check, FOR UPDATE lock, advisory lock on invoice_number) is preserved
-- verbatim. The function's interface is unchanged — same args, same return
-- shape — so JobDetail.tsx caller continues to work without modification.
--
-- No exempt marker needed — the body now properly reads + writes
-- idempotency_keys via the canonical helpers.
-- =============================================================================
...
BEGIN
  ...
  -- Canonical idempotency check (closes 2026-05-16 ultra-review P1 #1)
  ...
  -- Canonical idempotency save (closes 2026-05-16 ultra-review P1 #1)
  IF p_idempotency_key IS NOT NULL THEN
...
-- Verification: function exists with single overload + body has canonical idempotency helpers
DO $verify$
DECLARE
...
BEGIN
  ...
  IF v_body NOT LIKE '%check_idempotency(p_idempotency_key, ''transfer_job_to_invoice'')%' THEN
    RAISE EXCEPTION 'transfer_job_to_invoice: check_idempotency call not wired';
  END IF;
  IF v_body NOT LIKE '%save_idempotency(p_idempotency_key, ''transfer_job_to_invoice''%' THEN
    RAISE EXCEPTION 'transfer_job_to_invoice: save_idempotency call not wired';
  END IF;
  IF v_body NOT LIKE '%safe_cents_qty(v_share.price_override_cents%' THEN
    RAISE EXCEPTION 'transfer_job_to_invoice: safe_cents_qty wrap missing (regression from migration 334)';
  END IF;
END
$verify$;
