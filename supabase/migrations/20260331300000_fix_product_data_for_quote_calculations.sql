-- Fix product data that causes Quote Builder calculation errors.
--
-- Problem: 588 of 593 products have inventory_unit = NULL, 584 have product_form = NULL,
-- and 490 use rate_unit = 'oz' which doesn't exist in unit_conversions (only 'fl oz' does).
--
-- Effect: Quote calculations divide by getConversionFactor(NULL) = 1 instead of 128 (Gal),
-- inflating prices by up to 128x ("millions of dollars" bug).
--
-- Fix: Populate inventory_unit from unit_size, set product_form, normalize rate_unit,
-- and add missing 'oz' entry to unit_conversions.

-- ============================================================
-- 1. Add 'oz' to unit_conversions (490 products reference it)
-- ============================================================
INSERT INTO unit_conversions (id, unit, factor_oz, notes, unit_type)
VALUES (gen_random_uuid(), 'oz', 1, '1 fluid ounce (alias for fl oz)', 'liquid')
ON CONFLICT (unit) DO NOTHING;

-- Also add 'Oz' variant since 11 products use unit_size = 'Oz'
INSERT INTO unit_conversions (id, unit, factor_oz, notes, unit_type)
VALUES (gen_random_uuid(), 'Oz', 1, '1 fluid ounce (capitalized alias)', 'liquid')
ON CONFLICT (unit) DO NOTHING;

-- Add 'LB' variant since 1 product uses unit_size = 'LB'
INSERT INTO unit_conversions (id, unit, factor_oz, notes, unit_type)
VALUES (gen_random_uuid(), 'LB', 16, '1 pound (capitalized alias)', 'dry')
ON CONFLICT (unit) DO NOTHING;

-- Add 'Unit' since 7 products use unit_size = 'Unit'
INSERT INTO unit_conversions (id, unit, factor_oz, notes, unit_type)
VALUES (gen_random_uuid(), 'Unit', 1, 'Each / unit (alias for Ea)', 'both')
ON CONFLICT (unit) DO NOTHING;

-- Add 'qt' (lowercase) since 1 product uses rate_unit = 'qt'
INSERT INTO unit_conversions (id, unit, factor_oz, notes, unit_type)
VALUES (gen_random_uuid(), 'qt', 32, '1 quart lowercase (alias for Qt)', 'liquid')
ON CONFLICT (unit) DO NOTHING;

-- ============================================================
-- 2. Fix data conflicts before bulk update
-- ============================================================
-- "Treaty Extra" has product_form='dry' but unit_size='oz' (liquid) — fix to 'Dry oz'
UPDATE products SET unit_size = 'Dry oz'
WHERE product_name = 'Treaty Extra' AND unit_size = 'oz';

-- Dry products with empty unit_size — default to 'Lb'
UPDATE products SET unit_size = 'Lb'
WHERE product_form = 'dry' AND (unit_size IS NULL OR unit_size = '') AND inventory_unit IS NULL;

-- ============================================================
-- 3. Populate inventory_unit from unit_size where NULL
-- ============================================================
-- inventory_unit tells the system what unit the tier prices are denominated in.
-- It MUST match an entry in unit_conversions so getConversionFactor() works.
UPDATE products
SET inventory_unit = unit_size
WHERE inventory_unit IS NULL
  AND unit_size IS NOT NULL
  AND unit_size <> '';

-- For the few products with empty/null unit_size, default to 'Ea'
UPDATE products
SET inventory_unit = 'Ea'
WHERE inventory_unit IS NULL OR inventory_unit = '';

-- ============================================================
-- 4. Populate product_form based on inventory_unit
-- ============================================================
-- product_form filters the rate_unit dropdown to show only compatible units.
UPDATE products
SET product_form = CASE
  WHEN inventory_unit IN ('Gal', 'Qt', 'Pt', 'fl oz', 'oz', 'Oz') THEN 'liquid'
  WHEN inventory_unit IN ('Lb', 'LB', 'Dry oz', 'g', 'MG') THEN 'dry'
  ELSE NULL  -- 'Ea', 'Unit', etc. — show all units
END
WHERE product_form IS NULL;

-- ============================================================
-- 5. Fix save_quote() to fall back to unit_size when inventory_unit is NULL
-- ============================================================
-- The existing save_quote joins on p.inventory_unit. We update it to use
-- COALESCE(p.inventory_unit, p.unit_size) so it never gets a NULL lookup.
-- Read the current function and replace the inv_conv join.
...
BEGIN
  ...
  -- Key fix: COALESCE(p.inventory_unit, p.unit_size, 'Ea') ensures
  -- we never get NULL for the inventory unit conversion lookup.
  -- ============================================================
  WITH base AS (
    SELECT
      qi.id AS item_id,
