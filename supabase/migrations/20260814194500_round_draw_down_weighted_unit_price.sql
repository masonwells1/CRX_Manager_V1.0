-- Round the draw-down weighted unit price to whole cents.
--
-- STATUS: NOT APPLIED
-- LOCAL CANDIDATE.
--
-- A partial draw-down averages every quote line of one product into a single
-- order line. The averaged COST was already settled to whole cents; the averaged
-- unit PRICE was not, so it reached order_items.price_per_unit carrying
-- fractional cents whenever a product was booked on the same quote at two
-- different prices. trg_order_items_round_money settles total_price and derives
-- profit, but it does not touch price_per_unit, so the below-cost guard
-- zz_crx_below_cost_order_items -- which fires last on the row -- raised
-- INVALID_UNIT_PRICE_CENTS and rolled back the entire draw-down. A legitimate
-- draw of a legitimately booked quote could not be completed at all.
--
-- This forward-only migration changes only the private implementation behind the
-- governed five-argument public wrapper. It adds one rounding statement beside
-- the existing cost rounding and changes nothing else: the public signature,
-- authentication and role checks, below-cost approval context, idempotency
-- ordering, inventory and order behavior, grants, and the generated TypeScript
-- contract are all unchanged.
--
-- Money note: rounding here, before anything derives from the value, is what
-- keeps the stored unit price, line total, line profit and net margin on one
-- basis. Rounding later -- at the insert, or in a trigger -- would leave the
-- order header on the unrounded figure and the reports on the rounded one.
--
-- ORDERING: the preflight below pins the live implementation body. Branch
-- claude/restrict-draw-down-owner carries an unapplied migration
-- (20260813161614_restrict_draw_down_quote_owner.sql) that replaces this same
-- function body and pins the same md5. Whichever applies second will fail closed
-- on its preflight; the two must be reconciled into a single body before either
-- is applied live. This is deliberate -- a silent last-writer-wins clobber would
...
BEGIN
  IF v_impl IS NULL OR v_wrapper IS NULL THEN
    RAISE EXCEPTION 'DRAW_DOWN_PRICE_ROUND_DRIFT: expected wrapper chain is incomplete';
  END IF;
  ...
  IF (SELECT count(*) FROM pg_proc
    ...
    RAISE EXCEPTION 'DRAW_DOWN_PRICE_ROUND_DRIFT: unexpected overload set';
  END IF;
  ...
  IF (SELECT md5(p.prosrc) FROM pg_proc p WHERE p.oid = v_impl)
    ...
    RAISE EXCEPTION 'DRAW_DOWN_PRICE_ROUND_DRIFT: private implementation changed';
  END IF;
  ...
  IF (SELECT md5(p.prosrc) FROM pg_proc p WHERE p.oid = v_wrapper)
    ...
    RAISE EXCEPTION 'DRAW_DOWN_PRICE_ROUND_DRIFT: public wrapper changed';
  END IF;
  ...
  IF has_function_privilege('anon', v_impl, 'EXECUTE')
    ...
    RAISE EXCEPTION 'DRAW_DOWN_PRICE_ROUND_DRIFT: wrapper ACL changed';
  END IF;
END;
$preflight$;
...
BEGIN
  ...
  FOR v_draw IN SELECT * FROM jsonb_array_elements(p_draws) LOOP
    ...
    -- PRICEROUND<<< settle the weighted unit PRICE to whole cents on the same
    -- basis, and for the same reason, as the cost directly above. Several quote
    -- lines of one product at different prices average to a fractional cent --
    -- e.g. one unit at $1.00 and two at $1.01 average to $1.00666... . That raw
    -- value was inserted straight into order_items.price_per_unit;
    -- trg_order_items_round_money settles total_price and profit but never
    -- price_per_unit, so the below-cost guard (zz_crx_below_cost_order_items,
    -- which fires last) rejected the row with INVALID_UNIT_PRICE_CENTS and rolled
    -- the whole draw-down back. Rounding at the source keeps the stored unit
    -- price, line total, profit and margin on one value.
...
BEGIN
  ...
  IF v_impl IS NULL
     ...
     OR md5(v_impl_source) <> 'd6e3f919decf557f6b278902b42b4799'
     OR position('v_wavg_price := ROUND(v_wavg_price, 2);' in v_impl_source) = 0
     OR position('v_wavg_price := ROUND(v_wavg_price, 2);' in v_impl_source)
        > position('INSERT INTO order_items' in v_impl_source)
     OR position('v_wavg_price := ROUND(v_wavg_price, 2);' in v_impl_source)
        > position('v_line_total := ROUND(v_wavg_price * v_qty, 2);' in v_impl_source)
     OR NOT (SELECT p.prosecdef FROM pg_proc p WHERE p.oid = v_impl)
     OR (SELECT p.proconfig FROM pg_proc p WHERE p.oid = v_impl)
       IS DISTINCT FROM ARRAY['search_path=public, pg_temp']::text[]
     OR (SELECT p.proowner FROM pg_proc p WHERE p.oid = v_impl)
       <> 'postgres'::regrole THEN
    RAISE EXCEPTION 'DRAW_DOWN_PRICE_ROUND_POSTCONDITION_FAILED: private implementation drifted';
  END IF;
  ...
  IF has_function_privilege('anon', v_impl, 'EXECUTE')
     ...
     OR NOT has_function_privilege('authenticated', v_wrapper, 'EXECUTE')
     OR NOT has_function_privilege('service_role', v_wrapper, 'EXECUTE') THEN
    RAISE EXCEPTION 'DRAW_DOWN_PRICE_ROUND_POSTCONDITION_FAILED: wrapper boundary changed';
  END IF;
END;
$postflight$;
