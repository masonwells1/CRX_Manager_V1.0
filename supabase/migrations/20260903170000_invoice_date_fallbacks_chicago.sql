-- ============================================================================
-- invoice_date server-side fallbacks: America/Chicago business date, never UTC
-- ----------------------------------------------------------------------------
-- STATUS: NOT APPLIED
-- (This status line goes stale at apply time; the ledger is authoritative.)
--
-- PLAIN ENGLISH. The live database clock runs in UTC; the business runs in
-- America/Chicago. Four functions stamp an invoice's invoice_date from CURRENT_DATE
-- when the caller does not supply one, and CURRENT_DATE on live is the UTC calendar
-- day -- so a save between 7 pm and midnight Chicago time would be dated TOMORROW.
-- invoice_date is what the customer reads, what the accounting period is checked
-- against, and (Mason's 2026-09-03 decision, docs/manual/DECISION_LOG.md) the date the
-- payment terms run from, so it must be the Chicago business date. This is the
-- settled ~2026-07-10 rule ("a bare now()::date / CURRENT_DATE on live is a bug").
--
-- WHAT THIS CHANGES: each of the four bodies below is re-emitted from its LIVE
-- installed text (read read-only on 2026-09-03; three of them were created by RENAME,
-- so no single migration file carries their applied text) with exactly one delta --
-- the invoice_date fallback CURRENT_DATE becomes (now() AT TIME ZONE 'America/Chicago')::date.
-- Nothing else in any body moves:
--   1. _price_order_below_cost_impl_20260810        invoice_date = CURRENT_DATE  (G3 price-month)
--   2. _save_invoice_lineage_unaware_impl_20260827  COALESCE(payload invoice_date, CURRENT_DATE)
--   3. _save_field_app_invoice_impl_20260714        COALESCE(payload invoice_date, CURRENT_DATE)
--   4. _save_field_app_split_invoice_impl           the same fallback, in both the season
--      derivation and the INSERT. Its third CURRENT_DATE (a commission-record date) is
--      deliberately NOT changed: it is outside the decision this file implements and is
--      recorded as a follow-up in the changelog entry.
--
-- WHAT THIS DOES NOT CHANGE: no data is rewritten (existing invoice_date and due_date
-- values stay exactly as stamped -- Mason chose the branch that needs no backfill), no
-- row is deleted, no grant moves (CREATE OR REPLACE keeps each function's ACL), the
-- posting RPC and its due_date = invoice_date + terms rule are untouched, and the
-- client is untouched (both invoice screens already send the browser-local date).
--
-- PREFLIGHT PINS. Refuses to run unless each installed body is byte-for-byte either the
-- reviewed starting body (the live md5 read 2026-09-03) or this file's own candidate
-- body (an identical replay). A drifted body aborts the whole transaction untouched.
...
-- replay identical, postflight passes, and the fallback expression is shown to yield the
-- Chicago date at a UTC-evening instant where CURRENT_DATE would have been tomorrow.
-- ============================================================================
...
BEGIN
  ...
  SELECT count(*) INTO v_count FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_price_order_below_cost_impl_20260810';
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'PREFLIGHT_OVERLOAD: expected exactly 1 overload of public._price_order_below_cost_impl_20260810, found %. This migration replaces a body; it does not create one.', v_count;
  END IF;
  SELECT p.pronargs, p.prosecdef, p.proconfig, md5(p.prosrc) AS body_md5 INTO v_row
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_price_order_below_cost_impl_20260810';
  IF v_row.pronargs <> 4 THEN
    RAISE EXCEPTION 'PREFLIGHT_SIGNATURE: public._price_order_below_cost_impl_20260810 has % arguments, expected 4.', v_row.pronargs;
  END IF;
  ...
  SELECT count(*) INTO v_count FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_invoice_lineage_unaware_impl_20260827';
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'PREFLIGHT_OVERLOAD: expected exactly 1 overload of public._save_invoice_lineage_unaware_impl_20260827, found %. This migration replaces a body; it does not create one.', v_count;
  END IF;
  SELECT p.pronargs, p.prosecdef, p.proconfig, md5(p.prosrc) AS body_md5 INTO v_row
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_invoice_lineage_unaware_impl_20260827';
  ...
  SELECT count(*) INTO v_count FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_field_app_invoice_impl_20260714';
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'PREFLIGHT_OVERLOAD: expected exactly 1 overload of public._save_field_app_invoice_impl_20260714, found %. This migration replaces a body; it does not create one.', v_count;
  END IF;
  SELECT p.pronargs, p.prosecdef, p.proconfig, md5(p.prosrc) AS body_md5 INTO v_row
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_field_app_invoice_impl_20260714';
  ...
  SELECT count(*) INTO v_count FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_field_app_split_invoice_impl';
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'PREFLIGHT_OVERLOAD: expected exactly 1 overload of public._save_field_app_split_invoice_impl, found %. This migration replaces a body; it does not create one.', v_count;
  END IF;
  SELECT p.pronargs, p.prosecdef, p.proconfig, md5(p.prosrc) AS body_md5 INTO v_row
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_field_app_split_invoice_impl';
  IF v_row.pronargs <> 9 THEN
    RAISE EXCEPTION 'PREFLIGHT_SIGNATURE: public._save_field_app_split_invoice_impl has % arguments, expected 9.', v_row.pronargs;
  END IF;
  IF NOT v_row.prosecdef THEN
...
-- ----------------------------------------------------------------------------
-- _price_order_below_cost_impl_20260810: live body re-emitted with the invoice_date fallback delta only
-- ----------------------------------------------------------------------------
  ...
  IF v_remaining = 0 THEN
    ...
    LOOP
      ...
      UPDATE invoices SET
        ...
        invoice_date       = (now() AT TIME ZONE 'America/Chicago')::date,   -- G3: PRICE-MONTH. America/Chicago business date, never the UTC calendar day (2026-09-03)
        pricing_pending    = false,
        updated_at         = now()
      WHERE id = v_inv.id;
      v_swept := v_swept + 1;
...
-- ----------------------------------------------------------------------------
-- _save_invoice_lineage_unaware_impl_20260827: live body re-emitted with the invoice_date fallback delta only
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- _save_field_app_invoice_impl_20260714: live body re-emitted with the invoice_date fallback delta only
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- _save_field_app_split_invoice_impl: live body re-emitted with the invoice_date fallback delta only
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  v_season := COALESCE(
    v_season,
    compute_season(COALESCE((p_invoice->>'invoice_date')::date, (now() AT TIME ZONE 'America/Chicago')::date)),  -- America/Chicago business date, never UTC (2026-09-03)
    current_season());
...
DECLARE
  ...
  v_count integer;
BEGIN
  SELECT p.prosecdef, p.proconfig, md5(p.prosrc) AS body_md5, position(E'\r' IN p.prosrc) > 0 AS has_cr,
         (SELECT count(*) FROM regexp_matches(p.prosrc, 'CURRENT_DATE', 'g')) AS cd,
         (SELECT count(*) FROM regexp_matches(p.prosrc, 'AT TIME ZONE ''America/Chicago''', 'g')) AS chi
    INTO v_row
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_price_order_below_cost_impl_20260810';
  IF v_row.body_md5 <> 'bad627af481b79da93e5afbb1a3bc181' THEN
    RAISE EXCEPTION 'POSTFLIGHT_BODY: public._price_order_below_cost_impl_20260810 installed body md5 % <> candidate bad627af481b79da93e5afbb1a3bc181.', v_row.body_md5;
  END IF;
  IF v_row.has_cr THEN
    RAISE EXCEPTION 'POSTFLIGHT_EOL: public._price_order_below_cost_impl_20260810 installed body carries CR bytes; this file is LF.';
  END IF;
  IF v_row.cd <> 0 THEN
    RAISE EXCEPTION 'POSTFLIGHT_CURRENT_DATE: public._price_order_below_cost_impl_20260810 still contains % CURRENT_DATE token(s), expected 0.', v_row.cd;
  END IF;
  IF v_row.chi < 1 THEN
    RAISE EXCEPTION 'POSTFLIGHT_CHICAGO: public._price_order_below_cost_impl_20260810 carries % America/Chicago conversion(s), expected at least 1.', v_row.chi;
  END IF;
  IF NOT v_row.prosecdef OR NOT ('search_path=public, pg_temp' = ANY (v_row.proconfig)) THEN
    RAISE EXCEPTION 'POSTFLIGHT_SECURITY: public._price_order_below_cost_impl_20260810 must be SECURITY DEFINER with search_path=public, pg_temp (secdef %, config %).', v_row.prosecdef, v_row.proconfig;
  END IF;
  SELECT count(*) INTO v_count FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_price_order_below_cost_impl_20260810';
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'POSTFLIGHT_OVERLOAD: public._price_order_below_cost_impl_20260810 has % overloads after replacement.', v_count;
  END IF;
  ...
  SELECT p.prosecdef, p.proconfig, md5(p.prosrc) AS body_md5, position(E'\r' IN p.prosrc) > 0 AS has_cr,
    ...
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_invoice_lineage_unaware_impl_20260827';
  IF v_row.body_md5 <> 'e1f1e0e641bd22f23505a7afc4384b2b' THEN
    RAISE EXCEPTION 'POSTFLIGHT_BODY: public._save_invoice_lineage_unaware_impl_20260827 installed body md5 % <> candidate e1f1e0e641bd22f23505a7afc4384b2b.', v_row.body_md5;
  END IF;
  IF v_row.has_cr THEN
  ...
  END IF;
  IF v_row.cd <> 0 THEN
    RAISE EXCEPTION 'POSTFLIGHT_CURRENT_DATE: public._save_invoice_lineage_unaware_impl_20260827 still contains % CURRENT_DATE token(s), expected 0.', v_row.cd;
  END IF;
  IF v_row.chi < 1 THEN
    RAISE EXCEPTION 'POSTFLIGHT_CHICAGO: public._save_invoice_lineage_unaware_impl_20260827 carries % America/Chicago conversion(s), expected at least 1.', v_row.chi;
  END IF;
  IF NOT v_row.prosecdef OR NOT ('search_path=public, pg_temp' = ANY (v_row.proconfig)) THEN
  ...
  SELECT p.prosecdef, p.proconfig, md5(p.prosrc) AS body_md5, position(E'\r' IN p.prosrc) > 0 AS has_cr,
    ...
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_field_app_invoice_impl_20260714';
  IF v_row.body_md5 <> 'bf900b8bd31439b9fa2963b161e107ca' THEN
    RAISE EXCEPTION 'POSTFLIGHT_BODY: public._save_field_app_invoice_impl_20260714 installed body md5 % <> candidate bf900b8bd31439b9fa2963b161e107ca.', v_row.body_md5;
  END IF;
  IF v_row.has_cr THEN
  ...
  END IF;
  IF v_row.cd <> 0 THEN
    RAISE EXCEPTION 'POSTFLIGHT_CURRENT_DATE: public._save_field_app_invoice_impl_20260714 still contains % CURRENT_DATE token(s), expected 0.', v_row.cd;
  END IF;
  IF v_row.chi < 1 THEN
    RAISE EXCEPTION 'POSTFLIGHT_CHICAGO: public._save_field_app_invoice_impl_20260714 carries % America/Chicago conversion(s), expected at least 1.', v_row.chi;
  END IF;
  IF NOT v_row.prosecdef OR NOT ('search_path=public, pg_temp' = ANY (v_row.proconfig)) THEN
  ...
  SELECT p.prosecdef, p.proconfig, md5(p.prosrc) AS body_md5, position(E'\r' IN p.prosrc) > 0 AS has_cr,
    ...
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_field_app_split_invoice_impl';
  IF v_row.body_md5 <> '9288b8fb410f33b7c7d46ecfb76306fa' THEN
    RAISE EXCEPTION 'POSTFLIGHT_BODY: public._save_field_app_split_invoice_impl installed body md5 % <> candidate 9288b8fb410f33b7c7d46ecfb76306fa.', v_row.body_md5;
  END IF;
  IF v_row.has_cr THEN
    RAISE EXCEPTION 'POSTFLIGHT_EOL: public._save_field_app_split_invoice_impl installed body carries CR bytes; this file is LF.';
  END IF;
  IF v_row.cd <> 1 THEN
    RAISE EXCEPTION 'POSTFLIGHT_CURRENT_DATE: public._save_field_app_split_invoice_impl still contains % CURRENT_DATE token(s), expected 1.', v_row.cd;
  END IF;
  IF v_row.chi < 2 THEN
    RAISE EXCEPTION 'POSTFLIGHT_CHICAGO: public._save_field_app_split_invoice_impl carries % America/Chicago conversion(s), expected at least 2.', v_row.chi;
  END IF;
  IF NOT v_row.prosecdef OR NOT ('search_path=public, pg_temp' = ANY (v_row.proconfig)) THEN
    RAISE EXCEPTION 'POSTFLIGHT_SECURITY: public._save_field_app_split_invoice_impl must be SECURITY DEFINER with search_path=public, pg_temp (secdef %, config %).', v_row.prosecdef, v_row.proconfig;
  END IF;
  SELECT count(*) INTO v_count FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_save_field_app_split_invoice_impl';
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'POSTFLIGHT_OVERLOAD: public._save_field_app_split_invoice_impl has % overloads after replacement.', v_count;
  END IF;

  RAISE NOTICE 'POSTFLIGHT_OK: four invoice_date fallbacks now use the America/Chicago business date.';
END
$postflight$;
