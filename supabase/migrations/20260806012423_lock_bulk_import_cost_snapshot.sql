-- Preserve whole-cent line profit and one locked Product cost snapshot.
--
-- The prior live body reconciled aggregate profit but could leave individual
-- order-item profit beyond two decimal places. It also let the immutable-cost
-- trigger reread Product cost without holding a Product row lock, so a concurrent
-- governed cost update could split cost_per_unit from cost_at_time_cents. Lock
-- every requested Product in stable UUID order, derive one bigint-cent snapshot,
-- explicitly write it, and keep every line profit in whole cents.
