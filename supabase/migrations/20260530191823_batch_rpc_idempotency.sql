...
--   (batch_apply_all_prepayments + batch_void_invoices use the canonical
--    check_idempotency()/save_idempotency() helper indirection, which the
--    PreToolUse body-check hook does not see through. Both reproduced verbatim
--    from live; the ONLY changes are the added idempotency check/save + decls.)
-- ============================================================================
-- P2-3 · batch_apply_all_prepayments + batch_void_invoices: real idempotency
-- ============================================================================
-- Problem (review 2026-05-28 §5 P1-B follow-up / P2 item 3):
--   Both declared p_idempotency_key but the body never touched idempotency_keys
--   (tracked as 'gap' in rpcContracts.test.ts IDEMPOTENCY_BODY_EXEMPT). A
--   double-click / retry on "Apply all prepayments" or "Batch void" would run
--   the whole batch twice.
--
-- Fix:
--   Canonical check-at-top / save-at-end idempotency via the check_idempotency /
--   save_idempotency helpers. The CHECK is placed AFTER the auth/rate-limit
--   gates (so a replay still passes authz and never returns cached data to an
--   unauthorized caller) but BEFORE any mutation (the per-customer apply loop /
--   per-invoice void loop / the financial_audit_log INSERT).
--
--   batch_void_invoices RETURNS integer, so its result is stored as
--   jsonb_build_object('voided_count', v_count) and the cached integer is read
--   back with (v_existing->>'voided_count')::integer.
--
--   Everything else reproduced VERBATIM from live. After this lands, both RPCs
--   are removed from rpcContracts.test.ts IDEMPOTENCY_BODY_EXEMPT (the body-scan
--   test now detects the helper usage and would fail if they were left as gaps).
--
--   BUNDLED BUGFIX (approved by Mason 2026-05-30): the live batch_apply_all_
--   prepayments inserted entity_id = NULL into financial_audit_log, where
--   entity_id is NOT NULL — so the "Apply all prepayments" button failed on
--   EVERY click (0 audit rows ever written). Fixed the summary row to
--   entity_type 'batch' (a valid financial_audit_log_entity_type_check value),
--   entity_id = v_actor (the operator). This is the ONLY change beyond
--   idempotency, and only in batch_apply_all_prepayments.
--
-- Single overload each (verified live). Both SECURITY DEFINER, search_path
-- public, pg_temp (unchanged).
--
-- LIVE APPLY NOTE: this migration was applied to live in two steps — the
-- initial apply (stamp 20260530191823) used entity_type 'system', which a
-- post-apply smoke test caught as a violation of the financial_audit_log
-- entity_type CHECK (initially mis-read as "no CHECK constraints"). The fix
-- (entity_type -> 'batch') was re-applied as stamp 20260530192441
-- (batch_rpc_idempotency_entity_type_fix), a live-only correction with no
-- separate disk file. THIS file is the consolidated FINAL state ('batch') and
-- a fresh rebuild from it reproduces current live correctly.
-- ============================================================================

-- ─── batch_apply_all_prepayments (RETURNS jsonb) ─────────────────────────────
...
-- ─── batch_void_invoices (RETURNS integer) ───────────────────────────────────
