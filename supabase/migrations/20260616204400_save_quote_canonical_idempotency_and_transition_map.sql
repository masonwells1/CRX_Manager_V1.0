-- Fix (MED, large-RPC pass #5): two save_quote defects, in one verbatim-except-the-deltas reproduction.
--
-- (A) Idempotency cached a throwaway id. The old block ran an EXISTS check and, on a hit, returned
--     {status:'duplicate', quote_id: p_quote_id} — for a NEW quote p_quote_id is NULL, so a retried
--     new-quote save returned quote_id=null (QuoteBuilder.tsx reads result.quote_id). Worse, it stored
--     jsonb_build_object('quote_id', COALESCE(p_quote_id, gen_random_uuid())) BEFORE the quote was created —
--     a random UUID unrelated to the real id (INSERT ... RETURNING gives a different uuid). Moved to the
--     canonical check_idempotency()/save_idempotency() pattern (same as create_invoice_from_order): check at
--     the head, and cache the REAL rich result (status/quote_id/server_totals) at the TAIL. A sequential
--     retry now replays the real quote_id + totals. (The same client-generated key is only ever reused for a
--     retry of one logical save, not two independent concurrent creates, so tail-save is correct here;
--     check_idempotency uses expires_at, default now()+24h — same window the old created_at filter used.)
--
-- (B) Transition map diverged from the enforcer. save_quote's internal v_allowed_transitions advertised
--     draft -> [sent,revised,declined,expired] and expired -> [revised], but the DB guard trigger
--     _enforce_quote_status_transition allows draft -> [sent,cancelled] and treats expired as TERMINAL. The
--     extra edges passed save_quote's own check then died at the trigger ('Invalid quote status transition').
--     Trimmed v_allowed_transitions to a strict subset of the enforcer (draft->[sent], expired removed); no
--     legitimate transition is newly blocked (every removed edge was already rejected by the trigger), the
--     error just surfaces earlier + clearer. The enforcer trigger remains the authoritative gate.
--
-- Body reproduced live-verbatim except the 3 marked DELTA-SQ blocks. Baseline live md5 9d10588243ca32dd1bc9e0f5627057c4.
-- Source: nightly-debug (LEDGER: quote:save_quote:idempotency-random-uuid-placeholder + quote:save_quote:transition-map-diverges-from-enforcer-trigger).
-- Rollback: re-apply save_quote with the 3 DELTA-SQ blocks reverted (early EXISTS+INSERT idempotency, the
--   4-key transition map incl. expired->revised, and the inline RETURN jsonb_build_object at the tail).
