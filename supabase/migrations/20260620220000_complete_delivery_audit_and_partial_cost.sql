-- Overnight Bug Hunt — complete_delivery cluster (MED + LOW), one migration.
--
-- MED audit-log:complete_delivery:missing-invoice-created — complete_delivery
-- auto-creates a billable draft invoice on completion but writes NO
-- financial_audit_log 'invoice_created' row (only an activity_feed row). Add the
-- same provenance breadcrumb create_invoice_from_order / the unbilled-delivery
-- creator write, so the append-only ledger records this creator too.
--
-- LOW stale-derived-state:complete_delivery:partial-rebill-cost — the partial
-- re-bill branch (IF v_any_partial ...) trues up REVENUE (invoice_items.quantity
-- / extended_cents; invoices.total_amount_cents) but never recomputes the stored
-- invoices.total_cost_cents, so a partial completion leaves the stored header
-- cost sized to the full ordered qty (a draft-stage profit/COGS DISPLAY can be
-- inflated — InvoiceDetail prefers the stored value). Recompute it from the
-- trued lines: SUM(cost_cents * quantity) (cost_cents is PER-UNIT cents — do NOT
-- multiply by 100). get_monthly_summary already recomputes COGS from line items
-- and excludes drafts, so this is a coherence/display fix, not a ledger error.
--
-- Body reproduced verbatim from the live definition (read 2026-06-20) with ONLY
-- the two additions above — verified by a rolled-back line-level diff +
-- plpgsql_check. Latent on live data (quick_deliveries=0, delivery_remainders=0,
-- invoices=4 draft/0 posted). NOT applied live (Mason batch-approves).
-- complete_delivery is NOT reworked on feat/as-applied-invoices — no collision.
