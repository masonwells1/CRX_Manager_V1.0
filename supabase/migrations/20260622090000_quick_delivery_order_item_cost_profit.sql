-- 20260622090000_quick_delivery_order_item_cost_profit.sql
--
-- OVERNIGHT BUG HUNT Run 2 cycle 7, finding #16 (MEDIUM verifier / MEDIUM Codex; reporting accuracy).
-- dedupeKey: reporting:create_quick_delivery:order_items-missing-cost-profit-margin
--
-- WHAT WAS WRONG
-- create_quick_delivery's order_items INSERT (the DELTA-1 block) populated only
-- total_units_needed, price_per_unit, total_price (+ delivered/remaining/sort). It left
-- product_name, cost_per_unit, profit, and net_margin NULL. create_direct_order — the sibling
-- order-creation RPC — fills all of these. get_sales_detail_report and other per-line readers read
-- order_items.profit / net_margin / cost_per_unit directly, so quick-delivery lines show NULL/0
-- profit and margin and a blank product name, under-reporting margin on every quick delivery.
-- (The ORDER header totals are correct — the function recomputes orders.total_profit from
-- v_total_cents - v_total_cost_cents; only the PER-LINE columns were missing.)
--
-- THE FIX
-- Expand ONLY the order_items INSERT to also write product_name, cost_per_unit, profit, net_margin,
-- mirroring create_direct_order's exact per-line formula but using this RPC's own values:
--   price_per_unit = (v_item_price_cents / 100.0)   [tier price, already computed]
--   cost_per_unit  = COALESCE(v_product.current_cost, 0)   [dollars, same source as the header cost]
--   profit         = (price_per_unit - cost_per_unit) * qty
--   net_margin     = CASE price>0 THEN ((price - cost)/price)*100 ELSE 0
-- Every other statement in the function is reproduced verbatim from the live definition. No
-- signature/behavior change otherwise.
--
-- Latent today: 0 quick deliveries / 0 orders live. ROLLBACK: restore the original 8-column INSERT.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- BEGIN DELTA-1 (quick_delivery_column_fixes + OVERNIGHT FIX #16): order_items has NO
    -- "quantity" and NO "quantity_prebooked" columns (42703, B-b; never existed — v2 baseline
    -- 20260206172436). The per-line quantity lives in total_units_needed; per-product prebook
    -- accounting lives on inventory.quantity_prebooked, updated below in this same loop.
    -- OVERNIGHT FIX #16: also populate product_name, cost_per_unit, profit, net_margin (were
    -- NULL — under-reporting per-line margin), mirroring create_direct_order's formula with this
    -- RPC's values: price = tier price (cents/100), cost = current_cost dollars.
