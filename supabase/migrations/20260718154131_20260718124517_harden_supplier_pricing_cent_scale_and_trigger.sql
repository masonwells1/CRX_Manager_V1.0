...
--   * reject sub-cent costs and tier prices at the final Products trigger
--     boundary before any pricing branch can retain or round them;
...
-- The additive rollout deliberately preserves ordinary Product-page edits.
-- Enforce the money scale in the final pricing trigger so that compatibility
-- never permits a legacy direct save to retain a fractional-cent value.
...
BEGIN
  ...
  IF NEW.pricing_version IS DISTINCT FROM OLD.pricing_version THEN
    -- Preserve the currently deployed compatibility window: the legacy form
    -- resubmits the version it originally loaded, while governed writers may
    -- never choose a version themselves.
