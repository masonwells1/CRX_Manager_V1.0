-- Reject PostgreSQL's non-finite numeric values at the bulk-order import boundary.
--
-- numeric accepts NaN and +/-Infinity, and ordinary range checks do not reliably
-- reject them. Normalize legacy dollar inputs to cents before any write so every
-- stored price, cost, extended total, profit, inventory quantity, and commission
-- basis is finite and deterministic.
...
BEGIN
  ...
  LOOP
    ...
    IF v_price_per_unit::text IN ('NaN', 'Infinity', '-Infinity')
       ...
       OR v_price_per_unit < 0
       OR v_cost_per_unit < 0 THEN
      RAISE EXCEPTION 'ITEM_INVALID: price and cost must be finite and >= 0';
    END IF;

    -- These legacy columns store dollars as numeric. Round at the authoritative
    -- boundary so sub-cent caller values cannot flow into financial records.
  ...
  LOOP
    ...
    v_price_per_unit := round(COALESCE((v_item->>'price_per_unit')::numeric, 0), 2);
    v_cost_per_unit := round(COALESCE((v_item->>'unit_cost')::numeric, 0), 2);
