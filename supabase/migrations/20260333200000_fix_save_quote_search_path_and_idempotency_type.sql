-- Fix save_quote():
-- 1. Remove 'auth' from search_path (should be public, pg_temp only per project rules)
-- 2. Fix idempotency INSERT to use ::text instead of to_jsonb()
-- Note: activity_feed columns (event_type, related_entity_type, related_entity_id) are already correct
...
BEGIN
  ...
  -- Idempotency check (FIXED: use ::text not to_jsonb)
