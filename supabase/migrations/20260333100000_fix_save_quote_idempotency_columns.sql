-- Fix save_quote() idempotency column references
-- Bug: Migration 20260331300000 re-introduced wrong column names
-- (key, entity_type, entity_id) instead of (idempotency_key, operation, result)
-- in the idempotency_keys table references.
...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    ...
    VALUES (p_idempotency_key, 'save_quote', to_jsonb(COALESCE(p_quote_id, gen_random_uuid())));
  END IF;
