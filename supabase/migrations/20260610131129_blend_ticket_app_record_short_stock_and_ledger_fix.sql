-- ============================================================================
-- M2 + two latent crashes (2026-06-10 foundation ultra review remediation):
-- create_application_record_from_blend_ticket
--
-- 1. M2 — the inventory deduction loop had NO short-stock handling: a bare
--    quantity_available decrement, unlike complete_job's deliberate
--    warn-and-flag design (requires_review + "[SHORT STOCK]" note + short
--    count). NOT a hard block — the chemical is already physically applied
--    when this runs; aligned to complete_job's policy instead.
-- 2. M2b (latent 42703, proven 2026-06-10) — the ledger INSERT referenced a
--    `reference_id` column that does NOT exist on inventory_transactions;
--    the RPC would crash on the first product of any real blend ticket
--    (never exercised: 0 blend tickets in prod). The application-record id
--    now travels in `notes` instead.
-- 3. M2c (latent 22P02, proven 2026-06-10) — the idempotency save wrote
--    array_to_string(uuid[], ',') into idempotency_keys.result, which is
--    jsonb — invalid JSON, would crash at the end of the first successful
--    call; the replay branch likewise misparsed. Now to_jsonb(uuid[]) on
--    save and jsonb_array_elements_text on replay.
--
-- Also: deduction transaction_type switched 'delivered' -> 'job_applied' —
-- this is a field application (same operation complete_job logs as
-- job_applied), not a customer delivery (no delivery_id exists). Safe to
-- change: the loop has never executed in prod (see M2b).
--
-- Also (compliance-reviewer MED, this review round): the auth guard's
-- freeform error strings upgraded to the canonical machine-readable tokens
-- (AUTH_REQUIRED / ACTOR_MISMATCH / INSUFFICIENT_ROLE: <suffix>). Caller
-- (BlendTicketDetail.tsx) does no string matching — verified by grep.
--
-- Body otherwise reproduced verbatim from the live definition (read via the
-- catalog 2026-06-10, incl. the 20260609142548 FOR UPDATE + strict-actor
-- guards). Single overload confirmed live.
...
-- Verification: single overload; no reference_id reference; no bare string idempotency save
DO $$
...
BEGIN
  ...
  WHERE pronamespace = 'public'::regnamespace AND proname = 'create_application_record_from_blend_ticket';
  IF v_src ILIKE '%reference_id%' THEN
    RAISE EXCEPTION 'reference_id still referenced (column does not exist)';
  END IF;
  IF v_src ILIKE '%array_to_string(v_record_ids%' THEN
    RAISE EXCEPTION 'string idempotency save still present (result column is jsonb)';
  END IF;
  IF v_src NOT ILIKE '%requires_review%' THEN
    RAISE EXCEPTION 'short-stock flagging missing';
  END IF;
END $$;
