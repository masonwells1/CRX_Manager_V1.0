-- =============================================================================
-- Migration: 20260501100000_field_app_workflow_phase15.sql
-- Phase 15 — Sprint D-policy: drivers complete + auto-invoice restoration
--
-- Closes the two business-decision items from Sprint D that needed Mason's
-- input. Both are folded into a single complete_delivery rewrite so the
-- system stays self-consistent.
--
-- A1 (drivers can complete their assigned deliveries):
--   complete_delivery's role check now mirrors confirm_delivery's pattern —
--   admin/sales OR (driver AND v_actor = v_delivery.assigned_driver). The UI
--   was already showing the completion section to drivers, so this just
--   makes the RPC accept what the UI was offering.
--
-- B1 (auto-invoice on delivery completion):
--   The pre-Phase-1-rewrite version of complete_delivery used to create a
--   draft invoice from the delivered quantities. The rewrite dropped that,
--   leaving the UI promise ("draft invoice auto-created") stranded. This
--   migration restores the server-side auto-create with these guards:
--     - Skip if v_delivery.order_id IS NULL (orphan delivery)
--     - Skip if any non-voided/non-cancelled invoice already exists for the
--       order (avoids double-billing when create_invoice_from_order was
--       already called separately, or when create_quick_delivery already
--       made one)
--     - Use quantity_delivered, not quantity_ordered, so partial deliveries
--       bill the right amount
--   The result jsonb gains an `auto_invoice` field with `{id, invoice_number}`
--   when a fresh invoice was created — the existing DeliveryDetail.tsx
--   frontend already reads this field (lines 812-825) so no UI changes are
...
BEGIN
  ...
  -- Skip when:
  --   - delivery has no order_id (orphan)
  --   - any non-voided/non-cancelled invoice already exists for the order
  --     (covers create_invoice_from_order, create_quick_delivery, manual saves)
...
DECLARE c1 int;
BEGIN
  SELECT count(*) INTO c1 FROM pg_proc WHERE pronamespace='public'::regnamespace AND proname='complete_delivery';
  IF c1 != 1 THEN
    RAISE EXCEPTION 'VERIFICATION FAILED: complete_delivery has % overloads', c1;
  END IF;
  RAISE NOTICE 'Phase 15 verified — Sprint D-policy A1+B1 shipped';
END $$;
