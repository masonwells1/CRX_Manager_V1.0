-- H3 recovery follow-up: void_payment preserves invoice_line_allocations as
-- history while marking their allocation_set inactive. The invoice void guard
-- must block only allocations that are still active, otherwise the approved
-- unapply-then-void recovery path remains impossible.
-- Body reproduced from the last applied explicit definition; one predicate changed.
