-- ============================================================================
-- Refuse absent/null/non-finite job-field acreage and normalize intentional blank to zero.
-- STATUS: NOT APPLIED — DO NOT APPLY. LOCAL CANDIDATE; forward-only replacement of public.save_job; no data rewrite.
--
-- SOURCE: the effective six-argument SECURITY DEFINER body from
-- 20260903150000_job_chemicals_persist_driver.sql (live-reviewed md5
-- 18d08d5f40aea91fe13ac3e5a686c549). Actor, idempotency, ACL, search_path,
-- chemical refusals, derived totals, and every unrelated statement are preserved.
--
-- CHANGE: before any acreage COALESCE or write, refuse a missing acres_to_treat key or
-- JSON null with JOB_ACRES_NOT_FINITE. This migration also NEWLY accepts a non-null empty
-- acreage string at the direct RPC boundary and stores numeric zero; the UI already emits zero.
--
-- PREFLIGHT: one overload and the exact F06 source hash, with only this candidate's exact
-- body hash accepted for replay. POSTFLIGHT: function owner/security/search_path/ACL,
-- exact body, marker, guard order, and blank normalization.
-- ROLLBACK: re-emit the exact F06 function body and ACL from 20260903150000. No rows are
-- transformed by this migration.
-- PROOF: scripts/smoke/prove-save-job-field-acres-real-schema.mjs plus
-- scripts/smoke/prove-save-job-field-acres.mjs (network-disabled throwaway PostgreSQL 17).
-- ============================================================================
...
BEGIN
  ...
  -- F06 already added this column and CHECK. Pin their complete reviewed shape before
  -- replacing the function: presence alone would accept a NOT NULL/defaulted/generated
  -- column or a widened CHECK and silently preserve drift.
  IF NOT EXISTS (
    SELECT 1 FROM pg_attribute a
     WHERE a.attrelid = 'public.job_chemicals'::regclass
       AND a.attname = 'driver'
    ...
    RAISE EXCEPTION
      'PREFLIGHT_COLUMN_DRIFT: job_chemicals.driver must be nullable, default-free, non-generated text before replacing save_job.';
  END IF;

  SELECT pg_get_constraintdef(c.oid) INTO v_driver_def
    FROM pg_constraint c
   WHERE c.conrelid = 'public.job_chemicals'::regclass
     AND c.conname = 'job_chemicals_driver_chk';
  IF v_driver_def IS DISTINCT FROM 'CHECK (((driver IS NULL) OR (driver = ANY (ARRAY[''rate''::text, ''qty''::text]))))' THEN
    RAISE EXCEPTION
      'PREFLIGHT_CHECK_DRIFT: job_chemicals_driver_chk is missing or differs from the exact NULL | rate | qty definition (got %).',
      COALESCE(v_driver_def, '<none>');
  END IF;

  SELECT p.prosrc INTO v_src FROM pg_proc p WHERE p.oid = v_oid;

  -- 18d08d5f40aea91fe13ac3e5a686c549 is the one current live overload/body,
  -- sourced explicitly from 20260903150000. The second pin is this migration's exact
  -- candidate body so replay is safe without accepting any later hotfix by marker alone.
  IF md5(v_src) <> '18d08d5f40aea91fe13ac3e5a686c549'
     AND md5(v_src) <> '8acf34542105a90212ddb0a5e7c5d272' THEN
    RAISE EXCEPTION
      'PREFLIGHT_BODY_DRIFT: live body md5 is %, expected the reviewed F06 source pin or this file''s exact replay pin. Diff and re-review before applying.',
      md5(v_src);
  END IF;
...
BEGIN
  ...
  -- BODY MARKER: chem_unit_invariant_v4
  ...
  -- stops trusting.
  -- ==========================================================================
  -- JOB FIELD ACREAGE SHAPE GUARD: missing and JSON-null are invalid and
  ...
  FOR v_field IN SELECT * FROM jsonb_array_elements(p_fields) LOOP
    ...
    VALUES (
      ...
      COALESCE(NULLIF(v_field->>'acres_to_treat','')::numeric, 0),
...
-- overload, which is also what live carries (read-only 2026-09-04: one row,
-- pronargs 6). That replay is now executable evidence, not a hypothetical:
-- prove-save-job-field-acres-real-schema.mjs restores the full baseline, replays
-- every ordered post-baseline migration through F06, requires the permanent
-- save_job parity chain to pass, exposes F06's missing-acreage defect, applies this
-- exact candidate, then reruns permanent parity and focused A1-A4. The focused
-- prove-save-job-field-acres.mjs separately bootstraps exact F06 for pin, replay,
-- apply-abort and runtime mutation checks. Claim those two proven paths only.
--
-- asserting alone was the round-3 review finding.
--
-- caller-analysis: the one in-repository CRX Manager direct browser caller is
-- JobDetail.tsx (performSave near line 2360; save_job call near line 2468, via
-- src/lib/db.ts). Its ordinary
-- runJobSave route mints a key per attempt; the license-override route invokes
-- performSave directly and may retain a key after an uncertain outcome, where
-- actor/payload-bound replay is intentional. External direct callers were not
-- enumerated. The authenticated role remains granted below. The body raises
-- AUTH_REQUIRED when auth.uid() IS NULL and admits only active admin/sales_rep
-- profiles, while PUBLIC and anon execution remain explicitly revoked.
-- =============================================================================
...
-- prove-save-job-field-acres.mjs installs six security/ACL mutants -- downgrade to
-- SECURITY INVOKER, delete the search_path pin, grant PUBLIC, grant anon, remove the
-- authenticated grant, or remove the service_role grant -- and requires the apply to
-- abort with the corresponding named POSTFLIGHT_* error. So the assertions are exercised
-- against broken candidate files even though they cannot fail against a correct one.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'POSTFLIGHT_DRIVER_COLUMN: job_chemicals.driver is not nullable, default-free, non-generated text after replacement.';
  END IF;
  ...
  IF v_driver_def IS DISTINCT FROM 'CHECK (((driver IS NULL) OR (driver = ANY (ARRAY[''rate''::text, ''qty''::text]))))' THEN
    RAISE EXCEPTION
      'POSTFLIGHT_DRIVER_CHECK: job_chemicals_driver_chk is missing or differs from the exact NULL | rate | qty definition (got %).',
      COALESCE(v_driver_def, '<none>');
  END IF;
...
-- ----------------------------------------------------------------------------
-- Candidate-specific postflight. The general security/ACL postflight above is inherited
-- verbatim from F06; this block pins the new body and the ordering this fix depends on.
DO $postflight_field_acres$
DECLARE
  v_src text;
  v_missing_guard integer;
  v_null_guard integer;
  v_sum integer;
  v_job_insert integer;
  v_job_update integer;
  v_fields_delete integer;
  v_fields_insert integer;
  v_shares_delete integer;
  v_shares_insert integer;
  v_chems_delete integer;
  v_chems_insert integer;
  v_idempotency_insert integer;
BEGIN
  SELECT p.prosrc INTO v_src
    FROM pg_proc p
   WHERE p.oid = to_regprocedure('public.save_job(uuid,jsonb,jsonb,jsonb,uuid,text)');

  IF v_src IS NULL OR md5(v_src) <> '8acf34542105a90212ddb0a5e7c5d272' THEN
    RAISE EXCEPTION 'POSTFLIGHT_FIELD_ACRES_BODY: installed body md5 is %, expected this file''s reviewed body pin.', COALESCE(md5(v_src), '<none>');
  END IF;
  IF position('chem_unit_invariant_v4' IN v_src) = 0
     OR position('chem_unit_invariant_v3' IN v_src) > 0 THEN
    RAISE EXCEPTION 'POSTFLIGHT_FIELD_ACRES_MARKER: expected only chem_unit_invariant_v4 in the installed body.';
  END IF;

  v_missing_guard := position('WHERE NOT (f ? ''acres_to_treat'')' IN v_src);
  v_null_guard := position('WHERE jsonb_typeof(f->''acres_to_treat'') = ''null''' IN v_src);
  v_sum := position('SELECT COALESCE(SUM(COALESCE(NULLIF(f->>''acres_to_treat'','''')::numeric, 0)), 0)' IN v_src);
  v_job_insert := position('INSERT INTO jobs' IN v_src);
  v_job_update := position('UPDATE jobs' IN v_src);
  v_fields_delete := position('DELETE FROM job_fields' IN v_src);
  v_fields_insert := position('INSERT INTO job_fields' IN v_src);
  v_shares_delete := position('DELETE FROM job_field_shares' IN v_src);
  v_shares_insert := position('INSERT INTO job_field_shares' IN v_src);
  v_chems_delete := position('DELETE FROM job_chemicals' IN v_src);
  v_chems_insert := position('INSERT INTO job_chemicals' IN v_src);
  v_idempotency_insert := position('INSERT INTO idempotency_keys' IN v_src);
  IF v_missing_guard = 0 OR v_null_guard = 0 OR v_sum = 0 OR v_job_insert = 0 OR v_job_update = 0
     OR v_fields_delete = 0 OR v_fields_insert = 0 OR v_shares_delete = 0 OR v_shares_insert = 0
     OR v_chems_delete = 0 OR v_chems_insert = 0 OR v_idempotency_insert = 0
     OR NOT (v_missing_guard < v_null_guard AND v_null_guard < v_sum
       AND v_sum < v_job_insert AND v_sum < v_job_update
       AND v_sum < v_fields_delete AND v_sum < v_fields_insert
       AND v_sum < v_shares_delete AND v_sum < v_shares_insert
       AND v_sum < v_chems_delete AND v_sum < v_chems_insert
       AND v_sum < v_idempotency_insert) THEN
    RAISE EXCEPTION 'POSTFLIGHT_FIELD_ACRES_ORDER: both executable acreage guards must precede the acreage SUM and every business/idempotency DML anchor (missing %, null %, sum %).', v_missing_guard, v_null_guard, v_sum;
  END IF;
  IF position('COALESCE(NULLIF(v_field->>''acres_to_treat'','''')::numeric, 0)' IN v_src) = 0 THEN
    RAISE EXCEPTION 'POSTFLIGHT_FIELD_ACRES_BLANK: job_fields insert does not normalize intentional blank acreage to zero.';
  END IF;
END
$postflight_field_acres$;

COMMENT ON FUNCTION public.save_job(uuid, jsonb, jsonb, jsonb, uuid, text) IS $sjc$Saves a job with its fields, customer shares, and chemical lines. Enforces the chemical-unit invariant server-side and DERIVES total_cost_cents / total_price_cents from the chemical lines via safe_cents_qty, ignoring caller-supplied totals. THIRTEEN refusals, all raised before any write. Units: CHEM_UNIT_MISMATCH (rate unit and price unit provably disagree), CHEM_UNIT_FORM_MISMATCH (a DRY product measured or priced in fluid ounces on either side, in ANY spelling -- both sides are reduced to the unit they name, so a denominator does not hide it), CHEM_UNIT_UNSPECIFIED (a line that BILLS while its rate unit or stock unit is blank), CHEM_UNIT_UNSUPPORTED_CHARACTER (a priced line whose unit contains a character this function cannot read -- the fold refuses rather than discarding, because deleting an unreadable character erases a denominator instead of catching it), CHEM_RATE_UNIT_UNRECOGNIZED (a priced line naming a unit that is neither a normalize_rate_unit canonical output nor a live unit_conversions spelling), CHEM_RATE_DENOMINATOR_NOT_ACRES (a rate measured per anything but acres, in slash, spelled-out, hyphenated, stacked or leading form) and CHEM_STOCK_UNIT_IS_A_RATE (a PRICED line whose STOCK unit carries a denominator of its own -- the Unit says what the quantity counts and what the price is quoted per, so "oz/ac" there names a rate rather than a quantity; every earlier denominator rule examined rate_unit only, and normalize_rate_unit silently stripped the per-acre suffix off the stock side). Quantities: CHEM_QUANTITY_NOT_FINITE (negative, NaN or Infinity), CHEM_QUANTITY_NOT_DERIVED (quantity disagrees with rate x acres -- enforced on the units-EQUAL path too, since matching units prove what is counted and nothing about how many), CHEM_QUANTITY_ZERO_BUT_EXPECTED (a PRICED line recording zero applied where a positive quantity was derivable) and CHEM_QUANTITY_UNVERIFIABLE (a PRICED line whose quantity cannot be checked at all, which closes the bypass of simply omitting the rate). Job fields: JOB_ACRES_NOT_FINITE (a field acreage key that is missing or JSON null, or a nonblank acreage that is negative, NaN or Infinity -- every field must carry a finite, non-negative acreage before any write; a non-null empty acreage string is newly accepted at the direct RPC boundary and normalized to zero at insert; OTHER writers and direct DML remain outside this RPC guard). This twelfth refusal was added in review round 24 (2026-08-24); the first draft of this comment counted eleven and was corrected by the follow-up comment-only migration. Calculator driver: CHEM_DRIVER_INVALID (a chemical line whose `driver` is neither "rate", "qty" nor blank -- F06, 2026-09-03: the payload now records which field the operator typed so a reloaded line can re-derive the other side when the acreage changes; blank means unknown and is left exactly as saved; the value is stored on job_chemicals.driver and read by no refusal and no money derivation). ZERO-QUANTITY LINES ARE NOT FLATLY EXEMPT -- that was true before round 17 and is not now. The exit survives only where zero is genuinely right: customer_supplied, no PRICE (nothing can be under-charged), or no usable rate/acreage (nothing was expected). Per the Mason 2026-08-24 rule the money refusals key on PRICE, so a cost-only line can still misstate margin -- an accepted, recorded residual. THE LIMIT OF ALL OF IT: every refusal checks that units are INTERNALLY CONSISTENT; none checks that a rate is PLAUSIBLE. A milligram per acre of a $931/lb product passes every one of these and invoices six orders of magnitude low. See the banner block in the header of this migration file. CLIENT MIRROR (current as of 2026-09-04): JobDetail.tsx also mirrors CHEM_QUANTITY_NOT_DERIVED per line with the same tolerance, so an acreage change on a line whose driver is unknown is shown on screen instead of surfacing as a whole-save rollback. Earlier text: since PR #436 merged on 2026-08-25, JobDetail.tsx carries a save-BLOCKING client-side mirror -- chemRowDefects fails closed on blank or non-finite quantities, on a billing line whose rate unit or stock unit is blank (mirroring CHEM_UNIT_UNSPECIFIED), and on unrecognized rate denominators -- so the operator normally sees an on-screen refusal before the server does. This function remains the authoritative boundary: the client mirror is a courtesy for old tabs and direct API callers do not pass through it, and it must never be more lenient than the SQL. IDEMPOTENCY: a keyed save now goes through check_idempotency_intent (the same helper the return and commission-payment RPCs use), so the key is bound to the calling actor AND to a sha256 fingerprint of the requested job, fields and chemical lines, under an advisory lock. Reusing a spent key for a different operation raises IDEMPOTENCY_CROSS_OP_KEY_REUSE, from a different actor IDEMPOTENCY_ACTOR_MISMATCH, and with a changed payload IDEMPOTENCY_INTENT_MISMATCH -- the last of which is a REFUSAL WHERE THE OLD BODY SILENTLY RETURNED THE EARLIER SUCCESS AND SAVED NOTHING. An unchanged retry still replays to the same job, which is the whole point of the key.$sjc$;
