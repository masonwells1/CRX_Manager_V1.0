...
-- Phase 9 — Sprint A1 of the four-audit hot-fix series.
--
-- Fixes the auth-gate gap on three field-application billing RPCs flagged in:
--   docs/audits/2026-04-30-money-inventory-audit-findings.md (P1)
--   docs/audits/2026-04-30-security-permissions-audit-findings.md (P1)
--
-- Pattern (mirrors save_quote and Phase 7 start_job/complete_job):
--   v_actor := auth.uid()
--   reject if NULL (not authenticated)
--   reject if supplied actor (p_performed_by / p_created_by) is mismatched
--   reject if caller role is not admin/sales
--
-- Affected RPCs:
--   1. save_field_app_invoice            (was: Phase 8 body, no auth gate)
--   2. create_invoice_from_blend_ticket  (was: Phase 1 body, no auth gate)
--   3. post_invoice_group                (was: Phase 1 body, no auth gate)
-- =============================================================================


-- =============================================================================
-- 1. save_field_app_invoice — Phase 8 body + Phase 9 auth gate
-- =============================================================================
...
BEGIN
  ...
  -- Phase 8 (#4): cancel + detach orphan group child invoices
  IF v_existing_group_id IS NOT NULL THEN
    FOR v_orphan IN
...
-- =============================================================================
-- 2. create_invoice_from_blend_ticket — Phase 1 body + Phase 9 auth gate
-- =============================================================================
...
-- =============================================================================
-- 3. post_invoice_group — Phase 1 body + Phase 9 auth gate
-- =============================================================================
...
-- =============================================================================
-- 4. Verification (overload count only — per project SQL safety rules)
-- =============================================================================
...
BEGIN
  ...
  SELECT count(*) INTO c2 FROM pg_proc WHERE pronamespace='public'::regnamespace AND proname='create_invoice_from_blend_ticket';
  SELECT count(*) INTO c3 FROM pg_proc WHERE pronamespace='public'::regnamespace AND proname='post_invoice_group';
  IF c1 != 1 THEN RAISE EXCEPTION 'VERIFICATION FAILED: save_field_app_invoice has % overloads', c1; END IF;
  IF c2 != 1 THEN RAISE EXCEPTION 'VERIFICATION FAILED: create_invoice_from_blend_ticket has % overloads', c2; END IF;
  IF c3 != 1 THEN RAISE EXCEPTION 'VERIFICATION FAILED: post_invoice_group has % overloads', c3; END IF;
  RAISE NOTICE 'Phase 9 verified: 3 field-app billing RPCs each have 1 overload';
END $$;
