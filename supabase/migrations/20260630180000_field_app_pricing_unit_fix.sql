-- Migration 20260630180000 — PARKED-010 field-application billing unit fix (codex-driven hunt).
-- Codex-reviewed twice (VERDICT SHIP); frontend half committed as e695875e.
-- Fix the field-application chemical billing unit mismatch: save_field_app_invoice was
-- billing (rate-unit applied amount) x (price per the product's sold/inventory unit) with
-- NO conversion between the two units. For 525+ of 573 priced products the rate unit (oz,
-- pt, qt, dry oz) differs from the sold unit (gal, lb, qt), so the line overcharged by the
-- unit ratio: 128x (oz->gal), 32x (oz->qt), 16x (dry oz->lb), 4x (qt->gal). Verified on
-- live product data (e.g. 100 ac of a 16 oz/ac product @ $32.10/gal billed $51,360 vs the
-- correct $401.25). Latent today (0 field-app invoices live) but the first real one overcharges.
--
-- Two parts:
--   1) field_app_priced_quantity(qty, rate_unit, inventory_unit, product_form) — converts the
--      applied amount from the rate unit into the product's sold (pricing) unit. Identity when
--      the units already match (so products that were already correct are unchanged); NULL when
--      the units genuinely don't convert (e.g. an oz rate on a product sold "per unit"), which
--      the caller turns into a clear FIELD_APP_UNIT_UNCONVERTIBLE error rather than mis-billing.
--   2) save_field_app_invoice — price the chemical line on the converted quantity, and store the
--      invoice line's quantity/unit in the sold unit so quantity x unit_price = extended_cents
--      stays consistent. (The applied amount in the rate unit is still recorded in total_applied,
--      and the gal/lb equivalent in total_applied_gl_lb.)
--
-- Money + SECURITY DEFINER change -> must clear the migration-review + Codex gate before apply.

CREATE OR REPLACE FUNCTION public.field_app_priced_quantity(
  p_applied_qty numeric, p_rate_unit text, p_inventory_unit text, p_product_form text
) RETURNS numeric LANGUAGE plpgsql IMMUTABLE SET search_path TO 'public', 'pg_temp' AS $fapq$
DECLARE
...
-- ── corrected save_field_app_invoice follows ─────────────────────────────────
...
-- ── corrected preview_field_app_invoice_split (Codex blocker #2: preview must match save) ──
