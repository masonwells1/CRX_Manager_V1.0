...
--
-- ============================================================================
-- Final-wave-review F8 (HIGH) — restore tote_number copy step in complete_delivery
--
-- Bug. Migration `20260301100001_complete_delivery_tote_copy.sql` added a
-- post-loop UPDATE that copies `delivery_items.tote_number` to matching
-- `invoice_items.tote_number` after a delivery completes. Phase 15
-- (`20260501100000_field_app_workflow_phase15.sql`, deployed 2026-05-01)
-- rewrote `complete_delivery` for driver-app workflows but DROPPED the
-- tote_number copy step. The Wave 4 corrective migration
-- (`20260507190000`) faithfully restored Phase 15 features but
-- inherited Phase 15's omission.
--
-- Net effect since 2026-05-01: any delivery completed via the standard
-- (non-quick) path leaves invoice_items.tote_number = NULL. Eight
-- frontend files reference invoice_items.tote_number — load sheet PDFs
-- (loadSheetPdf.ts), delivery PDFs (deliveryPdf.ts), and the Invoice /
-- Delivery / NewDelivery / Deliveries pages all read it. Load sheets for
-- those deliveries are missing tote numbers.
--
-- This migration restores the copy step. The body is otherwise identical
-- to 20260507190000 (which itself is the Phase 15 body + P4-10 backdated
-- warn block). Function signature unchanged.
-- ============================================================================
