-- ============================================================================
-- Field-app parity #32 — Fuel surcharge money-lens fixes (3 MED + 1 LOW)
-- ============================================================================
-- Recreates THREE functions VERBATIM from their post-#32 bodies and applies
-- only the targeted fixes below. The #24 salesman_id CASE-gate and the #25
-- warehouse/vendor line persistence inside save_field_app_invoice are copied
-- unchanged (do NOT regress them).
--
-- MED 1 — preview-vs-save percent divergence on the app-service fee.
--   preview gated the app-service fee on a WHOLE-RECORD `v_app_service IS NOT NULL`
--   test. Postgres evaluates ROW(...) IS NOT NULL as FALSE when ANY column is NULL,
--   and application_services.vehicle_id / created_by are nullable — so an app
--   service with a null vehicle/created_by made preview DROP the app-fee from the
--   subtotal, understating the PERCENT fuel-surcharge basis while save used the
--   correct subtotal-incl-fee. FIX: preview now uses a real presence test (FOUND
--   after the SELECT INTO) and the SAME `IF NOT FOUND OR NOT is_active` guard save
--   already has, then gates the fee on `p_application_service_id IS NOT NULL`
--   exactly like save → preview total === save total for the percent basis.
--
-- MED 2 — unguarded JSON cast aborts the whole invoice save/preview.
--   compute_fuel_surcharge_cents read+cast `setting_value::jsonb` OUTSIDE any
--   exception handler. A non-JSON value in the admin-editable, CHECK-less
--   app_settings.fuel_surcharge row threw and propagated, aborting EVERY field-app
--   invoice save/preview. FIX: wrap the read+cast in BEGIN ... EXCEPTION WHEN others
--   THEN RETURN 0; END; so a malformed config behaves as disabled (returns 0).
--
-- (MED 3 / LOW are frontend-only — see invoicePdf.ts / fuelSurcharge.ts.)
-- ============================================================================

-- ── MED 2 — compute_fuel_surcharge_cents: guard the config read+cast ────────
CREATE OR REPLACE FUNCTION public.compute_fuel_surcharge_cents(p_acres numeric, p_subtotal_cents bigint)
 RETURNS bigint
 LANGUAGE plpgsql
 STABLE
...
DECLARE
  ...
  v_rate      numeric;
BEGIN
  -- MED 2: the read + ::jsonb cast is wrapped so a MALFORMED, non-JSON value in
  -- the admin-editable (CHECK-less) app_settings.fuel_surcharge row behaves as if
  -- the feature were disabled (returns 0) instead of throwing and aborting EVERY
  -- field-app invoice save/preview. Contract: malformed config → return 0.
  BEGIN
...
-- ── MED 1 — preview_field_app_invoice_split: app-service presence + is_active ─
-- Recreated VERBATIM from the post-#32 body, with ONLY the two MED-1 changes:
--   (a) after SELECT * INTO v_app_service, add the SAME `IF NOT FOUND OR NOT
--       v_app_service.is_active THEN RAISE EXCEPTION` guard save_field_app_invoice
--       has (so save and preview reject a missing/inactive service identically);
--   (b) the app-fee block is now gated on `p_application_service_id IS NOT NULL`
--       (matching save) instead of the broken whole-record `v_app_service IS NOT NULL`.
...
BEGIN
  ...
  IF p_application_service_id IS NOT NULL THEN
    ...
    -- MED 1: same presence + is_active guard as save_field_app_invoice, so a
    -- missing/inactive service is rejected identically in preview and save.
  ...
  LOOP
    ...
    -- MED 1: gate the app-service fee on `p_application_service_id IS NOT NULL`
    -- (identical to save_field_app_invoice) — NOT the broken whole-record
    -- `v_app_service IS NOT NULL`, which was FALSE whenever a nullable column
    -- (vehicle_id / created_by) was NULL and silently dropped the fee from the
    -- subtotal, understating the PERCENT surcharge basis vs save.
    IF p_application_service_id IS NOT NULL THEN
...
-- ── save_field_app_invoice: recreated VERBATIM (no logic change) ────────────
-- Copied unchanged from the post-#32 body so this migration owns the latest
-- version alongside the preview fix. Preserves: #24 salesman_id CASE-gate
-- (`salesman_id = CASE WHEN is_admin() THEN COALESCE(v_salesman_id, salesman_id)
-- ELSE salesman_id END`), #25 warehouse/vendor line persistence, and the #32
-- fuel-surcharge is_application_fee block. The presence + is_active guard here
-- is the one preview now mirrors (MED 1).
