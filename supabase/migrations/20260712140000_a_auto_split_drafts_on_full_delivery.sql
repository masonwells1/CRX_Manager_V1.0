-- Feature A: fully delivered field/acre-allocated orders now auto-create
-- per-owner split draft invoices, with the existing flag-and-notify path retained
-- for partial deliveries and split-engine failures.
...
BEGIN
  ...
  IF v_overload_count <> 1 THEN
    RAISE EXCEPTION 'A_AUTO_SPLIT_POSTCHECK: expected exactly one public.complete_delivery overload, found %', v_overload_count;
  END IF;
END;
$$;
