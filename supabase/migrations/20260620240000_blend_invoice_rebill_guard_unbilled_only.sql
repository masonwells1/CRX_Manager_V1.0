-- Overnight Bug Hunt — LOW blend:create_invoice_from_blend_ticket:prepaid-rebill-gap.
--
-- The re-bill guard only blocks payment_status='billed', so a ticket whose
-- payment_status is 'prepaid' or 'no_charge' (or NULL) can still generate a SECOND
-- invoice. Widen the guard to bill ONLY from 'unbilled' (the canonical pre-bill
-- state; CHECK = unbilled|billed|prepaid|no_charge). This still permits the
-- legit re-bill-after-void path: trg_sync_blend_ticket_payment resets a voided
-- invoice's ticket back to 'unbilled', which this guard allows.
--
-- ONLY change vs the live definition: the two-line payment_status guard. Body
-- reproduced verbatim (incl. the existing DELTA-BLEND-INV-APPSVC-GUARD /
-- DELTA-BLEND-INV-COLFIX / DELTA-AUDIT-ROW blocks) — verified by a rolled-back
-- plpgsql_check + line-level diff (file's exact text). Latent on live (0
-- blend-ticket invoices). NOT applied live (Mason batch-approves).
-- INSERTs only (invoice_type='field_application'); no interaction with the
-- field-app type-lock trigger (20260620210000), which fires on UPDATE only.
...
BEGIN
  ...
  END IF;
  -- Overnight bug-hunt LOW (20260620): bill ONLY from 'unbilled'. The old guard
  -- blocked just 'billed', letting a 'prepaid'/'no_charge'/NULL ticket re-bill.
  -- (Re-bill-after-void still works: the sync trigger resets to 'unbilled'.)
