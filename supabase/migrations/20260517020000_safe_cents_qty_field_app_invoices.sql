...
-- (Bodies use inline idempotency_keys lookup — pre-existing pattern preserved verbatim.
-- Only the safe_cents_qty wraps changed.)
-- ============================================================================
-- Follow-up sprint (deferred from 2026-05-13 audit, applied 2026-05-17) —
-- Wrap remaining (*_cents * qty) -> bigint instances in
-- create_invoice_from_blend_ticket + save_field_app_invoice with the canonical
-- safe_cents_qty() helper. Adds NULL safety: if cost_per_acre_cents (etc.) is
-- NULL the previous ROUND(NULL * v)::bigint produced NULL which corrupted
-- v_invoice_cost downstream. safe_cents_qty COALESCEs NULL -> 0.
-- transfer_job_to_invoice was already migrated in 20260516000000.
-- Math is otherwise identical: ROUND(p_cents::numeric * p_qty)::bigint.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.create_invoice_from_blend_ticket(
...
REVOKE ALL ON FUNCTION public.create_invoice_from_blend_ticket(uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_invoice_from_blend_ticket(uuid, uuid, text) TO authenticated;

CREATE OR REPLACE FUNCTION public.save_field_app_invoice(
  p_invoice_id              uuid,
  p_invoice                 jsonb,
  p_locations               jsonb,
  p_chemicals               jsonb,
...
REVOKE ALL ON FUNCTION public.save_field_app_invoice(uuid, jsonb, jsonb, jsonb, uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_field_app_invoice(uuid, jsonb, jsonb, jsonb, uuid, uuid, text) TO authenticated;
...
DECLARE
  ...
  v_sfai_count  int;
BEGIN
  SELECT (length(prosrc) - length(replace(prosrc, 'safe_cents_qty(', '')))/length('safe_cents_qty(')
    INTO v_cibt_count
  FROM pg_proc
  ...
  SELECT (length(prosrc) - length(replace(prosrc, 'safe_cents_qty(', '')))/length('safe_cents_qty(')
    INTO v_sfai_count
  FROM pg_proc
  WHERE proname = 'save_field_app_invoice' AND pronamespace = 'public'::regnamespace;

  IF v_cibt_count < 5 THEN
    RAISE EXCEPTION 'verification: create_invoice_from_blend_ticket missing safe_cents_qty (count=%, expected >=5)', v_cibt_count;
  END IF;
  IF v_sfai_count < 5 THEN
    RAISE EXCEPTION 'verification: save_field_app_invoice missing safe_cents_qty (count=%, expected >=5)', v_sfai_count;
  END IF;

  RAISE NOTICE 'safe_cents_qty wraps verified: create_invoice_from_blend_ticket=%, save_field_app_invoice=%', v_cibt_count, v_sfai_count;
END
$do$;
