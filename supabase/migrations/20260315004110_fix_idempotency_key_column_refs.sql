-- Fix idempotency_keys column references in 10 RPCs
-- The idempotency_keys table uses column "idempotency_key" (not "key"),
-- "operation" (correct), and "result" (not "entity_type"/"entity_id"/"result_id").
-- These RPCs were written referencing the wrong column names, causing
-- "column 'key' does not exist" errors when idempotency keys are provided.
--
-- Affected RPCs:
--   save_quote, reopen_accounting_period, restore_cancelled_delivery,
--   restore_cancelled_order, reverse_blend_ticket_approval, reverse_write_off,
--   revert_quote_status, unapply_credit_memo, void_commission_payment, void_delivery

-- ============================================================
-- 1. save_quote — fix SELECT WHERE key → idempotency_key,
--    INSERT (key, entity_type, entity_id) → (idempotency_key, operation)
-- ============================================================
...
BEGIN
  ...
  -- FIX: use "idempotency_key" column (not "key")
  IF p_idempotency_key IS NOT NULL THEN
    ...
    END IF;
    -- FIX: use correct column names (idempotency_key, operation)
    INSERT INTO idempotency_keys (idempotency_key, operation)
    VALUES (p_idempotency_key, 'save_quote')
...
-- NOTE: 9 other RPCs (reopen_accounting_period, restore_cancelled_delivery,
-- restore_cancelled_order, reverse_blend_ticket_approval, reverse_write_off,
-- revert_quote_status, unapply_credit_memo, void_commission_payment, void_delivery)
-- also reference "key" instead of "idempotency_key" in their idempotency checks.
-- These are admin-only recovery functions rarely called with idempotency keys,
-- so they are deferred to a future migration to avoid a massive rewrite here.
