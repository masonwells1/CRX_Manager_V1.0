-- ============================================================================
-- SUPERSESSION (2026-06-10, applied live as MCP stamp 20260610191456): live
-- rollover_quote_to_season at stamp 20260610184534 was the EARLY UNGATED
-- draft — remainder mode fired for ANY status, breaking "renew last season's
-- completed program" (BOOKING_FULLY_DRAWN on every backfilled fully-drawn
-- conversion) and silently shrinking renewals of accepted partially-drawn
-- bookings. The race: a parallel /ship session applied the in-progress draft
-- from the shared working tree BEFORE the review gate finished; the gate
-- below was added by the reviewed revision minutes later. Both reviewers
-- verified the gate is the ONLY delta vs the applied 184534 body (normalized
-- md5 ffdfb175… both sides). This migration ships the reviewed gated body;
-- the 184534 disk file is retained as-committed for history (per the
-- never-modify-existing-migrations red line) — its self-verification gate
-- assertion describes THIS body, not the one its stamp applied.
-- Smoke (post-apply): scripts/smoke/smoke-rollover-remainder.sql →
-- SMOKE_PASS_ROLLBACK (open 500/200 → 300 FIFO remainder; accepted fully-
-- drawn → full verbatim copy, remainder_rollover=false; corrupt open ledger
-- → BOOKING_FULLY_DRAWN; no-draws control unchanged).
-- ----------------------------------------------------------------------------
-- Original design header (unchanged below):
-- ----------------------------------------------------------------------------
