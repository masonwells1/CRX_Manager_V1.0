-- Fix: update_order_items fails with FK violation when removing items
-- that have linked invoice_items, order_line_allocations, return_items,
-- or delivery_remainders.  The old version only checked quantity_delivered = 0
-- but ignored other child-table references.
--
-- This version:
--   1. Checks ALL FK child tables before attempting to delete
--   2. Raises a clear, user-friendly error naming the blocking reference
--   3. For draft-invoice items (nullable FK), NULLs the order_item_id
--      rather than blocking the delete (safe because draft invoices are editable)
