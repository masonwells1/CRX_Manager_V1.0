     ...
     OR NOT EXISTS (
       SELECT 1
        ...
        WHERE p.oid = 'public.record_commission_earned_state()'::regprocedure
          ...
          -- the label repair (then 20260905020100; most recently renumbered
          -- 20260908130000 on 2026-09-08 so it sorts AFTER this file), a COSMETIC repair that refuses to run once
          ...
          --   5623b0d3... the body the label repair (now 20260908130000) produces (post-repair)
          --
          -- Both are fully-hardened recorders by every attribute asserted above; they
          -- differ only in the label snapshot text the repair (now 20260908130000) rewrites, which this guard
