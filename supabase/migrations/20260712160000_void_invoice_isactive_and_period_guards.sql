-- Money+Inventory night hunt 2026-07-10, cycle 1 (findings #1 + #2), Codex-gated.
-- Re-emits public.void_invoice with TWO strictly-tightening guard fixes; body is
-- otherwise byte-identical to the live function (proven by md5 of the canonical
-- function definition == 6a36e488909875d2db87cd1b845983ad in a rolled-back smoke
-- before apply).
--
--   #1 authz:void_invoice:is-active-missing (MEDIUM) — the admin gate looked up
--      role WITHOUT `AND is_active = true`, so a deactivated admin (is_active=false,
--      role still 'admin', session still valid) could still void invoices. Every
--      sibling money mutator (post/unpost/delete/create/save/finance-charges) filters
--      is_active. Fix: add `AND is_active = true` to the role lookup.
--
--   #2 lifecycle:void_invoice:closed-period-bypass (LOW) — the closed-period guard
--      check_period_open(invoice_date) ran ONLY for status='posted'. The status
--      enforcer allows paid->voided and overdue->voided, so voiding a paid/overdue
--      invoice dated in a CLOSED period skipped the check (would zero totals / restore
--      prepay / write a negative audit row against a closed period). Sibling
--      unpost_invoice period-checks unconditionally. Fix: gate on
--      status IN ('posted','paid','overdue'). (Dormant today: 0 closed periods,
--      0 paid/overdue invoices — forward-looking correctness.)
--
-- Both changes only ADD a guard; a legitimate active admin voiding an invoice in an
-- open period is unaffected. No signature / column / type change.
