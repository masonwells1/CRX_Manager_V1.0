...
-- Adds needed_by_date to quote_sections for planned programs
-- Creates RPCs for inventory hold management and expiring holds alerts
...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    ...
    VALUES (p_idempotency_key, 'planned_hold', p_quote_id);
  END IF;
...
-- Update save_quote to also handle needed_by_date in section inserts
-- Faithful copy of latest save_quote from 20260316300000 with needed_by_date added
...
BEGIN
  ...
  ELSE
    ...
    INSERT INTO quotes (
      id, quote_number, customer_id, created_by, tier, status, is_planned,
  ...
  LOOP
    INSERT INTO quote_sections (
      id, quote_id, section_name, sort_order, section_notes, section_header_notes, needed_by_date
    ) VALUES (
      gen_random_uuid(),
      v_quote_id,
...
-- Update create_quote_version to include needed_by_date in snapshot
...
-- Update restore_quote_version to include needed_by_date
