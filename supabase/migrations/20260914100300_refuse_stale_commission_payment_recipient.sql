     ...
     OR NOT EXISTS (
       SELECT 1
        ...
        WHERE p.oid = 'public.record_commission_earned_state()'::regprocedure
          ...
          -- 20260914100900 on 2026-09-14 so it sorts AFTER this file), a COSMETIC repair that refuses to run once
          ...
          --   5623b0d3... the body the label repair (now 20260914100900) produces (post-repair)
          --
          -- Both are fully-hardened recorders by every attribute asserted above; they
          -- differ only in the label snapshot text the repair (now 20260914100900) rewrites, which this guard
