...
-- Atomic Chicago business-date cutover for documents and their commission history.
--
-- The four document writers and two commission helpers are deliberately deployed
-- together. Applying either set on its own leaves a transaction window in which a
-- writer can persist an old UTC date that the commission helper then faithfully
...
-- post-image, preserving the retry/replay safety proof without widening drift.
-- ---------------------------------------------------------------------------
DO $commission_helper_preflight$
DECLARE
  r record;
...
BEGIN
  ...
  LOOP
    v_seen := v_seen + 1;
    IF r.overloads <> 1 THEN RAISE EXCEPTION 'PREFLIGHT_OVERLOAD: public.% has % overloads.', r.proname, r.overloads; END IF;
...
BEGIN
  ...
  SELECT o.order_date INTO v_order_date FROM public.orders o WHERE o.id = p_order_id;
  ...
  WITH split_rows AS (
    SELECT s, ord, row_number() OVER (ORDER BY ord) AS rn, count(*) OVER () AS split_count,
           s->>'recipient' AS recipient, NULLIF(btrim(s->>'recipient_user_id'), '')::uuid AS split_user_id,
           (s->>'percentage')::numeric AS percentage
      FROM jsonb_array_elements(p_commission_split->'splits') WITH ORDINALITY AS e(s, ord)
     WHERE (NULLIF(btrim(s->>'recipient'), '') IS NOT NULL OR NULLIF(btrim(s->>'recipient_user_id'), '') IS NOT NULL)
       AND (s->>'percentage')::numeric > 0
  ), calculated AS (
    SELECT sr.*, COALESCE(
      (SELECT p.id FROM public.profiles p WHERE p.id = sr.split_user_id AND p.is_active = true),
      (SELECT p.id FROM public.profiles p WHERE lower(trim(p.full_name)) = lower(trim(sr.recipient)) AND p.is_active = true
         AND (SELECT count(*) FROM public.profiles p2 WHERE lower(trim(p2.full_name)) = lower(trim(sr.recipient)) AND p2.is_active = true) = 1 LIMIT 1)
    ) AS resolved_user_id,
    CASE WHEN sr.rn = sr.split_count THEN GREATEST(ROUND(COALESCE(p_order_profit, 0), 2), 0) - COALESCE(
      SUM(public.compute_commission_amount(p_order_profit, sr.percentage)) OVER (ORDER BY sr.rn ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING), 0)
    ELSE public.compute_commission_amount(p_order_profit, sr.percentage) END AS reconciled_amount
    FROM split_rows sr
  )
  SELECT p_order_id, p_customer_id, COALESCE((SELECT p.full_name FROM public.profiles p WHERE p.id = c.resolved_user_id), c.recipient),
         c.resolved_user_id, c.percentage, c.reconciled_amount, COALESCE(p_order_profit, 0), v_order_date, 'pending'
    FROM calculated c;
...
BEGIN
  ...
  END IF;
  PERFORM public.validate_commission_split_json(p_commission_split);
  SELECT i.invoice_date INTO v_commission_date FROM public.invoices i WHERE i.id = p_invoice_id;
  ...
  WITH split_rows AS (
    ...
    CASE WHEN sr.rn = sr.split_count THEN GREATEST(ROUND(COALESCE(p_profit, 0), 2), 0) - COALESCE(
      SUM(public.compute_commission_amount(p_profit, sr.percentage)) OVER (ORDER BY sr.rn ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING), 0)
    ELSE public.compute_commission_amount(p_profit, sr.percentage) END AS reconciled_amount
    FROM split_rows sr
  )
  SELECT p_job_id, p_invoice_id, p_customer_id,
         COALESCE((SELECT p.full_name FROM public.profiles p WHERE p.id = c.resolved_user_id), c.recipient),
         c.resolved_user_id, c.percentage, c.reconciled_amount, COALESCE(p_profit, 0), v_commission_date, 'pending'
    FROM calculated c;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  RETURN v_count;
END;
...
DO $commission_helper_postflight$
DECLARE
  r record;
  v_default text;
  v_seen int := 0;
BEGIN
  ...
  LOOP
    ...
    IF r.proname = '_insert_commissions_for_order' THEN
      IF position('SELECT o.order_date INTO v_order_date' in r.prosrc) = 0 OR position('FROM public.orders o' in r.prosrc) = 0
         OR (SELECT count(*) FROM regexp_matches(r.prosrc, 'p_order_date', 'g')) <> 1
         OR (SELECT count(*) FROM regexp_matches(r.prosrc, 'v_order_date', 'g')) < 4 THEN
        RAISE EXCEPTION 'POSTFLIGHT_RESIDUAL: public._insert_commissions_for_order must derive and insert its order date; the caller argument is only a fallback.';
      END IF;
    ELSE
      IF position('SELECT i.invoice_date INTO v_commission_date' in r.prosrc) = 0 OR position('FROM public.invoices i' in r.prosrc) = 0
         OR (SELECT count(*) FROM regexp_matches(r.prosrc, '(^|[^_[:alnum:]])p_commission_date', 'g')) <> 1
         OR (SELECT count(*) FROM regexp_matches(r.prosrc, 'v_commission_date', 'g')) < 4 THEN
    ...
    END IF;
  END LOOP;
  IF v_seen <> 2 THEN RAISE EXCEPTION 'POSTFLIGHT_MISSING: expected two commission helpers, found %.', v_seen; END IF;
  ...
  IF v_default IS NULL OR position('America/Chicago' in v_default) = 0 THEN
    RAISE EXCEPTION 'POSTFLIGHT_ORDER_DATE_DEFAULT: orders.order_date is not Chicago-based.';
  END IF;
END;
$commission_helper_postflight$;

-- The remainder of this file replaces the four source-document writers. Its
-- pre/postflight pins preserve their full body, default, ACL, search_path and
-- idempotency contracts; the final cross-cutover postflight verifies all six.

-- Document dates follow the America/Chicago business day, never the UTC clock.
--
-- WHY THIS EXISTS
-- ---------------
-- The original split candidate made a commission inherit its SOURCE DOCUMENT's date,
-- but adversarial review (migration-drift-reviewer, BLOCKER B1) caught that four source
-- writers still stamped the document from the UTC clock. This unified migration converts
-- both sides under one writer-drain lock boundary, so no committed transaction can land
-- between the document and commission portions of the cutover.
--
...
DO $chicago_date_cutover_combined_postflight$
DECLARE
  v_seen int;
BEGIN
