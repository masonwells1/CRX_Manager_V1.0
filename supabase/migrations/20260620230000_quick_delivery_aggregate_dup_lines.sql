-- Overnight Bug Hunt — LOW concurrency:create_quick_delivery:duplicate-line-oversell.
--
-- create_quick_delivery validates inventory net-available PER RAW LINE in its first
-- loop, but the second loop prebooks PER RAW LINE. So one call listing the same
-- product_id on two lines passes the net-available check twice against the same
-- pre-existing quantity_prebooked, then deducts both -> net-free can go negative
-- (over-prebook). Defense-in-depth: the only prod caller (QuickDeliveryModal) already
-- merges duplicate products into one summed line; this closes a hand-crafted-payload
-- gap reachable only by an admin/sales_rep/driver calling the raw RPC.
--
-- FIX (exactly the ledger prescription): aggregate by product_id BEFORE the
-- net-available check. The validation loop now iterates one summed (product_id,
-- quantity) row per distinct product, so the net-available comparison sees the TOTAL
-- demand for each product. The loop BODY is byte-identical — the loop yields a
-- jsonb object so the existing v_item->>'product_id' / v_item->>'quantity' reads are
-- unchanged. The second (prebook) loop is intentionally left per-line (it produces the
-- order/delivery/invoice line items as sent); aggregating the VALIDATION is what
-- prevents the oversell.
--
-- Body reproduced verbatim from the live definition (read 2026-06-20) with ONLY the
-- first loop's source rewritten — verified by a rolled-back plpgsql_check + line-diff
-- (no unique line removed; the prebook-loop header remains exactly once). Latent on
-- live data (quick_deliveries=0). NOT applied live (Mason batch-approves).
