-- H3 (Live Foundation Gauntlet 2026-07-18) — void_invoice could strand
-- customer cash.
--
-- On the posted/paid/overdue path, void_invoice reverses prepay applications
-- (restores prepay_credits + customer balance) and credit-memo applications
-- (restores the other side), then DELETES invoice_line_allocations and zeroes
-- paid_amount_cents. But it does NOT re-bank direct payment cash: a payment made
-- via record_invoice_payment (paid_amount_cents > 0, payment row against the
-- order) or a payment allocation (invoice_line_allocations) is simply dropped —
-- the customer's money is stranded and no reconciliation catches the shortfall.
--
-- Safe fix (approved 2026-07-18): rather than guess the re-banking, REFUSE to
-- void a posted/overdue/paid invoice that still has direct cash applied
-- (paid_amount_cents > 0 OR any invoice_line_allocations). The admin must first
-- void/unapply those payments (void_payment re-banks the cash as customer
-- prepay), then void the invoice — at which point only prepay/credit
-- applications remain, which void_invoice already reverses correctly. Prepay-
-- only and credit-memo-only voids are unaffected (they carry no
-- paid_amount_cents / allocations). The draft/unposted branch is unaffected
-- (it can carry no payments). Body reproduced verbatim from the live catalog;
-- only the guard block was added.
...
BEGIN
  ...
  IF v_inv.paid_amount_cents > 0
     OR EXISTS (SELECT 1 FROM invoice_line_allocations WHERE invoice_id = p_invoice_id) THEN
