...
-- ============================================================================
-- Closes 20 of the 22 entries in the UNSCOPED_LOOKUP_GAP list from
-- src/lib/rpcIdempotencyScope.test.ts (verified live 2026-06-11).
--
-- CARVE-OUT (race-safety, rls-security-reviewer B1): create_planned_holds and
-- save_quote were part of the original 22 but are EXCLUDED from this sweep —
-- a parallel in-flight migration (20260611132115_planned_holds_drawn_sync,
-- written to disk but NOT yet applied at authoring time) rebuilds both
-- functions; whichever change applied second would silently revert the other.
-- They remain on the test's gap list (cap ratcheted 22 -> 2) and get scoped in
-- a follow-up stacked on that migration. Block numbering below preserves the
-- original 1..22 positions (7 and 19 are intentionally absent), and a
-- PRE-FLIGHT BASELINE GUARD aborts this whole apply if ANY remaining target's
-- live body drifts from the authored baseline before apply.
--
-- THE BUG CLASS (restore_quote_version class, Codex 2026-06-08 LOW, at scale):
-- these functions' idempotency lookup filtered ONLY on the key
-- (`WHERE idempotency_key = p_idempotency_key`) with no operation filter,
-- so a key collision across operations returns/honors ANOTHER operation's
-- cached row: the second RPC silently skips its work and reports success
-- (17 fns return the foreign cached result; the 5 EXISTS-style fns —
-- create_planned_holds, create_quote_from_template, rollover_quote_to_season,
-- save_quote, save_quote_template — falsely report 'duplicate').
-- Every INSERT already recorded the correct own-name operation; only the
-- LOOKUP was unscoped.
--
-- THE FIX (uniform, minimal): each function body below is VERBATIM from the
-- live catalog definition (read 2026-06-11) with exactly ONE change — the
-- lookup gains `AND operation = '<function's own name>'`. No other behavior
-- change. Fidelity proof: for every function, stripping the added clause
-- restores a body whose md5 equals the live body md5 (verified in-database
-- before writing this file, and re-verified from this file against live).
--
-- Functions swept (20):
--   batch_approve_blend_tickets, batch_post_invoices,
--   batch_reject_blend_tickets, complete_job,
--   create_invoice_from_blend_ticket, create_job_from_quote_section,
--   create_quote_from_template, create_quote_version,
--   create_split_invoices_from_order, delete_prepay_credit, edit_delivery,
--   edit_prepay_credit, post_invoice_group, reverse_receiving_record,
--   rollover_quote_to_season, save_blend_ticket_fields,
--   save_field_app_invoice, save_quote_template, start_job, void_payment
--
-- A terminal DO block asserts: one overload per function and an
-- operation-scoped lookup present in every live body after apply.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- PRE-FLIGHT BASELINE GUARD: abort the WHOLE apply if any target function's
-- live body has drifted from the md5 baseline this sweep was authored against
-- (fetched live 2026-06-11). A parallel session was active in this repo at
-- authoring time; this guard turns a silent clobber into a loud failure.
-- If it fires: re-fetch the named function from live, rebuild its block, and
-- refresh its baseline here — do NOT bypass.
-- ----------------------------------------------------------------------------
DO $baseline$
DECLARE
  r        record;
...
BEGIN
  ...
  LOOP
    SELECT md5(prosrc) INTO v_actual
    FROM pg_proc
    WHERE pronamespace = 'public'::regnamespace AND proname = r.fn;
    IF v_actual IS DISTINCT FROM r.expected THEN
      RAISE EXCEPTION 'SWEEP BASELINE DRIFT: % live md5 % != authored baseline % — re-author this function''s block from current live before applying',
        r.fn, COALESCE(v_actual, '<missing>'), r.expected;
    END IF;
  END LOOP;
END;
$baseline$;
...
-- ----------------------------------------------------------------------------
-- 1/22 batch_approve_blend_tickets
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3/22 batch_reject_blend_tickets
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 5/22 create_invoice_from_blend_ticket
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 6/22 create_job_from_quote_section
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 7/22 create_planned_holds — CARVED OUT (see header): rebuilt by the
-- parallel in-flight 20260611132115_planned_holds_drawn_sync; scope its
-- lookup in a follow-up stacked on that migration.
-- ----------------------------------------------------------------------------

-- ----------------------------------------------------------------------------
-- 8/22 create_quote_from_template
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 10/22 create_split_invoices_from_order
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 15/22 reverse_receiving_record
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 16/22 rollover_quote_to_season
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 17/22 save_blend_ticket_fields
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 19/22 save_quote — CARVED OUT (see header): rebuilt by the parallel
-- in-flight 20260611132115_planned_holds_drawn_sync; scope its lookup in a
-- follow-up stacked on that migration.
-- ----------------------------------------------------------------------------

-- ----------------------------------------------------------------------------
-- 20/22 save_quote_template
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- SELF-VERIFICATION: every swept function has exactly one overload, its
-- lookup is now operation-scoped to its own name, and NO bare unscoped
-- key-only lookup remains (every `idempotency_key = p_idempotency_key`
-- occurrence is immediately followed by `AND operation =`).
-- ============================================================================
DO $verify$
DECLARE
  v_fn        text;
...
BEGIN
  FOREACH v_fn IN ARRAY ARRAY[
    ...
    'batch_reject_blend_tickets','complete_job',
    'create_invoice_from_blend_ticket','create_job_from_quote_section',
    'create_planned_holds','create_quote_from_template','create_quote_version',
    'create_split_invoices_from_order','delete_prepay_credit','edit_delivery',
    'edit_prepay_credit','post_invoice_group','reverse_receiving_record',
    'rollover_quote_to_season','save_blend_ticket_fields',
    'save_field_app_invoice','save_quote','save_quote_template','start_job',
    'void_payment'
  ]
  LOOP
    ...
    IF v_overloads <> 1 THEN
      RAISE EXCEPTION 'SWEEP VERIFY FAIL: % has % overloads (expected 1)', v_fn, v_overloads;
    END IF;
    ...
    FROM regexp_matches(v_src, 'idempotency_key\s*=\s*p_idempotency_key', 'g');
    SELECT count(*) INTO v_scoped
    FROM regexp_matches(v_src,
      'idempotency_key\s*=\s*p_idempotency_key\s+AND\s+operation\s*=\s*''' || v_fn || '''', 'g');

    IF v_total < 1 THEN
      RAISE EXCEPTION 'SWEEP VERIFY FAIL: % has no idempotency lookup at all', v_fn;
    END IF;
    IF v_scoped <> v_total THEN
      RAISE EXCEPTION 'SWEEP VERIFY FAIL: % still has % unscoped lookup(s) (% total, % scoped)',
        v_fn, v_total - v_scoped, v_total, v_scoped;
    END IF;
  END LOOP;
END;
$verify$;
