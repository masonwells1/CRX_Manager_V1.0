-- Fix (2026-07-21, found by smoke-per-line-split-billing.sql G5 fail-first probe):
-- _save_field_app_split_invoice_impl referenced v_app_service.name at plan-build
-- for EVERY line kind, but the record is only assigned when a service line (or
-- top-level p_application_service_id) is processed first. On a cold session,
-- any save whose lines before the first service line are chemical/fuel/flat
-- crashed with 55000 'record "v_app_service" is not assigned yet'. Once a
-- service-first save warmed the connection's plan cache the same payload
-- succeeded — an intermittent, pool-warmth-dependent failure reachable from the
-- editor (nothing forces a service line, or service-first ordering).
--
-- Fix: capture the service name into a plain text variable (v_svc_name) at each
-- point the record is loaded, and reference ONLY that variable at plan-build.
-- No behavior change for payloads that worked before. Full function re-emit of
-- the 20260720233000 definition with those three edits.
