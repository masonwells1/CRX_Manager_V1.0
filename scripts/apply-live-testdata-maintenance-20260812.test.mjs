...
assert.equal(
  exportedFunctionSource({ toString: () => "function example() {\r\n  return true;\r\n}" }),
  "export function example() {\n  return true;\n}",
  "embedded function source is LF-stable even from a CRLF checkout",
);
...
assert.equal(
  producerProtection.outputs.codexGuard.includes(exportedFunctionSource(maintenanceProducerCommandMentioned)),
  true,
  "generated guard embeds the invocation matcher exercised below",
);
assert.equal(
  producerProtection.outputs.codexGuard.includes(exportedFunctionSource(maintenanceProducerInvocationAllowed)),
  true,
