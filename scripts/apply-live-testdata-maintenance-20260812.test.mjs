...
assert.equal(
  producerProtection.outputs.codexGuard.includes(normalizeLineEndings(`export ${maintenanceProducerCommandMentioned.toString()}`)),
  true,
  "generated guard embeds the invocation matcher exercised below",
);
assert.equal(
  producerProtection.outputs.codexGuard.includes(normalizeLineEndings(`export ${maintenanceProducerInvocationAllowed.toString()}`)),
  true,
