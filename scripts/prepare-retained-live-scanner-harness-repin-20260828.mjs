...
const EXPECTED_RETAINED_PRODUCER_BLOB = "d8c2db9f2bc92f736bb6da6cdc5a74abe26ec62d";
const NEW_INPUT_BLOB = "3875e085266f6f0395ea16ad2fa2032b56ae3373";
const EXPECTED_GENERATED_OUTPUT_BLOB = "79a11218ea45edb249d18669bb35723dd21ae151";
const EXPECTED_REPINNED_PRODUCER_BLOB = "0b55c622ccd87173a915e4b3402a3c75c76031b0";
const APPROVAL = "--mason-authorized-one-account-gate=2026-08-28";
const REVIEW_TOKEN = "RETAINED_HARNESS_REPIN_VERDICT";

const OLD_INPUT_LINE = 'const EXPECTED_INPUT_BLOB = "e09a88ff0df5c235ccb05e0df0ac818b622639d0";';
const OLD_OUTPUT_LINE = 'const EXPECTED_OUTPUT_BLOB = "0e947bc2a86cda1bdb4b2ad860b3aef5e023e264";';
...
async function generatedOutputFrom(provisionalProducer) {
  const evaluationPath = path.join(REPO_DIR, "scripts", `.crx-retained-repin-eval-${process.pid}-${Date.now()}.mjs`);
  try {
...
function reviewRepin(built) {
  const reviewDir = mkdtempSync(path.join(os.tmpdir(), "crx-retained-harness-repin-review-"));
  ...
  try {
    ...
    const prompt = [
      ...
      "Verify the manifest hashes, confirm CURRENT_PRODUCER to PROPOSED_PRODUCER changes only the exact",
      "scanner input and generated-output blob constants, confirm CURRENT_SCANNER matches the new input pin,",
      "and confirm GENERATED_OUTPUT is the deterministic output of the proposed producer with no guard weakening.",
