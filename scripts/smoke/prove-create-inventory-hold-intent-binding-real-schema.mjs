#!/usr/bin/env node
/**
 * Full-chain proof for the local 20260908130000 create_inventory_hold candidate.
 *
...
const CANDIDATE = path.join(ROOT, 'supabase', 'migrations', '20260908130000_bind_create_inventory_hold_receipt_to_intent.sql');
...
function stageSql(file, name) {
  ...
  let primaryError;
  try {
    writeFileSync(staged, readFileSync(file, 'utf8').replaceAll('\r\n', '\n'), 'utf8');
  ...
  try {
    ...
    if (error.code !== 'ENOENT') {
      if (primaryError) console.error(`[prover] failed to remove ${staged}: ${error.message}`);
...
const claims = `DO $claims$ BEGIN
  PERFORM set_config('request.jwt.claims', '{"sub":"${ADMIN}"}', true);
  PERFORM set_config('request.jwt.claim.sub', '${ADMIN}', true);
END $claims$;`;
function holdCall(productId, key) {
  return `SELECT 'HOLD_ID:' || (public.create_inventory_hold('${productId}'::uuid, '${CUSTOMER}'::uuid, 5, 'manual', DATE '2026-12-31', 'race', '${ADMIN}'::uuid, false, NULL, '${key}') ->> 'hold_id');`;
}
...
async function raceSameKey(productId, key) {
  ...
  const uuidRe = /^HOLD_ID:([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\r?$/m;
  const id1 = r1.stdout.match(uuidRe)?.[1];
  const id2 = r2.code === 0 ? r2.stdout.match(uuidRe)?.[1] : null;
