#!/usr/bin/env node
// Container proof for supabase/migrations/20260914100100_next_invoice_number_year_chicago.sql
//
...
const MIGRATION = path.join(REPO, "supabase", "migrations", "20260914100100_next_invoice_number_year_chicago.sql");
...
function main() {
  ...
  console.log(`\nContainer proof: 20260914100100_next_invoice_number_year_chicago\n`);
  ...
  let wrappableReason = "";
  try {
    assertWrappable(migrationSql, "20260914100100_next_invoice_number_year_chicago");
  } catch (error) {
    wrappable = false;
