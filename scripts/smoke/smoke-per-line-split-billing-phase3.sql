-- Rollback-only hard proof for Phase 3 per-line split billing.
-- Runs only in the network-isolated disposable container created by
-- prove-per-line-split-billing-phase3.mjs. The terminal PASS exception rolls
...
DECLARE
  ...
  v_sum numeric;
BEGIN
  IF current_database() <> 'crx_per_line_p3_disposable' THEN
  ...
  IF v_plan IS DISTINCT FROM v_plan_again THEN
    RAISE EXCEPTION 'P3_FAIL(feature_off): legacy preview was not returned unchanged: %', v_plan;
  END IF;
  ...
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_TEMP_OBJECT_COLLISION%' THEN
    RAISE EXCEPTION 'P3_FAIL(caller_owned_temp_relation_rejected): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_BILLING_SCOPE_DENIED%' THEN
    RAISE EXCEPTION 'P3_FAIL(cross_rep_job_scope): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_BILLING_SCOPE_DENIED%' THEN
    RAISE EXCEPTION 'P3_FAIL(cross_rep_field_scope): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_BILLING_SCOPE_DENIED%' THEN
    RAISE EXCEPTION 'P3_FAIL(cross_rep_invoice_scope): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_plan->>'grand_total_cents' <> '100' THEN
    RAISE EXCEPTION 'P3_FAIL(owner_invoice_scope): %', v_plan;
  END IF;
  ...
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_JOB_CONTEXT_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(null_invoice_job_rebind): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_INVOICE_FIELD_CONTEXT_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(direct_invoice_field_context): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_plan->>'grand_total_cents' <> '100'
    ...
    RAISE EXCEPTION 'P3_FAIL(50_50/manual_precedence): %', v_plan;
  END IF;
  IF length(v_lines#>>'{0,calculation_hash}') <> 64
     OR length(v_lines#>>'{0,vector_hash}') <> 64 THEN
    RAISE EXCEPTION 'P3_FAIL(hashes): %', v_plan;
  END IF;
  ...
  IF v_plan->>'calculation_hash' IS DISTINCT FROM v_plan_again->>'calculation_hash' THEN
    RAISE EXCEPTION 'P3_FAIL(hash_stability): % vs %', v_plan, v_plan_again;
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,calculation_hash}' =
    ...
    RAISE EXCEPTION 'P3_FAIL(hash_product_identity): % vs %', v_plan, v_plan_again;
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,calculation_hash}' =
     v_plan_again#>>'{billing_lines,0,calculation_hash}' THEN
    RAISE EXCEPTION 'P3_FAIL(hash_price_provenance): % vs %', v_plan, v_plan_again;
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,calculation_hash}' =
     v_plan_again#>>'{billing_lines,0,calculation_hash}' THEN
    RAISE EXCEPTION 'P3_FAIL(hash_override_reason): % vs %', v_plan, v_plan_again;
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,source_quantity}' <> '1.0000'
    ...
    RAISE EXCEPTION 'P3_FAIL(single_final_money_round): %', v_plan;
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,source_cost_cents}' <> '1'
    ...
    RAISE EXCEPTION 'P3_FAIL(penny_exact_cogs): %', v_plan;
  END IF;
  ...
  IF jsonb_array_length(v_plan->'billing_lines') <> 2
    ...
    RAISE EXCEPTION 'P3_FAIL(exact_job_chemical_snapshots): %', v_plan;
  END IF;
  ...
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_QUOTE_PRICE_AMBIGUOUS%' THEN
    RAISE EXCEPTION 'P3_FAIL(ambiguous_quote_rejected): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_QUOTE_SOURCE_AMBIGUOUS%' THEN
    RAISE EXCEPTION 'P3_FAIL(ambiguous_quote_source_rejected): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,shares,0,base_unit_price_cents}' <> '150'
    ...
    RAISE EXCEPTION 'P3_FAIL(unambiguous_quote_precedence): %', v_plan;
  END IF;
  ...
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_QUOTE_UNIT_UNCONVERTIBLE%' THEN
    RAISE EXCEPTION 'P3_FAIL(unconvertible_quote_unit_rejected): %', COALESCE(v_err, '<no error>');
  END IF;
  UPDATE public.quote_items
     SET price_unit = 'qt'
   WHERE id IN (
  ...
  IF v_plan#>>'{billing_lines,0,source_quantity}' <> '1.0000'
    ...
    RAISE EXCEPTION 'P3_FAIL(quote_price_unit_normalization): %', v_plan;
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,shares,0,base_unit_price_cents}' <> '100'
    ...
    RAISE EXCEPTION 'P3_FAIL(zero_share_quote_provenance_ignored): %', v_plan;
  END IF;
  ...
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_QUOTE_FIELD_COVERAGE_INCOMPLETE%' THEN
    RAISE EXCEPTION 'P3_FAIL(partial_quote_field_coverage_rejected): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,shares,0,base_unit_price_cents}' <> '100'
     ...
     OR v_plan#>>'{billing_lines,0,shares,1,base_price_source}' <> 'tier'
     OR v_plan->>'grand_total_cents' <> '150' THEN
    RAISE EXCEPTION 'P3_FAIL(tier_fallback): %', v_plan;
  END IF;
  ...
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_CHEMICAL_COST_REQUIRED%' THEN
    RAISE EXCEPTION 'P3_FAIL(missing_product_cost_rejected): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,source_cost_cents}' <> '0'
    ...
    RAISE EXCEPTION 'P3_FAIL(explicit_zero_product_cost_allowed): %', v_plan;
  END IF;
  ...
  IF v_shares#>>'{0,split_micro_pct}' <> '33333334'
    ...
    RAISE EXCEPTION 'P3_FAIL(exact_three_way_one_cent): %', v_plan;
  END IF;
  ...
  IF v_plan->>'grand_total_cents' <> '-13'
     ...
     OR v_sum <> -13 THEN
    RAISE EXCEPTION 'P3_FAIL(negative_half_cent): %', v_plan;
  END IF;
  ...
  IF (SELECT sum(total_amount_cents) FROM public.invoices
    ...
    RAISE EXCEPTION 'P3_FAIL(negative_plan_persistence): signed sales/COGS headers drifted';
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,price_basis}' <> 'per_person_price'
    ...
    RAISE EXCEPTION 'P3_FAIL(per_person_price_override): %', v_plan;
  END IF;
  ...
  IF jsonb_array_length(v_shares) <> 2
    ...
    RAISE EXCEPTION 'P3_FAIL(service_100_0_job_season): %', v_plan;
  END IF;
  ...
  IF v_shares#>>'{0,amount_cents}' <> '1'
    ...
    RAISE EXCEPTION 'P3_FAIL(flat_fee_one_cent): %', v_plan;
  END IF;
  ...
  BEGIN
    ...
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(flat_fee_quantity_freeze): <no error>';
  ...
  IF v_plan->>'customer_count' <> '1'
    ...
    RAISE EXCEPTION 'P3_FAIL(job_snapshot_precedence): %', v_plan;
  END IF;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'P3_FAIL(missing_job_price): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'P3_FAIL(negative_job_price): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_CHEMICAL_PRICE_REQUIRED%' THEN RAISE; END IF;
  END;
  ...
  IF v_err NOT LIKE '%PER_LINE_APPLICATION_SERVICE_CONTEXT_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(job_application_service_identity): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_JOB_CONTEXT_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(nonnull_invoice_job_requires_exact_context): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF v_err NOT LIKE '%PER_LINE_APPLICATION_SERVICE_CONTEXT_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(invoice_application_service_identity): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,source_quantity}' <> '4.0000'
    ...
    RAISE EXCEPTION 'P3_FAIL(customer_supplied_zero): %', v_plan;
  END IF;
  ...
  IF v_err NOT LIKE '%PER_LINE_CUSTOMER_SUPPLIED_PRICE_OVERRIDE_UNSUPPORTED%' THEN
    RAISE EXCEPTION 'P3_FAIL(customer_supplied_price_override): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_err NOT LIKE '%PER_LINE_FIELD_NOT_IN_JOB%' THEN
    RAISE EXCEPTION 'P3_FAIL(job_field_membership): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_err NOT LIKE '%PER_LINE_JOB_CHEMICAL_ID_REQUIRED%' THEN
    RAISE EXCEPTION 'P3_FAIL(job_chemical_identity_required): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,source_quantity}' <> '1600.0000'
    ...
    RAISE EXCEPTION 'P3_FAIL(job_snapshot_unit_price_pair): %', v_plan;
  END IF;
  ...
  IF v_err NOT LIKE '%PER_LINE_JOB_FIELD_SET_INCOMPLETE%' THEN
    RAISE EXCEPTION 'P3_FAIL(job_field_set_incomplete): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_plan#>>'{billing_lines,0,shares,0,customer_id}' <> v_b::text
    ...
    RAISE EXCEPTION 'P3_FAIL(field_owner_fallback): %', v_plan;
  END IF;
  ...
  IF v_err NOT LIKE '%PER_LINE_MODE_A_UNSUPPORTED%' THEN
    RAISE EXCEPTION 'P3_FAIL(mode_a_rejection): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_err NOT LIKE '%PER_LINE_CUSTOM_VECTOR_INCOMPLETE%' THEN
    RAISE EXCEPTION 'P3_FAIL(incomplete_vector_rejection): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_err NOT LIKE '%PER_LINE_CUSTOM_VECTOR_CUSTOMER_SET_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(duplicate_vector_rejection): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_count <> 1 OR has_function_privilege(
    ...
    RAISE EXCEPTION 'P3_FAIL(private_or_overload_guard): preview %, private_execute %',
      v_count,
  ...
  IF (
    ...
    RAISE EXCEPTION 'P3_FAIL(durable_source_cost_identity_columns): required audit columns are missing';
  END IF;
  ...
  IF v_err NOT LIKE '%field_app_billing_lines_kind_price_basis_ck%' THEN
    RAISE EXCEPTION 'P3_FAIL(flat_fee_basis_mismatch): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'P3_FAIL(job_chemical_completeness): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'P3_FAIL(job_service_completeness): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(share_customer_invoice_mismatch): <no error>';
  EXCEPTION
  ...
  IF v_err NOT LIKE '%invoice_line_shares_default_price_matches_base_ck%' THEN
    RAISE EXCEPTION 'P3_FAIL(default_price_must_match_base): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_err NOT LIKE '%PER_LINE_SOURCE_JOB_CHEMICAL_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(job_quantity_rewrite): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  END;
  IF v_err NOT LIKE '%PER_LINE_SOURCE_JOB_CHEMICAL_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(job_base_price_rewrite): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  BEGIN
    UPDATE public.invoice_line_shares
     ...
     WHERE id = '85000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(share_invoice_set_mismatch): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'P3_FAIL(share_item_quantity_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    INSERT INTO public.invoice_line_shares (
      ...
      ('85000000-0000-0000-0000-000000000021',
       ...
       repeat('e', 64), repeat('f', 64), v_admin);
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(ungrouped_multiple_invoices): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_SHARE_INVOICE_SET_MISMATCH%' THEN RAISE; END IF;
  END;
  ...
  BEGIN
    UPDATE public.invoice_items SET acres = 0.40
     WHERE id = '84000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(share_item_acres_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_SHARE_ITEM_IDENTITY_MISMATCH%' THEN RAISE; END IF;
  END;
  ...
  BEGIN
    UPDATE public.invoice_items SET unit_price_cents = 99
     WHERE id = '84000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(share_item_price_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_SHARE_ITEM_IDENTITY_MISMATCH%' THEN RAISE; END IF;
  END;
  ...
  BEGIN
    UPDATE public.invoice_items SET product_id = v_product_quote
     WHERE id = '84000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(share_item_product_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_SHARE_ITEM_IDENTITY_MISMATCH%' THEN RAISE; END IF;
  END;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'P3_FAIL(share_item_service_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_SHARE_ITEM_IDENTITY_MISMATCH%' THEN RAISE; END IF;
  END;
  ...
  BEGIN
    ...
    UPDATE public.invoices SET total_cost_cents = 50
     WHERE id = '83000000-0000-0000-0000-000000000010';
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(group_customer_set_mismatch): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'P3_FAIL(unshared_invoice_item): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    UPDATE public.invoice_line_shares
       SET amount_cents = 49
     WHERE id = '85000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(share_item_amount_mismatch): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    UPDATE public.field_app_billing_lines
     ...
     WHERE id = '82000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(source_amount_mismatch): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    UPDATE public.invoice_items
     ...
     WHERE id = '84000000-0000-0000-0000-000000000010';
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(source_quantity_mismatch): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    UPDATE public.invoice_items
     ...
     WHERE id = '84000000-0000-0000-0000-000000000010';
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(source_acres_mismatch): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    UPDATE public.invoices
     ...
     WHERE id = '83000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(invoice_header_mismatch): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    UPDATE public.invoices
     ...
     WHERE id = '83000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(pre_post_integrity_boundary): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_INVOICE_HEADER_TOTAL_MISMATCH%' THEN RAISE; END IF;
  END;
  ...
  BEGIN
    UPDATE public.invoice_items SET unit_size = 'wrong-unit'
     WHERE id = '84000000-0000-0000-0000-000000000010';
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(share_item_unit_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_SHARE_ITEM_IDENTITY_MISMATCH%' THEN RAISE; END IF;
  END;
  ...
  BEGIN
    UPDATE public.invoice_items SET cost_cents = 49
     WHERE id = '84000000-0000-0000-0000-000000000010';
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(share_item_cost_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_SHARE_ITEM_IDENTITY_MISMATCH%' THEN RAISE; END IF;
  END;
  ...
  BEGIN
    UPDATE public.invoices SET total_cost_cents = 24
     WHERE id = '83000000-0000-0000-0000-000000000010';
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(invoice_header_cost_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    UPDATE public.invoice_line_shares SET allocated_cost_cents = 24
     WHERE id = '85000000-0000-0000-0000-000000000010';
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(allocated_cost_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    UPDATE public.field_app_billing_lines SET source_total_cost_cents = 49
     WHERE id = '82000000-0000-0000-0000-000000000010';
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(source_total_cost_basis): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    UPDATE public.field_app_billing_lines
     ...
     WHERE id = '82000000-0000-0000-0000-000000000010';
    SET CONSTRAINTS ALL IMMEDIATE;
    RAISE EXCEPTION 'P3_FAIL(source_job_chemical_identity): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%PER_LINE_JOB_CHEMICAL_SET_INCOMPLETE%' THEN RAISE; END IF;
  END;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'P3_FAIL(post_snapshot_source_cost_identity): audit values missing';
  END IF;

  BEGIN
    UPDATE public.invoice_items SET cost_cents = 49
     WHERE id = '84000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(posted_item_cost_freeze): <no error>';
  ...
  BEGIN
    UPDATE public.invoice_items SET unit_size = 'wrong-unit'
     WHERE id = '84000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(posted_item_unit_freeze): <no error>';
  EXCEPTION WHEN raise_exception THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
    IF v_err NOT LIKE '%material split fields are frozen%' THEN RAISE; END IF;
  END;
  ...
  BEGIN
    UPDATE public.field_app_billing_lines SET description = 'Relabeled after post'
     WHERE id = '82000000-0000-0000-0000-000000000010';
    RAISE EXCEPTION 'P3_FAIL(posted_source_line_freeze): <no error>';
  EXCEPTION WHEN raise_exception THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'P3_FAIL(zero_sendable_pre_post): <no error>';
  EXCEPTION WHEN check_violation THEN
    GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'P3_FAIL(zero_suppressed_snapshot): immutable record missing';
  END IF;
  ...
  BEGIN
    PERFORM public.save_field_app_invoice_per_line(
      ...
      jsonb_build_array(jsonb_build_object(
        'field_id', v_field_50, 'applied_acres', 1, 'total_acres', 1
      )), '[]'::jsonb, v_admin, v_service, v_job_service, '{}'::jsonb, NULL
    );
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_IDEMPOTENCY_KEY_REQUIRED%' THEN
    RAISE EXCEPTION 'P3_FAIL(writer_requires_idempotency): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF jsonb_array_length(v_result->'invoice_ids') <> 2
     OR (SELECT count(*) FROM public.field_app_billing_lines
          WHERE billing_set_id = (v_result->>'billing_set_id')::uuid) <> 1
     OR (SELECT count(*) FROM public.invoice_line_shares s
          JOIN public.field_app_billing_lines l ON l.id = s.billing_line_id
     ...
     OR (SELECT count(*) FROM public.invoice_items item
          WHERE item.invoice_id IN (SELECT value::uuid FROM jsonb_array_elements_text(v_result->'invoice_ids'))
            AND item.extended_cents = 0 AND item.quantity = 0) <> 1
     OR (SELECT count(*) FROM public.financial_audit_log audit
          WHERE audit.operation_type = 'invoice_created'
            AND audit.entity_type = 'invoice'
            AND audit.entity_id IN (
              SELECT value::uuid FROM jsonb_array_elements_text(v_result->'invoice_ids')
            )
            AND audit.actor_user_id = v_admin) <> 2 THEN
    RAISE EXCEPTION 'P3_FAIL(writer_100_0_persistence): %', v_result;
  END IF;
  ...
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF COALESCE(v_err, '') NOT LIKE '%IDEMPOTENCY_ACTOR_MISMATCH%' THEN
  ...
  BEGIN
    PERFORM public.save_field_app_invoice_per_line(
      NULL, jsonb_build_object('invoice_date', '2026-07-21'),
      jsonb_build_array(jsonb_build_object(
        'field_id', v_field_50, 'applied_acres', 1, 'total_acres', 1
      )), '[]'::jsonb, v_admin, v_service, v_job_service, '{}'::jsonb,
      'phase3-writer-100-0'
  ...
  v_err := NULL;
  BEGIN
    UPDATE public.invoices SET total_amount_cents = total_amount_cents + 1
     WHERE id = (v_result#>>'{invoice_ids,0}')::uuid;
    SET CONSTRAINTS ALL IMMEDIATE;
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_INVOICE_HEADER_TOTAL_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(writer_header_assertion): %', COALESCE(v_err, '<no error>');
  END IF;
  SET CONSTRAINTS ALL DEFERRED;
  ...
  IF (SELECT count(*) FROM public.invoices
     ...
     OR (SELECT count(*) FROM public.invoices
          WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(v_result->'invoice_ids'))
            ...
            AND send_disposition = 'suppressed_zero_total'
            AND status = 'posted') <> 1
     OR (SELECT count(*) FROM public.invoice_line_share_post_snapshots snapshot
          WHERE snapshot.invoice_id IN (
            SELECT id FROM public.invoices
             WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(v_result->'invoice_ids'))
               AND total_amount_cents = 0
          )
            AND snapshot.invoice_total_amount_cents = 0
            AND snapshot.send_disposition = 'suppressed_zero_total') <> 1 THEN
    RAISE EXCEPTION 'P3_FAIL(writer_zero_post_snapshot): %', v_result;
  END IF;
  SET CONSTRAINTS ALL DEFERRED;

  -- The legacy transfer path is explicitly refused while the feature is ON.
  v_err := NULL;
  BEGIN
    PERFORM public.transfer_job_to_invoice(v_job_service, v_applicator, 'phase3-transfer-forged');
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF COALESCE(v_err, '') NOT LIKE '%ACTOR_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(transfer_actor_first): %', COALESCE(v_err, '<no error>');
  END IF;

  v_err := NULL;
  BEGIN
    PERFORM public.transfer_job_to_invoice(v_job_service, v_admin, 'phase3-transfer-block');
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF COALESCE(v_err, '') NOT LIKE '%PER_LINE_TRANSFER_REQUIRES_FIELD_APP_SAVE%' THEN
    RAISE EXCEPTION 'P3_FAIL(transfer_block): %', COALESCE(v_err, '<no error>');
  END IF;
  ...
  IF v_err NOT LIKE '%INSUFFICIENT_ROLE%' THEN
    RAISE EXCEPTION 'P3_FAIL(applicator_denied): %', COALESCE(v_err, '<no error>');
  END IF;

  RAISE EXCEPTION 'SMOKE_PASS_ROLLBACK: phase3 per-line save/post proofs passed';
END;
$proof$;
