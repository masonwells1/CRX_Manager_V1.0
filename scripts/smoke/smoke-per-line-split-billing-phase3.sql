...
BEGIN
  ...
  IF jsonb_array_length(v_result->'invoice_ids') <> 2
     ...
     OR (SELECT count(*) FROM public.invoice_items item
          WHERE item.invoice_id IN (SELECT value::uuid FROM jsonb_array_elements_text(v_result->'invoice_ids'))
            AND item.extended_cents = 0 AND item.quantity = 0) <> 1 THEN
  ...
  IF v_plan_again->>'invoice_group_id' IS DISTINCT FROM v_result->>'invoice_group_id'
     OR jsonb_array_length(v_plan_again->'invoice_ids') <> 2
     OR EXISTS (
       SELECT 1 FROM public.field_app_billing_sets
        WHERE id = (v_result->>'billing_set_id')::uuid
     )
     OR (SELECT count(*) FROM public.invoice_line_shares s
          JOIN public.field_app_billing_lines l ON l.id = s.billing_line_id
         WHERE l.billing_set_id = (v_plan_again->>'billing_set_id')::uuid) <> 2 THEN
    RAISE EXCEPTION 'P3_FAIL(writer_draft_edit): % -> %', v_result, v_plan_again;
  END IF;
  ...
  -- The public transfer wrapper rejects forged attribution at its own boundary,
  -- before the feature gate or private legacy function can run.
  v_err := NULL;
  BEGIN
    PERFORM public.transfer_job_to_invoice(v_job_service, v_rep_other, 'phase3-transfer-forged');
  EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
  IF COALESCE(v_err, '') NOT LIKE '%ACTOR_MISMATCH%' THEN
    RAISE EXCEPTION 'P3_FAIL(transfer_actor): %', COALESCE(v_err, '<no error>');
  END IF;
