...
function run() {
  ...
  console.log('PROOF — Saw: all Phase 2 calculator and relational-integrity cases remained green; the Phase 3 writer persisted the calculator-only 100/0 service plan as two real invoices/items/shares, rebuilt an existing draft group without RESTRICT-FK failure or orphan rows, preserved a zero-dollar suppressed child, posted both group members together with an immutable zero snapshot, replayed an identical key without duplicate rows, rejected the same key with changed payload, rejected a header/item mismatch, blocked transfer_job_to_invoice while the feature was ON, and retained owner/definer/search-path/ACL plus cleanup-failure guards.');
}
