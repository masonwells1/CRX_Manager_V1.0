...
const continuation = `
  ...
  const replayGuardSource = readFileSync(${JSON.stringify(path.join(ROOT, 'supabase', 'migrations', '20260908120200_commission_history_report_replay_guard.sql'))}, 'utf8');
  const latestRecipientLabelSource = readFileSync(${JSON.stringify(path.join(ROOT, 'supabase', 'migrations', '20260908120600_latest_commission_recipient_label.sql'))}, 'utf8');
