...
const SNAPSHOT_NAME = '2026-07-22-supplier-pricing-phase3-product-snapshot.json';
const MANIFEST_NAME = '2026-07-22-supplier-pricing-phase3-proposed-classification-manifest.json';
const privateArtifactDir = process.env.CRX_PHASE3_PRIVATE_ARTIFACT_DIR;
function option(name) {
  const index = process.argv.indexOf(name);
  return index === -1 ? null : process.argv[index + 1];
}
const snapshotPath = option('--snapshot') || (privateArtifactDir ? path.join(privateArtifactDir, SNAPSHOT_NAME) : null);
const manifestPath = option('--manifest') || (privateArtifactDir ? path.join(privateArtifactDir, MANIFEST_NAME) : null);
...
export function loadSnapshot(file = snapshotPath) {
  assert(file, 'private snapshot path required: set CRX_PHASE3_PRIVATE_ARTIFACT_DIR or pass --snapshot <path>');
...
function main() {
  ...
  if (args.has('--compare')) {
    assert(manifestPath, 'private manifest path required: set CRX_PHASE3_PRIVATE_ARTIFACT_DIR or pass --manifest <path>');
  ...
  if (args.has('--write')) {
    assert(manifestPath, 'private manifest path required: set CRX_PHASE3_PRIVATE_ARTIFACT_DIR or pass --manifest <path>');
    writeFileSync(manifestPath, output, { encoding: 'utf8' });
