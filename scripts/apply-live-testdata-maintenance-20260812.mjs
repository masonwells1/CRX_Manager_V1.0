...
export function buildProducerProtectionSources() {
  ...
  // Anchor on the exact boundary after the already-pinned matcher rather than
  // Function#toString(). Node's source rendering is not a stable source-code
  // serialization contract, so it made this one-use producer fail before it
  // could verify its own protected output.
  const matcherPrefix = "  return compact.includes(\"apply-live-testdata-maintenance-20260812.mjs\");\n}\n\n";
  const matcherAnchor = `${matcherPrefix}function normalize(value) {`;
  const invocationMatcherSource = normalizeLineEndings(`export ${maintenanceProducerInvocationAllowed.toString()}`);
  codexGuard = replaceExactly(
    codexGuard,
    matcherAnchor,
    `${matcherPrefix}${invocationMatcherSource}\n\nfunction normalize(value) {`,
