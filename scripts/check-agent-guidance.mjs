...
3. **Compare against the \`name\` high-water, never against a \`version\`.** \`supabase_migrations.schema_migrations\` carries both: \`name\` preserves the authored filename stamp and is what replay ordering depends on, while \`version\` is assigned by Supabase at apply time and can land on either side of \`name\`. \`.claude/schema-registry.json\`'s \`_meta.migrations_high_water\` is a **\`version\`**, so it is NOT valid evidence for this check, and neither is a bare \`max(version)\`. Comparing against a \`version\` emits a false **HIGH** on every migration authored before some later apply — \`.claude/hooks/session-staleness.mjs\` records the same trap in its CHECK 1 comments, and \`docs/reference/migration-history.md\` states the rule outright: "The ordering guard compares \`name\` stamps." If the only figure you can find is a \`version\`, treat it as no evidence and fall through to step 4 rather than comparing against it. One narrow per-row exception: a ledger row whose \`name\` embeds no 14-digit timestamp — early rows predating the timestamped convention — contributes \`<version>_<name>\` as that row's ordering signal, exactly as \`scripts/refresh-applied-migrations.mjs\` and \`.claude/hooks/migration-apply-guard.mjs\` synthesize it, so a timestamp-less row still constrains ordering instead of silently dropping out of the high-water. That per-row fallback never licenses comparing against a bare \`max(version)\` or the schema registry high-water.
...
5. Always note the post-apply obligation, and note it correctly: **do NOT rename the applied file to the server-assigned \`version\` when the ledger \`name\` already equals the disk filename** — which is the normal case on the governed \`apply_migration\` path, because the submitted name IS the filename. Renaming would destroy the authored stamp this check compares against and manufacture the exact drift the guard exists to catch; because a \`version\` can land BELOW its own \`name\` (live example: version \`20260813011751\` under name \`20260813070000\`), a renamed file can even sort under the live high-water and produce a false **HIGH** on the next run. Settled 2026-08-12 and recorded in \`docs/reference/migration-history.md\` — rows 878 and 879 and the \`20260813080000\` closeout each state the no-rename decision outright. The historical B7 rename applies only where the live \`name\` does NOT already equal the disk filename. Either way, record the server-assigned \`version\` in migration history before commit. This is a post-apply obligation, not a pre-apply finding.
`;
...
// 626 of 971 live ledger rows carry a bare-slug `name` with no 14-digit stamp
// (observed read-only 2026-08-24). Blanket "never use a version" would tell the
// reviewer to discard the only ordering signal such a row has, contradicting the
// `<version>_<name>` synthesis in refresh-applied-migrations.mjs and
// migration-apply-guard.mjs. Pin the per-row fallback so it cannot be dropped.
record(/no 14-digit timestamp[\s\S]*`<version>_<name>` as that row's ordering signal/i.test(migrationStampCheck), "migration drift reviewer keeps the per-row version fallback for timestamp-less ledger names");
...
// Settled 2026-08-12 (migration-history.md rows 878/879 + the 20260813080000
// closeout): an applied file is NOT renamed to the server-assigned version when
// the ledger `name` already equals the filename. The old charter text ordered the
// rename unconditionally, which would destroy the authored stamp CHECK 6 compares
// and — since a version can sort BELOW its own name — could push an applied file
// under the live high-water. Pin the no-rename rule and the history obligation.
record(/do NOT rename the applied file to the server-assigned `version` when the ledger `name` already equals the disk filename/i.test(migrationStampCheck), "migration drift reviewer forbids renaming an applied file that already matches its ledger name");
record(/record the server-assigned `version` in migration history before commit/i.test(migrationStampCheck), "migration drift reviewer requires the post-apply migration-history closeout");
