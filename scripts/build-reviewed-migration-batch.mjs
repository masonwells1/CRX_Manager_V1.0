...
function ledgerGuardInvariantLines(message) {
  ...
  return [
    ...
    "           AND t.tgtype = 7 AND t.tgenabled = 'A'",
...
export function buildAtomicMigrationSql({ migrationName, query, queryHash, requiredEarlierMigrations = [] }) {
  ...
  return [
    ...
    "        AND a.attname IN ('created_by', 'idempotency_key')",
    "        AND a.attnum > 0",
    "        AND NOT a.attisdropped) <> 2 THEN",
    "    RAISE EXCEPTION 'CRX migration ledger is missing created_by or idempotency_key';",
    "  END IF;",
    "",
    "  IF EXISTS (",
