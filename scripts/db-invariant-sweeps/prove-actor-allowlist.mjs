...
function sweep(entries) {
  ...
  return { ...packet, remaining: subtractAllowlist(packet.predicate, packet.rows, entries, packet.function_contracts) };
}
const uid = '11111111-1111-4111-8111-111111111111';
const other = '22222222-2222-4222-8222-222222222222';
// One text for the reviewed identity source, so the restore reproduces the pinned digest exactly.
const AUTH_UID_BODY = "'SELECT nullif(current_setting(''request.jwt.claim.sub'', true), '''')::uuid'";
...
try {
  ...
  // Same readiness contract as prove-preview-field-app-season.mjs. The image's entrypoint runs a
  // TEMPORARY server during initdb that listens on the Unix socket only, so a socket pg_isready can
  // report ready before the entrypoint stops it and starts the real one; a fixed sleep afterwards
  // only guesses when that restart ends. Require the init-complete log line, a TCP pg_isready that
  // the temporary server (listen_addresses='') cannot answer, and one real query (CodeRabbit #774).
  let ready = false;
  for (let attempt = 0; attempt < 120 && !ready; attempt += 1) {
    ...
    if (`${logs.stdout}\n${logs.stderr}`.includes('PostgreSQL init process complete; ready for start up.') &&
        docker(['exec', container, 'pg_isready', '-h', '127.0.0.1', '-U', 'postgres', '-d', 'postgres'], { allowFailure: true }).status === 0) {
      const probe = docker(['exec', container, 'psql', '-h', '127.0.0.1', '-U', 'postgres', '-d', 'postgres', '-Atqc', 'SELECT 1'], { allowFailure: true });
      ready = probe.status === 0 && probe.stdout.trim() === '1';
    }
    if (!ready) Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 250);
  }
  check(ready, 'disposable PostgreSQL started and answered a real query on the final server');
...
CREATE SCHEMA auth;
CREATE FUNCTION auth.uid() RETURNS uuid LANGUAGE sql STABLE AS ${AUTH_UID_BODY};
...
${deliverySafe}`);
  ...
  // Every actor entry pins auth.uid(), so a changed identity source must strand BOTH findings, not
  // just whichever one the matcher happens to reach first.
  const identityDrift = sweep(entries).remaining.map((row) => row.violation_key);
  check(identityDrift.includes(deliveryKey) && identityDrift.includes(wrapperKey),
    'changed identity source fails closed for every entry that pins it');
  // Restore it BEFORE the missing-dependency case. Left mutated, that check could pass on the stale
