...
const REVIEWED_STRUCTURAL_SIGNATURE_SOURCE = '{"id":"synthetic","sku":"SYN-TEST","product_name":"Synthetic test row","pricing_version":1,"updated_at":"2026-07-26T00:00:00.000Z"}\n';
...
try {
  ...
  assert(Object.keys(STRUCTURAL_SIGNATURE_EXEMPTIONS).every(repoPath => repoPath === 'scripts/supplier-pricing-phase3-private-artifacts.test.mjs'), 'reviewed structural exemptions must stay path-exact');
  assert.equal(isStructuralSignatureExempt('scripts/supplier-pricing-phase3-private-artifacts.test.mjs', STRUCTURAL_SIGNATURE_REASON), true);
  ...
  const exemptionRepo = fixtureRepo('containment-reviewed-structural-exemption'); const exemptionInitial = git(exemptionRepo, ['rev-parse', 'HEAD']).trim(); const exemptionPath = 'scripts/supplier-pricing-phase3-private-artifacts.test.mjs'; mkdirSync(path.dirname(path.join(exemptionRepo, exemptionPath)), { recursive: true }); writeFileSync(path.join(exemptionRepo, exemptionPath), REVIEWED_STRUCTURAL_SIGNATURE_SOURCE); await fixtureContainment(exemptionRepo);
  git(exemptionRepo, ['add', exemptionPath]); git(exemptionRepo, ['commit', '--quiet', '-m', 'synthetic reviewed structural fixture']); await fixtureContainment(exemptionRepo);
  const exemptionSibling = 'scripts/supplier-pricing-phase3-private-artifacts.test.mjs.copy'; writeFileSync(path.join(exemptionRepo, exemptionSibling), REVIEWED_STRUCTURAL_SIGNATURE_SOURCE); await containmentFails(exemptionRepo, exemptionSibling, STRUCTURAL_SIGNATURE_REASON); rmSync(path.join(exemptionRepo, exemptionSibling));
  ...
  await checkPrivateArtifactContainment({ root: exemptionRepo, ranges: [`${exemptionInitial}..${exemptionHead}`] });
...
`], {
  ...
  assert(containmentJob.includes("phase3_bootstrap_ancestor='d787b7e0e1c9cb5eb85c86b448e68b1ca43fce9c'"));
  ...
  if (fixtureBash()) {
    ...
    const simulatedBootstrapShell = containmentShell.replace("phase3_bootstrap_ancestor='d787b7e0e1c9cb5eb85c86b448e68b1ca43fce9c'", `phase3_bootstrap_ancestor='${bootstrapBase}'`);
