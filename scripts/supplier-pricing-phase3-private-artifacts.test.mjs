...
try {
  ...
  assert(containmentJob.includes("phase3_bootstrap_base='052b2171821dc7ffd965b4edb4b6de4ef8fda511'"));
  assert(containmentJob.includes('git -C "$GITHUB_WORKSPACE" cat-file -e "${phase3_base}:${phase3_checker_rel}"'));
  assert(containmentJob.includes('trusted base containment checker is incompatible; refusing candidate fallback'));
  ...
  assert(containmentJob.includes('node "$phase3_trusted_checker" --github-event --root "$GITHUB_WORKSPACE"'));
  ...
  if (fixtureBash()) {
    ...
    assert(`${bootstrapRun.stdout}${bootstrapRun.stderr}`.includes('CI_BOOTSTRAP_CANDIDATE_CHECKER_RAN'));
    ...
    assert(`${incompatibleRun.stdout}${incompatibleRun.stderr}`.includes('trusted base containment checker is incompatible; refusing candidate fallback'));
