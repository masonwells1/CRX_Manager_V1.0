...
try {
  ...
  assert.deepEqual(boundedPrePushCalls.find(args => args[0] === 'check-ref-format'), ['check-ref-format', 'refs/remotes/origin/phase3c-containment'], 'pre-push remote validation must use Git ref validation through an argument array');
  ...
  assert.deepEqual(namespacedRemoteCalls.find(args => args[0] === 'check-ref-format'), ['check-ref-format', 'refs/remotes/team/origin/phase3c-containment']);
  assert(namespacedRemoteCalls.find(args => args[0] === 'rev-list').includes('--remotes=team/origin'), 'Git-valid namespaced remotes must remain usable for first-push exclusion');
  ...
  for (const unsafeRemoteName of ['', '../origin', 'origin//mirror', 'origin..mirror', 'origin lock', 'origin@{mirror']) await assert.rejects(() => checkPrePushPrivateArtifactContainment({ root: boundedHistoryRepo, remoteName: unsafeRemoteName, stdin: `refs/heads/packet ${boundedHistoryHead} refs/heads/packet ${'0'.repeat(40)}\n` }), /valid remote name/);
  ...
  const ignoredPrivateArchiveRepo = fixtureRepo('containment-ignored-private-archive'); const ignoredPrivateArchivePath = 'private-packet.json.gz'; writeFileSync(path.join(ignoredPrivateArchiveRepo, '.gitignore'), '*.gz\n'); git(ignoredPrivateArchiveRepo, ['add', '.gitignore']); git(ignoredPrivateArchiveRepo, ['commit', '--quiet', '-m', 'ignore gzip files']); writeFileSync(path.join(ignoredPrivateArchiveRepo, ignoredPrivateArchivePath), Buffer.concat([Buffer.from([0x1f, 0x8b, 0x08]), Buffer.from(JSON.stringify({ format: POST_STAGE_A_SNAPSHOT_FORMAT }))])); await containmentFails(ignoredPrivateArchiveRepo, ignoredPrivateArchivePath, 'compressed archive container cannot be inspected');
  const ignoredToolArchiveRepo = fixtureRepo('containment-ignored-tool-archives'); const toolOwnedIgnoredRoots = ['node_modules', 'dist', 'dist-ssr', 'coverage', 'playwright-report', '.playwright-mcp', '.playwright-cli', 'graphify-out']; writeFileSync(path.join(ignoredToolArchiveRepo, '.gitignore'), `${toolOwnedIgnoredRoots.join('\n')}\n`); git(ignoredToolArchiveRepo, ['add', '.gitignore']); git(ignoredToolArchiveRepo, ['commit', '--quiet', '-m', 'ignore tool-owned roots']); for (const root of toolOwnedIgnoredRoots) { const toolArchivePath = `${root}/third-party.xlsx`; mkdirSync(path.dirname(path.join(ignoredToolArchiveRepo, toolArchivePath)), { recursive: true }); writeFileSync(path.join(ignoredToolArchiveRepo, toolArchivePath), compressedBytes); } await fixtureContainment(ignoredToolArchiveRepo); for (const root of toolOwnedIgnoredRoots) { const ignoredToolRootFileRepo = fixtureRepo(`containment-ignored-tool-root-file-${root.replaceAll('.', 'dot')}`); writeFileSync(path.join(ignoredToolRootFileRepo, '.gitignore'), `${root}\n`); git(ignoredToolRootFileRepo, ['add', '.gitignore']); git(ignoredToolRootFileRepo, ['commit', '--quiet', '-m', 'ignore tool root name']); writeFileSync(path.join(ignoredToolRootFileRepo, root), Buffer.from([0x1f, 0x8b, 0x08, 0x00])); await containmentFails(ignoredToolRootFileRepo, root, 'compressed archive container cannot be inspected'); }
  ...
  assert(containmentJob.includes("phase3_bootstrap_base='d3bac970804bf6130b6bf6259eed05fad0367a9c'"));
  ...
  assert(containmentJob.includes('expected bootstrap $phase3_bootstrap_base, event base $phase3_base'));
  ...
  if (fixtureBash()) {
    ...
    const simulatedBootstrapShell = containmentShell.replace("phase3_bootstrap_base='d3bac970804bf6130b6bf6259eed05fad0367a9c'", `phase3_bootstrap_base='${bootstrapBase}'`);
