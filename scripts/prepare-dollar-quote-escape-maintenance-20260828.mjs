...
const EXPECTED_INPUT_BLOB = "3875e085266f6f0395ea16ad2fa2032b56ae3373";
const EXPECTED_OUTPUT_BLOB = "419f4e8fc0b08566c6ebd139dde312d7553eb3f7";
const APPROVAL = "--mason-authorized-one-account-gate=2026-08-28";
const REVIEW_TOKEN = "DOLLAR_QUOTE_ESCAPE_MAINTENANCE_VERDICT";
...
export function buildMaintainedSource(input = normalize(readFileSync(TARGET_PATH, "utf8"))) {
  if (gitBlob(input) !== EXPECTED_INPUT_BLOB) throw new Error("refusing stale dollar-quote scanner input");
  const scannerStart = input.indexOf("function stripDollarQuotedCore(sql, keepBody)");
  const first = input.indexOf(OLD_SCANNER, scannerStart);
  if (scannerStart < 0 || first < 0 || input.indexOf(OLD_SCANNER, first + 1) >= 0) {
    throw new Error("dollar-quote scanner replacement anchor must occur exactly once after its declaration");
  }
...
function runArtifactReview({ input, output }) {
  const reviewDir = mkdtempSync(path.join(os.tmpdir(), "crx-dollar-quote-escape-review-"));
  ...
  try {
    ...
    const producerTest = readFileSync(path.join(REPO_DIR, "scripts", "prepare-dollar-quote-escape-maintenance-20260828.test.mjs"), "utf8");
    ...
    const prompt = [
      "Perform a read-only adversarial review of one deterministic security-parser maintenance transformation.",
      ...
      "Verify the manifest hashes and that the transformer is input-bound and output-bound.",
      "Verify OUTPUT changes only stripDollarQuotedCore so PostgreSQL E-strings skip backslash-escaped characters.",
      "Specifically prove an E-string containing $x$ cannot make a later $x$ in a comment swallow a real DELETE.",
      "Check for weakening, unrelated changes, alternate writes, non-E-string regressions, or a fail-open path.",
...
async function main() {
  ...
  if (verifyOnly) {
    process.stdout.write(`dollar-quote escape maintenance verified: ${built.blob}\n`);
    return;
  }
  ...
  if (gitBlob(normalize(readFileSync(TARGET_PATH, "utf8"))) !== EXPECTED_OUTPUT_BLOB) {
    throw new Error("post-write dollar-quote scanner blob mismatch");
  }
