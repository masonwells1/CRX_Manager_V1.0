...
deployed as v1 on 2026-09-22 UTC, and it needs only `ALLOWED_ORIGIN` plus the platform-provided
Supabase keys. The function-specific secrets below must be present wherever the corresponding
function uses them.

`customer-document-files` is now the **only** path for customer-document bytes. Migration
`20260914100700_customer_document_bytes_server_only.sql` was applied live on 2026-09-26 (ledger
version `20260926163005`), so no Storage policy lets a browser read or sign links in the
`customer-documents` bucket; it is done and must not be re-applied. Keep the function deployed:
undeploying it breaks the Documents tab entirely, because nothing else can serve those files.
