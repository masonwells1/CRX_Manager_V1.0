...
- **Regular, reversible code only** (no live migration in this job, no Edge Function deploy, no data deletion, every gate ran and came back green — including the Codex verdict when the change was Codex-worthy): land under Mason's standing authorization (2026-06-16, mechanics updated 2026-07-30) — invoke the installed trusted Git by its literal absolute executable path and push `HEAD:refs/heads/<branch>` → `gh pr create` → wait for the required Vercel check → **read CodeRabbit's review** (fix any real issue it raises; dismiss nitpicks with a one-line reason; never merge without reading it) → read the exact head with `gh pr view <n> --repo masonwells1/CRX_Manager_V1.0 --json headRefOid --jq .headRefOid` → substitute that literal 40-character value into the standalone merge command printed by `node scripts/land-pr.mjs <n>`. That command names the literal absolute trusted GitHub CLI path, numeric PR, repository, and exact head SHA. Verify the deploy and report the merge and evidence explicitly. Do not ask for a fresh yes — the green pipeline IS the authorization for this class of change. (Direct pushes to `main` are impossible: the `protect-main` ruleset rejects them for everyone.) `land-pr.mjs` watches STATE, not just checks: it updates the branch whenever the PR falls behind (re-running if main moves again), and then prints the guarded merge command. It never merges anything itself. Auto-merge is denied for every main-bound PR; a risky diff also needs the fresh exact-head proof before that immediate merge.
