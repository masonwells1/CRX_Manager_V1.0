...
3. **Apply** through `scripts/apply-migration-file.mjs <file>` (dry run first, then `--confirm`); the MCP `apply_migration` path cannot pass the guard's project binding. Record the apply in the change's `docs/changelog.d/` entry with the words "applied live" — the daily summary reads that.
