...
- **Regular, reversible code only** (no live migration in this job, no Edge Function deploy, no data deletion, every gate ran and came back green — including the Codex verdict when the change was Codex-worthy): land under Mason's standing authorization (2026-06-16, mechanics updated 2026-07-30) — `git push -u origin HEAD:refs/heads/<branch>` → `gh pr create` → wait for the required Vercel check → **read CodeRabbit's review** (fix any real issue it raises; dismiss nitpicks with a one-line reason; never merge without reading it) → read the exact head with `gh pr view <n> --repo masonwells1/CRX_Manager_V1.0 --json headRefOid --jq .headRefOid` → substitute that literal 40-character value into the standalone command `gh pr merge <n> --repo masonwells1/CRX_Manager_V1.0 --squash --match-head-commit <head-sha>` → verify the deploy, and report the merge and evidence explicitly. Do not ask for a fresh yes — the green pipeline IS the authorization for this class of change. (Direct pushes to `main` are impossible: the `protect-main` ruleset rejects them for everyone.) **Use `node scripts/land-pr.mjs <n>` to babysit the PR to a current, green state before the immediate merge** — branch protection requires the PR to be up to date with `main`, so a sibling merge flips the PR to `mergeStateStatus=BEHIND` and it stalls forever with every check green (PR #345 sat overnight this way, 2026-08-08). `land-pr.mjs` watches STATE, not just checks: it runs `gh pr update-branch` whenever the PR falls behind (re-running if main moves again), and then prints a merge command containing the literal inspected SHA. It never merges anything itself — merging stays with the gated `gh pr merge`. Auto-merge is denied for every main-bound PR; a risky diff also needs the fresh exact-head proof before that immediate merge.
