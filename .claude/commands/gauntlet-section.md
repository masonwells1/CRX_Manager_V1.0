...
Write to `docs/audits/gauntlet/<YYYY-MM-DD>-section-<NN>-<slug>-refresh.md` (real date from `date +%F`), following the shape of the existing section reports in that folder: verdict, scope, method, findings by severity with citations, verified-safe, and a recommended next action per finding.
