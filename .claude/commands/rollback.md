...
- **Edge function?** `list_edge_functions` via the Supabase MCP — check each function's version and updated timestamp; a fresh version on any function in `supabase/functions/` (the directory listing is authoritative) points to scenario (c). The Supabase dashboard shows the full version history.
