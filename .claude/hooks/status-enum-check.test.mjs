...
try {
  ...
  r = runHook("Edit", dirAsSql, {
    old_string: "-- status-enum-check: exempt (x)\n",
    new_string: "",
  });
  ...
  writeFileSync(migPath, CRLF("-- status-enum-check: exempt (stale marker, nothing left)\n"));
  r = runHook("Edit", migPath, {
    old_string: "-- status-enum-check: exempt (stale marker, nothing left)\n",
    new_string: "",
  });
