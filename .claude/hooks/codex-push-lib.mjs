...
const RISKY_PATH_RES = [
  ...
  /(^|\/)src\/lib\/sentry(\.ts|\/)/i,
  /(^|\/)\.claude\/hooks\//i,
  // Round thirteen (PR #605): `.codex/` by shape - hooks, hooks.json, config.toml and
  // sync-from-claude.ps1, the generator that writes into .codex/hooks/**. Supersedes the
  // three narrower entries this list used to carry.
  /(^|\/)\.codex\//i,
  // Round thirteen (PR #605): these three reach CI. package.json:57 runs
  // .claude/workflows/{gauntlet-sections-loop,truthful-review-states}.test.mjs and
  // scripts/check-agent-workflows.mjs (which reads .claude/commands/* by name) inside
  // `npm run test:agent-workflows` (ci.yml:468, .husky/pre-commit:43); check-agent-guidance.mjs
  // reads commands/preflight.md, commands/ship.md and skills/graphify/SKILL.md. A weakened
  // test still reports success, so the merge gate lies rather than fails.
  /(^|\/)\.claude\/commands\//i,
  /(^|\/)\.claude\/skills\//i,
  /(^|\/)\.claude\/workflows\//i,
  /(^|\/)\.github\/workflows\//i,
  ...
  // protected-surface-parity.test.mjs (PR #605, round twelve) - gate inputs read from
  // the working tree at decision time: the schema registry feeds three SQL PreToolUse
  // hooks and the migration review packet; the caller graph feeds grant-change-guard.
  // Both are regenerated by scripts, never hand-edited.
  /(^|\/)\.claude\/schema-registry\.json$/i,
  /(^|\/)\.claude\/caller-graph\.json$/i,
  // Already shell-denied by review-proof-guard; the parity test requires the three lists
  // to agree, and both decide whether the Claude and Codex agent manifests stay in sync.
  /(^|\/)scripts\/agent-manifest-parity\.mjs$/i,
  /(^|\/)scripts\/sync-agent-workflows\.mjs$/i,
