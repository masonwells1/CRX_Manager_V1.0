...
describe('create_inventory_hold receipt binding (20260908130000)', () => {
  ...
  it('pins the standalone insert barrier body and installs it before the rename', () => {
    ...
    const end = migration.indexOf('$insert_guard$;', start);
    expect(end).toBeGreaterThan(start);
    const hash = createHash('sha256').update(migration.slice(start, end), 'utf8').digest('hex');
  ...
  it('orders after its required predecessor and is still marked NOT APPLIED', () => {
    const ordered = readdirSync(join(root, 'supabase', 'migrations'))
      ...
      .sort();
    const predecessor = '20260908120000_close_pr535_live_gaps.sql';
    expect(ordered).toContain(predecessor);
    expect(ordered.indexOf(migrationName)).toBeGreaterThan(ordered.indexOf(predecessor));
