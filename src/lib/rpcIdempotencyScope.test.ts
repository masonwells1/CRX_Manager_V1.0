...
const INTERNAL_OPERATION_REFERENCES: Record<string, string[]> = {
  ...
  // Direct EXECUTE is revoked (verified live 2026-08-13: anon, authenticated
  ...
  // 20260813161614 replaces only this private body to add the quote-owner and
  // soft-delete guard. Both layers are one logical draw and must share the
  // public 'draw_down_quote' replay namespace — the live body already uses
  // that literal, and giving this layer its own namespace would create an
  // unreachable cache and let a replay slip past the shared one.
