...
function assignedSqlCallArguments(
  ...
  for (let i = open + 1; i < executable.length; i += 1) {
    ...
    if (executable[i] === ')') {
      if (depth === 0) {
        const finalArgument = executable.slice(start, i).replace(/\s+/g, ' ').trim();
        if (finalArgument || args.length > 0) args.push(finalArgument);
        return args;
      }
      depth -= 1;
    }
...
describe('Idempotency BODY verification (reads migration SQL)', () => {
  ...
  it('draw_down_quote binds replay intent and forwards one key through the private pricing chain', () => {
    ...
    expect(assignedSqlCallArguments(
      'v_result := public.zero_argument_probe();',
      'v_result',
      'public.zero_argument_probe',
    )).toEqual([]);
