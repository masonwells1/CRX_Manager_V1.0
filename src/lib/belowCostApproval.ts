...
import { appendBelowCostApproval, stripInternalNotes } from './internalNotes';
...
export function withBelowCostReason<T extends Record<string, unknown>>(
  ...
  if (!reason) {
    switch (operation) {
      ...
      case 'duplicate_quote':
        // These governed wrappers replaced older overloads. Sending the
  ...
  switch (operation) {
    ...
    case 'bulk_import_order':
      return { ...args, p_notes: appendBelowCostApproval(
...
export function stripBelowCostApprovalNote(notes: string | null | undefined): string | null {
  return stripInternalNotes(notes);
}
