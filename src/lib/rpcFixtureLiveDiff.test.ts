...
// -------------------------------------------------------------------------
// Checked-in live snapshot — 2026-07-11 baseline plus every function added
// through the 2026-07-14 live high-water mark, 374 distinct public names.
// -------------------------------------------------------------------------
...
const LIVE_PG_PROC_ADDITIONS_20260714 = [
  ...
  'run_weekly_db_backup',
  'stage_offline_action',
  'stamp_job_printed',
] as const;

const LIVE_PG_PROC_COUNT = 374; // count(DISTINCT proname), live query 2026-07-14.

const LIVE_FUNCTIONS = new Set([
  ...LIVE_PG_PROC_NAMES_CSV.split(','),
  ...LIVE_PG_PROC_ADDITIONS_20260714,
]);
...
describe('Live pg_proc snapshot integrity', () => {
  it('snapshot has the expected count, no duplicates, sorted', () => {
    const names = [...LIVE_FUNCTIONS].sort();
