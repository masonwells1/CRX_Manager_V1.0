...
export default function ApplicationServiceDetail() {
  ...
  // Fetch vehicles + customers for dropdowns
  useEffect(() => {
  ...
  const fetchService = useCallback(async () => {
    ...
    setForm({
      ...
      default_rate_per_acre: formatCentsToDollars(data.default_rate_per_acre_cents),
      cost_per_acre: formatCentsToDollars(data.cost_per_acre_cents),
  ...
  const fetchOverrides = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('customer_application_rates')
      .select('*, customer:customers(farm_name)')
      .eq('application_service_id', id)
  ...
  const handleSave = async () => {
    ...
    if (!isAdmin) {
      toast('error', 'Only admins can manage application services');
      return;
    }
    ...
    const payload = {
      ...
      default_rate_per_acre_cents: parseDollarsToCents(form.default_rate_per_acre),
      cost_per_acre_cents: parseDollarsToCents(form.cost_per_acre),
    ...
    try {
      if (isNew) {
        ...
        if (profile) logActivity({ event: 'app_service_created', description: `Application service "${form.name}" created`, performedBy: profile.id });
        toast('success', 'Application service created');
        setIsDirty(false);
        navigate(`/application-services/${result.data!.id}`);
      } else {
        ...
        if (profile) logActivity({ event: 'app_service_updated', description: `Application service "${form.name}" updated`, performedBy: profile.id });
  ...
  const handleAddOverride = async () => {
    if (!newOverride.customer_id || !newOverride.rate) {
      toast('error', 'Customer and rate are required');
      return;
    }
    try {
      const result = await supabase.from('customer_application_rates').insert({
        customer_id: newOverride.customer_id,
        application_service_id: id,
        rate_per_acre_cents: parseDollarsToCents(newOverride.rate),
        season: parseInt(newOverride.season) || currentSeason(),
        notes: newOverride.notes || null,
        created_by: profile?.id,
      }).select('*, customer:customers(farm_name)').single();
  ...
  const handleDeleteOverride = async (overrideId: string) => {
    try {
      const result = await supabase.from('customer_application_rates').delete().eq('id', overrideId).select();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button
          ...
          aria-label="Back to application services"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-nav-dark">
            {isNew ? 'New Application Service' : form.name}
          </h1>
          {!isNew && (
            <p className="text-sm text-secondary mt-0.5">
              {form.is_active ? 'Active' : 'Inactive'} &middot; {form.default_rate_per_acre ? `$${form.default_rate_per_acre}/ac` : 'No rate set'}
            </p>
          )}
        </div>
        {isAdmin && (
      ...
      <Card>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <Input
            ...
            required
            placeholder="e.g. Hagie Y-Drop Nitrogen"
          />
          <div>
            <label className="block text-sm font-medium text-nav-dark mb-1">Vehicle (optional)</label>
            <select
          ...
          <Input
            label="Default Rate / Acre ($)"
            type="number"
            step="0.01"
            min={0}
            value={form.default_rate_per_acre}
            onChange={(e) => updateField('default_rate_per_acre', e.target.value)}
          ...
          <Input
            ...
            step="0.01"
            min={0}
            value={form.cost_per_acre}
            onChange={(e) => updateField('cost_per_acre', e.target.value)}
      ...
      {/* Customer Rate Overrides — only show after service is saved */}
      {!isNew && (
        <Card>
          <h3 className="text-lg font-semibold text-nav-dark mb-4">Customer Rate Overrides</h3>
          <p className="text-sm text-secondary mb-4">
            Special per-acre rates for specific customers. These override the default rate above.
          </p>
          ...
          <div className="flex items-end gap-3 mb-4 pb-4 border-b border-gray-100">
            <div className="flex-1">
              ...
              <select
                value={newOverride.customer_id}
                onChange={(e) => setNewOverride((p) => ({ ...p, customer_id: e.target.value }))}
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-crx-green/20 focus:border-crx-green"
              >
            ...
            <div className="w-28">
              <label className="block text-xs font-medium text-secondary mb-1">$/Acre</label>
              <input
                type="number"
                step="0.01"
                min={0}
                value={newOverride.rate}
                onChange={(e) => setNewOverride((p) => ({ ...p, rate: e.target.value }))}
                placeholder="8.00"
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-crx-green/20 focus:border-crx-green"
              />
            </div>
            <div className="w-24">
              ...
              <input
                type="number"
                value={newOverride.season}
                onChange={(e) => setNewOverride((p) => ({ ...p, season: e.target.value }))}
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-crx-green/20 focus:border-crx-green"
              />
            </div>
            <div className="flex-1">
              ...
              <input
                ...
                onChange={(e) => setNewOverride((p) => ({ ...p, notes: e.target.value }))}
                placeholder="Optional note"
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-crx-green/20 focus:border-crx-green"
              />
            </div>
            <Button size="sm" icon={<Plus className="w-3 h-3" />} onClick={handleAddOverride}>
              Add
            </Button>
          </div>
          ...
          {/* Override list */}
          {overrides.length === 0 ? (
            <p className="text-sm text-secondary italic">No customer overrides. All customers use the default rate.</p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 text-left text-secondary">
                    ...
                    <th className="pb-2 font-medium text-right">Rate/Acre</th>
                    <th className="pb-2 font-medium text-center">Season</th>
                ...
                <tbody>
                  {overrides.map((o) => (
                    <tr key={o.id} className="border-b border-gray-50">
                      <td className="py-2 font-medium text-nav-dark">{o.customer?.farm_name || 'Unknown'}</td>
                      <td className="py-2 text-right font-mono">
                        <Badge variant="warning">${formatCentsToDollars(o.rate_per_acre_cents)}/ac</Badge>
                      </td>
                      <td className="py-2 text-center">{o.season}</td>
                      <td className="py-2 text-secondary">{o.notes || '-'}</td>
                      <td className="py-2">
                        <button
