...
For multi-file or risky work, present a short plain-English goal, definition of done, and plan, and name the files or systems expected to change. Unless Mason asked only for a plan, review, diagnosis, or status report, begin ordinary reversible in-scope work immediately after stating the plan. Mason's request to build, fix, finish, audit, or handle the task is approval for the normal local edits, investigation, tests, worktrees, and other reversible work needed to complete that scope. Do not stop after planning or ask "Should I continue?" while another safe, in-scope step is available. Tiny, obvious, reversible fixes may proceed directly.
