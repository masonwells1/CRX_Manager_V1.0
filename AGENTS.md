...
- **Autonomous landing (Mason, 2026-09-26).** When CodeRabbit has APPROVED the frozen final head, a fresh exact-SHA Sol review of that head is clean, and every required check is green, the agent merges by itself and applies that change's NON-destructive migration through the migration-apply-guard proof gate (both reviewer proofs and a fresh content-bound Sol proof, each under 30 minutes), in any session, with no ask. The merge and apply gates enforce it.
...
- Every change follows `.claude/commands/ship.md`: PR → Luna rounds → checks green → `ready-for-coderabbit` → CodeRabbit APPROVED (fixes stay on the same PR) → Sol **last** → apply its migration, if any → exact-head merge.
