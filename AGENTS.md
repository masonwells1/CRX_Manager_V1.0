...
- Risky money, inventory, auth, RLS, migration, permission, or other business-critical changes require a fresh independent `gpt-5.6-sol` high-effort review of the exact candidate SHA. A green status row is not proof that a review occurred, and neither is a clean Luna round. Run Sol **last**, after Luna is clean: the proof is bound to the HEAD it reviewed, so any commit after it — including a one-line fix — voids it and needs a fresh pass. A Luna round must never be routed through the proof wrapper, which unlinks the existing proof for that HEAD when it starts.
