...
**Instruction priority.** When sources disagree, follow the higher one: (1) the hard gates below and the hooks that enforce them, which only Mason's explicit approval in the current conversation opens; (2) Mason's current message and its explicit limits; (3) the rest of this file; (4) the selected workflow in `.claude/commands/` or `.claude/skills/` (or its generated `.agents/` adapter); (5) routed reference docs; (6) decision-log history, handoffs, and older prose. A lower source never loosens a higher one. Name a real conflict in one line, follow the higher source, and fix the lower one when it is in scope.
...
- Regular reversible code follows the protected path in `.claude/commands/ship.md`: branch → PR → required checks → applicable exact-SHA review → resolved agent findings → CodeRabbit review of the frozen head → exact-head merge → proportionate production verification.
