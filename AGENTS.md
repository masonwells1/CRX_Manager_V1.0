...
Standing push policy (Mason, 2026-06-16; mechanics updated 2026-07-30): regular, reversible code may land on `main` without a fresh approval once the full pipeline is green — review clean, tests passing, and the pre-push hook's typecheck/build succeeding. Since 2026-07-14, `main` is protected by the GitHub `protect-main` ruleset: direct pushes are impossible for everyone, so landing work means **push a branch → open a PR → checks pass (Vercel required) → read and resolve CodeRabbit's automated review → merge**. This applies to Claude, Codex, and Mason alike. Risky diffs additionally require a fresh, separate, exact-SHA adversarial proof pinned to `gpt-5.6-sol` at high reasoning effort. A merge to `main` deploys production via Vercel; the one-click rollback there is the accepted safety net. This authorization covers ordinary code only and never extends to the gated actions below.

Standing CodeRabbit review policy (Mason, 2026-07-17; adversarial model updated 2026-07-30; enforcement posture updated 2026-08-24): every PR on the public `CRX_Manager_V1.0` and `FarmRx` repos is automatically reviewed by CodeRabbit (an AI reviewer; the org is on a paid Pro Plus plan as of 2026-08-24). Configuration lives in each repo's `.coderabbit.yaml`. **CodeRabbit config sources do not merge**: the repo file outranks the repository and organization dashboard settings, and any key it omits falls through to CodeRabbit's own defaults rather than to the dashboard, unless per-level "Inheritance" is enabled. Both dashboards are currently empty and inert — change review behavior in `.coderabbit.yaml`, never in the web UI. Before merging, whoever is landing the work **reads CodeRabbit's review and fixes any real issue it raises**; genuine nitpicks may still be dismissed with a one-line reason.

Since 2026-08-24 the review profile is `assertive` (report everything, filter afterwards) and `request_changes_workflow` is on, so CodeRabbit withholds approval until its comments are resolved **and the latest commit has been reviewed**. Five `mode: error` pre-merge checks encode the CRX Hard Rules — RLS on new tables, `SECURITY DEFINER` search_path, mutating-RPC idempotency, exact whole-cent money, and no edits to applied migrations. **State their force accurately: a failing check posts a red X and withholds approval; it does not disable the merge button.** `protect-main` requires exactly three contexts — `Vercel`, `Lint, Type Check, Test, Build`, and `SQL Migration Validation` — with `required_approving_review_count: 0` and CodeRabbit absent from that list. The status context name is confirmed to be `CodeRabbit`; promoting it to a required check is a deliberate branch-protection change that still needs Mason's explicit OK, and is intentionally deferred until the error-mode checks have run long enough to show their false-positive rate. The PR author can override a failing check via "Ignore failed checks" in the Walkthrough comment.

CodeRabbit is the broad every-PR pass; a separate exact-SHA `gpt-5.6-sol` high-effort proof remains the hard gate for risky money/RLS/migration diffs. Both run — neither replaces the other. Active adversarial gates do not require Claude/Fable unless Mason explicitly asks for it. Canonical text: `docs/manual/DECISION_LOG.md` (2026-07-30 and 2026-08-24 entries).
...
- Delegate only significant, independent, bounded work whose saved time or reduced context noise exceeds the coordination cost. Keep tiny fixes and tightly sequential work with the coordinator. Ad-hoc Codex delegation may use at most three active workers; canonical workflow-defined fan-outs keep their own tested limits and are not reduced by this default.
...
- Use the lowest-cost model currently exposed that can reliably perform the contract; do not assume a remembered model or capability is available. Routine implementation and tests may go to a mid-tier worker, narrow read-only scans to a fast worker, while architecture and final review of money, inventory, units, auth, RLS, migrations, and other business-critical behavior remain with Sol at high effort.
