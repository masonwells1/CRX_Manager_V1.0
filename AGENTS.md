...
- Iterating code review runs on `gpt-6-luna` at xhigh (Mason's standing tier decision, 2026-09-20; re-pinned from the retired 5.6 class to GPT-6 on 2026-09-23). Fix and re-run until it is clean — no BLOCKER or HIGH remains, and every MED/LOW is fixed, refuted with evidence, or listed by name as a deferral in the report to Mason; that is the whole review for ordinary reversible work. Escalate a round to `gpt-6-sol` early only for genuinely complex work, and say why.
- Risky money, inventory, auth, RLS, migration, permission, or other business-critical changes require a fresh independent `gpt-6-sol` high-effort review of the exact candidate SHA. A green status row is not proof that a review occurred, and neither is a clean Luna round. Run Sol **last**, once no BLOCKER or HIGH remains from the Luna rounds (deliberately deferred MED/LOW do not block it, and requiring a literal `CLEAN` would make the gate unreachable on any change carrying one accepted nit): the proof is bound to the HEAD it reviewed, so any commit after it — including a one-line fix — voids it and needs a fresh pass. A Luna round must never be routed through the proof wrapper, which unlinks the existing proof for that HEAD when it starts.
