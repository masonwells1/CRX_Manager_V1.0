...
Standing CodeRabbit review policy (Mason, 2026-07-17; adversarial model updated 2026-07-30): every PR on the public `CRX_Manager_V1.0` and `FarmRx` repos is automatically reviewed by CodeRabbit (an AI reviewer, free on public repos; config in each repo's `.coderabbit.yaml`, which overrides the CodeRabbit dashboard settings). Before merging, whoever is landing the work **reads CodeRabbit's review and fixes any real issue it raises**; CodeRabbit is advisory — it comments and does not block, and its nitpicks may be dismissed with a one-line reason. CodeRabbit is the broad every-PR pass; a separate exact-SHA `gpt-5.6-sol` high-effort proof remains the hard gate for risky money/RLS/migration diffs. Both run — neither replaces the other. Active adversarial gates do not require Claude/Fable unless Mason explicitly asks for it. A merge-blocking required status check for CodeRabbit is planned (Mason chose "process now, hard-block soon") and will be added to `protect-main` once its exact check name is confirmed on a live PR. Canonical text: `docs/manual/DECISION_LOG.md` (2026-07-30 entry).
...
- Money is bigint cents. Never use floating-point math for stored or calculated money. **Narrow exception (Mason, 2026-08-10):** 12 enumerated pre-existing order, quote, and commission money columns stay PostgreSQL `numeric` dollars — an exact decimal type, not a binary float — rather than being converted. Each one **must** carry an enforced whole-cent *and* finite-value CHECK constraint; the 5 that do not yet carry one are outstanding work, not compliant, and no part of this exception licenses leaving them unconstrained or treating them as guarded. The exception covers only those 12 named columns and is closed to new ones: any money column added from 2026-08-10 onward is `bigint` cents. Canonical text, the column-by-column status, and the exact predicate: `docs/manual/DECISION_LOG.md` (2026-08-10 entry).
