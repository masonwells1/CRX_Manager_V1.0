...
Standing push policy (Mason, 2026-06-16; mechanics updated 2026-08-30): regular, reversible code may land on `main` without a fresh **in-chat** approval once the full pipeline is green — formal GitHub approval remains mandatory, review must be clean, tests must pass, and the pre-push hook's typecheck/build must succeed. Since 2026-07-14, `main` is protected by the GitHub `protect-main` ruleset: direct pushes are impossible for everyone, so landing work means **push a branch → open a PR → finish implementation and checks → freeze the candidate commit → apply `ready-for-coderabbit` → resolve one CodeRabbit review → verify its formal exact-head approval → merge**. This applies to Claude, Codex, and Mason alike. Risky diffs additionally require a fresh, separate, exact-SHA adversarial proof pinned to `gpt-5.6-sol` at high reasoning effort. A merge to `main` deploys production via Vercel; the one-click rollback there is the accepted safety net. This authorization covers ordinary code only and never extends to the gated actions below.

Standing CodeRabbit review policy (Mason, 2026-07-17; automation updated 2026-08-30): automatic reviews are disabled in `.coderabbit.yaml` for both public repos, `CRX_Manager_V1.0` and `FarmRx`. Do not trigger CodeRabbit when a PR opens or while Codex/Claude is still implementing, reviewing, updating the branch, or pushing fixes. First bring the PR up to date and green with auto-merge OFF. After the candidate commit is frozen and the separate Codex review is clean, record the head SHA and apply the `ready-for-coderabbit` label. The default-branch workflow rechecks the live head, draft/conflict/auto-merge state, actor permission, required checks, and every reported non-CodeRabbit check. It then records `coderabbit-review-requested`, removes the ready label, and posts exactly `@coderabbitai review` once. The hidden SHA is a dedupe and operator-verification marker; the generic GitHub Actions identity is not a separate security principal and its comment/status must never be treated as merge authorization. A new commit, reopened PR, draft conversion, base or auto-merge change clears both labels and requires a fresh green candidate; duplicate label events do not post duplicate reviews. If the gate fails, it removes the ready label and posts nothing — correct the named blocker and relabel. A failed comment post is checked against the live PR: dedupe state is kept if the command landed, otherwise both labels clear for a deliberate retry. Read the resulting review and fix every real issue; genuine nitpicks may be dismissed with a one-line reason. Before merging, verify live `main` protection still requires the branch current, one current approval with stale-review dismissal, and last-push approval — administrators are deliberately exempt since 2026-09-01, so no agent may act on that exemption — then require all three SHAs to be identical: the gate marker, an authenticated `APPROVED` CodeRabbit review `commit_id`, and the PR's final `headRefOid`. Recheck every reported check and auto-merge OFF immediately before `--match-head-commit <that-exact-sha>`; ordinary green CodeRabbit or generic Actions rows are insufficient. If a finding or base update creates a new commit, restart required checks, rerun the exact-HEAD Codex proof when the corrected diff is Codex-worthy, freeze and record the new SHA, and apply the ready label again for one follow-up incremental review. Never use `@coderabbitai resume`, because it re-enables automatic reviews, and reserve `@coderabbitai full review` for a deliberately justified complete reread.
...
Since 2026-08-24 the review profile is `assertive` (report everything, filter afterwards) and `request_changes_workflow` is on, so CodeRabbit withholds approval until its comments are resolved **and the latest commit has been reviewed**. Five `mode: error` pre-merge checks encode the CRX Hard Rules — RLS on new tables, `SECURITY DEFINER` search_path, mutating-RPC idempotency, exact whole-cent money, and no edits to applied migrations. Since Mason's 2026-08-28 approval, GitHub requires one approval, dismisses stale approvals after every new commit, requires approval from someone other than the last pusher, and requires the branch to be up to date with `main`. **Mason's manual review override (2026-09-01):** classic protection no longer enforces those rules for administrators, so Mason — and only Mason, by hand on the PR page — can merge a PR whose review is stuck, using GitHub's "Merge without waiting for requirements to be met" control. The `protect-main` ruleset was deliberately left with an empty bypass list, so the required Vercel/CI/SQL checks still bind everyone: **the override skips the review, never the tests.** The bypass is granted by admin rights on Mason's account, which is the same token every agent session runs on, so both merge gates hard-deny `gh pr merge --admin` and now verify `reviewDecision === "APPROVED"` directly instead of inferring an approval from a CLEAN `mergeStateStatus`. No agent may use the override for any diff or any deadline; a stuck review is handed to Mason with the reason. That approval check is head-bound only because stale-review dismissal is on — if it is ever turned off, both gates must also check that an APPROVED review's `commit_id` equals `headRefOid`. **Those approval rules come from classic branch protection on `main`, not from the `protect-main` ruleset.** The ruleset contributes the deletion, force-push, PR-required, and status-check rules, but requires zero approvals and does not dismiss stale reviews, so removing classic protection on the belief that the ruleset is equivalent would silently drop the merge gate to zero approvals. Both mechanisms are live; read the real state with `gh api repos/masonwells1/CRX_Manager_V1.0/branches/main/protection` and `gh api repos/masonwells1/CRX_Manager_V1.0/rulesets` before changing either. CodeRabbit's formal final approval is the normal merge-unlock path; the existing required build and deployment checks remain separate and unchanged. The PR author can override a custom check via "Ignore failed checks" in the Walkthrough comment, but a current formal approval is still required before merge.
