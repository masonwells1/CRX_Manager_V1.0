...
- **RPC hardening (all three):** SECURITY DEFINER with `SET search_path = public, pg_temp`; `REVOKE EXECUTE … FROM PUBLIC, anon` then `GRANT EXECUTE … TO authenticated`; and an **in-body role gate as the first executable statement** (`require_admin()` or the admin/sales check) — the EXECUTE grant is not the real gate, the in-body check is.
- **Blend-ticket propagation** — when an application record is created from a blend ticket, auto-insert `application_record_lots` rows from the existing `blend_ticket_products.lot_number` so blend-sourced applications get lots with no re-typing. This is a **`CREATE OR REPLACE` of `create_application_record_from_blend_ticket`** (find the LATEST live version — migrations `20260609142548` / `20260610145350` area): **reproduce its current body verbatim and add ONLY the lot-insert**, then verify no new overload, `search_path` + idempotency intact. It is *behaviorally* additive but it is NOT a no-touch — give it full migration-drift care (the #1 historical bug class; copy the body exactly, do not summarize).
...
5. `npm run typecheck && npm run lint && npm run build && npm run test` all green. Component/unit tests cover the new RPCs + editor + trace (a regression test for any bug found).
...
- **Delete data** — meaning live/customer/business data. (The `set_application_record_lots` RPC deleting + reinserting *its own* `application_record_lots` rows for one record is normal internal save behavior, not this gate.)
