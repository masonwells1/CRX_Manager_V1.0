...
2. **Build** — implement just this phase. Honor CLAUDE.md Hard Red Lines + canonical patterns (money=cents, RLS, `checkMutationResult`/`assertRpcResult`, idempotency, strict-actor, `search_path = public[, extensions], pg_temp`, lazy pages, Lucide, single db client). PreToolUse hooks block bad writes — a block = a real defect.
