...
7. **✅ DONE — TRACK B, FIRST ITEM: the automatic "as-applied is X% off our system acreage" alert (owner's applicator-mixup catch). SHIPPED LIVE 2026-06-23** (Mason: "ship to main"; branch `feat/applied-acres-divergence-alert` → main merge `90177c75`; Vercel `dpl_8zpi1tbV5cRmKEAPY89U5d2r5ssP` READY, rollback `dpl_7yTXs3oFWDcSoK9PZ7Y5KTM749se`). On the Field Application Invoice billing page (`src/pages/FieldApplicationInvoice.tsx`), each field row shows the billable **System acres** on file next to the **Applied acres** input and auto-flags ±10% over/under (per-row badge + top-of-tab banner). Frontend-only (no migration); new tested helper `acreDivergence()` in `fieldGeometry.ts`; advisory only (never changes the bill). Open: Mason's in-app real-file smoke. The *rest* of Track B (the per-acre billing tie-in into as-applied-invoices) stays BLOCKED until Mason's word.
