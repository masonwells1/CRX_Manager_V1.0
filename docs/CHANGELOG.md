...
An independent Codex database review then reproduced four integrity holes in the unapplied migration: moving a share checked only the destination line, reparenting checked only the old invoice item, a newly created logical line could commit with no shares, and concurrent writers could each commit a full vector for a final 200%. Phase 1 now validates both OLD and NEW references, adds a deferred no-empty-line constraint trigger, runs deferred RLS-table validation as locked-down `SECURITY DEFINER`, and serializes vector checks with one transaction advisory lock (one global lock avoids cross-line lock-order deadlocks). The durable network-isolated PostgreSQL 17 proof in `scripts/smoke/prove-per-line-split-billing-phase1.mjs` reproduced all four failures before the repair and now passes all four, including a real two-session race. The feature flag remains OFF and nothing was applied live.
