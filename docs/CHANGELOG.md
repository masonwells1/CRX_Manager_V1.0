...
The applied pricing correction remains byte-immutable under `scripts/applied-migration-artifacts/`. Its active migration-path replay copy has the same schema effect and differs only by conditionally skipping the embedded row-trigger assertion when a clean catalog has no Product to update; the seeded disposable smoke separately proves the real cent-scale rejection. The repository also now carries the exact sources and rollback smokes for the already-live prepay/payment active-role gate (`20260718153744`), cancelled-order quote escape hatch (`20260718152837`), and paid-invoice void refusal (`20260718154810`), eliminating the live-registry/source gap.

PR #168 review then exposed latent payment-void and cancelled-order lifecycle defects. `void_payment` correctly retains allocation rows as history, but `void_invoice` treated those inactive rows as still-applied cash and would later delete them; transaction review also counted inactive history as a live credit. A cancelled whole-conversion quote could also be reopened after partial delivery or surviving financial activity, and three invoice entry points could create a new whole-order draft after cancellation; meanwhile the unused `restore_cancelled_order` RPC changed only the order status and rebuilt none of cancellation's inventory, hold, commission, or invoice effects. Forward migration `20260718193000_correct_payment_void_and_quote_retry_contract` is prepared but not yet live; it makes only active payment sets block voiding and reports, preserves payment-allocation history, retires the unused direct-payment and status-only order-restore RPCs, binds quote retries to exact intent, and blocks unsafe quote reopening. A serialized invoice trigger rejects deleted/voided orders and all cancelled-order billing except the audited completed-delivery recovery RPC, whose short-lived capability is stored in an owner-only, RLS-protected table; source lineage and those recovered items become immutable, and posting revalidates exact delivered quantities/cents. Exact cancel/void cleanup and accounting updates on already-posted invoices remain available. No production database change occurs until Mason approves that exact migration.
