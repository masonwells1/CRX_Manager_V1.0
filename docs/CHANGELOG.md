...
- The Phase 3 privacy guard now uses a syntax-independent catalog-row signature in every non-binary selected file (two UUIDs plus `id`/`product_id`, `product_name`, and `sku`): it has no SQL, path, extension, comment, or formatting bypass. The 16 pre-existing synthetic fixtures/tests/migration matches are permitted only by exact normalized path-and-whole-file canonical-LF SHA-256; platform LF/CRLF variations are equivalent, while any other content change refuses.
