...
PR #168 review then exposed latent payment-void and cancelled-order lifecycle defects. `void_payment` correctly retains allocation rows as history, but `void_invoice` treated those inactive rows as still-applied cash and would later delete them; transaction review also counted inactive history as a live credit. A cancelled whole-conversion quote could also be reopened after partial delivery or surviving posted/paid financial activity, while the unused `restore_cancelled_order` RPC changed only the order status and rebuilt none of cancellation's inventory, hold, commission, or invoice effects. Forward migration `20260718193000_correct_payment_void_and_quote_retry_contract` is prepared but not yet live; it makes only active payment sets block voiding and reports, preserves payment-allocation history, retires the unused direct-payment and status-only order-restore RPCs, binds quote retries to exact intent, and blocks unsafe quote reopening. No production database change occurs until Mason approves that exact migration.
