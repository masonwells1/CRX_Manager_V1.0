...
Built Phase 1 of per-line-item custom split billing on branch `claude/billing-splits-plan-8ih4jg`: additive migration `20260718210000` creating `field_app_billing_sets` / `field_app_billing_lines` / `invoice_line_shares` (one parent per non-null invoice group; indexed foreign keys; immutable micro-percent allocation snapshot; per-line Σ=100,000,000 constraint trigger; freeze-while-posted share and linked-item material-field triggers; billing-staff SELECT-only RLS/privileges), plus `invoices.send_disposition` ($0 not-to-send gate) and the `feature_per_line_split_billing=false` flag. The never-applied migration is strictly above the read-only verified live `20260718203206` high-water; production remains unchanged. Types added; typecheck/build/tests green. NOT applied to live — awaiting final exact-hash reviews, the §6.1 baseline billing cycle, and Mason's explicit apply approval. Spec: `docs/plans/per-line-item-split-billing-spec-2026-07-17.md`.
