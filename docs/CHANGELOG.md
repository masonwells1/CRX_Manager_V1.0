...
The final payment replay proof exposed an adjacent live regression in the July 14 shared idempotency trigger: `allocate_payment`'s versioned atomic claim could complete once, but every ordinary replay was intercepted before its cached-response branch and told to retry forever. Reviewed migration `20260716160000_fix_idempotency_committed_replay_guard` was applied as Supabase ledger version `20260716165801`. It restores committed replay only for the recognizable empty `allocate_payment_v1` claim while preserving the duplicate-work rollback error for legacy functions that write final idempotency results after mutation.

The final Sonnet 5 adversarial review found one delivery replay authorization gap and one bulk-PO identity edge case. Reviewed migration `20260716173342_authorize_delivery_before_replay` was applied as ledger version `20260716174220`: the public `complete_delivery` wrapper now validates canonical actor identity and the caller's current active admin/sales authority or assigned-driver relationship before the unchanged internal implementation can inspect a committed idempotency result. Direct execution of that internal implementation is revoked. Its pre-apply rollback run and post-apply live smoke both completed a real delivery, then reused the same committed key as a different active unauthorized user and received `Not authorized to complete this delivery`. Bulk PO import intents now include the original stable source-file index, so two distinct same-name/same-content documents import separately while a retry of either document reuses its own key and PO number. The permanent allocate-payment smoke registry description was also clarified to point reviewers at its already-existing exact committed-replay regression coverage.
...
Post-apply PO proof used a real active sales rep and ended in `SMOKE_PASS_ROLLBACK`: direct submitted creation and save-based status changes were rejected, draft save/edit and both idempotent replays succeeded, dedicated submission and cancellation succeeded, integer-cent totals were exact, and a non-office user was denied. The delivery, invoice, AP/finance/prepay, inventory-position, and full allocate-payment/customer-statement rollback chains also passed against the final live functions. All 17 database invariant sweeps returned zero unallowlisted findings, and the schema registry was rebuilt from live introspection to high-water `20260716184406` with the new generated cents columns and applied migration name (also incorporating the overlapping CRM session's already-live migrations).
