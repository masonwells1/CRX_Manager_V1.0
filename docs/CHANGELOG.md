...
PR #168 review then exposed a latent payment-void lifecycle defect: `void_payment` correctly retains allocation rows as history, but `void_invoice` treated those inactive rows as still-applied cash and would later delete them; transaction review also counted inactive history as a live credit. Forward migration `20260718193000_correct_payment_void_and_quote_retry_contract` is prepared but not yet live; it makes only active payment sets block voiding and reports, preserves all payment-allocation history, retires the unused direct-payment RPC under a locked empty-table precondition, and moves quote-status retries onto the shared advisory-lock helper with exact action binding. No production database change occurs until Mason approves that exact migration.
