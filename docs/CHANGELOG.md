...
A later fresh Sol `FIX` rejected pre-repair candidate `b2d55f77`: NIT wording
could still hide explicitly required work, and containment did not cover
quoted-printable, percent/form, SQLite, Product SQL column-list, one-layer
nested transfer, valid deletion-record, or noncanonical ignored-backup cases.
The cleanroom repair adds bounded three-level transfer decoding, canonical-only
ignored local paths, and regressions for
every reproduced seam. These repairs are uncommitted proof work, not an
accepted Phase 3C candidate; PR #246 remains PARKED pending a new exact-SHA
packet and fresh required reviews.
...
ever touching the parent vendor row, so it did not serialize with `delete_vendor` — interleaved,
a vendor could be soft-deleted while carrying an open AP liability. The fix re-emits the live
function body byte-identically (md5-verified against live `pg_proc`) plus one added gate: after
locking the payment and bill, it locks the vendor row with the same `deleted_at IS NULL … FOR
UPDATE` predicate `delete_vendor` uses, raising `VENDOR_DELETED` if the vendor is already
soft-deleted. Lock-order analysis in the migration header shows no deadlock cycle. Deliberate
strict consequence: payments on a deleted vendor's bills cannot be voided until the vendor is
restored (no restore RPC exists yet — flagged as a possible follow-up).

Governance: the first `write-apply-proofs.mjs` run returned one BLOCKER (the migration lacked its
own EXECUTE revocation); after restating the exact live ACL both reviewer charters returned CLEAN,
and Mason approved in-chat ("I approve when get verdict back to make it all live you have full
authority"). Applied via the sanctioned MCP path; Supabase assigned ledger version
`20260726201208` and the disk file was B7-renamed to match. **Post-apply proof:** the migration's
in-transaction verification passed; live body md5 exactly matches the on-disk function body; the
vendor-liveness gate, `VENDOR_DELETED` error, and period gate are all present live; ACL unchanged
(anon still denied). Migration-history row 823, `KNOWN_ISSUES.md`, the pending-migration test
gate, and the schema-registry high-water were updated in the same change.
...
## 2026-07-22 — Supplier Pricing Phase 3 Stage A prepared (later applied live, still dormant)
...
- Generated a private 604-row provisional classification manifest: every row remains unresolved and pending owner review; 21 name-only no-return candidates are evidence flags only. The raw snapshot and manifest are excluded from this public repository; only aggregate counts and their verification checksums are published. After Stage A is separately applied, a future Stage-A-aware generator must capture the new schema; the pre-Stage-A generator cannot consume it. Any later classification migration must route each Product through the lock-taking `set_product_phase3_metadata` RPC rather than update governed columns directly. Manifest SHA-256: `57da3f92d2e02253f4be7fa88a8626a3191bdbbc13ab140a57f33af6e84f7b91`. Final exact-SHA Opus and Sol verdicts are external PR evidence and are required before push or acceptance; this changelog does not self-certify them.
- Preparation itself performed no Product mutation, feature-flag enablement, merge, or AI/OCR supplier-PDF extraction. The reviewed migration was later applied live on 2026-07-23; it remains dormant pending a separately approved classification/enablement stage.
