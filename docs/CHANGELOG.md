...
- **Landing-time Codex review — 2 low-severity findings accepted with a tracked follow-up (SHIP-WITH-FOLLOWUPS):** a fresh Codex review of the diff surfaced two safe-failing issues in the already-live SQL that the build-time review missed — (P1) M4's partial-batch `financial_audit_log` records the full requested `invoice_ids` array even for failed posts, on a **zero-caller** RPC that can't run in prod; (P2) Feature A can skip the auto-split when two same-day final deliveries of one allocated order race, falling back to the pre-existing **manual** split-billing (no misbill). Both accepted as low-risk; a careful, separately-reviewed follow-up migration will fix them (P1: store only posted IDs; P2: lock the order row before the all-delivered check).
