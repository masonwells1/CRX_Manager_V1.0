...
Added a network-isolated PostgreSQL 17 proof harness (`scripts/smoke/prove-per-line-split-billing-phase2.mjs`) that loads the checked-in Phase 1 + Phase 2 migrations into a uniquely named tmpfs Docker database and rolls every fixture back. It proves feature-OFF delegation, 50/50, exact three-way, 1¢ allocation, full-precision conversion with one final money round, signed half-cent −13¢ → −7¢/−6¢, manual/quote/tier/service price precedence, per-person override, 100/0 service with the zero row retained, flat fee, snapshot/default/fallback precedence, stable hashes, Mode A rejection, malformed-vector rejection, role denial, one public preview overload, and a non-browser-executable private calculator. A spec reconciliation caught and fixed one real 1¢ defect before closeout: same-price money had briefly multiplied the rounded 4dp quantity instead of the full-precision converted quantity. The regression `1.00004 × 12,500¢ = 12,501¢` now pins the rule; independent Codex delta review returned `APPROVE`.

**PARKED / not shippable yet:** PR #166's inherited Phase 1 migration contains invalid PostgreSQL `COMMENT ON COLUMN ... IS 'literal' || 'literal'` syntax. The Phase 2 proof normalizes only that exact comment in memory for diagnostics and reports the blocker; the checked-in Phase 1 file is not edited here. Phase 1 objects and the feature flag remain absent from live Supabase, the §6.1 real baseline billing cycle is not confirmed, and two read-only Claude wrapper attempts (Opus 4.8 and Sonnet 5) completed with zero permission denials but returned empty verdicts, so Claude provenance is `BLOCKED`, not clean. Live invariant sweeps also found one unrelated pre-existing quote commission row (`Q-2026-2059`, blank recipient); no live data was changed. No migration was applied, no flag was flipped, and Phase 3 was not started.
