...
All six invoice-adjacent send points now re-read `invoices.send_disposition` immediately before sending, and the prepared `send-email` edge function independently requires an invoice resource, validates its customer, and refuses anything except `sendable` inside the server boundary. The edge change is committed only for later owner-controlled deployment. Contract tests pin stored-cent PDF/statement/customer-summary rendering and prohibit quantity-times-price recomputation. This branch applied nothing and did not deploy the edge function. A final read-only live check found that another workflow had concurrently applied older inactive split-schema/calculator/save migrations at ledger versions `20260720213000`, `20260720214000`, and `20260720233000`; the live feature-flag row is absent, the split tables contain zero rows, and production still exposes only the legacy four-argument preview with no `save_field_app_invoice_per_line`. The reviewed pending Phase 1–3 chain therefore requires a separate drift reconciliation and fresh timestamps above live high-water `20260720235246` before any promotion.

The first exact-commit Codex review found three release blockers. The enabled preview payload used `p_options` instead of the calculator's actual `p_line_overrides` argument; proof notices could omit the invoice resource to bypass the edge disposition lookup; and a legacy un-split invoice with several grower recipients could fail every secondary proof send. The preview key now matches the migration. For proof emails, the edge derives the job/customer from the server-created notification idempotency key and then gates the matching child invoice—or the sole legacy source invoice—while preserving pre-invoice Job-page notices. The browser uses the same sole-invoice fallback for legacy shared invoices. All three findings have focused regressions and require a fresh exact-HEAD review.

The first HEAD-bound risky-diff proof then found that `sendable` alone did not retire a cancelled or voided invoice. Both browser and edge gates now reject cancelled, voided, or soft-deleted invoices; the server-derived proof lookup excludes retired rows before selecting the recipient child. Focused lifecycle regressions are green, and the HEAD-bound proof must be regenerated on the repaired commit.

The Phase 4-only reconciliation review also asked that the JobDetail customer-resource path be made explicit. Post-application notices now compare a caller resource ID with the server-derived invoice only when the caller actually supplied `resource_type='invoice'`; customer-resource notices remain valid and are still gated by the invoice derived from the server-owned notification record.

The fresh Claude review found two real enabled-path gaps. Job-backed preview/save payloads did not carry the frozen `job_chemical_id` values required by the shared calculator, and ungrouped single-customer drafts stopped rehydration because the browser looked up billing sets only by `invoice_group_id`. The invoice loader now matches each ordered invoice item to its server-loaded job-chemical source, sends those IDs in both RPC payloads, and fails closed if any source cannot be verified. Ungrouped drafts now resolve their one billing set through invoice item → line share → billing line, while grouped drafts retain the direct group lookup. Typecheck and all 57 focused Phase 4 contracts pass. The same review's email-column deployment concern was resolved by the earlier read-only live catalog check: production already has `invoices.send_disposition`; no live change was made.
...
The network-isolated PostgreSQL 17 proof loads the exact five-migration chain and rolls back all fixtures. It persisted a real 100/0 service split as two invoices/items/shares, rebuilt existing grouped and single-recipient invoices with restrictive child foreign keys in the correct delete order, posted the zero-dollar child with `suppressed_zero_total` and an immutable snapshot, proved required idempotency plus exact replay and changed-payload conflict, rejected a corrupted header, blocked the legacy transfer path while enabled, and retained owner/definer/search-path/ACL guards. Production remains unchanged: the feature flag is OFF/absent, no split migration is applied, and the read-only live high-water was `20260720203000` at the Phase 3 review.

Mason authorized a fresh Phase 3 review cycle on 2026-07-20. The two prior round-3 blockers are now fixed: the writer authorizes the concrete job/invoice/customer scope before any cached result can be returned, binds cached replay to the authenticated actor, and stamps both new and rebuilt child invoices with the source job season used by the calculator. The rollback harness now proves cross-actor replay rejection and a 2025 source job retained on invoices created in 2026. Fresh exact-SHA Codex and Claude reviews are required before the draft PR; Phase 4 begins only after that clean handoff.

The first paired review exposed a production-fixture mismatch: calculator provenance labels such as `job_snapshot` and `application_service_default` were being written directly into the legacy `invoice_items.price_source` column, whose live CHECK permits only `quoted`, `tier`, or `manual`. The writer now maps detailed provenance to that legacy vocabulary while preserving the full label in `invoice_line_shares.base_price_source`; the disposable schema carries the exact CHECK and proves the service path stores `manual`. The same repair round retires an empty source draft when a one-recipient invoice is expanded into a multi-recipient group. `field_app_location_shares.amount_cents=0` remains the established legacy behavior because that table describes field-acre ownership; per-line money is authoritative in `invoice_line_shares`, and Phase 4 renderers must read its stored cents. Draft re-saves continue to use activity history while the financial audit records newly created invoice entities.

The fresh reconciliation review then found one more destructive-conversion risk: a caller could pass a legacy draft that had never been created by the per-line engine, causing its existing item graph to be replaced. The writer now requires a matching server-created `field_app_billing_sets` record before editing any existing single invoice or group and rejects legacy conversion. The PostgreSQL 17 rollback proof creates a legacy item, observes the refusal, and confirms the original cents and item remain intact. Exact-HEAD Codex review is clean at `5b92c716`; draft PR #183 is mergeable with Vercel and CodeRabbit green.
...
Added a network-isolated PostgreSQL 17 harness (`scripts/smoke/prove-per-line-split-billing-phase2.mjs`) that loads the exact checked-in Phase 1 + Phase 2 migrations verbatim into a uniquely named tmpfs Docker database and rolls every fixture back. It exercises feature-OFF delegation, 50/50, exact three-way, 1¢ allocation, full-precision conversion with one final money round, signed half-cent −13¢ → −7¢/−6¢, exact job-chemical snapshots, customer-supplied zero billing, non-job manual/quoted/tier and source-season service pricing, per-person override, 100/0 service with the zero row retained, flat fee, snapshot/default/fallback precedence, stable hashes, Mode A rejection, malformed-vector rejection, role denial, one public preview overload, and a non-browser-executable private calculator. A spec reconciliation caught and fixed one real 1¢ defect before closeout: same-price money had briefly multiplied the rounded 4dp quantity instead of the full-precision converted quantity. The regression `1.00004 × 12,500¢ = 12,501¢` pins the rule.

The first exact-SHA branch review timed out while tracing quote-price provenance, but that trace exposed a real mispricing path before closeout: the calculator searched every quote section sharing an input field/product and selected the lowest quote-item UUID. A later complete branch-wide review found the deeper source-of-truth problem: job-backed lines recalculated caller quantities and current quote/tier prices instead of using frozen `job_chemicals`; same-product quote rows were therefore ambiguous, customer-supplied chemicals could be charged, and service rates used `current_season()` instead of the job/invoice season. New red proofs reproduced browser-tampered 98/99-unit plans, two same-product job rows receiving the wrong price, customer-supplied charges, an outside-job field, and prior-season service repricing. The calculator now requires the complete exact `job_chemical_id` set, derives product/quantity/price/customer-supplied state from those frozen rows, rejects fields outside `job_fields`, forces customer-supplied lines to $0, and resolves service rates from the source job/invoice season. Non-job drafts preserve manual→quoted→tier pricing and now reject conflicting selected-field quote prices rather than guessing. The exact two-row regression now returns 2.0000 × 150¢ and 3.0000 × 275¢ regardless of tampered caller fields or duplicate quote products.

The next complete exact-branch review found a real partial-job-field allocation gap and real cross-table persistence gaps; each received a failing PostgreSQL proof before repair. Its separate unit-conversion finding was disproved by current source: `job_chemicals.quantity`, `unit`, and per-unit cents are one frozen measure. `JobDetail` uses `reconcileChemAutofillUnits` to convert quantity and cents together, quote-to-job copies `quote_items.price_unit` into `job_chemicals.unit`, and the established transfer path multiplies the frozen pair directly. Converting only the quantity again would underbill valid jobs, so a regression now preserves `1,600 oz × 25¢/oz = 40,000¢` even when the product inventory unit is gallons. A job-backed request must include the exact complete `job_fields` set, because the schema has only a job-wide chemical quantity and cannot safely reallocate it across a caller-selected subset. New additive migration `20260718210500` closes Phase 1's cross-table relationships without editing the original migration: deferred, locked, owner-only guards require share customer = child invoice customer, share amount = item extension, exact group-customer membership on every line, source cents/quantity/acres reconciliation, no unshared child items, invoice job identity, and header total = item cents. The same complete assertion runs synchronously before a split invoice posts, ahead of the immutable snapshot trigger. The network-isolated proof now also corrupts each relationship and sees the expected fail-closed error; the feature remains OFF and none of these migrations are live.

The exact-SHA review of that repair then found three further authority/relationship gaps, all reproduced with red proofs before repair. A job-backed payload could still promote a browser `manual_override` over the frozen `job_chemicals` price, and an arbitrary active application service could replace the job/invoice service. Job-backed prices are now rebuilt strictly from the frozen row, while service identity is derived from and cross-checked against the source job and invoice. Durable shares now must point to live invoices inside their billing set's exact group; an ungrouped set must use one common child invoice. The child item must also match its share's quantity, rounded two-decimal invoice acres, effective unit price, chemical product, and service identity in addition to its extended cents. The disposable proof corrupts each relationship independently, and container-removal failure is now fatal rather than a warning. Exact-SHA review is being rerun on the repaired bytes; no production state changed.

That exact-SHA rerun at `fd547bf5` found two final HIGH fail-closed defects plus one schema-drift hardening gap. Production permits nullable `job_chemicals.price_per_unit_cents`, but the proof fixture had declared it non-null and unresolved snapshot/tier prices were normalized to $0. The fixture now matches production, and missing or negative chemical/service prices raise before any line can be returned. Phase 2 now also aborts installation unless the server flag exists and is exactly `false`, preventing a stale `true` row from routing traffic into the new engine. New additive preflight migration `20260718205500` runs before the byte-stable Phase 1 migration and refuses any stale per-line table, index, or `invoices.send_disposition` column, so Phase 1's `IF NOT EXISTS` clauses cannot preserve weaker definitions by name. The isolated proof reproduces each failure; migrations remain unapplied and the flag remains OFF.

The next exact-SHA review at `81ca0c24` found that the calculator froze job-chemical identity, pricing unit, and COGS but the durable line/post history did not yet preserve all three; it also found that a $0 split child could remain `sendable`. The additive relational-guard migration now stores and hashes source job-chemical ID/unit/per-unit cost, requires invoice-item unit/COGS and invoice-header cost to reconcile, freezes material item and logical-line fields while posted, and captures both source and item values in immutable post history. Zero-dollar suppression is now exact: zero split children must be `suppressed_zero_total`, nonzero children must be `sendable`, and the same complete assertion runs before posting. Disposable proofs cover draft corruption, post-time mutation, immutable snapshot contents, zero-sendable rejection, and a valid posted suppressed-zero record. Phase 1 remains byte-stable; the flag remains OFF and nothing is live.

The exact-SHA review at `f105ab2b` then found three Phase 2 release blockers and one audit-hash gap. The public `SECURITY DEFINER` preview role-gated sales reps but did not preserve invoice/job/field ownership; invoice/job matching accepted a supplied job when the invoice's `job_id` was null; non-job chemicals skipped the settled quote tier in manual → quoted → tier precedence; and the calculation hash omitted material identity/provenance/reason fields. Each case failed first in the disposable PostgreSQL proof. The calculator now gates invoice, source-job, and field-only contexts to the owning/assigned rep, binds invoice/job IDs with null-safe exact equality, binds direct invoices to their primary customer's fields, accepts exactly one distinct selected-field quote price while rejecting conflicting quotes, and hashes a canonical server-resolved line/share JSON record including product/service identity, base-price source, override modes, and reasons. The combined proof is green; the feature remains OFF, no migration is applied, and a new exact-SHA review is required before push.

The next exact-SHA review at `64766630` found two further HIGH persistence defects. The relational validator proved only source lines that already existed, so a Phase 3 writer could omit an entire job chemical or the job's required application service and still reconcile every remaining cent. Flat-fee shares also returned `allocated_quantity=NULL` while production `invoice_items.quantity` is non-null; the old disposable fixture was incorrectly nullable and hid that interface failure. New red proofs reproduced both. The guard now proves the inverse job-source set, requires the exact single job service line, and rejects missing/substituted rows. Flat-fee source lines and every child fee item now use quantity `1`; their cents still split by largest remainder. The harness persists the exact calculator JSON into production-shaped durable tables and rejects quantity tampering. The combined PostgreSQL 17 proof is green again; both edited migration hashes require fresh content-bound and exact-SHA reviews before any push.

The exact-SHA review at `63df9f58` then found three HIGH release blockers, each reproduced before repair. Child invoice COGS independently rounded frozen unit cost × allocated child quantity, so a 1¢ source split 50/50 became 2¢; non-job quote lookup could select a stale or unrelated quote sharing only field/product; and the new `SECURITY DEFINER` wrapper would trust any pre-existing private legacy helper. Source COGS is now rounded once, stored durably, and allocated by its own deterministic largest-remainder cents pass; child headers sum that ledger exactly. Quote pricing now requires one eligible quote header bound to the share customer, source season, active lifecycle, and unexpired sent/revised state (accepted quotes remain eligible), with both price and quote-source ambiguity rejected and provenance recorded as `quoted:<quote_id>`. Phase 2 now aborts unless the live-approved public four-argument legacy preview has the exact body hash, owner, language, volatility, pinned search path, and ACL, and it separately refuses a pre-existing private target. The network-isolated proof exercises malicious-helper drift, public-body drift, stale/declined/deleted/expired/wrong-season quote exclusion, same-price/different-source ambiguity, and a 1¢ COGS source split into 1¢/0¢ children. The combined proof is green. The calculator migration's content-bound security and drift reviews are clean. The guard migration's security review is clean and its drift reviewer found no source/type mismatch, but correctly withholds approval until prerequisite Phase 1 exists live and the registry can confirm its 52-column snapshot insert. A new commit-SHA review remains required before push. The flag is still OFF and production is unchanged.

The exact-SHA review at `b7b35f33` found that negative return plans still could not cross the future Phase 3 persistence boundary: preview calculated −25¢ COGS, but the new durable total/allocation columns rejected negative values, and the COGS allocator used `floor()` directly on signed ideals, reversing the settled customer tie-break (`−12/−13` instead of `−13/−12`). It also found the legacy preview TypeScript union omitted the new `flat_fee` result kind. A red PostgreSQL proof first reproduced the reversed signed COGS. The durable totals now accept signed cents while keeping the frozen per-unit cost non-negative, COGS uses the same ABS/sign largest-remainder algorithm as sales, and the exact −13¢ sales / −25¢ COGS return plan persists as child sales `−7/−6` and child COGS `−13/−12` through every deferred relational guard. The TypeScript union now includes `flat_fee`, and its type-contract regression enumerates every server-emitted line kind. The combined proof is green again. Fresh content-bound security and drift reviews are CLEAN for the calculator; guard security is CLEAN, while guard drift remains intentionally blocked only because the prerequisite Phase 1 snapshot table is not live, with all 52 persisted columns confirmed against pending DDL and TypeScript. Fresh exact-SHA reviews remain required for the new commit.

The exact-SHA review at `1af4cbc2` found two more HIGH non-job money defects. Quote lookup used `quote_items.price_per_unit` without its paired `price_unit`, so a `$10/quart` quote on a gallon product could be billed as `$10/gallon`; product lookup also coerced `products.current_cost=NULL` to zero before the existing required-cost guard. Red database proofs first reproduced the `$10` total for one quoted gallon and the missing-cost plan returning without error. Eligible quote prices are now normalized through `field_app_priced_quantity` into the durable product inventory unit before ambiguity comparison, selection, or multiplication (`$10/quart` → `$40/gallon`); unconvertible quote units fail closed. Missing cost now remains NULL and raises `PER_LINE_CHEMICAL_COST_REQUIRED`, while an explicitly stored zero cost remains valid. The combined disposable proof is green, and fresh content-bound security and drift reviews are CLEAN on these exact migration bytes. A new commit-SHA review is still required before push.

The exact-SHA review at `9a17f0fd` found two further HIGH trust/quote-coverage defects. The privileged calculator reused five predictable `pg_temp` relations through `IF NOT EXISTS`, so an authenticated caller could pre-create a same-named object before entering the `SECURITY DEFINER` frame. It also allowed one selected field's quote to price a customer's aggregate chemical quantity from other selected fields, even when the customer had no share on the quoted field. Authenticated-role and two-field red proofs reproduced both paths. Each call now rejects any caller-owned helper relation and drop/recreates only calculator-owned temp tables, preserving safe repeated previews in one transaction. A server-derived required-field set now ties quote candidates to actual field/customer contribution; field-default lines require every contributing field, custom/even nonzero shares require every selected field, zero-share-field quotes are ignored, and partial coverage raises `PER_LINE_QUOTE_FIELD_COVERAGE_INCOMPLETE`. The full disposable proof is green again, and fresh content-bound security and drift reviews are CLEAN on the new bytes; a new commit-SHA review remains required.

The exact-SHA review at `e2dbe602` found two more HIGH durable-integrity defects and one MEDIUM migration-drift gap. A writer could coherently double a job-backed line's stored source quantity, child quantities, sales, COGS, and invoice headers because the validator did not compare the durable quantity with `job_chemicals.quantity`; the same balanced rewrite could relabel the frozen base price/provenance. A `flat_fee` line could also use `per_person_price`, bypassing canonical source-cent reconciliation, while `CREATE OR REPLACE` functions were not preflighted for same-signature owner collisions or rechecked after installation. Each path failed first in the network-isolated PostgreSQL proof. The additive guard now binds rounded durable quantity plus every share's base price/source to its frozen job row, keeps default effective price equal to base unless an explicit reasoned override is used, and couples `flat_fee` kind/basis on working and immutable-history rows. The preflight rejects every feature-owned same-signature function, and post-install guards assert exact owner, definer mode, volatility, language, pinned search path, and role ACLs for the trigger/calculator chain. Phase 1 remains byte-stable; the combined proof is green, the feature remains OFF, nothing is applied live, and fresh content-bound plus exact-SHA reviews are required before push.

The Phase 1 parser and invariant blockers were repaired on PR #166 and merged into this branch before final Phase 2 verification. The combined checked-in migrations now run verbatim; the diagnostic-only Phase 1 workaround is no longer used. Phase 1 objects and the feature flag remain absent from live Supabase, the §6.1 real baseline billing cycle is not confirmed, and the flag stays OFF. Live PostgreSQL was independently confirmed as 17.0.6. Live invariant sweeps also found one unrelated pre-existing quote commission row (`Q-2026-2059`, blank recipient); no live data was changed. No migration was applied and no flag was flipped.
...
Repaired the unapplied Phase 1 migration's final `COMMENT ON COLUMN invoices.send_disposition` statement: PostgreSQL does not allow string concatenation after `IS`, so the three fragments are now one literal. A network-isolated PostgreSQL 17 parser probe reproduced the original `syntax error at or near "||"` and accepted the corrected statement while preserving the exact comment text. Production remains unchanged; live read-only counts show the Phase 1 objects/flag are absent and the required legacy field-app billing baseline has not yet occurred.

An independent Codex database review then reproduced four integrity holes in the unapplied migration: moving a share checked only the destination line, reparenting checked only the old invoice item, a newly created logical line could commit with no shares, and concurrent writers could each commit a full vector for a final 200%. Phase 1 now validates both OLD and NEW references, adds a deferred no-empty-line constraint trigger, runs all invariant validation as locked-down `SECURITY DEFINER`, and serializes vector checks with one transaction advisory lock (one global lock avoids cross-line lock-order deadlocks). A fail-closed preflight refuses to install the FORCE-RLS validators if the migration owner lacks `BYPASSRLS`; live read-only catalog evidence confirmed the real Supabase migration role is `postgres` with `rolsuper=false` and `rolbypassrls=true`. The durable network-isolated PostgreSQL 17 proof in `scripts/smoke/prove-per-line-split-billing-phase1.mjs` reproduced all four integrity failures plus the incomplete trigger-security posture before repair and now passes every case, including a real two-session race. The feature flag remains OFF and nothing was applied live.

A later full Codex pass reproduced four more authority/immutability gaps before release: parent `ON DELETE CASCADE` paths could erase posted share history, inherited table privileges left `TRUNCATE` available to `authenticated`, browser updates could change the supposedly server-controlled `send_disposition`, and a nonzero invoice could be marked `suppressed_zero_total`. The unapplied migration now uses restrictive share-parent foreign keys, revokes all client table privileges before granting authenticated SELECT only, protects disposition changes with a SECURITY INVOKER/BYPASSRLS-authority trigger, and constrains suppression to zero totals. Each defect failed first and now passes in the same disposable proof; a valid server-side zero-total suppression remains allowed. Production remains unchanged.

The next exact-commit Codex review found two additional release blockers: the parent-table SELECT policies let every authenticated applicator see unrelated billing-set identifiers and source prices, and the two override-reason constraints accepted whitespace-only text. Both were reproduced first in the disposable proof. Parent/source-line reads are now limited to billing staff (`admin`/`sales_rep`) until an applicator assignment boundary is deliberately implemented, and custom split/price overrides require at least one non-whitespace character. The proof now exercises both cases directly.

The following exact-commit review found one more concurrency/idempotency ambiguity: the ordinary invoice-group index allowed two durable billing-set parents for the same non-null `invoice_group_id`. A failing proof now creates that duplicate explicitly; the migration replaces the ordinary index with a partial UNIQUE index, so ungrouped drafts may still have NULL group IDs while a real group has exactly one parent.

The next exact-commit review reproduced a freeze-boundary race: a share mutation could read a draft invoice, then commit after another transaction posted it. The share freeze trigger now locks every affected invoice row in stable ID order before rechecking status, matching the posting lock boundary. The durable proof starts a real share writer and concurrent poster; it failed before the repair and now proves the poster cannot cross the in-flight share mutation.

The final exact-commit pass reproduced a parent-reparent bypass: an authorized `invoice_items.invoice_id` update could move a shared item from a posted invoice to a draft invoice, after which the supposedly frozen allocation became editable. A new locked-down trigger makes the invoice parent immutable while an item has a split-billing share (draft workflows may deliberately delete/rebuild shares first; posted workflows cannot), and share mutations now lock affected item rows before invoice rows so a concurrent insert cannot validate one parent and follow the item onto another. The exact bypass failed first and now passes in the disposable proof.

The subsequent exact-commit pass reproduced one final snapshot-divergence path: a privileged writer could change a posted split item's quantity, acres, unit price, or extended cents while its allocation share remained frozen. The linked-item trigger now locks the invoice posting boundary and rejects those four material-field changes after posting while preserving ordinary draft edits. The same review found five uncovered foreign-key indexes (`primary_customer_id`, both new `created_by` references, `product_id`, and `application_service_id`); all now exist. Permanent disposable tests failed on all four material updates before repair and now prove the posted rejection, draft-edit success, and `5/5` index posture.

The following exact-hash review found that reversible unpost/edit/repost could overwrite the only working allocation record, losing what had actually been posted. Phase 1 now includes `invoice_line_share_post_snapshots`: each transition into a posted state captures a complete, self-contained copy of the share, source line, child item, invoice header, hashes, actor, and post sequence. History has no foreign-key dependency on editable working rows, rejects privileged UPDATE/DELETE/TRUNCATE, and remains intact after an unposted rebuild deletes those rows. The disposable proof failed before the table existed and now preserves two distinct post cycles. The same review's proof-harness finding is closed: a nonzero Docker cleanup result now fails the run instead of reporting a false PASS.

The next exact-hash review found that the two-session vector proof accidentally placed both child items on one invoice, allowing the invoice-row lock to mask whether the global vector lock really prevented cross-invoice write skew, and that immutable history omitted the billing set's rounding-policy version. The fixture now races shares on two distinct child invoices and still commits exactly one 100% vector. Every post snapshot also copies `rounding_policy_version`; the repost proof changes the working policy from v1 to v2 and preserves both versions beside their original cents after all working rows are deleted.

The final pre-push review then reproduced two release blockers. First, `is_sales_rep()` granted every sales rep global read access to every new billing set and source line; the policies now require the rep to be the set creator or the creator/salesman on a child invoice in that set's group. Second, a split invoice could change to `posted` with `posted_at=NULL`, skip the history trigger, and still freeze its editable rows. The snapshot trigger now rejects any split invoice whose posted-state and timestamp disagree before it can cross that boundary. Both cases failed first in the disposable proof and now pass: an unrelated rep sees `0|0` sets/lines while the owner sees `1|1`, and the timestamp-less post aborts with no state change.

The next exact-HEAD review caught a lifecycle regression in that timestamp rule: the existing `void_invoice` RPC deliberately preserves `posted_at` when a posted/paid/overdue invoice becomes `voided`. The rule now recognizes `voided` as a post-derived terminal state that retains its timestamp but does not create a second post snapshot. A new regression proof failed before the repair and now ends `voided|true|1` (status, timestamp present, one immutable post history row).

The following exact-SHA review found three more fail-closed gaps. The working-share and post-history RLS policies trusted invoice ownership without rechecking `is_sales_rep()`, so a deactivated rep with a retained session could still read allocation prices and history; both policies now require a current active sales role, and the proof changes the same actor from active (`1|1` visible shares/history) to inactive (`0|0`). Share and linked-item material triggers also treated only posted/timestamped invoices as frozen, allowing a cancelled invoice to receive or alter allocation data; every state except clean `draft`/`unposted` is now immutable. Finally, the invoice suppression CHECK used a globally non-unique constraint name; the migration deliberately recreates the table-local exact constraint, and a same-named decoy now coexists at `1|1` while nonzero suppression is still rejected. All five unsafe outcomes failed first and now pass in the disposable PostgreSQL proof.

Built Phase 1 of per-line-item custom split billing on branch `claude/billing-splits-plan-8ih4jg`: additive migration `20260718210000` creating `field_app_billing_sets` / `field_app_billing_lines` / editable-while-draft `invoice_line_shares` / append-only `invoice_line_share_post_snapshots` (one parent per non-null invoice group; indexed foreign keys; exact micro-percent vectors; per-line Σ=100,000,000 constraint trigger; freeze-while-posted share and linked-item material-field triggers; durable unpost/repost history; owner/assigned-invoice-scoped sales-rep SELECT plus admin access), plus `invoices.send_disposition` ($0 not-to-send gate) and the `feature_per_line_split_billing=false` flag. The never-applied migration is strictly above the read-only verified live `20260718203206` high-water; production remains unchanged. Types added; typecheck/build/tests green. NOT applied to live — awaiting final exact-hash reviews, the §6.1 baseline billing cycle, and Mason's explicit apply approval. Spec: `docs/plans/per-line-item-split-billing-spec-2026-07-17.md`.
