...
After the migration was re-stamped above the advancing live high-water, a fresh exact-HEAD Codex pass found that `invoice_group_id` was still being treated as sales-rep read authority even though an existing invoice INSERT policy lets a rep choose that UUID; soft-deleted child invoices also retained access. Phase 1 now delegates billing-set/source-line reads only through a locked-down SECURITY DEFINER predicate over server-created `invoice_line_shares` membership and a non-deleted invoice. The proof shows group-only forgery `0|0`, legitimate server-bound membership `1|2`, and the same membership after soft deletion `0|0`. The same review found that `ON CONFLICT DO NOTHING` could preserve a pre-existing enabled feature flag; the migration now locks/reads the surviving value and aborts with `PER_LINE_SPLIT_BILLING_FLAG_MUST_BE_OFF` unless it is exactly `false`. Both cases failed first and now pass without changing production.

Built Phase 1 of per-line-item custom split billing on branch `claude/billing-splits-plan-8ih4jg`: additive migration `20260720231000` creating `field_app_billing_sets` / `field_app_billing_lines` / editable-while-draft `invoice_line_shares` / append-only `invoice_line_share_post_snapshots` (one parent per non-null invoice group; indexed foreign keys; exact micro-percent vectors; per-line Σ=100,000,000 constraint trigger; freeze-while-posted share and linked-item material-field triggers; durable unpost/repost history; owner/assigned-invoice-scoped sales-rep SELECT plus admin access), plus `invoices.send_disposition` ($0 not-to-send gate) and the `feature_per_line_split_billing=false` flag. The never-applied migration was re-stamped above the read-only verified live `20260719092832` high-water; production remains unchanged. Re-check and re-stamp again at apply time if production advances. Types added; typecheck/build/tests green. NOT applied to live — awaiting final exact-hash reviews, the §6.1 baseline billing cycle, and Mason's explicit apply approval. Spec: `docs/plans/per-line-item-split-billing-spec-2026-07-17.md`.
