...
- Prepared an additive `product_families` / Product metadata and return-credit enforcement foundation. `RETURN_POLICY_NO_RETURN` is the exact refusal code; pre-Stage-A compatibility remains `unknown` / default metadata values. Direct `return_items` writes defer only their Product FK inside the statement, then take sorted namespaced Product advisory locks and force the FK immediate again, preventing metadata/direct-write races without row-lock ordering. Metadata idempotency replays are bound to the full optimistic-update request and fail closed on changed Product or target values; its transaction-local write authorization resets immediately after the governed update, so a later direct metadata write is refused. The preserved cancel and credit-memo-unapply wrappers now acquire the linked sorted Product lock set before their first inventory or ledger mutation; `reject_return` remains safely covered by its first durable action, the privileged status-trigger backstop. The replacement prehashes are pinned to the approved July 22 live public-schema dump at highwater `20260722202622`, not the older July 19 baseline; the content-bound Stage A migration is LF-pinned so its reviewed `pg_proc` body hashes survive Windows checkout. Rollback-matrix and two-session real-schema concurrency proofs cover wrapper/direct-item/unapply races, metadata idempotency mismatch and authorization-scope refusal, a regression-sensitive received-cancel versus approved-receive shared-Product race, autocommit timing, cancellation reversal, late reject/cancel/unapply rollback, and an unrelated credit-memo unapply that voids only its memo (`return_reverted=false`).
- Added a 604-row provisional classification manifest: every row remains unresolved and pending owner review; 21 name-only no-return candidates are evidence flags only. Manifest SHA-256: `bf85cc649657735fa26ba8c7e753d653c76ba238ce63c7605ce723393ea322c4`. Final exact-SHA Fable and Sol verdicts are external PR evidence and are required before push or acceptance; this changelog does not self-certify them.
