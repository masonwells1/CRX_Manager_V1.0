...
- Bulk `products` SQL inserts now use a comment-aware parser (quoted schema/table names, aliases, multiline columns, and `VALUES`/catalog `SELECT` forms); the four pre-existing disposable fixtures are allowed only after that signature matches and their exact path-and-SHA-256 matches, so any byte change, appended comment, or formatting change is refused.
