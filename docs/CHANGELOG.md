...
Built Phase 1 of per-line-item custom split billing on branch `claude/billing-splits-plan-8ih4jg` (commit c4a5b6a): additive migration `20260718210000` creating `field_app_billing_sets` / `field_app_billing_lines` / `invoice_line_shares` (immutable micro-percent allocation snapshot; per-line Σ=100,000,000 constraint trigger; freeze-while-posted trigger; staff-read-only RLS with no client DML), plus `invoices.send_disposition` ($0 not-to-send gate) and the `feature_per_line_split_billing=false` flag. The never-applied migration was re-stamped above the verified live `20260718193000` high-water before review; production remains unchanged. Types added; typecheck/build/tests green; rls-security + migration-drift + types-drift reviewers CLEAN. NOT applied to live — awaiting the §6.1 baseline billing cycle and Mason's explicit apply approval. Spec: `docs/plans/per-line-item-split-billing-spec-2026-07-17.md`.
