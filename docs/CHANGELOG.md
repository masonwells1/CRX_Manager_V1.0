...
## 2026-07-18 — Per-line split billing Phase 2 calculator built and PARKED (flag OFF, not applied)
...
Added a network-isolated PostgreSQL 17 harness (`scripts/smoke/prove-per-line-split-billing-phase2.mjs`) that loads the checked-in Phase 1 + Phase 2 migrations into a uniquely named tmpfs Docker database and rolls every fixture back. Because Phase 1 is currently invalid, the default command fails hard; the explicit `--diagnose-blocked-phase1` mode labels its output `DIAGNOSTIC`, normalizes only the declared Phase 1 comment in memory, and isolates Phase 2 without certifying the migration pair. It exercises feature-OFF delegation, 50/50, exact three-way, 1¢ allocation, full-precision conversion with one final money round, signed half-cent −13¢ → −7¢/−6¢, manual/quote/tier/service price precedence, per-person override, 100/0 service with the zero row retained, flat fee, snapshot/default/fallback precedence, stable hashes, Mode A rejection, malformed-vector rejection, role denial, one public preview overload, and a non-browser-executable private calculator. A spec reconciliation caught and fixed one real 1¢ defect before closeout: same-price money had briefly multiplied the rounded 4dp quantity instead of the full-precision converted quantity. The regression `1.00004 × 12,500¢ = 12,501¢` now pins the rule; independent Codex delta review returned `APPROVE`.

**PARKED / not shippable yet:** PR #166's inherited Phase 1 migration contains invalid PostgreSQL `COMMENT ON COLUMN ... IS 'literal' || 'literal'` syntax. The checked-in Phase 1 file is not edited here. Phase 1 objects and the feature flag remain absent from live Supabase, the §6.1 real baseline billing cycle is not confirmed, and the final Claude Opus 4.8 review returned `NEEDS-WORK`: it agreed that no Phase 2 money/RLS defect was found, confirmed the Phase 1 blocker, and raised one Phase 2 harness-integrity issue that was fixed by making the default command fail hard and the workaround explicitly diagnostic. Live PostgreSQL was independently confirmed as 17.0.6, satisfying Claude's version follow-up for `pg_input_is_valid`. Live invariant sweeps also found one unrelated pre-existing quote commission row (`Q-2026-2059`, blank recipient); no live data was changed. No migration was applied, no flag was flipped, and Phase 3 was not started.
