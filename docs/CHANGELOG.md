...
The applied correction remains byte-immutable. Historical companion migrations `20260718124516` and `20260718124518` install and immediately remove a revoked, empty-catalog-only statement trigger so that the correction's embedded row-trigger assertion also passes during a zero-Product rebuild without seeding or deleting business data.
