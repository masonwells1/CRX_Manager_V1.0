...
The July 14 workflow gauntlet found five real issues, all independently confirmed by an issue-only Claude review and then reconciled by Terra, Luna, and Sol. Three queued migrations close the database gaps without changing existing identities: commission payout policies now use the active-aware `is_admin()` helper through a single-evaluation InitPlan, `batch_apply_prepayments` enforces the admin-only route contract for the batch wrapper before idempotency replay or mutation, and `is_admin()` itself now source-controls the hardened `public, pg_temp` search path required for SECURITY DEFINER helpers while qualifying `public.profiles`. A new live invariant predicate permanently detects regression of the commission policies. The workflow map now renders all nine quote states and its generator enforces an exact `QuoteStatus` node set; stale retired-RPC prose was removed. The checked-in public-function snapshot was refreshed from 364 to the verified live 374 names, and an AST-based test now proves every literal production RPC call exists in that catalog. Disposable-database proof confirmed the queued migrations compile, active admins pass, sales reps and inactive admins are rejected, and the new commission predicate falls from five live-equivalent violations to zero after migration. Live-vs-source proof also confirmed `is_admin()` already has `search_path=public, pg_temp` and the expected active-admin body, and the queued money-function rewrite preserves the current production body before adding the admin gate. No live migration was applied as part of this change. During proof, the offline-action provenance note was corrected: live `schema_migrations` records this branch's two checked-in offline-action migrations by `name` under MCP apply-time versions (`20260714171331`, `20260714171800`) and separately records a sibling review-resolution migration under `20260714172135`, so the docs now warn only not to infer apply state from filename prefixes.
