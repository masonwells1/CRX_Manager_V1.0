...
Phase 3 is PARKED after the binding third exact-Codex round. Rounds 1–2 found and the branch fixed grouped and single-recipient rebuild failures caused by restrictive child foreign keys. Round 3 then found two unresolved release blockers: a cached idempotency response is returned before target customer/job authorization, and newly created invoices use `current_season()` instead of the source job season that priced the plan. Per the build-loop cap, no fourth review/fix round was started, no draft PR was opened, and dependent Phase 4 was not started.
