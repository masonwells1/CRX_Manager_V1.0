...
Added a network-isolated PostgreSQL 17 harness (`scripts/smoke/prove-per-line-split-billing-phase2.mjs`) that loads the exact checked-in Phase 1 + Phase 2 migrations verbatim into a uniquely named tmpfs Docker database and rolls every fixture back. It exercises feature-OFF delegation, 50/50, exact three-way, 1¢ allocation, full-precision conversion with one final money round, signed half-cent −13¢ → −7¢/−6¢, manual/quote/tier/service price precedence, per-person override, 100/0 service with the zero row retained, flat fee, snapshot/default/fallback precedence, stable hashes, Mode A rejection, malformed-vector rejection, role denial, one public preview overload, and a non-browser-executable private calculator. A spec reconciliation caught and fixed one real 1¢ defect before closeout: same-price money had briefly multiplied the rounded 4dp quantity instead of the full-precision converted quantity. The regression `1.00004 × 12,500¢ = 12,501¢` pins the rule.

The first exact-SHA branch review timed out while tracing quote-price provenance, but that trace exposed a real mispricing path before closeout: the calculator searched every quote section sharing an input field/product and selected the lowest quote-item UUID, rather than the quote section attached to the source job. A decoy proof placed a lower-ID $9.99 quote beside the job's $1.50 quote and reproduced the wrong 999¢ plan. The calculator now resolves `jobs.quote_section_id` from the effective job/invoice context and restricts quote lookup to that exact section; without job context it does not guess a quote. The same decoy proof now returns 150¢ from the source job.
