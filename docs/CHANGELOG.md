...
The applied correction remains byte-immutable and its repository filename now starts with its assigned live version `20260718154131`. Disposable smoke-only setup/cleanup adapters install and immediately remove a revoked, empty-catalog-only statement trigger so the immutable artifact can also compile in the zero-Product proof without creating retroactive production migrations or touching business data.
