...
- Money must remain exact whole cents: new storage uses bigint cents; documented legacy
  PostgreSQL numeric-dollar columns retain exact `numeric` arithmetic and finite whole-cent
  constraints once clean. Parse decimal input exactly instead of multiplying a binary float.
