...
- [ ] Land the work the standard way (Mason's autonomous-landing rule, 2026-09-26): commit on a branch → open a PR → Luna rounds and required checks (Vercel) → bring the branch current → freeze the candidate head → apply `ready-for-coderabbit` (the trusted workflow waits out running checks, validates the candidate and attaches `coderabbit-review-dispatch` once) → CodeRabbit APPROVED that exact commit, every real finding resolved → run the exact-SHA `gpt-6-sol` proof LAST (it expires in 30 minutes) → apply the change's non-destructive migration through `scripts/apply-migration-file.mjs` if it has one → recheck every reported check and auto-merge OFF → merge with `--match-head-commit <that-exact-sha>`. The merge gates enforce the CodeRabbit approval, the newest-run-green checks and the Sol proof; `CHANGES_REQUESTED`, `--auto` and `--admin` block. A fix goes on the SAME PR: push it, let the trusted synchronize run record the new candidate epoch, wait for checks, relabel — it earns one follow-up review; no replacement PR. Label writes and skipped statuses are not review proof; preserve pending or uncertain request state and follow `docs/reference/coderabbit-native-review.md` before retrying. Labels are dedupe evidence, not an independent trust identity. Direct pushes to `main` are impossible (the `protect-main` ruleset). A destructive migration, an Edge Function deploy, and any secrets/auth/billing/permissions change stay Mason's.
