...
- [ ] Land the work the standard way: commit on a branch → open a PR → finish the separate Codex review and required checks (Vercel) → bring the branch current → freeze and record the candidate head → recheck every reported check and auto-merge OFF → merge with `--match-head-commit <that-exact-sha>`. CI is the merge gate: an approving review is NOT required (Mason removed it 2026-09-02) but a `CHANGES_REQUESTED` verdict still blocks; when CodeRabbit HAS approved, that approval's `commit_id` and the live PR head must match. **Do not apply `ready-for-coderabbit`, and do not post `@coderabbitai review` by hand (measured 2026-09-07).** The label's workflow posts as `github-actions[bot]`, which CodeRabbit does not answer — ~24 uses, zero reviews. The hourly `crx-hourly-coderabbit-slot` task requests one review per hour under Mason's account; reviews are rationed to roughly one grant per hour fleet-wide, so a hand-posted request collides with that job and consumes the slot another PR was waiting for. Do not stall a green landing waiting for a review that is not required. A fix or base update restarts checks and needs a newly frozen and recorded SHA. **One security exception:** any `SECURITY DEFINER` migration that accepts a caller-supplied actor parameter (`p_performed_by`, `p_actor*`, `p_user*`, or any actor-shaped name outside that pattern such as `p_target_id`) DOES still require a real CodeRabbit review before merge — **whether or not the body appears to bind it to `auth.uid()`**, because the documented residual gaps are re-binding after a passing `ACTOR_MISMATCH` check, temp-table laundering, names outside the pattern, and cross-routine delegation, so an apparent binding does not exclude the migration — `actor-binding-check.mjs` is capped as best-effort (2026-09-01) and `docs/reference/agent-guardrails.md` records that for the re-binding, laundering, naming-scope and cross-routine gaps only the exact-SHA Codex proof and the CodeRabbit review stand. Hold the merge and tell Mason rather than merging that class unreviewed. Direct pushes to `main` are impossible (the `protect-main` ruleset); agents land reviewed code themselves under the standing policy — Mason does not hand-commit code.
