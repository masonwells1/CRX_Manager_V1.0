...
Active runtime state stays under the shared Git common directory. Before a job may reach `live`, the factory must generate a content-hashed, human-readable closeout packet under `docs/audits/factory/jobs/<job-id>.md` containing the ticket hash, approval receipt summary, behavior result, proof manifest, reviewer verdicts, landing commit/base, production verification, and terminal ledger hash. That packet lands with the job's ordinary documentation and survives a re-clone. The shared active-state directory will also be added to the existing off-site CRX backup inventory before multi-lane activation.
