...
| **1.9a** | **[REV3] Brand attaches to the existing lot chain, at receiving — not to a new parallel field.** The app already tracks physical provenance end to end: `receiving_records.lot_number` → `blend_ticket_products.lot_number` → `application_record_lots` (which carries `quantity_from_lot`). A lot number **is** a specific branded batch. The brand reference therefore hangs off the receiving record / lot, and delivery, blend and application records inherit it through the chain they already carry. **Do not add a `brand_id` to delivery or application lines** — that builds a second provenance system that can contradict the lot data | Receive a lot of "Lima 6" against the LV6 ester spec; without any further data entry, the application record built from that lot reports Lima 6 and Lima 6's EPA number |
| **1.9a-i** | **[REV3] Brand selection is required at receiving — Mason, 2026-08-18.** Asked whether picking the brand can be a required step at the moment the crew types the lot number, Mason chose yes: it is the cheapest capture point, because the person unloading is holding the jug | Receiving cannot be completed without a brand once the spec has brand rows defined. A spec with no brand rows yet does not block receiving |
| **1.9a-ii** | **[REV3] Split loads record every contributing brand with its amount — Mason, 2026-08-18.** Asked whether paperwork must show both brands when a drum runs out mid-load, Mason chose "both brands, with amounts." `application_record_lots` already models multiple lots with a quantity each, so this needs no new shape — it needs the brand to ride the lot | An application record drawing 30 gal from an Ag Saver lot and 15 gal from a Slam lot shows both brands, both EPA numbers, and both quantities on the customer's paperwork |
| **1.9a-iii** | **[REV3] Records snapshot the brand's EPA registration number at write time; they do not dereference the brand row later.** Otherwise correcting a typo in a brand's registration number silently rewrites historical spray records | Change a brand's EPA number after a delivery is recorded; the existing delivery and application records still show the number that was in force when they were written |
