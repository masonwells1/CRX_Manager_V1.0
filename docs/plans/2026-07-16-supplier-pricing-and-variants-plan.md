...
**Status:** Phase 1a frontend merged in PR #163 and deployed to production. Live: additive bootstrap, zero-cost guard, legacy repeat-save compatibility, forward hardening, RPC-only Product pricing flows, and pricing-only `.xlsx` worksheet. Pending owner-gated production step: deploy and verify the `process-document` supplier-pricing OCR retirement. Parked: the strict direct-write cutover until the frontend rollback window closes.
