...
**Status:** Phase 1a frontend merged in PR #163 and deployed to production. Live: additive bootstrap, zero-cost guard, legacy repeat-save compatibility, forward hardening, RPC-only Product pricing flows, pricing-only `.xlsx` worksheet, and verified `process-document` v19 supplier-pricing OCR retirement. Parked: the strict direct-write cutover until the frontend rollback window closes.
