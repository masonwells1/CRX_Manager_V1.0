...
- LANDING CHECKLIST (every PR):
  ...
  3. Freeze the head. CodeRabbit review of THAT head. The request path is the repository contract: the `ready-for-coderabbit` label workflow (`.claude/commands/ship.md`, `.claude/hooks/pr-merge-guard.mjs`, `docs/reference/coderabbit-native-review.md`), which runs the trusted final-review checks before dispatch. A hand-posted `@coderabbitai review` is allowed only where the label path cannot serve the candidate (as of 2026-09-13 it needs a fresh PR for every new head, because incremental reviews are disabled) and only with Mason's per-request authorization in this fleet's chat, recorded in the coordinator's reservation record (requester, head, time, acknowledgement). A hand-posted request never substitutes for the trusted checks; the merge still needs the exact-head review delivered plus every step below. (Amended 2026-09-13 by Mason's decision from the 2026-09-11 wording "requested by Mason or by the one session he authorized in chat"; making the label path serve re-reviewed heads is open item PR #647.)
