...
- ⏭ **v1c NEXT (UI):** create-rush-order entry point (button/page → calls `create_rush_order`), Orders nav `needs_pricing` count badge, rep notification on creation (frontend `notificationTriggers`). Then **v2** = `price_order` RPC + pricing screen + invoice unit/extended/total sync (invoice_date = pricing date per G3) + deferred-commission insert on final profit; **v3** = `check_unpriced_orders` cron (48h/7d) + MonthEndClose "0 needs_pricing" checklist + credit surfacing.
...
- **Iter 4 (2026-06-13):** #2 v1b. Migration `20260613180000_create_rush_order.sql` (FILE-ONLY): NEW SECDEF `create_rush_order` + `order_items.suggested_price` column + Orders `needs_pricing` filter + `OrderItem.suggested_price` type. W1 gate confirmed live. In-house gate CLEAN (rls 0/0/0+1 NIT; drift 6/6 safety +1 MED doc-row fixed; types 0/0/0; tsc green). Doc counts 442→443 (+RPC 219→220); AGENTS.md regen. Smoke `02b-create-rush-order.sql`. codex=PENDING (pre-G5 batch). NOT applied/pushed. v1c (UI) next.
