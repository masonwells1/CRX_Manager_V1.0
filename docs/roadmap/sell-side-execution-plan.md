...
- ⏭ **NEXT (UI consolidation, frontend-only):** create-rush-order entry (button/page → `create_rush_order`) + pricing screen (price-only inputs defaulting to `order_items.suggested_price` → `price_order`) + Orders nav `needs_pricing` count badge + rep notification on creation (`notificationTriggers`). Then **v3** = `check_unpriced_orders` cron (48h/7d) + MonthEndClose "0 needs_pricing" checklist + credit surfacing. After UI + v3 → #2 DONE.
