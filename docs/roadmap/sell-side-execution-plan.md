...
- 🔄 **UI consolidation (frontend, in progress):**
  ...
  - ⏭ **Remaining UI:** create-rush-order entry (NewOrder "ship now / price later" mode → `create_rush_order`, items {product_id,qty}); Orders nav `needs_pricing` count badge; rep notification on rush-order creation (`notificationTriggers`).
- Then **v3** = `check_unpriced_orders` cron (48h/7d) + MonthEndClose "0 needs_pricing" checklist + credit surfacing. After UI + v3 → #2 DONE.
...
- **Iter 6 (2026-06-13):** #2 UI — pricing screen. `OrderDetail.tsx` "Set Pricing" panel (needs_pricing orders, admin/sales_rep) → `price_order` with `suggested_price` defaults. Frontend-only (no migration). compliance-reviewer CLEAN; tsc + lint green. NOT applied/pushed. Remaining UI: create-rush-order entry (NewOrder mode) + nav badge + notify; then v3.
