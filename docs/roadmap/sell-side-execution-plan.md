...
- 🔄 **v3 (in progress):**
  ...
  - ⏭ **Remaining v3:** MonthEndClose "0 needs_pricing" checklist item (read the month-end-close page first to wire it correctly); credit-surfacing for unpriced orders is vague → scope minimally or `DEFERRED` with note. After these → **#2 DONE**.
...
- **Iter 9 (2026-06-13):** #2 v3 — cron. Migration `20260613200000_check_unpriced_orders_cron.sql` (FILE-ONLY): orders dedupe stamps + SECDEF `check_unpriced_orders()` (48h/7d notify, deduped) + pg_cron 06:10. In-house gate CLEAN (rls 0/0/0/0+1 info; drift 0/0/0+2 info; types 0/0/0). Doc counts 444→445 (+RPC 221→222); AGENTS.md regen. Smoke `02d-check-unpriced-orders.sql`. codex=PENDING. Remaining #2: MonthEndClose checklist + credit-surfacing (then #2 DONE). NOT applied/pushed-to-main.
