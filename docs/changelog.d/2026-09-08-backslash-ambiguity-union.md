...
**Proof.** `guards.test.mjs` 201 -> 205. Two assertions fail in OPPOSITE
readings — the round-2 shape needs the escape, the round-3 shape needs the
literal — so together they pin the union: no single reading satisfies both.
(**Corrected 2026-09-09:** this paragraph first said "two of the ADDED
assertions". Only one of the pair was added here; the round-2 assertion came
from `c8f3c5727`. The pair does pin the union, but it spans two commits — a
reader taking this commit alone would not find it. Codex round 4 caught the
attribution.) A control asserts the documented `UPDATE ... -- [E2E]` exemption
still works. `migration-apply-guard.test.mjs` 113 green; full
`npm run test:correction-guards` green. The deployed hook process still DENIES
all twelve must-deny shapes with `REAL-DATA-OK` absent and ALLOWS all four
must-allow shapes, and the same 12 of 29 sweep predicates still clear it — the
second pass adds no false positives.
