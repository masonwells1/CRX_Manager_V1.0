...
Claude's verified exact-head review of 2443913336cb6286b9e40ea3ef5340a71b6c094b
cleared the code/migrations and returned only two LOW stale capture references.
Point the current-state link and the superseded September 8 ledger block at the
actual September 14 current capture, and align the current-state historical header
to that same date. Preserve dated historical publications and every SQL byte.

Documentation drift checks remain required. Sol's 1200-second review timed out
...
with the supported longer 2400-second limit, then all protected delivery and actual
