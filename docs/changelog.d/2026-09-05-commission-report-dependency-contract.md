...
- Expanded the wrapper contract to pin its set-returning, strictness,
  leakproof, parallel-safety, and cost attributes instead of allowing a replay
  to silently normalize unreviewed catalog state.
- Added disposable-PostgreSQL mutations for wrapper cost drift plus child body,
  overload, anonymous grant, security-mode, and search-path drift. Each must
  abort before the replay can certify the report contract.
