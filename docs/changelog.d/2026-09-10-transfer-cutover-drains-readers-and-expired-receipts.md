...
- takes `ACCESS EXCLUSIVE` on `idempotency_keys` first, bounded by its existing
  15-second `lock_timeout`. It waits for every transaction that has read or written the
  table and keeps new ones out until commit. While it waits, receipt reads and writes
  across the app queue behind it for at most that long.
...
- `scripts/smoke/prove-transfer-invoice-intent-binding.mjs` (disposable PostgreSQL 17,
  ...
  Expired receipts for other operations, and bound transfer receipts, survive.
- A second disposable instance falsifies each new statement. Without the purge, the same
  ...
  nothing.
- The static proof pins the lock, its place after `lock_timeout` and before every
  preflight, and the purge's exact predicate and place.
