...
GitHub workflow accepts one exact current-main commit, reviewed PR-head commit, durable review
comment, timestamped migration filename, and SHA-256.
...
Before Mason can approve a run, GitHub verifies the exact owner-authored review-attestation comment,
the PR whose head received that review, the PR's merge as the still-current `main`, and byte-identical
migration Git blobs before and after merge. Every third-party action in the credential-bearing path
...
durable-comment and merged-PR mismatches, PostgreSQL transaction aliases, non-atomic commands,
