...
- Normalize PostgreSQL Unicode-escaped configuration identifiers before scanning owner-privileged routine bodies, failing closed for unparseable `SET` or `RESET` targets; bind linked-worktree proof hashes and ordering checks to the same session-owned applied-migrations ledger.
