...
**Apply order.** A read-only ledger read on 2026-09-11 found 1001 rows and 994 distinct names. The newest version is `20260909023300` and the effective high-water is `20260908120000_close_pr535_live_gaps`, which this file sorts above. PR #624's `20260908130000` migration is not applied and sorts below this file. So #624's migration must go live first. Otherwise, once this one lands, the ordering guard will refuse #624's migration and it will need a new number. The live-ledger capture in `migration-history.md` and the ledger sentence in `CURRENT_STATE.md` have been updated to match that read. They use the same wording as open PR #646, so the two branches merge without conflict. Both manual docs' "Last verified" stamps now read 2026-09-11.
