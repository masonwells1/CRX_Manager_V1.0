...
- **The migration `20260908130000` is still NOT applied to live.** Nothing in this change
  ...
  needing Mason's explicit in-conversation approval. The requirement recorded on
  September 7 to apply it before exposing the browser fix is historical: the
  September 13 delivery note permits the browser retry fix to ship while this
  migration remains parked. Merge readiness does not prove server-side binding
  is active in production; that requires the separately approved live apply.
