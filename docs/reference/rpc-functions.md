...
- `complete_cycle_count(...)` and `reverse_completed_cycle_count(...)` lock every affected inventory row before delegating to the canonical implementation. `complete_cycle_count` also accepts an optional cycle-count item revision, rejecting a completion if another item save has advanced the authoritative revision.
...
- `save_job(p_job_id, p_job_payload, p_fields, p_chemicals, p_performed_by, p_idempotency_key)` — upsert job + fields + chemicals with actor/idempotency binding, server-derived acreage and chemical totals, and chemical-unit refusals. **LOCAL CANDIDATE 2026-09-04:** expands `JOB_ACRES_NOT_FINITE` to reject a missing `acres_to_treat` key or JSON null with distinct diagnostics before any acreage coalescing/write. It also newly accepts a non-null empty acreage string from direct RPC callers and stores numeric zero; the UI already sends zero. A same-day read-only live survey found 0 invalid stored acreage rows across 5 `job_fields` rows. Re-emits the effective F06 body with exact source/candidate replay pins; not applied live.
...
- `receive_return()` — receive returned items and update inventory; retry is bound to the authenticated actor and exact return id. **Applied live 2026-09-01 (`20260827041100`):** every usable linked item is restored, with a missing Main Warehouse inventory row created transactionally instead of silently skipping restock. Linked return lines must retain the authoritative order-item product and unit; runtime lineage mismatches raise `RETURN_CREDIT_UNIT_MISMATCH`, while a warehouse/product stock-unit mismatch raises `RETURN_CREDIT_INVENTORY_UNIT_MISMATCH`. Apply-time preflights reject any unhandled open restock mismatch before replacing a function. The sole verified legacy source-free RMA is pinned by return/item id and converts `15 ea` through its authoritative `2.5 Gal` container size to exactly `37.5 Gal`; the compatibility branch cannot match any other row.
- `issue_return_credit()` — issue a server-derived credit memo for a return, integrated with AR; retry is bound to the authenticated actor and exact return id. **Applied live 2026-09-01 (`20260827041100`):** linked usable returns gain negative-quantity credit lines that reverse historical COGS recognized on paid, overdue, and posted source invoices. Rollout and runtime guards reject ambiguous cost credits without order-item lineage; a recognized source invoice cannot leave the recognized set or be deleted while an active linked return credit depends on it; and active credit/source cost lines remain immutable until the credit is voided/unapplied. This behavior is live as of 2026-09-01.
...
- `get_commission_balance_report(p_as_of_date)` — **APPLIED LIVE 2026-09-03 (`20260903150100`, ledger `20260903202611`):** admin-only earned/paid/outstanding balances beginning on the first complete Chicago day after the immutable database cutover (`2026-09-04`). Earned selects the latest append-only commission state before the cutoff; paid independently sums signed settlement events, so cancelled/deleted paid rows remain visible as negative outstanding. The partial cutover day, every earlier date, and future dates fail closed with the supported boundary. **LOCAL CANDIDATE `20260905200600`, not applied:** recipient labels select the latest earned-state observation at the requested cutoff (including paid-only output), replacing alphabetical historical-name selection without changing money or counts.
- `get_commission_payment_detail_report(p_as_of_date)` — **APPLIED LIVE 2026-09-03 (`20260903150100`):** companion admin-only reconciliation rows from immutable settlement-event snapshots, with payment number/date, recipient, commission line, order/job source, customer, commission date, and exact settled amount. Fully reversed items are absent after their void cutoff.
- `get_commission_history_report(p_as_of_date)` — **APPLIED LIVE 2026-09-03 (`20260903230000`, ledger `20260904040643`):** read-only `SECURITY INVOKER` wrapper that returns the two applied commission report datasets together from one stable SQL statement snapshot. It inherits each child RPC's admin guard and returns the requested date plus `balance_rows` and `payment_detail_rows` arrays.
...
- `create_commission_payment()`, `post_commission_payment()`, `void_commission_payment()` — create snapshots exact `commission_payment_items`; post marks the payout effective; **APPLIED LIVE 2026-09-03 (`20260903150100`):** void also stamps `voided_at`/`voided_by` so prior cutoff reports remain reproducible.
...
> **Live contract (2026-07-31):** The signatures below reflect the applied production state through migration `20260731001654_ap_period_close_boundary_hardening`. The helper-function idempotency pattern is `check_idempotency` / `save_idempotency` — see `docs/reference/sql-canonical-patterns.md`.

- `create_vendor_bill(p_vendor_id, p_purchase_order_id, p_bill_number, p_bill_date, p_due_date, p_payment_terms, p_subtotal_cents, p_adjustment_cents, p_notes, p_idempotency_key, p_confirm_po_overage, p_po_overage_reason)` → uuid — admin-only. Creates vendor bill with auto-calculated due_date from payment terms (Net 15/30/45/60/90/Due on Receipt). Calls `check_period_open(bill_date)` (PR-04). Validates `subtotal > 0` AND `total = subtotal + adjustment > 0` (PR-04 + audit-fix-2 codex F4). Verifies vendor exists + is not soft-deleted. PR-22b additionally checks PO-to-bill vendor consistency (raises `VENDOR_PO_MISMATCH`) and posts a soft-warn notification when bill total drifts >5% from the linked PO's authoritative line-rounded `total_cost_cents` (`20260717010000`), avoiding aggregate-rounding false drift. `20260831161000` (**applied live 2026-09-03**, ledger version `20260903024550`) added the two trailing cumulative-overage confirmation parameters and requires a logged reason above 105% of authoritative PO cost. Audit log entry with `operation_type='vendor_bill_created'`. Canonical idempotency.
...
- `update_vendor_bill(p_bill_id, p_subtotal_cents, p_adjustment_cents, p_bill_date, p_due_date, p_notes, p_idempotency_key, p_confirm_po_overage, p_po_overage_reason)` → jsonb — admin-only (PR-14). Edits an unpaid bill with no active payments. Re-runs `check_period_open(bill_date)` since the date may change. Recomputes `total_cents`. `bill_number` is NOT editable (uniqueness invariant). `20260831233000` (**applied live 2026-09-03**, ledger version `20260903124710`) added the two trailing cumulative-overage confirmation parameters and applies the same reasoned 105% PO threshold to edits. That migration also placed an actor- and payload-bound replay wrapper in front of this RPC; the mature implementation it delegates to is `_section9_update_vendor_bill_intent_impl_20260831`. Audit log with `operation_type='vendor_bill_updated'`. Returns `{success, bill_id, old_total_cents, new_total_cents}`. Canonical idempotency.
...
- `delete_vendor(p_vendor_id, p_idempotency_key)` → jsonb — admin-only (PR-25). Soft-delete via `deleted_at`. Refuses if any unpaid `vendor_bills` exist. PO check intentionally omitted (purchase_orders uses legacy `vendor` TEXT column, not `vendor_id`; the vendor-FK migration remains out of scope).
- `get_ap_aging(p_as_of_date)` — current-only AP aging by `due_date`, with vendor breakdown: Current (due today or later) / 1-30 / 31-60 / 61-90 / over 90 days past due. Dates other than the server-owned Chicago business date fail closed until durable historical bill-state history exists. PR-22 dropped the unused `p_idempotency_key` parameter (read-only RPC).
- `get_ap_dashboard_summary()` — KPI totals: total_owed, due_this_week, due_this_month, overdue_amount, bill counts, recent vendor bills. `due_this_month` ends at the Chicago calendar month-end, not a rolling 30-day boundary.
...
- `preview_field_app_invoice_split(p_locations jsonb, p_chemicals jsonb, p_application_service_id uuid DEFAULT NULL, p_invoice_id uuid DEFAULT NULL, p_invoice_date date DEFAULT NULL)` -> jsonb `{per_customer[], grand_total_cents, customer_count, shares_detail}` — **Phase 1 new:** read-only preview returning the same per-customer breakdown that `save_field_app_invoice` would produce, without writing anything. Backs the "Preview" button on the field app invoice page. p_invoice_id (2026-06-24): when editing a split group, pass the invoice id so preview excludes customers whose only group invoice was soft-deleted (matches save_field_app_invoice deleted-stays-deleted); NULL = no exclusion. **Unit-price fix (2026-07-01):** mirrors `save_field_app_invoice` — prices each line on the converted `field_app_priced_quantity(...)` so the on-page preview matches the invoice. **p_invoice_date (2026-09-06, CRX-SEC-001, migration `20260906120000`, APPLIED LIVE 2026-09-08 under ledger version `20260908045843` — live is now the 5-argument signature, verified from `pg_proc` and through a real PostgREST call in both the 5- and 4-argument shapes):** the application-fee season now follows the invoice, not the server clock. An existing group member, or a single-customer edit of `p_invoice_id`, prices at that row's STORED season; only a genuinely new invoice uses `compute_season(COALESCE(p_invoice_date, America/Chicago today))` — the same branch order `save_field_app_invoice` uses, so the previewed per-acre rate matches the billed one. Omitting the argument still resolves through the DEFAULT and falls back to the Chicago business date.
...
- `complete_cycle_count(p_cycle_count_id, p_completed_by, p_idempotency_key, p_expected_item_revision DEFAULT NULL)` -> void — **rewritten:** replaced `GREATEST(0, ...)` clamp with `RAISE EXCEPTION` when math would drive on-hand negative (E2a — "block, don't drift"). Closes the audit's "ledger ↔ on-hand permanent disagreement" finding. Item-first locking plus the optional authoritative revision prevents a completion from silently applying a stale cross-tab snapshot; local follow-up `20260831235900` also makes item inserts lock/revalidate the parent before becoming visible, closing the late-insert race.
...
- `update_cycle_count_item(p_item_id, p_counted_qty, p_notes, p_performed_by, p_idempotency_key)` -> jsonb — locks the item before parent `cycle_counts`, validates `status='in_progress'`, computes variance + variance_pct server-side, and returns the new `item_revision`. Replaces direct PostgREST `.update()` from React. Admin-only.
