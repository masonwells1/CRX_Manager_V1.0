...
- `draw_down_quote(p_quote_id, p_draws, …)` — **partial booking draw-down** (sell-side roadmap #1 v1, `20260610145253`): pulls part of a sent/revised quote's booked quantities into a new confirmed order at the quote's locked booking value; tracks per-product balances in `quote_product_draws`; decrements the quote's active holds FIFO (drawn qty moves to `quantity_prebooked` — Net Free invariant); overdraw blocked (`BOOKING_OVERDRAWN`); final draw sets the quote `accepted`. admin/sales_rep, strict-actor, idempotent. **Pending authorization follow-up (NOT APPLIED):** `20260813161614` requires a sales rep to own the locked quote, preserves the admin path, and hides soft-deleted quotes before replay or mutation. **Known edge:** same-product lines at distinct cent prices can average to a fractional-cent unit price and safely roll back at the whole-cent guard. A reviewed cumulative-rounding candidate was removed because it could underbill invoice reconstruction and reuse allocation after a reversible job draw; no replacement is prepared.
