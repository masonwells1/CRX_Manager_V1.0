...
- `restore_cancelled_order(p_order_id, p_reason, p_performed_by, p_idempotency_key)` → jsonb — **live behavior remains the legacy admin status-only restore until prepared migration `20260718220000` is approved/applied.** That migration replaces it with a non-mutating, fully ungranted tombstone because the old body restored only `orders.status` and did not reconstruct inventory reservations, holds, commissions, or cancelled draft invoices.
