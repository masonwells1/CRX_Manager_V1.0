...
- `generate_batch_statements()` — generate batch PDF statements for multiple customers. Pending local migration `20260803131507` includes customers whose only positive open invoices are overdue, rejects forged actors, and authorizes the authenticated admin before selecting customers (including an empty result).
