...
## Offline Action Receipts — applied live with MCP-stamped versions

**2026-07-14 read-only verification:** production contains `offline_action_receipts`, its staging guard, and the offline-action RPC names. Matching `supabase_migrations.schema_migrations` by `name` shows this branch's two checked-in offline-action migrations applied live as `20260714024811_offline_action_receipts` → `version=20260714171331` and `20260714070000_offline_action_receipt_stage_limits` → `version=20260714171800`. The live ledger also records the sibling review-resolution migration name `20260714122626_offline_action_review_resolution` under `version=20260714172135`; that source is not part of this branch. Do not infer application state from the filename prefix alone; MCP applies can write apply-time versions while preserving the original filename in `name`.

- `stage_offline_action(p_client_action_id, p_operation, p_entity_id, p_schema_version, p_client_created_at, p_payload, p_idempotency_key)` → jsonb — active-user staging gate for only `complete_delivery` and `complete_job`; validates/canonicalizes bounded payloads, checks current target/assignment visibility, and atomically claims the permanent action UUID + idempotency key. Exact replay returns the existing receipt; changed UUID/key reuse raises. Queued follow-up `20260714070000` serializes new staging per actor and raises `OFFLINE_STAGE_RATE_LIMIT` after 250 new receipts in a rolling 24 hours or while 500 unresolved review rows already exist; exact action-ID replays remain readable at either limit. The SQL default remains `NULL` to preserve CRX's one-overload mutator convention, but the function rejects it and `StageOfflineActionArgs` requires the key for app callers.
- `process_offline_action(p_client_action_id, p_idempotency_key)` → jsonb — owner-bound processor. Locks the receipt, calls exactly one current canonical completion RPC, validates its result, and commits the business mutation + `succeeded` receipt in the same database transaction. Deterministic business conflicts roll back their subtransaction and become sanitized `needs_review`; retryable infrastructure failures rethrow and leave the receipt safely `received`. `ProcessOfflineActionArgs` requires the key even though the SQL signature keeps the project-standard default.
- `get_offline_action_status(p_client_action_id)` → jsonb — sanitized status lookup for the active owner or active admin/sales office user. It never returns the saved request payload or idempotency key.

The receipt table has no authenticated direct table grants; all writes are RPC-only. The rollback smoke registry entry `stage_offline_action` covers all three RPCs plus both canonical completion branches.
