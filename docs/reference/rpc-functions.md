# RPC Functions Reference (324 callable overloads across 316 names + 79 trigger/internal functions — live DB as of 2026-07-16)

> Function inventory verified live against Supabase project `rhyzpcqhnizqbxphqdkr` (`pg_proc` joined to `pg_namespace` where `nspname='public'`, split by `prorettype = 'trigger'::regtype`): **324 callable overloads across 316 distinct names**, **79 trigger/internal function names**, **395 total public function names**. This supersedes the earlier 2026-06-29 branch-HEAD snapshot (270 + 56), which was measured against a local throwaway DB before the field-app parity migrations and subsequent sprints shipped live.
>
...
- `complete_delivery(...)` requires an active profile and uses the effective Chicago completion date for the closed-period warning and generated draft invoice. Since live migration `20260716173342`, current admin/sales authority or assigned-driver access is verified before any committed idempotency result can be returned; the unchanged implementation is internal and direct external execution is revoked. `void_delivery(...)` now relies on the active-aware admin helper.
...
- `complete_delivery()` — requires in_progress, creates remainder rows for partial deliveries, copies `tote_number` from delivery items. **Live replay-authorization correction (`20260716173342`, ledger `20260716174220`):** the public wrapper checks canonical actor identity plus the caller's current active admin/sales role or assigned-driver relationship before delegating to the directly non-executable internal implementation, so a leaked committed key cannot expose a cached result to an unauthorized active user. **Live closed-period correction (`20260716152906`):** the delivery transition trigger calls `check_period_open()` on the effective Chicago completion date; a closed date rejects and atomically rolls back the full RPC, including the older warning rows, inventory/order changes, draft invoice, and idempotency write. **F8 (2026-05-07):** restored the post-loop `tote_number` copy step that Phase 15 dropped — load sheets now correctly carry tote numbers again.
...
- `void_delivery()` — void a completed delivery, reversing inventory transactions. **Live closed-period correction (`20260716152906`):** the same non-bypassable delivery transition trigger checks the original Chicago completion date (scheduled date only as a legacy fallback); a closed date rejects and rolls back every reversal, draft-invoice cancellation, lifecycle write, warning, audit, and idempotency side effect.
...
- `allocate_payment()` — allocate/re-allocate payment amounts across invoices. **Live replay correction (`20260716165801`):** its versioned atomic idempotency claim can now fall through the shared insert trigger on an already-committed same-request replay, letting the RPC return the cached result; cross-operation key reuse and legacy duplicate final-result inserts still raise and roll back.
