...
- `create_quote_version(p_quote_id uuid, p_performed_by uuid, p_method text, p_idempotency_key text, p_expected_row_version bigint)` — snapshots full quote state (sections + items) for version history. SECURITY DEFINER, search_path = public, pg_temp. Since `20260813080000` this is the **only** write path to `quote_versions`: the browser roles' INSERT policy and write grants were revoked (**CRX-SEC-1**), so direct PostgREST inserts no longer work. Signature verified against live `pg_proc` on 2026-08-19 UTC (the evening of 2026-08-18 local).
- `restore_quote_version(p_quote_id uuid, p_version_id uuid, p_performed_by uuid, p_idempotency_key text, p_expected_row_version bigint, p_below_cost_reason text)` — restores quote from a version snapshot as revised draft. Note `p_quote_id` comes **first**, before `p_version_id`. SECURITY DEFINER, search_path = public, pg_temp. Signature verified against live `pg_proc` on 2026-08-19 UTC (the evening of 2026-08-18 local).
