...
- `save_field()` — upsert a field and its billing defaults. **Live actor-binding hardening (`20260729222311`):** derives activity attribution from `auth.uid()`, rejects a non-null mismatched `p_performed_by` before replay lookup or writes, and preserves legacy callers that pass NULL by recording the authenticated user.
