...
### Supplier Pricing Phase 2 — governed Product cost basis (code-only; not live)

`20260721231000_supplier_cost_basis_phase2.sql` adds two RPC-only, RLS-enabled tables. `product_cost_basis` is the append-only selected-cost ledger with one active row per Product and explicit supplier-observation, received-PO-item, or manual provenance. `product_cost_basis_change_rows` binds that provenance plus the expected Product pricing version and active-basis identity to an existing Phase 1a preview, preventing both pricing drift and same-dollar concurrent-selection overwrite. The migration fills the PO conversion snapshot with `1` and persists the effective Product inventory-unit identity only when a PO line's unit matches that inventory unit. Insert and first-receipt triggers discard caller-supplied snapshot/provenance values and derive only that safe same-unit fact; mismatched supplier/package units remain unresolved. Supplier links are likewise eligible only while their confirmed conversion unit matches the Product's current effective inventory unit. Preview, apply, workspace, and history normalization all reject stale unit evidence. While Phase 2 is enabled, a Product with an active basis cannot change effective inventory unit. A controlled flag-off unit correction atomically closes the old active basis, so it cannot be silently reinterpreted and the next governed approval must establish a fresh basis. While Phase 2 is disabled, a received-line correction clears stale snapshot provenance; once enabled, cost-defining fields for received lines are immutable. Money remains bigint cents.

The feature flag `app_settings.supplier_cost_basis_enabled` defaults to `false`. The migration baseline copies current governed Product costs into history but does not update `products.current_cost`, tier prices, or `cost_history`. While the flag remains off, the deployed Phase 1a Product editor stays compatible and its governed cost changes atomically append matching manual basis rows, so migration-first rollout does not create a Product-edit outage or ledger drift. When explicitly enabled later, cost changes must use the Phase 2 wrapper; the wrapper delegates all price math and writes to the existing Phase 1a engine rather than introducing a second calculator. This section documents branch code only and is not evidence of a live migration or enabled feature.

Release order is fail-closed: apply and verify the migration while the flag remains off, refresh the live RPC snapshot/generated types and remove the temporary queued-RPC test bridge, then publish the frontend. Enabling supplier/actual-purchase selection is a separate owner-approved action after those checks; deploying the frontend before its RPCs exist is intentionally blocked by `rpcFixtureLiveDiff.test.ts`.
...
| product_cost_basis *(Phase 2 code-only)* | RPC only | RPC only | RPC only (close active row) | - |
| product_cost_basis_change_rows *(Phase 2 code-only)* | RPC only | RPC only | - | - |
