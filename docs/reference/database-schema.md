...
`20260721231000_supplier_cost_basis_phase2.sql` adds two RPC-only, RLS-enabled tables. `product_cost_basis` is the append-only selected-cost ledger with one active row per Product and explicit supplier-observation, received-PO-item, or manual provenance. `product_cost_basis_change_rows` binds that provenance plus the expected Product pricing version and active-basis identity to an existing Phase 1a preview, preventing both pricing drift and same-dollar concurrent-selection overwrite. The migration also fills the PO conversion snapshot with `1` only when a PO line's unit matches the Product inventory unit. Insert and first-receipt triggers discard caller-supplied snapshot/provenance values and derive only that safe same-unit fact; mismatched supplier/package units remain unresolved instead of being guessed. While Phase 2 is disabled, a received-line correction clears stale snapshot provenance; once enabled, cost-defining fields for received lines are immutable. Money remains bigint cents.
