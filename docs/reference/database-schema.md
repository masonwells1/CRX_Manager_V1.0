...
`20260721193553_supplier_cost_basis_phase2.sql` adds two RPC-only, RLS-enabled tables. `product_cost_basis` is the append-only selected-cost ledger with one active row per Product and explicit supplier-observation, received-PO-item, or manual provenance. `product_cost_basis_change_rows` binds that provenance plus the expected Product pricing version and active-basis identity to an existing Phase 1a preview, preventing both pricing drift and same-dollar concurrent-selection overwrite. Money remains bigint cents.
