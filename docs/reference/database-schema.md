...
- `commission_history_cutover` - **LOCAL CANDIDATE `20260903150100`, NOT LIVE:** one immutable row records the real cutover timestamp, the first complete reportable Chicago day, and a count/digest of the opening commission population. UPDATE/DELETE/TRUNCATE are refused; earlier dates fail closed.
- `commission_earned_state_ledger` - **LOCAL CANDIDATE `20260903150100`, NOT LIVE:** append-only bigint-cent snapshots of each commission's report-relevant state, with opening rows at the real cutover and later revisions at actual wall-clock transition time, stable recipient/source/customer labels, RLS, RESTRICT lineage, and mutation/truncate refusal triggers.
- `commission_settlement_events` - **LOCAL CANDIDATE `20260903150100`, NOT LIVE:** append-only signed bigint-cent events; posting writes positive frozen payment-item snapshots and voiding writes matched negative reversals. Historical paid totals and detail read this ledger independently from current earned state.
