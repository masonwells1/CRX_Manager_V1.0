...
- `idempotency_keys` - Idempotent operation cache (idempotency_key UNIQUE, operation, result jsonb, expires_at — auto-cleanup after 24h). **LOCAL — NOT APPLIED:** migration `20260802162805_bind_idempotency_to_mutation_intent` adds optional server-derived `request_fingerprint` + `request_actor_id` bindings.
