...
- `below_cost_approvals` - **LOCAL ONLY pending apply (`20260810180002`)** immutable audit for active-admin below-cost overrides (operation, entity/line/product identity, actor, reason, locked current unit cost, sale unit price, quantity and total shortfall cents). Admin-readable only; no authenticated INSERT/UPDATE/DELETE policy; PostgreSQL trigger is the sole writer in the same transaction as the sale.
