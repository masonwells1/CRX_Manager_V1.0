...
- `payments` - Legacy payment records (DEPRECATED — use allocation_sets + invoice_line_allocations instead). Live currently has zero rows. Forward migration `20260718193000` is prepared, not applied: it will retire and fully ungrant the unused `record_invoice_payment` writer, aborting if a row appears before apply.
- Cancelled whole-conversion quotes - Live escape-hatch behavior remains permissive until prepared migration `20260718193000` is applied. The prepared contract blocks reopening when delivered or any nonterminal invoice/paid-commission activity survives and retires the unused `restore_cancelled_order` status-only RPC, whose historical body did not reconstruct cancellation's inventory or financial effects. A new serialized invoice trigger rejects every invoice linked to a deleted/voided order and every whole-order invoice linked to a cancelled order; its sole cancelled-order exception is the audited completed-delivery recovery RPC, backed by the private RLS-protected `invoice_delivery_recovery_capabilities` table with no application-role grants. Invoice source lineage and recovered items are immutable, posting revalidates the completed delivery plus exact quantities and cents, and canonical cancel/void/accounting lifecycle updates remain available.
...
`supabase/migrations/20260717042803_supplier_pricing_phase1a.sql` is the applied-live additive bootstrap: a dedicated `products.pricing_version`, private/RLS-enabled `pricing_workbook_exports` + export rows, `pricing_change_sets` + approved rows + preview rows, three admin-only pricing RPCs, collision-safe fingerprints, durable apply idempotency, and trigger-owned history for governed writes. Supabase ledger version/name: `20260717042803` / `20260717120000_supplier_pricing_phase1a`. Forward correction `20260718154131_20260718124517_harden_supplier_pricing_cent_scale_and_trigger.sql` is live as version `20260718154131` under its preserved live name; its byte-identical live artifact is pinned separately while the active replay copy conditionally skips only the no-row assertion on an empty catalog. It hardens exact-cent pricing boundaries, durable replay, per-acre preview parity, grants, and the 5,000-row export ceiling. The Phase 1a frontend is live from PR #163. `scripts/.staging-migrations/20260717121000_supplier_pricing_phase1a_cutover.sql` remains a separate post-frontend cutover that removes direct pricing/history grants and makes the governed path mandatory; it is not current production schema. Production `process-document` v18 still has the old supplier-pricing OCR routes until its separately approved deployment.
