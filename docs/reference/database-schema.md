...
- `payments` - Legacy payment records (DEPRECATED — use allocation_sets + invoice_line_allocations instead). Live currently has zero rows. The unused `record_invoice_payment` writer is retired and ungranted by the applied gauntlet corrections.
- Cancelled whole-conversion quotes and terminal-order invoices - Quote reopen/history guards and the governed split terminal lifecycle are live through ledger `20260719060256`. Prepared PR #168 migration `20260719120000` preserves those definitions and closes the remaining invoice/order mutation routes: direct header/item mutation fails closed; full cancellation delegates through the private provenance-aware implementation; approved partial close uses a bound short-close request; and completed-delivery recovery requires a private transaction claim plus exact historical line evidence. The private capability tables use deny-all RLS and no application-role grants. No PR #168 production database change occurs until the exact migration is reviewed and approved.
