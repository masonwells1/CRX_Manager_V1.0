...
| Browser and PostgreSQL text folding are different runtime boundaries | For durable cross-device document identity, normalize both sides with the reviewed NFKC → lowercase → NFKC sequence and pin a non-ASCII SHA-256 parity vector in the migration. Lock and require an empty claim table before changing the algorithm; otherwise old and new hashes can represent the same document. Never treat a browser-only normalized key as the final uniqueness boundary. |
