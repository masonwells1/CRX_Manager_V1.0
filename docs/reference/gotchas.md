...
| **`transfer_job_to_invoice` (job→invoice) parity** | Bring it up to the standard its siblings already enforce: (1) **strict actor** — bind `v_actor := auth.uid()` and `RAISE 'ACTOR_MISMATCH'` if `p_performed_by` differs (like `save_job` / `complete_job`); use the bound actor for `created_by` + `activity_feed`. (2) **write the `invoice_created` `financial_audit_log` row** (like `create_invoice_from_order`). (3) **derive the invoice header from the summed line items**, not the client-trusted `jobs.total_price_cents`. (4) On the multi-customer share path, **reconcile `invoice_shares` to the header unconditionally** (not only when a price override exists) and largest-remainder the pennies. | This one RPC carries 4 of the recurring billing-bug classes at once (forgeable actor, missing audit row, header≠sum(lines), shares don't sum to total). A parked strict-actor fix already exists on `feat/as-applied-invoices` — coordinate so it's fixed once. |
| **Cancel/void must not strand a batched commission** | A commission queued into a `commission_payments` batch stays `status='pending'` until the batch is posted. Before zeroing a pending commission, **all three** cancel paths (`cancel_order`, `void_order`, `cancel_delivery`) must skip — or first void/adjust — commissions linked into a non-voided batch: `... AND NOT EXISTS (SELECT 1 FROM commission_payment_items cpi JOIN commission_payments cp ON cp.id=cpi.commission_payment_id WHERE cpi.commission_id = c.id AND cp.status <> 'voided')` (the guard already in `update_order_items`). And `post_commission_payment` must add `AND status='pending'` (and `RAISE` if fewer rows update than the batch has items). | Cancelling/voiding an order (or cancelling a delivery) zeroes a still-pending commission that's already in an unposted batch; the batch keeps the stale snapshot; `post_commission_payment` then flips the cancelled row `cancelled → 'paid'` with no status guard → **real money paid out for a cancelled order** + a `financial_audit_log` row for the stale amount. |
| **Prepay credit ↔ invoice must be same customer** | `apply_prepay_to_invoice` must verify the credit and invoice belong to the same customer before applying: after loading `v_credit` + `v_inv` (both `FOR UPDATE`), `IF v_credit.customer_id IS DISTINCT FROM v_inv.customer_id THEN RAISE EXCEPTION 'CUSTOMER_MISMATCH ...'`. This one guard also protects `batch_apply_prepayments` (forwards caller pairs verbatim). | No customer-match check exists, so a privileged direct/bulk RPC call can debit Customer A's `prepay_credits.balance_cents` to settle Customer B's invoice and wrongly decrement B's `customers.prepay_balance_cents` → wrong-account money + both prepay caches desynced. The UI scoping to one customer is client-side only; the DB never re-validates. |
