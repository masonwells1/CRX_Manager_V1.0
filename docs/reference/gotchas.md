...
| General Invoice Detail rewrites do not preserve `invoice_items.order_item_id` | `_save_invoice_scoped_impl` rebuilds line items without that source-line field. Editing a delivery/order-generated draft in the general editor can therefore strip the lineage a later return needs and conservatively leave its COGS reversal at zero. Until the server writer carries the field, do not edit generated delivery/order invoices there; void/recreate them or use the governed source workflow. |
