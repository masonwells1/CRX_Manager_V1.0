...
| `migration-apply-guard.mjs` | Supabase MCP `apply_migration` calls — refused unless `.claude/session-state/migration-review-<name>.json` proof exists from a recent (<30 min) `rls-security-reviewer` + `migration-drift-reviewer` run, with `queryHash` binding the proof to the exact transmitted SQL. ALSO (settled 2026-07-13 policy), three flag-driven rule-sets: **flag absent** → interactive rules (Mason's in-chat OK is the prose gate). **`AUTOPILOT.on` active** → hands-free rules: (a) a DESTRUCTIVE migration — apply-time `DROP TABLE`, `DROP SCHEMA/DATABASE/OWNED`, `ALTER TABLE … DROP COLUMN`, `TRUNCATE`, ANY top-level `DELETE FROM`, or `MERGE INTO` (`destructiveMigrationCheck` in `live-testdata-lib.mjs`; function bodies exempt, `DO` blocks count, no table allowlist) — is refused even with a clean proof; (b) the reviewer proof must carry a nonempty `queryHash` exactly matching the transmitted SQL; (c) a separate content-bound Codex proof `codex-review-mig-<safe-name>.json` must exist with `queryHash` matching the transmitted SQL, `verdict` in clean/ship/ship-with-followups, and a fresh (<30 min) `timestamp` — the second-model gate must have actually run and PASSED on this exact SQL. **Flag exists but expired/malformed** → the authorization LAPSED: ALL applies blocked until Mason re-arms or disarms (`--off` deletes the flag). ALSO a **one-shot replay check**: migrations listed in `supabase/baselines/one-shot-migrations.json` are population-bound repairs that must never run twice, so any submission that matches one is refused unless a digest-, project- and time-bound `.claude/session-state/one-shot-replay-override.json` authorizes that exact SQL (consumed on sight; the registry fails closed if absent or unparseable). Matching runs on two independent layers: normalized TEXT (name, whole-body containment, shared write statement), and — since 2026-08-12, round 23 — SEMANTICS, via `apply-time-dml-lib.mjs`, which extracts the `(table, column)` pairs the SQL writes **at apply time** and refuses on any overlap. Since round 27 that overlap is compared **table-level**: a trigger scoped `BEFORE UPDATE OF total_price, profit, cost_per_unit, total_units_needed` recomputes the registered money from a sibling column, so `SET profit = profit` re-ran the registered repair while sharing no column with it. The column is still reported, but never narrows the match; measured cost is two override prompts across all 882 migrations. Apply-time includes `DO` blocks. A `CREATE FUNCTION`/`PROCEDURE` body is deferred only until something calls it (round 24), so bodies are parsed and filed by name, and any routine the migration actually invokes — through `CALL`, `PERFORM`, `SELECT`, a `FROM` clause, or another helper — has its writes folded back in transitively; overloads sharing a name are unioned rather than resolved, which can only over-report. Round 28 closed the other half of round 24: attaching a trigger still runs nothing, but the attachment survives its statement, so every `CREATE TRIGGER` is recorded as `{table, function}` and the attached function's writes are folded in as soon as this file writes that relation — re-checked after every fold-in round, since a body folded in this round can fire a trigger on a relation it writes. Attaching a mutating function to a scratch table and inserting one row was the reproducer; a trigger function that lives only in the database is reported as an unknown call and fails closed. Statement ordering is deliberately not modelled (DML above the `CREATE TRIGGER` is charged anyway), and measured cost is one override prompt across all 882 migrations. Since round 25 the reader **fails closed by default**: unresolvable targets, a call into a routine that lives only in the database, unparseable SQL, and — the round-25 inversion — **any write verb whose relation the reader cannot read** all refuse rather than pass, so a construct nobody anticipated fails toward the override prompt instead of through it. Round 26 extended that to `format` in every shape: a placeholder can split the write verb itself (`format('UP%sATE public.order_items SET …', 'D')` runs an UPDATE that no literal in it spells), so the round-25 inversion never fired on it. The accepted price, pinned by its own test, is that an `EXECUTE` whose operand is a format call reaches the override prompt even when the statement it assembles only reads; `format` used as an ordinary value expression is untouched. Round 29 closed the opposite case — an operand the reader COULD read. A single literal counted as resolved and was then scanned only for bare DML verbs, so `EXECUTE 'SELECT public.tmp_fix()'` reported nothing at all: a call is not a DML verb. Mapping an `EXECUTE` back to its own operand is unreliable here (a deferred routine body emits an empty literal placeholder without pushing a literal, and a nested dollar-quoted body numbers its literals against its own code), so position is not used: once a body is known to run dynamic SQL at all, **every** apply-time literal in it that reads as a statement is parsed as SQL and folded in — writes, routine definitions, trigger attachments and calls, transitively. A file that executes nothing is untouched, which is what keeps it affordable. The accepted over-report, pinned by its own test, is a `RAISE` message that begins with a statement verb and a table name; measured over all 882 migrations 3 files change classification, all 3 were already `unresolved` and none is a registered one-shot, so the cost is zero new prompts. Row locks, trigger timing and column lists, foreign-key actions, policy applicability, privileges and rules are the enumerated exceptions that keep a bare write verb and stay silent. Whole-column rewrites count: `ALTER COLUMN … TYPE … USING <expr>` and `COPY` are writes. Either layer alone is enough to refuse, which is why re-spelling a repair — parentheses, alias, cast, quoting, CTE, upsert — no longer evades it. Measured across the tree, zero non-registered migrations overlap the registered repair, so the semantic layer adds no friction. Tests: `apply-time-dml-lib.test.mjs` | B7/B8/B9 class — applying migrations without parallel-session review; plus unattended data loss (no PITR on the free plan), Codex-gate skips, edited-after-review SQL, and lapsed-authorization applies in hands-free runs |
| `applied-snapshot-invalidate.mjs` | **PostToolUse**, not a blocker — deletes the primary linked checkout's `.claude/session-state/applied-migrations.json` after EVERY Supabase MCP `apply_migration` call (successful or not). The no-argument refresh producer resolves that same verified primary link even when invoked from a worktree, so it cannot write evidence where the harness-pinned guard will ignore it. `migration-apply-guard.mjs` accepts the snapshot as ordering evidence while it is under 24 hours old, but elapsed time cannot prove no migration ran in between: capture a snapshot, apply a newer migration, then attempt an older one in the same session, and the still-"fresh" snapshot omits the newer row and lets the older migration through. An apply — not the clock — is the real invalidator. The next apply then blocks on missing evidence and demands a fresh capture (`node scripts/refresh-applied-migrations.mjs`). Fail-loud-ish: errors surface in `additionalContext` and never throw, and it only ever removes a regenerable cache file. | Codex P1 on PR #348 — an out-of-order live apply sneaking past a stale-but-"fresh" snapshot, the same class that silently reverted the `batch_apply_prepayments` actor guard on 2026-07-15 |
...
| `session-staleness.mjs` | Schema registry behind registry-relevant migrations (without false alarms for cron/data-only migrations); uncommitted files from a prior session; weekly DB backup missing or older than 8 days (`backups/LATEST-OK.json`, stamped by `scripts/backup-db.mjs` — Supabase FREE plan has no point-in-time recovery). Linked worktrees resolve the marker through Git's common directory so the canonical checkout's gitignored backup is visible instead of being falsely reported as missing. |
...
**The scanner resolves views before it exempts schemas (round 27).** A write to a
relation the scanner cannot account for is refused, with a fixed allowlist of
Supabase/PostgreSQL infrastructure schemas out of scope. `pg_temp` is on that
allowlist because a temp table is scratch — but a temp *view* is a writable alias
for permanent rows (`CREATE TEMP VIEW oi_shim AS SELECT * FROM public.order_items;`
then `UPDATE pg_temp.oi_shim SET profit = 0;` rewrites real rows, since a
single-table view is automatically updatable). View resolution therefore runs on
the bare relation name **before** the schema exemption, and the reported finding
keeps the schema-qualified identity. No migration in the repo writes through a
`pg_temp`-qualified relation, so this costs the corpus nothing.

**The scanner fires trigger attachments (round 28).** A mutating function appearing
under a `CREATE`/`ALTER`/`DROP`/`GRANT` statement head is skipped as a definition,
which is right for `CREATE FUNCTION` and wrong for `CREATE TRIGGER ... EXECUTE
FUNCTION`. A plain `INSERT` was also skipped outright — it adds rows rather than
rewriting them — so attaching a mutating function to a table the file just created
and inserting one row reported nothing at all. Trigger attachments are now indexed
in the same pass that indexes created tables, and fire on every relation any DML
names: INSERT, UPDATE, DELETE, MERGE, TRUNCATE, COPY. Creating a trigger and never
firing it stays silent, so an ordinary `updated_at` trigger costs nothing.

**A column type change is a whole-table rewrite (round 29).** Every channel in the
scanner reads a DML verb, and `ALTER TABLE t ALTER COLUMN c TYPE ty USING <expr>`
spells none of them while rewriting every row — the `USING` expression is evaluated
per row and its result is what gets stored. It is reported as a `retype` rewrite and
needs the same approved-set digest `TRUNCATE` does, because no narrower binding
exists when all rows changed. Both spellings are read (`TYPE` and `SET DATA TYPE`),
and the action list is walked FORWARDS from each `ALTER` — reading backwards from the
