...
>
> Last reconciled against `.claude/settings.json` hook wiring on 2026-07-13 (guard-hardening pass: `mcp-tool-guard.mjs` and `review-proof-guard.mjs` added; `bash-safety.mjs`, `hold-latch-lib.mjs`, `codex-push-lib.mjs`, `live-testdata-lib.mjs`, `idempotency-body-check.mjs` broadened — see their rows below).
...
| `review-proof-guard.mjs` | Native Write/Edit, MCP filesystem paths, or shell commands that directly name `.claude/session-state/claude-review-push.json` or `codex-review-*.json`; wired for both Claude and Codex. Legitimate review wrappers write internally without naming the proof path in the tool command. | Prevents accidental or one-line self-certification of the second-model review gate. |
...
|------|--------------------------|
| `production-action-guard.mjs` | **Pending activation by clean Claude re-review + Mason's merge OK:** non-risky diffs may target `main` (Husky pre-push still enforces typecheck/build). Risky diffs require `.claude/session-state/claude-review-push.json` with `{ "claude_ran": true, "verdict": "clean" | "blockers-fixed", "head_sha": "<exact pushed commit>", "timestamp": "<ISO-8601>" }`; the timestamp must be 0–30 minutes old, and missing/stale/future/wrong-SHA/wrong-verdict/BOM/malformed proof denies closed. Only a successful clean-worktree `node scripts/run-claude-review.mjs --scope base-main` run using the fixed `claude` executable with a SHIP/SHIP-WITH-FOLLOWUPS verdict writes that proof; NEEDS-WORK/failure revokes it, no standalone verdict writer exists, and direct proof-file access is denied. Force intent on any branch and bulk push modes (`--all`, `--branches`, `--mirror`, `--prune`) deny before target/diff classification. Both guards recognize `git`/`git.exe`/quoted paths, resolve `git -C`, inspect chained pushes, reject shell cwd or git-environment overrides, honor Codex tool `workdir`/`cwd` as the inspected repository, and fail closed on unresolved refs/diffs. Server-side routes (`gh pr merge`, relative/full-URL direct REST merges, GitHub MCP `merge_pull_request`) additionally require `mergeStateStatus=CLEAN` plus a non-empty rollup where every check is completed success/neutral/skipped. GraphQL merges, unrecognized mutating `gh api` calls, direct GitHub write tools, and `node_repl` deny closed; read-only GitHub tools remain available. `execute_sql` accepts only one read-only SELECT/WITH/EXPLAIN/SHOW statement and a narrow allowlist of read-only PostgreSQL functions; custom/application RPC calls deny. `git push origin :main`, protected `master`/`production` routes, deploy/live-migration commands, non-read-only SQL, and edge-function/live-service mutation remain blocked. Allow emits no hook output; deny emits structured JSON; the manifest grants this heavier guard 15 seconds. This is deterministic honest-agent workflow enforcement, not a cryptographic sandbox against deliberately obfuscated arbitrary shell code. |
...
**Ledger guard (pre-commit, HARD — Mason 2026-07-13):** `scripts/check-ledger-update.mjs` runs FIRST in `.husky/pre-commit` and BLOCKS any commit that stages agent-surface/policy files (`.claude/commands|skills|hooks|workflows|settings.json`, any `.codex/` file, `AGENTS.md`, `CLAUDE.md`, `.husky/`, `scripts/check-*|validate-*|verify-*`, `sync-agent-workflows.mjs`) without also staging at least one ledger update (`docs/CHANGELOG.md`, any `docs/manual/*.md`, this file, or a `docs/loops/` ledger). Converts the "record every command/policy change in the same commit" prose rule into a hard gate. Tests: `scripts/check-ledger-update.test.mjs` (in `test:correction-guards`). Never bypass with `--no-verify`; never satisfy it with a throwaway line.
