...
|------|--------------------------|
| `production-action-guard.mjs` | **Pending activation by clean Claude re-review + Mason's merge OK:** non-risky diffs may target `main` (Husky pre-push still enforces typecheck/build). Risky diffs require `.claude/session-state/claude-review-push.json` with `{ "claude_ran": true, "verdict": "clean" | "blockers-fixed", "head_sha": "<exact pushed commit>", "timestamp": "<ISO-8601>" }`; the timestamp must be 0–30 minutes old, and missing/stale/future/wrong-SHA/wrong-verdict/BOM/malformed proof denies closed. Main-bound force intent (`--force`, `-f`, `--force-with-lease`, `+` refspec) is denied before diff classification. Server-side routes (`gh pr merge`, direct `gh api .../pulls/<n>/merge`, GitHub MCP `merge_pull_request`) additionally require `mergeStateStatus=CLEAN` plus a non-empty rollup where every check is completed success/neutral/skipped. `git push origin :main`, protected `master`/`production` routes, deploy/live-migration commands, non-read-only SQL, and edge-function/live-service mutation remain blocked. Allow emits no hook output; deny emits structured JSON; the manifest grants this heavier guard 15 seconds. Claude writes the proof after a real review with `node scripts/write-claude-push-proof.mjs --verdict clean|blockers-fixed`. This proof is freshness/SHA binding for honest-agent workflow enforcement, not cryptographic provenance. |
