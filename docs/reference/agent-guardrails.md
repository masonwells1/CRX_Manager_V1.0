...
|------|--------------------------|
| `production-action-guard.mjs` | **Pending activation by Claude review + Mason's merge OK:** non-risky diffs may target `main` (Husky pre-push still enforces typecheck/build). Risky diffs require `.claude/session-state/claude-review-push.json` with `{ "claude_ran": true, "verdict": "clean" | "blockers-fixed", "head_sha": "<exact pushed commit>", "timestamp": "<ISO-8601>" }`; the timestamp must be 0–30 minutes old, and missing/stale/future/wrong-SHA/wrong-verdict/BOM/malformed proof denies closed. The same classifier/proof gate covers `git push`, refspec and `git -C` forms, `gh pr merge`, and GitHub MCP `merge_pull_request`. `git push origin :main`, protected `master`/`production` routes, force/deploy/live-migration commands, non-read-only SQL, and edge-function/live-service mutation remain blocked. Allow emits no hook output; deny emits structured JSON. Claude writes the proof after a real review with `node scripts/write-claude-push-proof.mjs --verdict clean|blockers-fixed`. |
