...
|------|--------------------------|
| `production-action-guard.mjs` | **Pending activation by clean Claude re-review + Mason's merge OK:** non-risky diffs may target `main` (Husky pre-push still enforces typecheck/build). Risky diffs require `.claude/session-state/claude-review-push.json` with `{ "claude_ran": true, "verdict": "clean" | "blockers-fixed", "head_sha": "<exact pushed commit>", "timestamp": "<ISO-8601>" }`; the timestamp must be 0–30 minutes old, and missing/stale/future/wrong-SHA/wrong-verdict/BOM/malformed proof denies closed. Only a successful clean-worktree `node scripts/run-claude-review.mjs --scope base-main` run with a SHIP/SHIP-WITH-FOLLOWUPS verdict writes that proof; NEEDS-WORK/failure revokes it, and no standalone verdict writer exists. Force intent on any branch and bulk push modes (`--all`, `--branches`, `--mirror`, `--prune`) deny before target/diff classification; both guards resolve `git -C` and inspect chained pushes. Server-side routes (`gh pr merge`, relative/full-URL direct REST merges, GitHub MCP `merge_pull_request`) additionally require `mergeStateStatus=CLEAN` plus a non-empty rollup where every check is completed success/neutral/skipped. GraphQL merges, unrecognized mutating `gh api` calls, direct GitHub write tools, and `node_repl` deny closed; read-only GitHub tools remain available. `git push origin :main`, protected `master`/`production` routes, deploy/live-migration commands, non-read-only SQL, and edge-function/live-service mutation remain blocked. Allow emits no hook output; deny emits structured JSON; the manifest grants this heavier guard 15 seconds. This is deterministic honest-agent workflow enforcement, not a cryptographic sandbox against deliberately obfuscated arbitrary shell code. |
