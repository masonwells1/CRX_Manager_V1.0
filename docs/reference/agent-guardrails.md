...
with no alias there is nothing to launder. The proof file does not exist before
the wrapper mints it, so a junction followed by a dynamically assembled hard-link
command could otherwise make the alias as the final write, leaving no later file
operation for identity checking. Agent shell routes therefore deny filesystem
link creators as a class (`mklink`, `ln`, `fsutil`, and PowerShell link/reparse
...
recognized safe literal (`File` or `Directory`) or the
...
closed, while ordinary file and directory creation is unaffected. A real
