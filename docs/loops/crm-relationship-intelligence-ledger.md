...
| 2.2 facts UI + review queue | terra | GATE | — | PROOF — Ran: full gate (lint 0 err, typecheck, 3503 vitest, build) orchestrator-run after types regen · Saw: green; orchestrator review caught + fixed 3 issues terra's gate missed: saveFact never invoked (`void saveFact` → `void saveFact()`), 2 null-vs-generated-type mismatches on the fact RPCs |
| 2.3 prep card | terra | GATE | — | PROOF — same gate · Saw: green; assertRpcResult + formatCents patterns verified; tel: link uses phone_e164; prep card renders on info tab, knowledge tab wired |
