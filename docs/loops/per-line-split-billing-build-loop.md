...
| 4 — UI + mail gates | **IN PROGRESS** | `codex/per-line-split-billing-phase4-ui` | PROOF — local deterministic hard proof currently green: exact flag read; preview/save share the options builder and call the Phase 2/3 RPCs; Mode A query precedes both preview and write; stored server cents render; five UI surfaces plus the second field-app send path and the edge function refuse `suppressed_zero_total` | Review pending after full pipeline and render proof | flag remains OFF; edge-function change is code-only and will not be deployed |
