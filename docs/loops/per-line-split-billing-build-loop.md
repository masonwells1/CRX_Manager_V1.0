...
| 2 — calculator | **IN PROGRESS** | `claude/split-billing-phase2-calculator` / PR #167 | PROOF — Ran: exact preflight + Phase 1 + relational-guard + Phase 2 migrations verbatim in network-isolated PostgreSQL 17 with rollback-only assertions · Saw: stale objects/functions and enabled flag rejected; installed function owner/definer/search-path/ACL posture asserted; pre-existing private helper, drifted public legacy body/security/ACL, and authenticated caller-owned predictable temp helper rejected; flag OFF delegated unchanged; owner/outsider sales-rep job/invoice/field scope and exact context binding; 50/50, exact 3-way, 1¢, and signed sales −13→−7/−6 plus signed COGS −25→−13/−12 persisted through the durable graph; one final money round after full-precision conversion; source COGS rounded once and a 1¢ source allocated exactly 1¢/0¢; exact job-chemical identity/quantity/unit/frozen price/provenance/COGS for duplicate products plus coherent quantity/price rewrite rejection; browser tampering ignored; nullable/negative frozen prices rejected; complete durable job-chemical/service source sets and exact service identity; 1,600 oz × 25¢/oz preserved; complete job-field-set enforcement; customer-supplied $0/COGS; non-job manual→customer/season/lifecycle-bound quote→tier with quote prices normalized from their paired unit (`$10/qt`→`$40/gl`), complete contributing-field coverage required, zero-share-field/stale/unrelated quotes ignored, and unconvertible units, missing product cost, partial coverage, and price/source ambiguity rejected; source-season service pricing; per-person override; default-price/base coupling; 100/0 and flat-fee output persisted with quantity 1 and exact kind/basis coupling; job snapshot/default/fallback; exact grouped/ungrouped invoice membership and share-item identity; allocated COGS/source/header parity; posted freeze/history; zero-send disposition; canonical hashes; Mode A/vector/role rejection; overload/privilege posture; cleanup failure fatal | exact-SHA review at `b7b35f33` found signed return COGS could preview but not persist and `flat_fee` was absent from the preview TypeScript union; both reproduced/repaired. Exact-SHA review at `1af4cbc2` found quote price/unit pairing was ignored and NULL product cost was silently treated as zero. Exact-SHA review at `9a17f0fd` found caller-owned temp relations could cross the SECDEF boundary and one field's quote could price other fields. Exact-SHA review at `e2dbe602` found coherent job quantity/base-price rewrites, flat-fee basis mismatch, and same-signature function ownership drift. Every finding failed first in the disposable proof and is repaired; the combined proof is green. Fresh content-bound security + drift and exact-SHA Codex + Claude reviews remain required before push | exact reviewed Phase 1 SHA `7cd00757` remains byte-stable; additive preflight `20260718205500` and guard `20260718210500`; flag remains OFF; no live apply; §6.1 baseline still unmet; unrelated live quote Q-2026-2059 has a blank commission recipient |
