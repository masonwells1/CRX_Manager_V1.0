...
| 4 — UI + mail gates | **DONE — draft PR handoff pending** | `claude/split-billing-phase4-ui` / draft PR pending | PROOF — Ran: focused rendered editor/settings/send/rendering contracts plus local Vite boot and live read-only catalog check · Saw: exact micro-percent input, all recipient prices, complete override vector, fail-closed Mode A/flag/rehydration gates, frozen job-chemical IDs in preview/save, ungrouped item→share→line billing-set recovery, six browser pre-send reads, server-derived proof/invoice disposition+lifecycle enforcement, stored-cent renderer contracts, live `send_disposition`, missing live flag, zero split rows, legacy four-arg preview, and no new writer | Codex review cycles fixed preview args, proof bypass, legacy recipients, and terminal sends; Claude found missing job-chemical IDs and blocked ungrouped rehydration, then returned SHIP-WITH-FOLLOWUPS with no blocker/high after repair; 57 focused tests and full suite green | clean Phase 4 branch is based on current `origin/main` so it does not inherit the separate colliding migrations; this branch applied/deployed nothing; PARK Phase 1–3 apply; edge not deployed |
