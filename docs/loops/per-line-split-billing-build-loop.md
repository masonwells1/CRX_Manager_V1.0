...
| 3 — save/post RPC | **PARKED — 3-round cap** | `claude/split-billing-phase3-rpc` / no PR | PROOF — Ran: exact preflight + Phase 1 + guards + Phase 2 + Phase 3 writer in network-isolated PostgreSQL 17, with rollback-only fixtures · Saw: calculator-driven 100/0 persistence, grouped and single-recipient rebuilds, posted suppressed-zero snapshot, idempotent replay/conflict, header rejection, audit rows, transfer refusal, and owner/ACL guards passed | security CLEAN; exact Codex rounds 1–2 found/fixed grouped and single-recipient restrictive-FK rebuild failures; round 3 found unresolved authorization-before-replay and source-job-season defects, so the binding cap parks this phase | flag OFF; no live apply/push/PR; live high-water read-only verified `20260720203000` on 2026-07-20 |
| 4 — UI + mail gates | **PARKED — dependency blocked** | — | — | — | Phase 3 is parked; no safe writer contract exists for UI integration; edge-fn deploy remains a Mason gate |
