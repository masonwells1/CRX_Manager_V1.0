...
| 3 — save/post RPC | **DONE (draft PR)** | `codex/per-line-split-billing-phase3-rpc` / PR #181 | PROOF — Ran: Phase 1 + guards + Phase 2 + Phase 3 verbatim in network-isolated PostgreSQL 17 · Saw: calculator cases remained green; ordinary drafts refused unchanged; 100/0 save produced two invoices/items/shares; draft group edit rebuilt dependency-first; actor/group-bound replay and authorization, conflict, stale-header, source-season, zero suppression, post snapshot, and transfer gates green; full pipeline 3,659 passed / 117 skipped | Fresh exact-HEAD Codex review CLEAN at `99f29682` after the partial-group authority, stale original header, and source-season repairs; pushed to draft PR #181 | flag OFF; no live apply; §6.1 baseline unmet; CodeRabbit must run before merge |
| 4 — UI + mail gates | **DONE (draft PR)** | `codex/per-line-split-billing-phase4-ui` / PR #182 | PROOF — Ran: `PHASE4_HARD_PROOF_PASS`, typecheck, lint, production build, focused UI/email tests, full suite, agent-workflow and correction-guard suites, docs drift · Saw: exact OFF-by-default flag read; job-owned complete field set; preview/save share one options builder and call the Phase 2/3 RPCs; Mode A query precedes preview and write; exact micro-percent parsing; stored server cents render; group and single-invoice post paths; five UI surfaces plus the second field-app send path and code-only edge defense refuse `suppressed_zero_total`; full suite 3,673 passed / 117 skipped | Exact Phase 4 review found and verified the job-field repair. Follow-up suggestion for an in-app flag switch was dismissed as outside the authoritative controlled-rollout contract; flag enablement remains a Mason-gated live action | flag OFF; no live apply; edge function not deployed; stacked draft PR #182; CodeRabbit must run before merge |
