...
| 2 — calculator | **DONE (draft PR)** | `claude/split-billing-phase2-calculator` / PR #167 | PROOF — Ran: re-stamped preflight `20260720230000` + reviewed Phase 1 `20260720231000` + guards `20260720232000` + calculator `20260720233000` verbatim in network-isolated PostgreSQL 17 · Saw: helper ownership collision and enabled flag rejected before clean install; all signed sales/COGS, quote-unit/provenance/coverage, job authority, flat-fee, temp-object, membership, header, history, and calculator cases green | content-bound security/drift CLEAN; exact-HEAD Codex CLEAN and Claude SHIP-WITH-FOLLOWUPS at `d869bf43`; Vercel SUCCESS; CodeRabbit skipped while draft | flag OFF; no live apply; §6.1 baseline unmet; re-check live high-water at apply |
| 3 — save/post RPC | **DONE — draft review handoff** | `claude/split-billing-phase3-rpc` / draft PR #183 | PROOF — Ran: exact preflight + Phase 1 + guards + Phase 2 + Phase 3 writer in network-isolated PostgreSQL 17, with rollback-only fixtures · Saw: calculator-driven 100/0 persistence stamped with source-job season and live-valid price source, grouped/single rebuilds, single→group source retirement, posted suppressed-zero snapshot, actor-bound replay, payload conflict, header rejection, audit rows, transfer refusal, and owner/ACL guards passed | exact-SHA Codex CLEAN; Claude SHIP-WITH-FOLLOWUPS with no blocker/high after its first-round live `price_source` and empty-source-draft findings were fixed | flag OFF; no live apply; live high-water read-only verified `20260720211454` on 2026-07-20 |
