...
| 3 — save/post RPC | **IN PROGRESS** | `claude/split-billing-phase3-rpc` / draft PR pending | PROOF — Ran: exact preflight + Phase 1 + guards + Phase 2 + Phase 3 writer in network-isolated PostgreSQL 17, with rollback-only fixtures · Saw: the calculator-only 100/0 service plan persisted as two invoices/items/shares, including a posted suppressed-zero child and immutable snapshot; required idempotency, exact replay, payload conflict, header-integrity rejection, audit rows, transfer-path refusal, and owner/ACL guards passed | content-bound security CLEAN; drift parked only on intentionally unapplied prerequisites and doc/high-water gates; exact-HEAD reviews pending | flag OFF; no live apply; live high-water read-only verified `20260720203000` on 2026-07-20 |
