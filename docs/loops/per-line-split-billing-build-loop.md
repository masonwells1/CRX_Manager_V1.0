...
| 4 — UI + mail gates | **IN PROGRESS — review repairs** | `claude/split-billing-phase4-ui` / draft PR pending | PROOF — Ran: focused rendered editor/settings/send/rendering contracts plus local Vite boot · Saw: exact micro-percent input, all recipient prices, complete override vector, fail-closed Mode A/flag/rehydration gates, six browser pre-send reads, server-derived proof/invoice `sendable` enforcement, and stored-cent renderer contracts green; authenticated end-to-end UI remains unavailable without test credentials | first exact-commit Codex review found 3 P1s (preview arg, proof edge bypass, legacy shared-recipient send); all fixed, exact-HEAD rerun pending; Claude blocked on session limit until 19:40 CT | flag OFF; no live apply; send-email edge not deployed; production unchanged |
