...
| 4 — UI + mail gates | **DONE — draft PR open** | `claude/split-billing-phase4-ui` / PR #187 | PROOF — Ran: focused rendered editor/settings/send/rendering contracts plus local Vite boot and live read-only catalog check · Saw: exact micro-percent input, all recipient prices, complete override vector, fail-closed Mode A/flag/rehydration gates, frozen job-chemical IDs in preview/save, ungrouped item→share→line billing-set recovery, six browser pre-send reads, server-derived proof/invoice disposition+lifecycle enforcement, stored-cent renderer contracts, live `send_disposition` contract (`normal` / `suppressed_zero_total`, 12 current rows all `normal`), missing live flag, zero split rows, legacy four-arg preview, and no new writer | exact-HEAD Codex CLEAN; exact-HEAD Claude SHIP-WITH-FOLLOWUPS with no blocker/high; full commit and push pipelines green; duplicate-product source ambiguity is server-rejected by `SPLIT_DUPLICATE_PRODUCT` | reconciled against current `origin/main`; this branch applied/deployed nothing; feature stays OFF; edge not deployed; CodeRabbit/Vercel PR gates pending |
