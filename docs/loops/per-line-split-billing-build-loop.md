...
| 4 — UI + mail gates | **IN PROGRESS — Claude review pending** | `claude/split-billing-phase4-ui` / draft PR pending | PROOF — Ran: focused rendered editor/settings/send/rendering contracts plus local Vite boot and live read-only catalog check · Saw: exact micro-percent input, all recipient prices, complete override vector, fail-closed Mode A/flag/rehydration gates, six browser pre-send reads, server-derived proof/invoice disposition+lifecycle enforcement, stored-cent renderer contracts, missing live flag, zero split rows, legacy four-arg preview, and no new writer | exact-commit Codex fixed 3 P1s; HEAD-bound proof then fixed terminal-invoice send and returned CLEAN at `c8eef6b9`; Claude blocked on session limit until 19:40 CT | this branch applied/deployed nothing; separate live older split chain collides with pending objects/timestamps; PARK apply; edge not deployed |
