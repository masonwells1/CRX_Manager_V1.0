...
| 3 — save/post RPC | **IN PROGRESS — owner-authorized fresh review cycle** | `claude/split-billing-phase3-rpc` / draft PR pending | PROOF — Ran: exact preflight + Phase 1 + guards + Phase 2 + Phase 3 writer in network-isolated PostgreSQL 17, with rollback-only fixtures · Saw: calculator-driven 100/0 persistence stamped with the source job season, grouped and single-recipient rebuilds, posted suppressed-zero snapshot, same-actor replay, cross-actor replay rejection, payload conflict, header rejection, audit rows, transfer refusal, and owner/ACL guards passed | prior round-3 blockers fixed: authorization now precedes cache lookup, replay is actor-bound, and child invoices inherit the source job season; fresh exact-SHA Codex + Claude review pending | flag OFF; no live apply; live high-water read-only verified `20260720211454` on 2026-07-20 |
