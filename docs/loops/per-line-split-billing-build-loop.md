...
| 3 — save/post RPC | **IN REVIEW** | `codex/per-line-split-billing-phase3-rpc` | PROOF — Ran: Phase 1 + guards + Phase 2 + Phase 3 verbatim in network-isolated PostgreSQL 17 · Saw: calculator cases remained green; 100/0 save produced two invoices/items/shares; draft group edit rebuilt dependency-first without RESTRICT-FK failure or orphans; replay/conflict/header mismatch/zero suppression/post snapshot/transfer actor+feature gates green; full pipeline 3,659 passed / 117 skipped | RLS/security CLEAN; exact-HEAD Codex round 1 found draft-edit deletion order, round 2 found cross-actor cached replay; both fixed with regressions; final exact-HEAD review pending | flag OFF; no live apply; drift review's only HIGHs are the intentionally absent prerequisite schema and deferred apply-time high-water check; fresh clean proof required at Mason's later apply gate |
