...
| 3 — save/post RPC | **PARKED (review cap)** | `codex/per-line-split-billing-phase3-rpc` | PROOF — Ran: Phase 1 + guards + Phase 2 + Phase 3 verbatim in network-isolated PostgreSQL 17 · Saw: calculator cases remained green; 100/0 save produced two invoices/items/shares; draft group edit rebuilt dependency-first without RESTRICT-FK failure or orphans; replay/conflict/header mismatch/zero suppression/post snapshot/transfer actor+feature gates green; full pipeline 3,659 passed / 117 skipped before final parked repair | RLS/security CLEAN; exact-HEAD Codex round 1 found draft-edit deletion order, round 2 found cross-actor cached replay, round 3 found ordinary-draft adoption; all fixed with regressions, but the three-round cap requires a fresh review cycle before publication | flag OFF; no live apply; Phase 4 remains blocked on Phase 3 certification; drift review's only HIGHs are intentionally absent prerequisites and deferred apply-time high-water proof |
