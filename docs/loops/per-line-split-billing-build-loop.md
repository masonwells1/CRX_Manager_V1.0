...
Phase 1 (schema) is isolated in migration `20260720231000` and draft PR **#166**,
...
|---|---|---|---|---|---|
| 1 — schema | **DONE (draft PR)** | `claude/billing-splits-plan-8ih4jg` / PR #166 | PROOF — Ran: exact `20260720231000` in network-isolated PostgreSQL 17 plus concurrent sessions · Saw: integrity/history/RLS/race cases green; forged group-only/server-bound/deleted membership reads `0|0` / `1|2` / `0|0`; enabled flag rejected and preserved | content-bound security + drift CLEAN; exact-HEAD Codex CLEAN and Claude SHIP-WITH-FOLLOWUPS at `86bd40bb`; CodeRabbit SUCCESS; Vercel/CodeQL pending after branch push | flag OFF/absent; not applied; above live high-water `20260719092832`; re-check at apply; §6.1 baseline unmet |
| 2 — calculator | **DONE (draft PR)** | `claude/split-billing-phase2-calculator` / PR #167 | PROOF — Ran: re-stamped preflight `20260720230000` + reviewed Phase 1 `20260720231000` + guards `20260720232000` + calculator `20260720233000` verbatim in network-isolated PostgreSQL 17 · Saw: helper ownership collision and enabled flag rejected before clean install; all signed sales/COGS, quote-unit/provenance/coverage, job authority, flat-fee, temp-object, membership, header, history, and calculator cases green; typecheck/lint/build green; full suite 3,658 passed / 117 skipped with the repository's heavy-workbook timeout raised for the run only | content-bound security + drift CLEAN; exact-HEAD Codex CLEAN at `d869bf43`; Vercel and preview checks SUCCESS | flag OFF; no live apply; §6.1 baseline unmet; re-check live high-water at apply; CodeRabbit skipped while PR remains draft and must run before merge |
| 3 — save/post RPC | **DONE — draft review handoff** | `claude/split-billing-phase3-rpc` / draft PR #183 | PROOF — Ran: exact preflight + Phase 1 + guards + Phase 2 + Phase 3 writer in network-isolated PostgreSQL 17, with rollback-only fixtures · Saw: calculator-driven 100/0 persistence stamped with source-job season and live-valid price source, grouped/single rebuilds, legacy-draft conversion refusal with its item graph preserved, single→group source retirement, posted suppressed-zero snapshot, actor-bound replay, payload conflict, header rejection, audit rows, transfer refusal, and owner/ACL guards passed | fresh reconciliation review found and fixed unsafe legacy-draft replacement; exact-HEAD Codex CLEAN at `5b92c716`; prior Claude SHIP-WITH-FOLLOWUPS had no blocker/high after its first-round live `price_source` and empty-source-draft findings were fixed | flag OFF; no live apply; live high-water read-only verified `20260720211454` on 2026-07-20 |
| 4 — UI + mail gates | **DONE — draft PR handoff pending** | `claude/split-billing-phase4-ui` / draft PR pending | PROOF — Ran: focused rendered editor/settings/send/rendering contracts plus local Vite boot and live read-only catalog check · Saw: exact micro-percent input, all recipient prices, complete override vector, fail-closed Mode A/flag/rehydration gates, frozen job-chemical IDs in preview/save, ungrouped item→share→line billing-set recovery, six browser pre-send reads, server-derived proof/invoice disposition+lifecycle enforcement, stored-cent renderer contracts, live `send_disposition` contract (`normal` / `suppressed_zero_total`, 12 current rows all `normal`), missing live flag, zero split rows, legacy four-arg preview, and no new writer | Codex review cycles fixed preview args, proof bypass, legacy recipients, terminal sends, and live `normal` compatibility; Claude found missing job-chemical IDs and blocked ungrouped rehydration, then returned SHIP-WITH-FOLLOWUPS with no blocker/high after repair; focused tests and full suite green | branch reconciliation against current `origin/main` pending; this branch applied/deployed nothing; PARK Phase 1–3 apply; edge not deployed |
