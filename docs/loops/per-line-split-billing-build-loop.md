...
| 4 — UI + mail gates | **IN PROGRESS — verification/review** | `claude/split-billing-phase4-ui` / draft PR pending | PROOF — Ran: focused rendered editor/settings/send/rendering contracts plus local Vite boot · Saw: exact micro-percent input, all recipient prices, complete override vector, fail-closed Mode A/flag/rehydration gates, six browser pre-send reads, server-edge `sendable` enforcement, and stored-cent renderer contracts green; authenticated end-to-end UI remains unavailable without test credentials | exact-SHA Codex + Claude reviews pending | flag OFF; no live apply; send-email edge not deployed; production unchanged |
