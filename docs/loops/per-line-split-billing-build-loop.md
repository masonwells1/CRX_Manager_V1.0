...
|---|---|---|---|---|---|
| 1 — schema | **IN PROGRESS** | `claude/billing-splits-plan-8ih4jg` / PR #166 | PROOF — Ran: the exact `20260720231000` migration in a network-isolated PostgreSQL 17 container plus real concurrent sessions · Saw: trigger ownership posture `6/6/6/6`; newly uncovered foreign-key indexes `5/5`; source-line move, posted-share reparent, shared-item parent reparent, posted linked-item quantity/acres/unit/extended edits, posted parent-cascade deletion, empty-line, authenticated TRUNCATE, duplicate invoice-group parent, unrelated applicator read, cross-sales-rep source read, timestamp-less post/freeze, blank split/price reasons, browser disposition change, nonzero suppression, cancelled-invoice share/item mutation, 200% write-skew, and share-write/post race attempts rejected; the owning rep sees its `1|1` set/line while another rep sees `0|0`; the same rep sees `1|1` shares/history while active and `0|0` after role removal; the invoice and same-named decoy CHECKs both exist (`1|1`); valid posted→voided retains `posted_at` and exactly one history instance (`voided|true|1`); draft linked-item material edits and valid server zero-total suppression accepted; unpost/edit/repost retained two distinct immutable post sequences with rounding policies v1/v2 even after every working source row was deleted, while privileged history UPDATE/TRUNCATE were rejected; two distinct child invoices raced, exactly one vector writer committed, and the final vector remained exactly 100,000,000; a share writer held the item/invoice lock boundary so a concurrent poster or parent reparent could not move the snapshot across invoices; nonzero container cleanup now fails the proof; live Phase 1 objects/flag absent; live migration role `postgres` has BYPASSRLS | exact-SHA Codex found active-role, terminal-freeze, and constraint-scope gaps; all five unsafe outcomes now have red→green proofs; post-restamp exact-HEAD Codex then found forgeable/deleted group authority and enabled-flag fail-open; both now have red→green proofs (`0|0` group-only, `1|2` server-bound, `0|0` deleted, enabled flag rejected); fresh content-bound + exact-HEAD Codex + Claude publication proofs pending | flag OFF/absent; not applied live; migration `231000` was re-stamped above verified live high-water `20260719092832`; re-check at apply time; §6.1 baseline is still unmet (live field-app locations/shares/groups/job-linked invoices all zero) |
