...
|---|---|---|---|---|---|
| 1 — schema | **IN PROGRESS** | `claude/billing-splits-plan-8ih4jg` / PR #166 | PROOF — Ran: the exact `20260718210000` migration in a network-isolated PostgreSQL 17 container plus real concurrent sessions · Saw: trigger ownership posture `6/6/6/6`; newly uncovered foreign-key indexes `5/5`; source-line move, posted-share reparent, shared-item parent reparent, posted linked-item quantity/acres/unit/extended edits, posted parent-cascade deletion, empty-line, authenticated TRUNCATE, duplicate invoice-group parent, unrelated applicator read, cross-sales-rep source read, timestamp-less post/freeze, blank split/price reasons, browser disposition change, nonzero suppression, 200% write-skew, and share-write/post race attempts rejected; the owning rep sees its `1|1` set/line while another rep sees `0|0`; draft linked-item material edits and valid server zero-total suppression accepted; unpost/edit/repost retained two distinct immutable post sequences with rounding policies v1/v2 even after every working source row was deleted, while privileged history UPDATE/TRUNCATE were rejected; two distinct child invoices raced, exactly one vector writer committed, and the final vector remained exactly 100,000,000; a share writer held the item/invoice lock boundary so a concurrent poster or parent reparent could not move the snapshot across invoices; nonzero container cleanup now fails the proof; live Phase 1 objects/flag absent; live migration role `postgres` has BYPASSRLS | exact-HEAD Codex timed out only after reporting two blockers; both now repaired with red→green proofs; final exact-hash Codex + Claude verdicts pending | flag OFF/absent; not applied live; migration `210000` is above verified live high-water `20260718203206`; §6.1 baseline is still unmet (live field-app locations/shares/groups/job-linked invoices all zero) |
| 2 — calculator | **IN PROGRESS** | `claude/split-billing-phase2-calculator` / PR #167 | Combined Phase 1 + Phase 2 verbatim PostgreSQL 17 proof pending after the final Phase 1 repair merge | final exact-hash Codex + Claude reviews pending | exact Phase 1 SHA `d1022e4a` merged locally; flag remains OFF; no live apply; unrelated live quote Q-2026-2059 has a blank commission recipient |
