...
| 4 — UI + mail gates | **IN PROGRESS — blocker repair review** | `claude/split-billing-phase4-ui` / draft PR pending | PROOF — Ran: focused rendered editor/settings/send/rendering contracts plus local Vite boot and live read-only catalog check · Saw: exact micro-percent input, all recipient prices, complete override vector, fail-closed Mode A/flag/rehydration gates, frozen job-chemical IDs in preview/save, ungrouped item→share→line billing-set recovery, six browser pre-send reads, server-derived proof/invoice disposition+lifecycle enforcement, stored-cent renderer contracts, live `send_disposition`, missing live flag, zero split rows, legacy four-arg preview, and no new writer | exact-commit Codex fixed 3 P1s; HEAD-bound proof fixed terminal-invoice send; fresh Claude found missing job-chemical IDs and blocked ungrouped rehydration, now fixed with 57 focused tests green; fresh exact-HEAD reviews pending | this branch applied/deployed nothing; separate live older split chain collides with pending objects/timestamps; PARK apply; edge not deployed |
