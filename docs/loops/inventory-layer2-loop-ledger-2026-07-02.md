...
### save_quote drawn-product guard — CLOSED via A3.6-b; extension = OPTIONAL fast-follow
The save_quote drawn guard (block reducing a line below its drawn qty) currently counts ORDER draws only. Codex #2's actual STRAND vulnerability is closed by the A3.6-b guard-hardening above. Extending save_quote to also count job draws prevents an INERT orphan-draw intermediate state (hold still protects stock; accept blocked; terminal blocked; rollover GREATEST-clamps). It needs reproducing the 11KB hot-path save_quote (pricing-math transcription risk) → deliberately deferred as a targeted fast-follow, not rushed into this batch. Spec: replace the `v_drawn_guard` SELECT's `FROM quote_product_draws d` with a `UNION ALL` of quote_product_draws + job_product_draws aggregated per product, comparing new booked vs total(order+job) drawn.
