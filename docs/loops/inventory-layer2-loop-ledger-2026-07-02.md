...
## Part B COMPLETE (B1–B4 all built + reviewer-clean + green)
Remaining before the single apply/merge decision: (1) batched `/codex-review` over Part B (B1–B4), (2) final combined Codex over the whole A+B branch, (3) present ONE batched apply/merge decision. Deferred Part-A polish items (save_quote guard, db-invariant sweep, rollover/settlement trio) are LOW-risk and tracked above — decide with Mason whether to fold in before apply or ship as a follow-up.
