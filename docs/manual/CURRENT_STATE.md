...
**Last verified:** 2026-08-09. The document was re-read against current `origin/main`, the four newest on-disk migrations, and read-only production ledger/catalog evidence. Operational counts called out below remain separately dated snapshots where noted.

**2026-08-09 candidate-migration detail:** production now ends at ledger version `20260809130108` (`team_note_completion_rpc_and_assignment_notify`; 946 rows). Files `20260808150100` through `20260808150400` are **local candidates, not applied live** and now sit below the live high-water, so they must be restamped and re-reviewed before any future apply. Production therefore still lacks the restored `batch_apply_prepayments` actor wrapper, the cancellation `quantity_remaining = 0` follow-up, the inventory `TRUNCATE` revocation and legacy-`payments` comment, and the two forward-looking money-rounding triggers. Read-only proof still finds one cancelled line with non-zero remaining quantity, authenticated inventory `TRUNCATE`, 46 fractional `order_items.total_price` rows, 3 fractional commissions, and 19 negative inventory rows. The rounding candidate intentionally does not repair the 49 historical money rows.
