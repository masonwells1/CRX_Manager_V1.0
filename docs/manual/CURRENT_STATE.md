...
**Last verified:** 2026-08-09 (post-apply). Live ledger high-water is `20260809130108` (`team_note_completion_rpc_and_assignment_notify`). The database half of the Team Board delegation fix is live: `complete_team_note` authorizes the creator, current assignee, or an active admin through an actor-bound idempotent SECURITY DEFINER path, and the assignment trigger creates `task_assigned` notifications while suppressing self-assignment and inactive recipients. A fresh RLS review found that an inactive profile with a still-valid JWT can satisfy the legacy creator-only insert policy and make the owner-run trigger notify an active teammate; forward-only follow-up `20260809154649_active_team_note_assignment_actor` is written but NOT LIVE, and now requires an active actor in both the insert policy and trigger. The compatible frontend remains pending in PR #351, so delegated completion is not fully shipped until the follow-up applies and that PR merges. Live catalog/grant checks, all 26 standing invariant predicates, and a genuine schema-registry refresh passed for the installed migration; the revised rollback-only behavior smoke remains pending external execution because the Codex production guard refuses its intentional transaction-local writes. The earlier profile role-lock, CRM fact RPC, bulk-import lifecycle, idempotency, statement disclosure, and historical AR report protections remain live as documented below. Operational counts below remain the separately dated 2026-07-18 snapshot.
