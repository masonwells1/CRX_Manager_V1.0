...
**Last verified:** 2026-08-10 — the live ledger high-water and the pricing/profit aggregates described immediately below were re-read from the live database on 2026-08-10. The section 2 counts table remains the separately dated 2026-08-09 snapshot; the 2026-08-07 feature/postflight detail and deployment log were not re-checked.

**2026-08-10 live re-read.** The live ledger now contains 954 rows at high-water **`20260810025159`** (`20260810022500_backfill_stale_line_profit`). Its source file is on open PR #364 and is not yet in `origin/main`; do not duplicate that sibling session's work. The two re-issued PR #350 pricing migrations (`20260810135537` and `20260810135538`) are absent from the live ledger. Current aggregate money checks found 35 fractional `order_items.total_price` rows, 3 fractional active `commissions.commission_amount` rows, zero stale `order_items.profit` rows, and one remaining order-header-versus-lines profit mismatch. The full invariant sweep matrix was not rerun in this re-read.
