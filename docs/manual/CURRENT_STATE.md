...
**Multi-price quote-draw edge remains open; the unsafe allocation candidate was removed.** The live private `draw_down_quote` implementation can average same-product quote lines into a half-cent unit price, which the live whole-cent guard correctly rejects. Review proved the proposed cumulative-rounding/floored-unit-price repair could underbill a reconstructed invoice by one cent and could reuse a cent allocation after a reversible job draw was cancelled. The pending ownership migration now preserves that fail-closed boundary explicitly: more than one distinct booked price or cost cohort for the requested product raises `AMBIGUOUS_BOOKING_MONEY_COHORTS` before order money is derived. No allocation fix is prepared; the durable design still needs cent-priced cohorts or an immutable allocation/revenue ledger.
...
**Quote draw-down ownership/deletion and cent boundary — local candidate, not applied.** Exact review found that the live private draw implementation locks a quote but does not bind an active sales rep to `quotes.created_by`, does not reject `deleted_at`, and can carry a fractional-cent weighted unit price into an order line that the live whole-cent constraint rejects. `20260813161614_restrict_draw_down_quote_owner.sql` is a forward-only body replacement behind the unchanged governed public wrapper. It requires the owning rep or an active admin after the quote lock, hides soft-deleted quotes, rejects distinct same-product price or cost cohorts before averaging, and only then settles an unambiguous single cohort to whole cents before deriving order money. The access checks remain before idempotency replay or mutation, and the container rollback chain now proves both authorization and the `$10.00`/`$10.01` money denial. No live state has changed.
...
**Quote-section RPC ownership — local candidate, not applied.** `20260814063000_quote_sections_rpc_owned.sql` removes the application roles' remaining write-capable privileges and obsolete write policies from `quote_sections` while preserving RLS-backed SELECT and governed owner-side writers. Its container-only smoke proves authenticated INSERT/UPDATE/DELETE/TRUNCATE CASCADE are denied, service-role direct writes are absent, and SELECT still works. The related browser conversion-cancellation race is closed without a database change: cancelling below-cost approval leaves the already-committed quote accepted and makes no compensating write, so a concurrent edit or order cannot be overwritten. No live state has changed.
