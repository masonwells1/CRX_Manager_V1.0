...
**Last verified:** 2026-07-29 (read-only Supabase query against `supabase_migrations.schema_migrations` confirms **920 ledger rows** and live high-water `20260729035923` = `application_service_atomic_save`, applied live by a concurrent session at 04:00 UTC. This session did **not** independently verify what `application_service_atomic_save` does — only that it is applied and holds the high-water. The migration immediately below it, `20260729015706` = `application_service_cost_admin_only`, was also applied live by a concurrent session and **was** verified here. It revokes `authenticated`'s SELECT on `application_services.cost_per_acre_cents`, closing the last finding of the 2026-07-27 SECDEF pricing-bypass audit — drivers and sales reps can no longer read internal per-acre cost off the table. Re-verified live this date: `has_column_privilege('authenticated', …, 'cost_per_acre_cents', 'SELECT')` = false, the customer-facing `default_rate_per_acre_cents` = true, and `metabase_ro` retains its direct grant. **Its branch `fix/application-service-cost-admin-only` is applied live but not yet merged into `main`** — live is ahead of `main` on this file until that PR lands. The previous stamp, still accurate for everything else, read 918 rows at `20260728233459`; `secdef_pricing_reads_office_only` is applied live. The live July 27 table hardening and July 28 RPC hardening now jointly restrict quote pricing, per-customer rates, rebate terms, and the two remaining `SECURITY DEFINER` pricing readers to the intended office roles. Regression guard: `scripts/db-invariant-sweeps/predicates/office-only-pricing-secdef-gates.sql`. Both halves of the anon-EXECUTE revoke are also applied live — ledger `20260728231350` for the 40 functions in no RLS policy and `20260728233459` for the `is_admin`/`is_applicator`/`is_driver` role helpers — so logged-out callers no longer reach any of the 43, including the six `next_*_number()` allocators, `calculate_billing_splits` and `check_period_open`, which had no auth gate of any kind. See `docs/manual/KNOWN_ISSUES.md` §0c.)
