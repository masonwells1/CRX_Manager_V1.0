...
**Last verified:** 2026-07-31 (UTC ledger date; evening 2026-07-30 Chicago). Live high-water is `20260731001654_ap_period_close_boundary_hardening` with 932 rows. The AP migration is B7-reconciled, exact function/ACL/RLS/policy readback passed, the registered Section 9 smoke ended `SMOKE_PASS_ROLLBACK`, and zero fixtures or closed periods remained. The schema registry was rebuilt from six live-introspection queries to this high-water and includes the immediately preceding Quote/Customer row-version migration. Operational counts below remain the separately dated 2026-07-18 snapshot.
**Update triggers:** refresh when a major feature ships or quarterly, whichever first.

**Quote/customer row-version reconciliation:** the compatible frontend is live and the migration applied as ledger `20260730235031`. Its separate release lane still owns the B7 disk rename, generated TypeScript refresh, and its final postflight documentation; this AP closeout does not claim those steps on its behalf.
