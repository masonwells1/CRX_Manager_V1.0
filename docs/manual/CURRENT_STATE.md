...
**Last verified:** 2026-07-30 (post-apply B7 closeout: ledger rows `20260730114102_vendor_bill_period_close_lock`, `20260730124308_close_accounting_period_idempotency_recheck`, `20260730140808_accounting_period_immutable_date_math`, and validation-only `20260730174628_vendor_bill_month_lock_helper_acl_postflight` are live. The final row was authored as `20260730170743`, B7-renamed to the server-assigned disk/ledger version, occurs exactly once at high-water with 930 ledger rows, and changed no schema or business rows. It requires the month-lock helper's unique `postgres` owner, SECURITY INVOKER mode, `search_path=public, pg_temp`, and owner-only EXECUTE ACL, plus unique `postgres`-owned SECURITY DEFINER caller routines; API roles are denied helper access. The schema registry is refreshed to this version/name. The final readback confirmed the validated whole-month constraint and close RPC use exactly two time-zone-independent casts each; 9 period rows remain valid and no business rows changed. The sole close overload retains `postgres` owner, SECURITY DEFINER mode, `search_path=public, pg_temp`, ACL boundary, two idempotency reads, and the month lock. The registered Section 9 PO/AP chain reached expected `ERROR P0001 SMOKE_PASS_ROLLBACK` with zero remnants. All 21 standing invariant predicates were executed live and are CLEAN: 7 raw/7 allowlisted/0 new rows across 5 predicates. Operational counts below remain the separately dated 2026-07-18 snapshot.)
**Update triggers:** refresh when a major feature ships or quarterly, whichever first.

**Pending quote/customer row-version rollout:** deploy and production-verify the compatible frontend first while `20260730201230_quote_customer_row_version_guard.sql` remains unapplied; its extra JSON token is ignored by the old RPCs. Obtain separate explicit approval for the live migration, apply it in a bounded maintenance window, require a browser/PWA refresh for cached bundles, then run postflight/live smoke. Regenerate generated schema/types only in that follow-up. Cached old bundles fail closed after apply until refreshed; the already-deployed compatible bundle avoids an all-user outage. No rollout toggle is required.
