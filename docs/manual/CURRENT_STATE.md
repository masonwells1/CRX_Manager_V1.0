...
**Last verified:** 2026-08-25 UTC **for the live migration ledger and the `quote_versions` blast radius only** — everything else in this document carries its own older date and was *not* re-read in this pass. The live ledger now holds **976 rows** (was 971) and its highest `version` is **`20260825142708`**, carrying submitted migration name `20260820120000_save_job_enforce_chem_unit_invariant_and_derive_totals`. Five migrations applied between the previous stamp and this one, all by other sessions: `20260816110000_draw_down_cutover_barrier` (ledger `20260824185408`), `20260816120000_draw_down_split_order_lines_by_price_tier` (`20260825025241`), `20260817120000_carry_allocated_line_cents_through_lifecycle` (`20260825033106`), `20260819232000_bind_draw_down_receipts_to_intent` (`20260825034622`), and `20260820120000_save_job_enforce_chem_unit_invariant_and_derive_totals` (`20260825142708`) — the draw-down chain plus the save_job chem-unit invariant. That last one supersedes this document's older statement that `20260820120000` is "written and proven but **not applied**": it applied live on 2026-08-25. Note the ledger `version` and the submitted `name` disagree in ordering terms for all five — the version is the apply stamp, the name is the file — which is why `.claude/hooks/migration-ordering-lib.mjs` compares the **name**, and by that comparison the highest timestamp-prefixed name is `20260820120000`. **Also read live in this pass, for the pending `20260825190000_quote_version_restore_trust_boundary`:** `quote_versions.restore_trusted_at` is **absent** (the migration is unapplied), the table holds **3 rows across 2 quotes**, carries exactly **one** policy, and both `create_quote_version` and `_restore_quote_version_below_cost_impl_20260810` have exactly **one** overload each — so the migration's preconditions hold and its blast radius is unchanged from the 2026-08-25 measurement. `.claude/schema-registry.json` was **not** re-derived in this pass and is still stamped to the `20260816174353` high-water, so it is now five applies behind.

**Superseded 2026-08-22 header, kept for provenance.** Verified 2026-08-22 UTC for the ledger only — a read-only re-read returning 971 rows, high-water `20260816174353`, 345 of 971 names timestamp-prefixed, every figure **unchanged** from the previous pass; that pass also read the four live `job_chemicals` rows while measuring the blast radius of parked migration `20260820120000` (history row 891), which is written and proven but **not applied**. The `quote_versions` write surface, the return-idempotency helper contract, and the section 2 counts were last read live **2026-08-19 UTC** and are carried forward on that reading, not re-verified since. **The live ledger has 971 rows.** Its highest `version` is `20260816174353`, carrying submitted migration name `20260813080000_lock_quote_versions_writes_to_rpc`, which is also the highest *timestamp-prefixed* `name` — so both orderings agree on the same row. (Only **345** of the 971 ledger names carry a 14-digit timestamp prefix — 346 if the single 8-digit `20260207_gap_analysis_fixes.sql` is counted (the `.sql` suffix is
