...
**Last verified:** 2026-08-11 (UTC) — the live ledger was re-read read-only and has **moved past** the 960-row post-apply release check recorded earlier the same day: high-water is now `20260811220045`, **961 ledger rows / 954 distinct names**, up from `20260810235207` / 958 / 951 at the previous read. The three new rows were applied live on 2026-08-11 by concurrent sessions, not by this one: `20260811183317` (`20260811122851_assign_customers_sales_rep`), `20260811183437` (`20260811130000_bind_commission_payout_idempotency_to_intent`) and `20260811220045` (`20260811200000_blend_ticket_order_whole_cent_totals`). Carried forward from the 960-row check: the Customer 360 RPC recorded by `20260811183317` is live with one overload, `SECURITY DEFINER`, `search_path=public, pg_temp`, no PUBLIC/anon EXECUTE, and authenticated/service access; the schema registry, generated Supabase types, and live `pg_proc` fixture were genuinely refreshed through that 960-row high-water, and so trail the current 961-row ledger by the blend-ticket row alone. Branch `claude/wave-a-money` carries six local migration candidates prefixed `20260811…` that are **not applied** — confirmed absent from the ledger by the same read — so no statement below describes state they created; because they now sit *below* the live high-water they must be renumbered forward before any apply. The whole-cent money counts and post-apply constraint state carry their 2026-08-10 read-only re-read. Ledger versions are UTC, which is why some stamps read a day ahead of the local session date. The section 2 counts table carries its 2026-08-09 re-read; Team Board deployment details below remain current; the 2026-08-07 feature/postflight detail below and the deployment log were **not** re-checked, and the operational counts remain the separately dated 2026-07-18 snapshot.
