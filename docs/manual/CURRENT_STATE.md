...
**Last verified:** 2026-08-06. Live ledger high-water is `20260806000752` (`authorize_bulk_import_product_cost`). The Section 4 bulk-order-import lifecycle hardening is live through four migrations: imports are draft-only, inventory-aware, activity-logged, idempotent, and commission-safe; every imported line now snapshots its authoritative cost from the live Product row, including when a caller supplies a lower value. Post-apply catalog/grant checks, rollback-only behavior proof, all 21 standing invariant predicates, and a live schema-registry refresh passed. The earlier idempotency, statement disclosure, and historical AR report protections remain live as documented below. Operational counts below remain the separately dated 2026-07-18 snapshot.
...
- **2026-08-06:** Section 4 bulk-order-import lifecycle hardening is live through `20260806000752_authorize_bulk_import_product_cost`. The import RPC now creates draft orders only, reserves inventory through the normal prebook/ledger model, records order activity, binds idempotent retries to the original payload, rejects non-finite numeric values, and snapshots every line's Product `current_cost` for profit and commission calculations. Live catalog and grants, an active-sales-rep rollback smoke with a deliberately false zero caller cost, zero fixture residue, all 21 invariant predicates, and schema-registry refresh passed.
