...
**Last verified:** 2026-07-27 (live migration watermark **unchanged** at `20260726223520`, re-read this date via the Supabase connector: 911 ledger rows, max version `20260726223520`. Nothing has been applied live since 2026-07-26. The newer on-disk file `20260727123055_inline_role_checks_require_active_profile.sql` is **drafted and committed but deliberately NOT applied** — it awaits Mason's explicit OK and a `write-apply-proofs.mjs` gate run, so the live-state description below still holds as written. Until it is applied, 38 RLS policies still gate on `profiles.role` without requiring `profiles.is_active = true`, so a deactivated user holding a valid JWT retains access through them; see `docs/manual/KNOWN_ISSUES.md`. Prior stamp, still accurate: 2026-07-26, live migration watermark `20260726223520` — four migrations applied live that day, all verified via Supabase MCP post-apply: the section-9 `void_vendor_payment` vendor-liveness gate (`20260726201208`), the admin-only `reactivate_vendor` RPC (`20260726212043`), and two RLS policies on the `vendors` table making the new Show Inactive view work — admin read access to soft-deleted vendors (`20260726215154`) tightened to active admins only (`20260726223520`). Vendor "Delete" is now framed as Deactivate/Reactivate in the UI. Supplier Pricing Stage A remains dormant (`supplier_cost_basis_enabled=false`); `process-document` v19 still rejects supplier price/product lists before OCR; operational counts remain the dated 2026-07-18 snapshot below)
