...
**Last verified:** 2026-07-31 (UTC ledger date; evening 2026-07-30 Chicago). Live high-water is `20260731001654_ap_period_close_boundary_hardening` with 932 rows. The AP migration is B7-reconciled, exact function/ACL/RLS/policy readback passed, the registered Section 9 smoke ended `SMOKE_PASS_ROLLBACK`, and zero fixtures or closed periods remained. The immediately preceding Quote/Customer guard is B7-reconciled as `20260730235031_quote_customer_row_version_guard`; both row-version columns/triggers, lifecycle boundaries, and child-table ACLs passed live postflight, four rollback-only chains ended `SMOKE_PASS_ROLLBACK`, zero fixtures remained, and all 21 standing invariant predicates had zero unallowlisted findings. The schema registry was rebuilt from six live-introspection queries through the AP high-water and records both migrations and both row-version columns. Operational counts below remain the separately dated 2026-07-18 snapshot.
