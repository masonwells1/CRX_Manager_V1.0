...
**Pricing-audit follow-through remains local and unapplied.** The reporting snapshot, immutable quote-cost snapshot, and database-enforced below-cost approval migrations are re-issued as `20260810180000`–`20260810180002`, above the current live high-water. They were rebased onto the live whole-cent `save_quote` and canonical commission bodies, then all three applied cleanly to a disposable database restored from a fresh production schema dump. Both rollback-only below-cost smokes passed with zero residue, including mutation proof for pre-rounding version restore and canonical conversion commission basis. Exact-SHA review and the live-apply approval gate still remain; the earlier PR #350 stamps must not be used.
...
**Branch `claude/wave-a-money` — six unapplied migration candidates.** That branch carries six local migration files prefixed `20260811…` that are **not applied**, confirmed absent from the ledger by a read-only re-read on 2026-08-11 at the then-current high-water `20260811220045` / 961 rows. No statement in this document describes state they created. Because all six sit *below* the live high-water — now `20260812003315` — they must be renumbered forward before any apply is even considered.
