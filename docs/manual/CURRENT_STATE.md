...
**Last verified: 2026-08-25 UTC for the migration ledger and schema registry only** — a read-only
re-read taken after the draw-down rollout completed. **The live ledger has 975 rows**, `max(version)`
`20260825034622`, and current live effective ordering high-water `20260819232000` (name
`20260819232000_bind_draw_down_receipts_to_intent`). All four migrations of the draw-down chain are
applied live: the cutover barrier (ledger version `20260824185408`) and, later on 2026-08-24 with
Mason's explicit in-chat approval, the tier split (`20260825025241`), the allocated-line-cents
lifecycle carry (`20260825033106`), and the receipt intent binding (`20260825034622`).
`.claude/schema-registry.json` was regenerated from live introspection on 2026-08-25 and records
`migrations_high_water` `20260825034622`, so the registry matches this ledger high-water. The
authoritative rollout record — per-migration SHA-256 pins, proofs, and postflight — is the block at
the top of `docs/reference/migration-history.md`; the matching issue entries are in
...
figure in this document; every section below keeps its own older date. In particular, parked
migration `20260820120000` (history row 891, the `save_job` chemical-unit invariant) is still
written and proven but **not applied** — that is unchanged by this rollout.
