...
**Last verified:** 2026-08-10 — the live ledger and the whole-cent money counts were re-read from the live database on 2026-08-10 (read-only). The section 2 counts table carries its 2026-08-09 re-read; the 2026-08-07 feature/postflight detail below and the deployment log were **not** re-checked on either date.

**2026-08-10 live re-read.** Live ledger high-water is **`20260810025159`** (`backfill_stale_line_profit`), **954 ledger rows**. Three `20260810` rows are applied: `20260810000427` — the version Supabase assigned to merged file `20260809230500_single_canonical_line_profit.sql` (history row 862) — plus `20260810010308_active_team_note_assignment_actor` and `20260810025159_backfill_stale_line_profit`, **whose files are not in `origin/main`**; both live on the unmerged branch `claude/session-orchestration-setup-d73e6c` (commits `c3f2a387` and `a0a69a62`). Live is therefore ahead of `main` until that branch lands. The backfill moved `order_items.total_price` fractional rows from 46 to 35; the 3 fractional `commissions` rows are unchanged. Full per-column conformance figures are in `docs/manual/KNOWN_ISSUES.md` under the same date.

**2026-08-10 — three migrations drafted, proven, NOT applied.** History rows 863–865 (`20260810150000`, `20260810150500`, `20260810151000`) fix the commission-basis defect, round `quotes.total_cost` and the `quote_items` line money, and add whole-cent CHECK constraints to the 7 already-clean money columns. All three executed end-to-end against a throwaway PostgreSQL 17 with every post-condition passing and mutation-tested to fail closed. **Nothing was applied to live and no live data changed** — applying needs Mason's explicit OK, in the order 863 → 864 → 865. This is also the disposition of CodeRabbit's "use bigint cents" Major finding on PR #354: closed **won't-fix with a hard guard substituted**, rationale in `docs/audits/2026-08-10-order-profit-bigint-cents-evaluation.md`.

**2026-08-09 live re-read.** At the time of that read, live ledger high-water was **`20260809130108`** (`team_note_completion_rpc_and_assignment_notify`), 946 ledger rows — exactly one row above the 2026-08-07 high-water. That migration was applied from a concurrent session and **has no file in this repository**; see the note at the end of `docs/reference/migration-history.md`.
