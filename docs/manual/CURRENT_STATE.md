...
**Historical quote-version restore quarantine — local candidate, not applied.** The pending RPC-only write boundary stops future browser-forged `quote_versions`, but the three rows already present live were created while authenticated callers could insert directly and cannot be proven server-authored after the fact. `20260813170000_quarantine_legacy_quote_version_restore.sql` records every version existing at its apply boundary in a private, forced-RLS, immutable quarantine table and makes the governed restore wrapper reject those ids before reaching snapshot writes. A rollback smoke proves a quarantined legacy restore is denied while a version created after the boundary still restores normally. Apply order requires `20260813080000_lock_quote_versions_writes_to_rpc.sql` first. No live state has changed.
