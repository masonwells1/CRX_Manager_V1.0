...
**Last verified:** 2026-08-14 UTC, read-only ledger and catalog re-check. The live ledger has **970 rows** and its high-water **version** is `20260813011751`, carrying submitted migration name `20260813070000_pin_return_idempotency_helper_contract`. **Nothing at or above `20260813080000` is applied** — the quote-version RPC-only write lock and the quote-version restore trust boundary are written but NOT live. The schema registry high-water matches that live version, so the schema-aware hooks are validating against current live structure; migration files on disk whose *name* stamps run ahead of it are the unapplied set, not drift. Draw-down catalog proof this session: exactly one overload each of `draw_down_quote(uuid,jsonb,uuid,text,text)` and the private `_draw_down_quote_below_cost_impl_20260810(uuid,jsonb,uuid,text)`, both `SECURITY DEFINER` with `search_path=public, pg_temp` and `postgres` ownership; the private implementation is not executable by `authenticated`; their stored-body md5s are `87bf7adcdc63d94684676da5ab09bfde` and `970960d8490b63b972bc8c60fe207a5b`, matching the pins the pending ownership fix gates on. The prior 2026-08-12 Customer 360 release check (962 rows, `20260812003315`, submitted name `20260811230423_log_customer_sales_rep_assignment`, full six-query registry refresh) remains valid history and is not restated as current. No table, column, enum, generated column, or function signature was observed to change, so generated Supabase types and the 566-name `pg_proc` fixture remain structurally current. Team Board deployment details below remain current; operational counts below remain the separately dated 2026-07-18 snapshot.
