...
**Last verified:** 2026-08-03. Live ledger high-water is `20260803010917` (`20260802162805_bind_idempotency_to_mutation_intent` in the server ledger; B7-reconciled disk file `20260803010917_bind_idempotency_to_mutation_intent`). The idempotency intent-binding migration is live and post-apply verified: both columns exist, public wrappers are authenticated/service-role only, private implementations are postgres-only, auth guards fire, and no partial or unexpired legacy receipts remain. Local candidate `20260803131507_fix_statement_balance_disclosure` is reviewed but **not live** and must not be counted as production state. Operational counts below remain the separately dated 2026-07-18 snapshot.
