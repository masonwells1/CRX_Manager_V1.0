...
**Pending quote/customer row-version rollout:** deploy and production-verify the compatible frontend first while `20260730031925_quote_customer_row_version_guard.sql` remains unapplied; its extra JSON token is ignored by the old RPCs. Obtain separate explicit approval for the live migration, apply it in a bounded maintenance window, require a browser/PWA refresh for cached bundles, then run postflight/live smoke. Regenerate generated schema/types only in that follow-up. Cached old bundles fail closed after apply until refreshed; the already-deployed compatible bundle avoids an all-user outage. No rollout toggle is required.
