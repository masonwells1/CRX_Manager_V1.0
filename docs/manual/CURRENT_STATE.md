...
**Last verified:** 2026-07-29 (live post-apply read: **923 ledger rows**, high-water `20260729125314_application_service_cost_exact_text`. The three approved migrations are live: profile-directory security at `20260729125227`, contact-sync search-path pins at `20260729125251`, and exact-text application-service cost at `20260729125314`. Live postflights passed: all 11 profile rows are represented in the RLS-protected, security-invoker directory view; both contact-sync functions remain invoker functions with fixed search paths; admins receive exact text cents; and a sales rep is denied with SQLSTATE 42501.)
