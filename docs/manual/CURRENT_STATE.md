...
**Last verified:** 2026-08-03. Live ledger high-water is `20260803221244` (`fix_statement_balance_disclosure`; B7-reconciled disk file `20260803221244_fix_statement_balance_disclosure`). The idempotency intent-binding migration remains live and post-apply verified: both columns exist, public wrappers are authenticated/service-role only, private implementations are postgres-only, auth guards fire, and no partial or unexpired legacy receipts remain. The statement balance-disclosure migration is also live and post-apply verified: its private as-of helpers have fixed search paths and revoked public execution, its posting-timestamp constraint is validated, and a read-only authenticated-admin check exercised both public statement RPCs. A prior candidate revision reached rollback-only smoke; no separate final-revision rollback rehearsal was captured. Operational counts below remain the separately dated 2026-07-18 snapshot.
