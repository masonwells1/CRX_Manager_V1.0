...
**Last verified:** 2026-08-13 UTC, read-only release-coordination check. The live ledger has 970 rows and ends at version `20260813011751`, carrying submitted migration name `20260813070000_pin_return_idempotency_helper_contract`. The pending quote-version write boundary (`20260813080000`) and restore trust boundary (`20260813180000`) are not applied: `quote_versions.restore_trusted_at` is absent, RLS remains enabled, and one mutating policy remains. No Wave A or quote-version candidate was applied by this work. The schema registry still records the older `20260812003315` high-water and must not be called current until the written-but-unapplied candidates are definitively parked or applied and the registry is regenerated from live introspection. Historical deployment details below retain their separately dated evidence.

**Wave A — six parked, unapplied migration drafts.** PR #393 moved the drafts out of `supabase/migrations/` into `scripts/.staging-migrations/`; they remain absent from the live ledger. They are not eligible for apply at their parked `20260813010000`–`20260813060000` names. If the work resumes, the coordinated target is `20260813100000`–`20260813150000`, subject to a fresh live-ledger re-read, exact-artifact review, and explicit live-apply approval. No statement in this document describes schema or data state those drafts would create.
