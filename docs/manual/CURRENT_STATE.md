...
**Quote draw-down ownership/deletion and cent boundary — local candidate, not applied.** Exact review found that the live private draw implementation locks a quote but does not bind an active sales rep to `quotes.created_by`, does not reject `deleted_at`, and can carry a fractional-cent weighted unit price into an order line that the live whole-cent constraint rejects. `20260813161614_restrict_draw_down_quote_owner.sql` is a forward-only body replacement behind the unchanged governed public wrapper. It requires the owning rep or an active admin after the quote lock, hides soft-deleted quotes, settles both weighted price and cost to whole cents before deriving order money, performs the access checks before idempotency replay or mutation, and ships with a container-only two-rep rollback chain. No live state has changed.
