...
**The schema registry matches the live schema shape but is one COMMENT-only ledger row behind.**
`.claude/schema-registry.json` records `migrations_high_water` `20260825142708`, refreshed after the
`save_job` body apply. The later `20260826150000` apply changed only `pg_description`, so no column,
constraint, status, policy, grant, or function body represented by the registry changed.
