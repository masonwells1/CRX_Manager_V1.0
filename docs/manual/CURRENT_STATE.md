...
**Last verified:** 2026-08-05. Live ledger high-water is `20260805151605` (`20260805124533_fix_historical_ar_report_cutoffs`; B7-reconciled disk file `20260805151605_fix_historical_ar_report_cutoffs`). The idempotency intent-binding and statement balance-disclosure migrations remain live and post-apply verified. The historical AR report migration is also live and verified: AR Aging and Customer Balance Listing use posted-as-of eligibility, cutoff credit reconstruction, and fail closed on unknowable later activity; the listing includes overdue and excludes unposted invoices. Both RPCs have one overload, fixed search paths, deliberate authenticated/service grants with anon revoked, and the private reconstruction helper is postgres-only. Admin current reports and sales-rep current listing executed read-only; sales-rep historical access failed with the expected admin-only error; the rollback smoke passed with zero fixture residue. Operational counts below remain the separately dated 2026-07-18 snapshot.
