...
**Last verified:** 2026-08-13 UTC, read-only recovery check. The live ledger has 970 rows and ends at assigned version `20260813011751`, carrying submitted name `20260813070000_pin_return_idempotency_helper_contract`. The six recovered money/pricing versions after the prior 962-row Customer 360 high-water are `20260812034831`, `20260812034951`, `20260812145628`, `20260812151606`, `20260812154028`, and `20260812154757`; this recovery change carries five exact applied sources plus the explicitly documented digest-bound public replay form of the sensitive historical repair. The two later return-credit versions are `20260812212323` and `20260813011751`, whose sources are carried by the return-credit release PR rather than this recovery PR. The schema registry in this recovery change still reflects the prior introspection and is explicitly tracked for a fresh live rebuild; it must not be described as current until that separate refresh lands. Team Board deployment details below remain current; operational counts below remain the separately dated 2026-07-18 snapshot.

**Multi-price quote-draw correction — local candidate, not applied.** Exact-commit review found that the live private `draw_down_quote` implementation can average same-product quote lines into a half-cent unit price, which the live whole-cent guard then rejects. `20260813053545_allocate_multi_price_quote_draws.sql` preserves the governed public wrapper and allocates each partial draw from cumulative rounded booking value. The captured-schema proof and the two-draw `$10.00` + `$10.01` rollback regression pass with zero residue. The submitted name is absent from the fresh 970-row live ledger; no production behavior described here depends on it yet.

**Wave A — six unapplied migration candidates.** This tree carries `20260813010000` through `20260813060000`; all six submitted names are **absent** from the fresh 970-row live ledger. No statement in this document describes state they created. The live ledger has advanced since they were stamped, so the whole set must be revalidated and any colliding/older stamp renumbered forward before an apply is considered.
