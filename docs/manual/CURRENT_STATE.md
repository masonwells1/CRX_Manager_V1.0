...
**Last verified:** 2026-07-27 (live migration watermark now `20260727174805`, re-read this date via the Supabase connector: **914 ledger rows**, max version `20260727174805`. Three migrations applied live this date. First, `inline_role_checks_require_active_profile` (`20260727145843`) — 38 `ALTER POLICY` statements closing the systemic gap where RLS policies inlined a `profiles.role` check without also requiring `profiles.is_active = true`; post-apply live proof: residual inline-role gaps **38 → 0**, 48 policies now require an active profile, and under live role simulation the one deactivated `sales_rep` now sees **0** vendors and **0** vendor_bills while an active admin still sees 13 and 4. **Account deactivation is now complete**, closing the two follow-ups that migration left open: `broad_reads_require_active_profile` (`20260727174657`) added the role-agnostic `is_active_profile()` helper and rewrote the **31** wide-open PERMISSIVE SELECT policies (not six — the earlier figure was wrong, corrected by live re-enumeration) so they require an active profile — wide-open read policies **31 → 0**, with an active admin still seeing `products=604 team_notes=53` and the deactivated user seeing `products=0 team_notes=0` under live role simulation; and `deactivation_revokes_auth_access` (`20260727174805`) made deactivation real — it now bans the auth user (`banned_until` = the finite sentinel `9999-12-31 23:59:59+00`) and deletes their `auth.sessions`/`auth.refresh_tokens`, with reactivation clearing the ban, plus a `trg_guard_last_active_admin` trigger that refuses to deactivate the last active admin. Behavioral proof, rolled back: `BEFORE sessions=924 tokens=924` → `DEACTIVATED sessions=0 tokens=0` → `REACTIVATED ban=NULL` → last-admin deactivation blocked. **Residual window, unavoidable:** an already-issued access token stays valid until it expires (~1 hour); it simply cannot be renewed. **Read access is not narrowed by role** — every active user reads exactly what they read before; deciding which roles *should* see each table remains open. Also still open: the schema baseline is ahead of its own ledger high-water so a from-zero rebuild cannot complete (disaster-recovery only, production unaffected) — see `docs/manual/KNOWN_ISSUES.md`. Prior stamp, still accurate: 2026-07-26, live migration watermark `20260726223520` — four migrations applied live that day, all verified via Supabase MCP post-apply: the section-9 `void_vendor_payment` vendor-liveness gate (`20260726201208`), the admin-only `reactivate_vendor` RPC (`20260726212043`), and two RLS policies on the `vendors` table making the new Show Inactive view work — admin read access to soft-deleted vendors (`20260726215154`) tightened to active admins only (`20260726223520`). Vendor "Delete" is now framed as Deactivate/Reactivate in the UI. Supplier Pricing Stage A remains dormant (`supplier_cost_basis_enabled=false`); `process-document` v19 still rejects supplier price/product lists before OCR; operational counts remain the dated 2026-07-18 snapshot below)
