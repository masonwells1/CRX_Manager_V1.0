...
**Last verified:** 2026-08-05. Live ledger high-water is `20260805171334` (`20260805170848_fix_customer_balance_paid_invoice_totals`; B7-reconciled disk file `20260805171334_fix_customer_balance_paid_invoice_totals`). The idempotency intent-binding and statement balance-disclosure migrations remain live and post-apply verified. The historical AR report migrations are also live and verified: AR Aging and Customer Balance Listing use posted-as-of eligibility, cutoff credit reconstruction, and fail closed on unknowable later activity; the listing includes overdue, excludes unposted, and keeps fully paid invoices in cumulative invoiced/paid/count totals while limiting open-balance fields to positive balances. Both RPCs have one overload, fixed search paths, deliberate authenticated/service grants with anon revoked, and the private reconstruction helper is postgres-only. Admin current reports and sales-rep current listing executed read-only; sales-rep historical access failed with the expected admin-only error; the expanded rollback smoke passed with zero fixture residue. Operational counts below remain the separately dated 2026-07-18 snapshot.
