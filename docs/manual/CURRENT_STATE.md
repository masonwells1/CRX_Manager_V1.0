...
**Last verified:** 2026-08-06. Live ledger high-water is `20260806012423` (`lock_bulk_import_cost_snapshot`). The Section 4 bulk-order-import lifecycle hardening is live through six migrations: imports are confirmed-only, inventory-aware, activity-logged, actor/payload-bound for replay, and commission-safe; every imported line uses one locked bigint-cent Product cost snapshot, retains whole-cent profit, and commission profit is reread from the trigger-canonical order header. Post-apply catalog/grant checks, fractional-line rollback proof, all 21 standing invariant predicates, and a live schema-registry refresh passed. The earlier idempotency, statement disclosure, and historical AR report protections remain live as documented below. Operational counts below remain the separately dated 2026-07-18 snapshot.
...
- **2026-08-06:** Section 4 bulk-order-import lifecycle hardening is live through `20260806012423_lock_bulk_import_cost_snapshot`. The import RPC creates confirmed orders only, reserves inventory through the normal prebook/ledger model, records order activity, binds retries to the original actor/payload, rejects non-finite values, locks Product cost into one bigint-cent immutable snapshot, keeps line profit whole-cent, and creates commissions from trigger-canonical stored profit. Live catalog and grants, an active-sales-rep rollback smoke with false caller cost plus fractional lines and changed-intent replay, zero fixture residue, all 21 invariant predicates, and schema-registry refresh passed.
