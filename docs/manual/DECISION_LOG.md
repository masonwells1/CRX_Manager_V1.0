...
- Review prompts must request every finding and filter later; never instruct a reviewer to "only report high-severity issues" or "be conservative" (Opus 5 obeys literally and reports less). **OPEN (P2):** `overnight-bug-hunt.js:51`, `money-inventory-hunt.js:52`, and `whole-codebase-audit.js:29` cap each agent at its 8–10 "most significant" findings, so ranking happens before the skeptic pass that was meant to filter. Removing the caps costs materially more per run — Mason decides whether to drop them or narrow the rule to exclude bounded overnight sweeps.
- **OPEN (P2):** `money-inventory-hunt.js` pins `effort: 'high'` at `:293` and `:334`, so the `xhigh` row of the effort mapping does not reach the money/inventory night hunt. Resolve as part of the effort sweep; raising it costs more per run.
