...
**Last verified: 2026-08-05.** Live ledger high-water is `20260805151605`. The prior payload-blind replay risk for `save_invoice` and `create_quick_delivery` remains fixed live. The Section 2 statement balance-disclosure migration remains live under `20260803221244`. The historical AR report defects are now fixed live under `20260805151605`: AR Aging reconstructs the selected cutoff instead of reusing current mutable balances, and Customer Balance Listing includes overdue invoices, excludes unposted work, and makes historical execution admin-only. Post-apply catalog, grants, active admin/sales-rep execution, historical role denial, schema-registry refresh, and rollback-smoke residue checks passed.
