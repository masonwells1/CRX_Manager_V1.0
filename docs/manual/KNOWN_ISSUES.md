...
**Last verified: 2026-08-14 UTC, read-only.** Live ledger high-water **version** is `20260813011751` at **970 rows**, carrying submitted name `20260813070000_pin_return_idempotency_helper_contract`. **Nothing at or above `20260813080000` is applied**, so the quote-version RPC-only write lock and the quote-version restore trust boundary are written but not live. The schema registry high-water matches that live version. Draw-down catalog proof this session: one overload each of the public `draw_down_quote(uuid,jsonb,uuid,text,text)` wrapper and the private `_draw_down_quote_below_cost_impl_20260810(uuid,jsonb,uuid,text)`, both `SECURITY DEFINER` / `search_path=public, pg_temp` / `postgres`-owned, with the private implementation not executable by `authenticated`; their stored-body md5s match the pins the pending fix gates on. Ledger versions are UTC and Supabase applies may assign a version different from the submitted filename, so match the recorded name when reconciling an apply. The prior 2026-08-12 Customer 360 post-apply check (962 rows at `20260812003315`) and the historical Team Board, money, and commission-payout details below remain separately dated evidence rather than claims that their older high-waters are current.
