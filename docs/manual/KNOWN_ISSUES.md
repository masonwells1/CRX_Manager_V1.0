...
| `supabase/migrations/20260718205500_per_line_split_billing_phase1_preflight.sql` | Fail-closed absence check before the unapplied Phase 1 schema | Built and proven locally; intentionally not applied while the feature remains OFF and exact-hash reviews are incomplete | Apply only as the first migration in the eventual owner-approved per-line chain; any stale object requires investigation, not bypass |
| `supabase/migrations/20260718210500_per_line_split_billing_relational_guards.sql` | Cross-table money, exact signed source/allocated COGS ledger for sales and returns, inverse job-chemical/required-service completeness, exact service uniqueness, flat-fee quantity-1 persistence, item/source identity, pricing-unit/COGS, invoice membership, customer, zero-send disposition, header, posted freeze, and immutable-history guards for the unapplied per-line split schema | Built and proven locally, including durable negative-return COGS. Fresh security review is clean; drift remains H1 only because the prerequisite Phase 1 snapshot table is absent live, although all 52 persisted columns match its pending DDL and TypeScript interface | Keep with the Phase 1/2 migration chain; after Phase 1 apply, refresh the live registry and rerun drift review before any guard apply |
| `supabase/migrations/20260718211000_per_line_split_billing_phase2_calculator.sql` | Phase 2 deterministic preview calculator with owner/assignment-scoped SECDEF reads, caller-owned temp-object rejection, exact invoice/job/field context binding, manual→customer/season/lifecycle-bound quote→tier pricing with quote-unit normalization and complete contributing-field coverage, exact signed sales/COGS residual allocation, quantity-1 child flat fees, full audit-identity hashes, strict missing-price/cost rejection, frozen job price/service/unit/COGS authority, disabled-flag install gate, and a compatibility wrapper that fingerprints the approved legacy body/security/ACL | Built behind the OFF flag and intentionally not applied. Exact-SHA reviews found and the disposable proof now covers signed return COGS persistence, quote price/unit pairing, unconvertible/partial/zero-share quote fields, NULL product-cost rejection, and predictable temp-object defense; fresh content-bound security and drift reviews are clean on the current repaired bytes | Complete the fresh commit-SHA reviews plus the baseline billing cycle; do not apply or enable the feature flag yet |
...
- **Per-line-item custom split billing (field-app)** — DESIGN SPEC complete + review-hardened; Phase 1
  ...
  temp-object boundary, and legacy-helper trust repairs, but
