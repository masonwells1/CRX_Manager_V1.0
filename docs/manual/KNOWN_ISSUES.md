...
**`quote_items.cost_at_quote_cents` — declaration and registry gap both CLOSED 2026-08-13.** Added by `20260812115236` and running in production since 2026-08-12. `src/types/index.ts` declares it as an optional field because partial projections may omit it; the column is trigger-stamped and the browser never writes it. The registry gap is now closed too: `.claude/schema-registry.json` was rebuilt on 2026-08-13 and carries `cost_at_quote_cents` in `columns.quote_items`, so the schema-aware hooks no longer validate against a column set missing it. Verified by reading both files directly, not by re-reading this note — an automated reviewer flagged the `src/types/index.ts` half as stale and was itself half-wrong, having assumed the registry half still stood. **Separate and still open:** the registry's applied-migration high-water is `20260813011751`, and the session-staleness hook reports it as behind **two** files. That number is narrower than it sounds and the definition matters, because read as "two migrations are unapplied" it is wrong. Measured 2026-08-13: **seven** files on disk carry a stamp above the high-water (`20260813020000`, `030000`, `040000`, `050000`, `060000`, `070000`, `080000`). The hook then drops `20260813070000`, which is applied live and present in the registry's `applied_migration_names` — Supabase assigns a server-side version that can sort below the filename stamp, so name membership, not the numeric compare, is what refutes it. It drops four more via `migrationMayAffectSchemaRegistry`: `020000`, `040000`, `050000` and `080000` replace function bodies and revoke grants, which cannot change the table/column/constraint surface the registry describes. What remains is `030000` (30 finiteness CHECK constraints) and `060000` (delivery-before-invoice) — the only two whose application would make the four schema-aware hooks validate against a stale registry. So "two" counts *registry-invalidating* unapplied files, not unapplied files; the unapplied count is six, all tracked in the Wave A entry below.

**Wave A — six unapplied migration candidates, now CONFIRMED BLOCKED on ordering.** This tree carries submitted names `20260813010000` through `20260813060000`; all six remain absent from the fresh 970-row live ledger. Nothing in this document describes state they created. The earlier instruction to "revalidate and renumber any colliding stamp" is no longer a precaution — it is measured. The ledger's newest **submitted name** is now `20260813070000_pin_return_idempotency_helper_contract` (assigned version `20260813011751`), applied by the concurrent return-credit session, whose files live on `codex/remediate-section-08-returns-credit-20260812` and are pushed but not merged. `checkMigrationOrdering` compares filename timestamps, not assigned ledger versions, so every one of the six now sorts older than an applied migration. Running the real guard from `.claude/hooks/migration-ordering-lib.mjs` against the live 970-row name list on 2026-08-13 returns **REFUSE for all six**, `newestApplied=20260813070000`. They must be re-stamped forward — for the second time — before any apply is attempted, and because concurrent sessions keep advancing the ledger, the re-stamp is only durable if it happens immediately before the apply in the same session. `20260813080000_lock_quote_versions_writes_to_rpc` (CRX-SEC-1) is stamped above the high-water and passes the same check unchanged.
...
the applying sessions' transcripts and land in the same change as this entry, as history rows 880-885.
...
## CLOSED 2026-08-13 — `quote_items.cost_at_quote_cents` is now declared in both the registry and types

**Was: MED, silent drift.** The column exists live as `bigint` on `quote_items` (read-only
`information_schema` check, 2026-08-12). When this entry was written it appeared in neither
`.claude/schema-registry.json` nor the `QuoteItem` interface in `src/types/index.ts`.

Both halves are now closed, each verified by reading the file rather than by trusting this entry:
`src/types/index.ts` declares `cost_at_quote_cents?: number | null` (added by `c7062a3f`), and the
registry was rebuilt on 2026-08-13 and lists the column in `columns.quote_items`. The
registry-aware hooks therefore no longer validate against a column set that omits it.

Kept rather than deleted because the closure is the useful record: drift of this kind is invisible
until something reads the field, so the evidence that it was checked — and how — is worth more than
the absence of an entry. **Not to be confused with** the still-open freshness gap, where the
registry's applied-migration high-water trails migration files present on disk; that is a question
about the ledger snapshot, not about this column.
