...
**Last verified: 2026-08-14 UTC for repository state; live facts remain from the 2026-08-13 read-only recovery check.** The live ledger has 970 rows and ends at assigned version `20260813011751`, carrying submitted name `20260813070000_pin_return_idempotency_helper_contract`. The six recovered money/pricing sources in this change are live at assigned versions `20260812034831`, `20260812034951`, `20260812145628`, `20260812151606`, `20260812154028`, and `20260812154757`; the later return-credit sources are now reconciled from merged PR #388. The schema registry and captured-ledger proof snapshot both match the 970-row high-water. Ledger versions are UTC and Supabase may assign a version different from the submitted filename, so reconcile by both recorded version and name. Historical Team Board, money, and commission-payout details below remain separately dated evidence rather than claims that their older high-waters are current.

**OPEN IN PRODUCTION / SAFE BOUNDARY PREPARED — same-product quote lines at multiple price or cost cohorts cannot yet be partially drawn.** The live private `draw_down_quote` implementation calculates one weighted average unit price per product. A booking split across `$10.00` and `$10.01` lines can therefore produce `$10.005`; the whole-cent order-line trigger correctly rejects it, so the transaction safely rolls back but the valid draw cannot finish. A review briefly found that the pending `20260813161614` candidate rounded that average to `$10.01`, which could turn the booked `$20.01` into `$20.02`. The candidate now explicitly rejects more than one distinct booked price or cost cohort with `AMBIGUOUS_BOOKING_MONEY_COHORTS` before line/header/commission math or any durable draw write. The container rollback smoke constructs the `$10.00`/`$10.01` case and proves zero order/draw residue. The functional limitation remains until a future design allocates cent-priced cohorts or records an immutable revenue allocation ledger.

**Repository/production gap reopened and re-closed on 2026-08-12 — six migrations, and the prevention gap is still OPEN.** The high-water quoted in the paragraph above (`20260812003315`, 962 rows) was overtaken the same day. Six further migrations applied live on 2026-08-12 — ledger versions `20260812034831`, `20260812034951`, `20260812145628`, `20260812151606`, `20260812154028`, `20260812154757`, carrying submitted names `20260812010000_blend_ticket_order_header_runtime_assert`, `20260812011000_restore_quote_version_whole_cent_money`, `20260812115235_snapshot_cost_reporting`, `20260812115236_quote_items_cost_at_quote_snapshot`, `20260812115237_enforce_below_cost_admin_approval`, `20260812115238_repair_historical_order_line_cents` — **applied by concurrent sessions that never landed their files.** For part of that day none of the six existed on `main`, on any pushed branch, or in any local worktree, so SQL touching order money, quote cost, report math and a new approval table was running against production with no one able to review it, and a clean rebuild from `main` would have produced a schema without it. The **files** side is now closed without falsifying ledger identity: five exact payloads were recovered verbatim from the applying sessions' transcripts (not reconstructed from live `prosrc`, which loses the header, preconditions and review history). The sensitive sixth payload remains private because it embeds customer-linked financial preimage data; the public tree records its exact applied byte count and SHA-256 but contains no different SQL under that applied version. A separately named digest-bound recovery replay outside `supabase/migrations/` derives its candidates without disclosing identifiers or values. The recovered function bodies were md5-verified against live `pg_proc.prosrc`, and all six are documented in history. **The prevention side is not closed.** Nothing yet stops the next session from applying a migration and not landing its file; this is the third occurrence (2026-08-09, 2026-08-11 via PR #371, 2026-08-12) and the only current defence is that someone notices. A durable fix — a hard guard that reconciles the live ledger against tracked files and fails a check when they disagree — is not written.

**`quote_items.cost_at_quote_cents` and the live schema registry are reconciled.** Added by submitted migration `20260812115236` (live ledger version `20260812151606`) and running in production since 2026-08-12. `src/types/index.ts` declares it as optional because partial projections may omit it; the column is trigger-stamped and the browser never writes it. The registry includes the pricing objects and matches live high-water `20260813011751`. Five active local candidates remain intentionally unapplied: `20260813080000`, `20260813090000`, `20260813161614`, `20260814041419`, and `20260814063000`. The schema-registry warning is expected until those candidates are governed and applied or parked; do not regenerate the registry from unapplied SQL. The six Wave A drafts are parked outside `supabase/migrations/` and are not part of schema staleness.

**Stale below-cost note reuse (`20260814041419_fresh_below_cost_reason`) — local candidate, NOT APPLIED.** The applied recursive JSON parser treats a historical `Below-cost approved:` marker inside persisted invoice or quote notes as a reason when the current attempt supplies no explicit value. That can authorize a later save against different locked costs with an old explanation. The forward candidate makes presence of the current-attempt key authoritative, including explicit null, while preserving the existing private function contract and ACL. The browser side now supplies the key on every JSON-backed attempt and strips prior markers from legacy note transports. Until the migration applies, production retains the old parser behavior.

**Quote-section direct write boundary — local candidate, NOT APPLIED; conversion-cancellation race — code fix prepared.** Live `authenticated` currently lacks direct quote-section INSERT/UPDATE/DELETE, but still holds `TRUNCATE`, `TRIGGER`, and `REFERENCES`; RLS cannot stop TRUNCATE, and the quote-section foreign key cascades into cost-snapshot-bearing quote items. `20260814063000` removes every write-capable browser grant and the obsolete write policies while preserving SELECT. Separately, the browser previously compensated a cancelled below-cost conversion by directly updating accepted back to sent without a quote lock, row-version guard, or order check. The prepared UI fix removes that compensation entirely: the quote remains accepted, no second mutation runs, and staff can retry conversion from the accepted state. The 970-row captured-ledger reconstruction and rollback smoke pass; production's database boundary remains unchanged until apply.

**Wave A — six parked migration drafts.** PR #393 moved submitted names `20260813010000` through `20260813060000` to `scripts/.staging-migrations/`. They are intentionally absent from `supabase/migrations/`, are not armed for apply, and create no live state. Their content is pinned by the six SHA-256 values recorded in migration history; any future promotion must restamp them above the then-current live high-water and repeat the governed proof pipeline.

**Restore owner-implementation ACL (`20260813090000_restrict_restore_quote_owner_impl`) — local candidate, NOT APPLIED.** The governed public restore wrapper already takes a key-only advisory lock through `check_idempotency`, rejects `IDEMPOTENCY_CROSS_OP_KEY_REUSE`, and is backed by the enabled `guard_idempotency_key_insert` trigger. The narrower remaining bypass was direct `service_role` EXECUTE on `_restore_quote_version_owner_impl`, which could skip both that check and the below-cost wrapper. The permission-only candidate pins those live bodies and revokes direct owner-implementation execution from every application role while preserving `postgres`. No live state has been changed.

**Quote draw-down owner/deletion bypass (`20260813161614_restrict_draw_down_quote_owner`) — local candidate, NOT APPLIED.** The live governed public wrapper is actor-bound and below-cost-aware, but its private implementation only checks that the caller is an active admin or sales rep. After locking the quote it neither rejects `deleted_at` nor requires a sales rep to match `quotes.created_by`; quotes are broadly readable, so possession of another quote id is enough to reach the money/inventory lifecycle. The candidate fingerprints the exact private and public bodies, replaces only the private body, hides deleted quotes, applies the established `NOT_QUOTE_OWNER` boundary for non-admin reps, and runs both checks before any idempotency replay or write. A container-only rollback chain creates two active reps plus a deleted revised quote, proves both denials, proves owning-rep/admin controls reach `EMPTY_DRAW`, and asserts zero order/draw-ledger residue. No live state has been changed.

**CRX-SEC-1 (`20260813080000_lock_quote_versions_writes_to_rpc`) — reviewed, live-verified, NOT APPLIED.** `quote_versions.snapshot_data` became an authoritative cost source when `20260812115236` went live, while the table still carried a direct INSERT grant for `authenticated`; a caller able to write a row could write their own cost basis, and the below-cost trigger cannot see it because that trigger compares price against cost and this lowers the cost. Production currently has three historical rows created while that path was open. Their shape is plausible, but the old policy did not bind every snapshot field to a server-derived value, so provenance cannot be proven retrospectively. The candidate now closes both halves atomically: one ACCESS EXCLUSIVE lock spans capture of every pre-boundary id, creation of a private forced-RLS immutable `quote_version_restore_quarantine`, removal of browser mutation grants/policies, and replacement of the governed restore wrapper so captured ids raise `QUOTE_VERSION_RESTORE_QUARANTINED` before snapshot writes. A server-created version cannot land between the classification and cutover: it commits before the lock and joins the legacy set, or waits until the transaction commits and remains restorable. Two review findings are worth carrying forward independently. **First, a false-fail trap for anyone writing a test or smoke against `create_quote_version`:** the RPC ends in an unconditional `UPDATE quotes SET status = 'sent'` with no admin override, and `_enforce_quote_status_transition` admits `'sent'` only from `draft`, `revised` and `accepted`, plus the unchanged-status early return. Any fixture picking a quote in another status fails inside the lifecycle rule, not in whatever is under test — and because the oldest live quote is `cancelled`, the obvious `ORDER BY created_at LIMIT 1` selects exactly the wrong row. **Second, the exploitation check in that file, and any similar scan, proves only the absence of forgery *of the shape it detects*** — its threshold is half the catalog cost, so a line understated by a third passes silently. A clean result is a reason to close the hole, never evidence that the hole was never used. Every precondition the file asserts was run read-only against production on 2026-08-13 UTC and passes: RLS on with FORCE off; owner `postgres` with `rolbypassrls` true and **`rolsuper` false**, which is what keeps the `has_table_privilege` assertions from being vacuous; all grants made by `postgres`, so the REVOKE cannot silently no-op on a foreign grantor; no column-level ACLs; exactly the two expected policies; one overload of each of the three pinned functions at the pinned signatures; **zero** routines with a `BEGIN ATOMIC` body in any non-system schema, so the source-text writer scan is not blind; one writer, the expected owner-side definer; and zero unevaluable or below-threshold snapshot cost lines. The captured-ledger disposable proof and rollback smoke cover the deny and post-boundary positive-control paths. **No live state has been changed.**
...
**Historical 2026-08-10 money baseline (superseded for order-line prices by the 2026-08-12 repair below).** Whole-cent conformance then measured: `orders.total_price` 0 dirty, `orders.total_cost` 0, `orders.total_profit` 0, `order_items.profit` 0, `quotes.total_price` 0, `quotes.total_profit` 0, `quote_items.total_price` 0, `quote_items.profit` 0; dirty values were `order_items.total_price` 35/288, `quotes.total_cost` 2/4, `commissions.commission_amount` 3/35, and `commissions.order_profit` 3/35. The 35 order-line prices were repaired by assigned migration `20260812154757`, as proven by the database-gate entry below. The quote and commission rows remain separately tracked debt; this paragraph is historical evidence and must not be read as the current order-line count.
...
**2026-08-12 pricing rollout evidence (superseded for ledger high-water by the 2026-08-13 header above).** Live then ended at `20260812154757` (`20260812115238_repair_historical_order_line_cents`) with 968 rows. The four pricing/cent-repair statements were reconciled under their B7-assigned filenames, and the schema registry was genuinely regenerated through that high-water.
...
**Severity: HIGH when found. No production runtime outage was observed on August 12, 2026 — the code
worked. The problem was that, when discovered, its source did not exist anywhere in version control,
so nobody could review the SQL running in production.** The
...
At discovery, the newest migration on `origin/main` that was both present and applied was
`20260812003315_log_customer_sales_rep_assignment`. **Six migrations had been applied live after it,
and none of their files existed on `main`, on any of the sixteen other pushed branches, or in any
local worktree under `C:\CRX_Manager`** — searched by both filename and by the identifiers they
install on 2026-08-12. The files are recovered in the resolution recorded below:
...
live shape failed there. Wave A migration `20260813015000` is exactly such a migration — it retargets
...
recovery ship as **one** change: `20260813015000` names
...
**Resolution of the files side.** Five files were recovered as the exact `apply_migration` payloads
from the applying sessions' transcripts and land in the same change as this entry. The sixth,
`20260812115238`, keeps its exact applied payload in the access-controlled applying-session record and
lands a public replay source that derives the redacted row set, then proves it against the original
fixed full-snapshot digest, counts, and financial impact before writing. This preserves disaster
recovery without publishing customer-linked identifiers and prices. None was reconstructed from live
`prosrc`, which loses headers, preconditions and review history. Function bodies extracted from the
five exact recovered texts md5-match live `pg_proc.prosrc` at identical length.
...
## RESOLVED IN THIS RELEASE — `quote_items.cost_at_quote_cents` is declared in the pricing registry and types

The column exists live as `bigint` on `quote_items` (read-only `information_schema` check,
2026-08-12). This release restores it to the generated database types and to the `QuoteItem`
interface as an optional read field because partial projections may omit it; browser code never
writes the trigger-owned snapshot. The schema registry includes it and now matches the later
return-credit live high-water `20260813011751`; there is no remaining registry-freshness task for
this column.
