...
**Last verified: 2026-08-11 UTC, post-apply release check.** The live ledger has 960 rows and ends at `20260811183437` (`20260811130000_bind_commission_payout_idempotency_to_intent`). Customer 360 ownership assignment is live immediately below it as server-assigned version `20260811183317` (submitted `20260811122851_assign_customers_sales_rep`), closing the stale-rep and partial-assignment race with an active-admin-only atomic RPC. Live catalog/grant verification passed, and the schema registry, generated Supabase types, and `pg_proc` fixture were refreshed through the current high-water. The registered live rollback smoke is still pending because Codex's live-data guard requires a separate `REAL-DATA-OK` even though the script terminates with `SMOKE_PASS_ROLLBACK`; the exact same migration and smoke chain passed in disposable PostgreSQL before apply. Team Board deployment details below remain current.
...
### OPEN 2026-08-09 — one remaining HIGH commission finding from the Section 7 gauntlet refresh (owner decision SETTLED: Option B; 3.4 parked, 3.5 resolved live)

Source: `docs/audits/gauntlet/2026-08-09-section-07-commissions-splits-payouts-voids-refresh.md` (original verdict REMEDIATION REQUIRED, 0 BLOCKER / 2 HIGH). Both were proven against **live** `pg_proc.prosrc`, not just disk. Neither was an access-control defect — RLS, admin-only payout reads, and RPC-only mutations all held. Gauntlet summary rows **3.4** and **3.5**. Finding 3.5 is now resolved live by migration `20260811130000`, applied under server version `20260811183437`; only 3.4 remains open.
...
| 3.5 | **RESOLVED LIVE 2026-08-11 — payout idempotency receipts were keyed to the operation, not the intent.** | `20260811130000_bind_commission_payout_idempotency_to_intent.sql`, applied as server version `20260811183437` | The three public payout wrappers now require a real key and bind replay to the authenticated actor plus a normalized request fingerprint. Exact intent replays; actor or payload mismatch fails closed instead of reporting another payout's cached success. |
...
### 3.5 RESOLVED LIVE 2026-08-11 — APPLIED AS SERVER VERSION `20260811183437`

Option B was implemented on branch `fix/commission-payout-intent-binding` and merged through PR #378. Migration `20260811130000_bind_commission_payout_idempotency_to_intent` applied live as Supabase-assigned ledger version `20260811183437`. Treat 3.5 as resolved in production; 3.4 remains the deliberately parked reporting-accuracy defect.

| Piece | File | State |
| --- | --- | --- |
| Migration — renames the three payout bodies to `_<name>_intent_impl_20260809` (money logic never retyped) and creates public wrappers that bind each receipt to `request_actor_id` + a SHA-256 `request_fingerprint`; adds the `check_idempotency_intent` helper | `supabase/migrations/20260811130000_bind_commission_payout_idempotency_to_intent.sql` | Applied live as server version `20260811183437`; live catalog reflected in the regenerated 566-function fixture |
...
**Historical review record:** both Codex reviews returned DO NOT SHIP on 2026-08-09 (sol and terra, independently). Every confirmed finding was fixed before PR #378 merged and the migration applied live as server version `20260811183437`. What those reviews caught, and what changed:
