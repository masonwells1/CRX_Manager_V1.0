...
**Last verified: 2026-08-06.** Live ledger high-water is `20260806000752` (`authorize_bulk_import_product_cost`). The Section 4 bulk-order-import lifecycle gap is fixed live through four migrations: imports are draft-only, inventory-aware, activity-logged, payload-bound for replay, non-finite-safe, and always use Product `current_cost` for profit and commission snapshots. Post-apply catalog/grant checks, active-sales-rep rollback smoke, all 21 standing invariant predicates, schema-registry refresh, and zero-residue checks passed. The earlier idempotency, statement-disclosure, and historical AR report protections remain live as documented below.
