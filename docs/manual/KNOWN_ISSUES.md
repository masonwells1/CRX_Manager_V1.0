...
**Last verified: 2026-07-17** (full document re-read and targeted money/inventory plus supplier-pricing refresh against current code and live DB through ledger `20260717045420`; older open/deferred claims retain their dated evidence below; owner-facing combined list: root `TODO.md`)
...
### Supplier Pricing Phase 1a rollout gap — repository fix not deployed

The additive pricing RPC/bootstrap is live, but the pre-deploy zero-cost guard, frontend, strict cutover, and `process-document` retirement are not. Until `20260717120500_supplier_pricing_zero_cost_guard.sql` is separately reviewed and applied, the governed live calculator can accept a margin-driven zero cost; therefore the new frontend must not deploy first. The active production Edge Function is still v18 and retains the old `price_list` / price-bearing `product_list` OCR paths. The repository rejects those document types before OCR, but production will not inherit that rule until a separately approved Edge Function deployment. Do not describe supplier-price OCR as retired live before that deployment is verified.
...
| `scripts/.staging-migrations/20260717120500_supplier_pricing_zero_cost_guard.sql` | Reject margin-driven zero cost before the governed RPC frontend deploys | Applying is still a live migration and needs the normal gated session; unlike the cutover it preserves deployed legacy-mode behavior | Additive bootstrap live and verified; fresh migration review/apply approval and proof; apply and verify before frontend deployment |
