...
**Last verified: 2026-08-07.** Live ledger high-water re-confirmed via `list_migrations` as `20260806023048` (`surface_bulk_import_inventory_warnings`); the `20260807153000` profile role-lock INSERT and `20260807120000` CRM fact RPC migrations are PARKED, not applied. The Section 4 bulk-order-import lifecycle gap is fixed live through six migrations: imports are confirmed-only, inventory-aware, activity-logged, actor/payload-bound for replay, non-finite-safe, Product-cost-authoritative, whole-cent per line, and create commissions from trigger-canonical stored profit. Post-apply catalog/grant checks, fractional active-sales-rep rollback smoke, all 21 standing invariant predicates, schema-registry refresh, and zero-residue checks passed. The earlier idempotency, statement-disclosure, and historical AR report protections remain live as documented below.
...
| `supabase/migrations/20260807120000_log_customer_fact_rpc.sql` | `log_customer_fact` RPC: retry-safe, role-gated, actor-pinned CRM fact intake replacing the direct `customer_facts` insert in `CustomerFacts.tsx` | **PARKED 2026-08-07** — awaiting migration-review + apply gates + Mason's OK. History row 855. Frontend cutover block is staged as a comment in `CustomerFacts.tsx` and applies post-apply. | Migration-review gate + Codex proof + Mason's in-chat approval. |
...
| `supabase/migrations/20260807153000_profile_role_lock_covers_insert.sql` | Extends `_guard_profile_role_lock` to `BEFORE INSERT OR UPDATE` so a logged-in non-admin cannot re-insert their own `profiles` row with `role = 'admin'` (§0d follow-up; closes the admin self-escalation path) | **PARKED 2026-08-07** — awaiting migration-review + apply gates + Mason's OK. Paired red/green predicate `profile-role-lock-insert-arm.sql` currently returns 2 rows (red) and goes to 0 after apply. History row 855. | Migration-review gate + Codex proof + Mason's in-chat approval (or armed hands-free run per the 2026-07-13 policy). Service-key edge-function creation path (`auth.uid()` null) is unaffected; that residual is documented in the file header. |
