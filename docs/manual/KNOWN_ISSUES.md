...
**Last verified: 2026-07-17** (targeted money/inventory and supplier-pricing refresh against current code and live DB through ledger `20260717045420`; older open/deferred claims retain their dated evidence below; owner-facing combined list: root `TODO.md`)
...
| `scripts/.staging-migrations/20260717030000_supplier_pricing_phase1a_cutover.sql` | Close direct Product pricing and cost-history writes after the governed RPC frontend is deployed | Applying it before frontend deployment breaks current Product-page pricing and prevents a safe frontend rollback | Additive bootstrap live and verified; RPC frontend deployed; rollback window closed or forward DB rollback ready; fresh apply approval/proof |
...
- **2026-07-15/17** — Schema registry was regenerated from live introspection; its current high-water is `20260716210032`. The live ledger has since advanced through `20260717020549`; the supplier-pricing apply workflow must regenerate the registry before the RPC frontend is committed. Roadmap tickets T1/N2 remain done.
