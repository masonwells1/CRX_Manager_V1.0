...
| `supabase/migrations/20260718210500_per_line_split_billing_relational_guards.sql` | Cross-table money, exact source/allocated COGS ledger, inverse job-chemical/required-service completeness, exact service uniqueness, flat-fee quantity-1 persistence, item/source identity, pricing-unit/COGS, invoice membership, customer, zero-send disposition, header, posted freeze, and immutable-history guards for the unapplied per-line split schema | Built and proven locally. Security review is clean; drift review intentionally remains H1 until prerequisite Phase 1 exists live and the registry can verify the snapshot table | Keep with the Phase 1/2 migration chain; after Phase 1 apply, refresh the live registry and rerun drift review before any guard apply |
| `supabase/migrations/20260718211000_per_line_split_billing_phase2_calculator.sql` | Phase 2 deterministic preview calculator with owner/assignment-scoped SECDEF reads, exact invoice/job/field context binding, manual→customer/season/lifecycle-bound quote→tier pricing, exact sales/COGS residual allocation, quantity-1 child flat fees, full audit-identity hashes, strict missing-price rejection, frozen job price/service/unit/COGS authority, disabled-flag install gate, and a compatibility wrapper that fingerprints the approved legacy body/security/ACL | Built behind the OFF flag and intentionally not applied while the combined Phase 1 + 2 final proof/reviews run | Complete exact content-bound and commit-SHA reviews plus the baseline billing cycle; do not apply or enable the feature flag yet |
...
- **Per-line-item custom split billing (field-app)** — DESIGN SPEC complete + review-hardened; Phase 1
  ...
  exact COGS, quote-provenance, and legacy-helper trust repairs, but both remain parked pending their
  exact review/publish gates. The production flag and Phase 1 objects
