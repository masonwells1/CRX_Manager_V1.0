...
**Last verified: 2026-07-29** (live post-apply read: **924 ledger rows**, high-water `20260729163243_harden_profile_directory_followups`. The four approved migrations applied in order: `20260729125227_secure_profile_public_directory`, `20260729125251_pin_contact_sync_search_path`, `20260729125314_application_service_cost_exact_text`, then `20260729163243_harden_profile_directory_followups`. Live postflights passed. The final migration left `service_role` with SELECT-only directory access, preserved the owner-run synchronization trigger, and exact profile/directory parity remained 11/11 with zero mismatches. Rollback-only `service_role` and authenticated role probes both passed. Earlier resolved and deferred claims below remain unchanged.)
...
| `scripts/.staging-migrations/workflow-waves-parked/SUPERSEDED-dispatch-backfill.sql` (was `PARKED-dispatch-backfill.sql`, renamed 2026-07-29) | One-time backfill of `job_location_dispatches` for legacy-assigned open jobs | **RETIRED 2026-07-29 — no longer parked, DO NOT APPLY.** Obsoleted by the live sync triggers, so it can never have work to do again. Verified read-only against live 2026-07-29: its own count query returns 0 rows (same as when parked on 2026-07-10), and `trg_sync_job_location_dispatch_on_applicator_change` on `jobs` + `trg_sync_job_location_dispatch_on_field_insert` on `job_fields` are both live, so assigning an applicator or adding a field writes the dispatch row automatically and no new legacy job can appear. | Nothing — it is not waiting on Mason. Do **not** re-run the count query "just in case" and apply it: this is a business-data write whose trigger coverage is the reason it is dead. If a dispatch row is ever genuinely missing, diagnose the trigger, do not resurrect this backfill. |
