...
- **Phase 3C byte and candidate scan budgets need earlier maintenance than the ancestry cap.** The measured first-push proof consumed about 1.61 GiB of the fail-closed 2 GiB `MAX_TOTAL_STRUCTURAL_SCAN_BYTES` budget and 71,289 of the 100,000 `MAX_STRUCTURAL_SCAN_CANDIDATES` budget. Re-measure and adjust those bounds with adversarial memory/time proof before either reaches 80%; a breach intentionally blocks push and CI rather than scanning an unbounded repository.
