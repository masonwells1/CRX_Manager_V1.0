...
- **Offline work recovery rollout is queued, not live** — the feature branch now includes permanent server receipts, browser retention until proven success, distinct cap/backlog handling, a safe device review panel, and an audited office `already_completed` / `abandoned` path. The three database migrations have **not** been applied and the frontend must not merge ahead of them. Still deferred: signature/photo persistence, idempotent email/notification replay, cross-tab leasing, operation-specific conflict preconditions, and a general duplicate-action policy. Browser storage remains device-local until the phone reconnects and stages its receipt, so destroying or clearing that storage before reconnection can still lose work. Source: `docs/audits/2026-07-13-agent-pair-review-offline-work-durability.md` and `docs/roadmap/offline-work-stage1b-receipt-design-2026-07-13.md`.
