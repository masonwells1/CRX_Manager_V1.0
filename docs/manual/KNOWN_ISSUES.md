...
**Last verified: 2026-08-03.** Live ledger high-water is `20260803221244`. The prior payload-blind replay risk for `save_invoice` and `create_quick_delivery` is fixed live: receipts bind to a server-derived request fingerprint and authenticated actor, while mismatches reopen the already-committed entity instead of duplicating it. Post-apply grants, auth guards, columns, and receipt binding state were verified. The Section 2 statement balance-disclosure migration is live under the B7-reconciled version `20260803221244`; post-apply helper, grant, constraint, newline-normalized ledger-content, and read-only RPC execution checks passed. A prior candidate reached rollback-only smoke; no separate final-revision rollback rehearsal was captured.
