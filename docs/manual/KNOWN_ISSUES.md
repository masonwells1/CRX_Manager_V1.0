...
**Last verified: 2026-07-29** (live post-apply read: **924 ledger rows**, high-water `20260729163243_harden_profile_directory_followups`. Four approved migrations applied in order: `20260729125227_secure_profile_public_directory`, `20260729125251_pin_contact_sync_search_path`, `20260729125314_application_service_cost_exact_text`, then `20260729163243_harden_profile_directory_followups` (#276; authored `20260729151850`, B7-renamed to the server-assigned version). Live postflights passed, including role-impersonation proof that admins receive exact text cents and a sales rep is denied with SQLSTATE 42501. Earlier resolved and deferred claims below remain unchanged.)
...
- **RESOLVED 2026-07-29 — applied live as ledger version `20260729163243`** (disk file B7-renamed to match; body unchanged), merged in #276. `harden_profile_directory_followups` replaces the duplicated active-profile predicate with `public.is_active_profile()`, reduces `service_role` on the directory table/view to SELECT-only, and removes direct application-role EXECUTE from the trigger-only synchronizer. Source and live-catalog review found no direct directory writers; profile writes continue through the owner-run trigger. Live catalog re-read after the apply confirms all three: the policy qual is `( SELECT is_active_profile() AS is_active_profile)`, `service_role` holds no write privilege on either the table or the view (SELECT retained on both), and the synchronizer's ACL is `{postgres=X/postgres}` — no application role can call it directly. The mirror is intact at 11 = 11 rows.
