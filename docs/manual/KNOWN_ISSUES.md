...
**`quote_items.cost_at_quote_cents` is live but not in the type layer.** Added by `20260812115236` and running in production since 2026-08-12. It is **absent from `.claude/schema-registry.json` and from `src/types/index.ts`**, so the schema-aware hooks validate against a column set that no longer matches the database and TypeScript does not know the field exists. Refresh the registry from introspection and add the field before anything reads or writes it from the front end.
