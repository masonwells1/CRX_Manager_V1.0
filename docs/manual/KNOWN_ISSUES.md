...
| `supabase/migrations/20260718210500_per_line_split_billing_relational_guards.sql` | Cross-table money, item identity, invoice membership, customer, source, and header guards for the unapplied per-line split schema | Built and proven locally, but intentionally not applied while the feature remains OFF and exact-hash reviews are incomplete | Keep with the Phase 1/2 migration chain; complete reviews and the baseline cycle before any owner-approved apply |
| `supabase/migrations/20260718211000_per_line_split_billing_phase2_calculator.sql` | Phase 2 deterministic preview calculator, strict missing-price rejection, frozen job price/service authority, disabled-flag install gate, and compatibility wrapper | Built behind the OFF flag and intentionally not applied while the combined Phase 1 + 2 final proof/reviews run | Complete exact-hash reviews and the baseline billing cycle; do not apply or enable the feature flag yet |
