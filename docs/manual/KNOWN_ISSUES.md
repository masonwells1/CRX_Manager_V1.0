...
**Last verified: 2026-07-18** (full document re-read plus supplier-pricing and targeted per-line split-billing refresh against current code and live DB through high-water `20260718193000`; older open/deferred claims retain their dated evidence below; owner-facing combined list: root `TODO.md`)
...
| `supabase/migrations/20260718210000_per_line_split_billing_phase1_schema.sql` | Phase 1 schema and feature-flag foundation for per-line split billing | Built and proven but intentionally not applied: the required §6.1 real legacy billing baseline has not occurred and live apply remains an owner gate | Complete the baseline billing cycle, finish draft-PR review, then obtain Mason's explicit live-apply approval |
| `supabase/migrations/20260718205500_per_line_split_billing_phase1_preflight.sql` | Fail-closed absence check before the unapplied Phase 1 schema, including every new feature-owned function signature | Built and proven locally against stale tables/indexes/columns and an authenticated-owned same-signature trigger; intentionally not applied while the feature remains OFF and exact-hash reviews are incomplete | Apply only as the first migration in the eventual owner-approved per-line chain; any stale object/function requires investigation, not bypass |
| `supabase/migrations/20260718210500_per_line_split_billing_relational_guards.sql` | Cross-table money, exact signed source/allocated COGS ledger for sales and returns, frozen job quantity/base-price/provenance binding, inverse job-chemical/required-service completeness, exact service uniqueness, flat-fee kind/basis plus quantity-1 persistence, item/source identity, pricing-unit/COGS, invoice membership, customer, zero-send disposition, header, posted freeze, immutable-history, and installed-function security guards for the unapplied per-line split schema | Built and proven locally, including durable negative-return COGS and coherent quantity/price rewrite rejection. Fresh content-bound reviews must be rerun on the repaired bytes; live drift remains expected until prerequisite Phase 1 exists | Keep with the Phase 1/2 migration chain; after Phase 1 apply, refresh the live registry and rerun drift review before any guard apply |
| `supabase/migrations/20260718211000_per_line_split_billing_phase2_calculator.sql` | Phase 2 deterministic preview calculator with owner/assignment-scoped SECDEF reads, caller-owned temp-object rejection, exact invoice/job/field context binding, manual→customer/season/lifecycle-bound quote→tier pricing with quote-unit normalization and complete contributing-field coverage, exact signed sales/COGS residual allocation, quantity-1 child flat fees, full audit-identity hashes, strict missing-price/cost rejection, frozen job price/service/unit/COGS authority, disabled-flag install gate, and a compatibility wrapper plus final owner/definer/search-path/ACL assertions | Built behind the OFF flag and intentionally not applied. Exact-SHA reviews found and the disposable proof now covers signed return COGS persistence, quote price/unit pairing, unconvertible/partial/zero-share quote fields, NULL product-cost rejection, predictable temp-object defense, same-signature function drift, flat-fee basis coupling, and coherent job quantity/base-price rewrites; fresh reviews are required on the current repaired bytes | Complete the fresh content-bound and commit-SHA reviews plus the baseline billing cycle; do not apply or enable the feature flag yet |
...
- **Per-line-item custom split billing (field-app)** — DESIGN SPEC complete + review-hardened; Phase 1
  ...
  repaired and its disposable PostgreSQL proof is green; Phase 2's calculator proof is also green after
  exact signed COGS (including durable returns), quote-provenance/unit/field-coverage, missing-cost,
  temp-object boundary, legacy-helper trust, function-ownership drift, flat-fee basis, and frozen
  job quantity/base-price/provenance repairs, but
