...
| `supabase/migrations/20260718211000_per_line_split_billing_phase2_calculator.sql` | Phase 2 deterministic preview calculator with owner/assignment-scoped SECDEF reads, exact invoice/job/field context binding, manual→customer/season/lifecycle-bound quote→tier pricing, exact signed sales/COGS residual allocation, quantity-1 child flat fees, full audit-identity hashes, strict missing-price rejection, frozen job price/service/unit/COGS authority, disabled-flag install gate, and a compatibility wrapper that fingerprints the approved legacy body/security/ACL | Built behind the OFF flag and intentionally not applied. An exact-SHA review found and the disposable proof now covers signed return COGS persistence; fresh content-bound security and drift reviews are clean on the repaired bytes | Complete the fresh commit-SHA reviews plus the baseline billing cycle; do not apply or enable the feature flag yet |
...
- **Per-line-item custom split billing (field-app)** — DESIGN SPEC complete + review-hardened; Phase 1
  ...
  exact signed COGS (including durable returns), quote-provenance, and legacy-helper trust repairs, but
