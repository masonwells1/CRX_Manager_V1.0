...
**OPEN — same-product quote lines at multiple cent prices can block a valid partial draw.** The live private `draw_down_quote` implementation calculates one weighted average unit price per product. A booking split across `$10.00` and `$10.01` lines can therefore produce `$10.005`; the whole-cent order-line trigger correctly raises `INVALID_UNIT_PRICE_CENTS`, but the valid draw is rolled back. The local forward migration `20260813053545_allocate_multi_price_quote_draws.sql` fixes this without changing the public RPC: authoritative order-line totals are cumulative cent deltas, while the stored unit price is floored to cents so below-cost checks never round in the seller's favor. The captured-schema proof and an exact two-partial-draw rollback regression pass. This issue remains open until that migration is separately reviewed, applied, and verified live.
...
**`quote_items.cost_at_quote_cents` is restored in the app types and registry; the overall registry refresh remains open.** Added by submitted migration `20260812115236` (assigned/disk version `20260812151606`) and running in production since 2026-08-12. `src/types/index.ts` declares it as optional because partial projections may omit it; the column is trigger-stamped and the browser never writes it. The pricing rollout regenerated `.claude/schema-registry.json` through live high-water `20260812154757`, so this column and the pricing objects are present. The two later return-credit applies advanced live to `20260813011751`, making the registry as a whole stale again; refresh it after the return-credit release reconciles those sources.

**Wave A — six unapplied migration candidates.** This tree carries submitted names `20260813010000` through `20260813060000`; all six remain absent from the fresh 970-row live ledger. Nothing in this document describes state they created. Because the live ledger advanced after they were stamped, revalidate the complete set and renumber any colliding/older stamp before any apply is considered.
...
writes the trigger-owned snapshot. The pricing schema registry includes it through assigned version
`20260812154757`. The later return-credit applies advanced the live ledger to `20260813011751`, so
the registry still needs a fresh live rebuild when the return-credit release is reconciled; that is
an overall registry-freshness task, not an undeclared quote-cost-column defect.
