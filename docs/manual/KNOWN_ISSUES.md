...
**Last verified: 2026-08-10.** Live ledger high-water is **`20260810025159`** (`20260810022500_backfill_stale_line_profit`, 954 ledger rows). Its source file is owned by open PR #364 and is not yet in `origin/main`. The two re-issued PR #350 pricing migrations (`20260810135537` and `20260810135538`) are not live. Current read-only aggregates show 35 fractional `order_items.total_price` rows, 3 fractional active `commissions.commission_amount` rows, zero stale `order_items.profit` rows, and one remaining order-header-versus-lines profit mismatch. This 2026-08-10 pass rechecked the ledger and those aggregates only; the last full 27-predicate sweep remains the dated 2026-08-09 run described below.
