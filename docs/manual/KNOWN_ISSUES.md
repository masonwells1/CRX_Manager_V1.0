...
**`quote_items.cost_at_quote_cents` is live but not in the type layer.** Added by `20260812115236` and running in production since 2026-08-12. `src/types/index.ts` now declares it (optional, because no current query selects it — the column is trigger-stamped and the browser never writes it). It remains **absent from `.claude/schema-registry.json`**, so the four schema-aware hooks still validate against a column set that no longer matches the database. The registry is behind by more than this one column — the session-staleness hook reports its high-water at `20260812003315` against a live high-water of `20260812154757` — so the fix is a full `--from-introspection` rebuild, deliberately kept out of the recovery PR because a whole-schema resync would bury a six-file record in an unrelated diff.
