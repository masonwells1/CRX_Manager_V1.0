...
|---|---|---|---|
| `supabase/migrations/20260726210000_void_vendor_payment_vendor_liveness.sql` | Section 9 follow-up MEDIUM-1: `void_vendor_payment` doesn't serialize with `delete_vendor`, so a vendor can be soft-deleted while a concurrent void re-opens one of its bills (deleted vendor carrying an open AP liability). Guard-only fix: adds a `deleted_at IS NULL … FOR UPDATE` vendor lock + `VENDOR_DELETED` error to the live body (byte-identity md5-proven). | Drafted 2026-07-26, pending the governed apply gate + Mason's explicit in-chat approval. Strict consequence to flag: payments on a soft-deleted vendor's bills can't be voided until the vendor is restored, and no vendor-restore RPC exists yet. | Gate pass (`write-apply-proofs.mjs`, both charters CLEAN) then Mason's OK to apply. |
