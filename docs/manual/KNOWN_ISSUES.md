...
**OPEN — same-product quote lines at multiple cent prices can block a valid partial draw.** The live private `draw_down_quote` implementation calculates one weighted average unit price per product. A booking split across `$10.00` and `$10.01` lines can therefore produce `$10.005`; the whole-cent order-line trigger correctly raises `INVALID_UNIT_PRICE_CENTS`, so the transaction safely rolls back but the valid draw cannot finish. A reviewed local candidate that floored the stored unit price and allocated rounded cumulative totals was removed: reconstructing an invoice from quantity × unit price could underbill by a cent, and using reversible job draws in the cumulative allocation could allocate the same cent twice after cancellation. No safe forward migration is currently prepared; the durable design needs cent-priced cohorts or an immutable allocation/revenue ledger.
