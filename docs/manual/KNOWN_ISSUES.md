...
**OPEN IN PRODUCTION / FIX PREPARED — same-product quote lines at multiple cent prices can block a valid partial draw.** The live private `draw_down_quote` implementation calculates one weighted average unit price per product. A booking split across `$10.00` and `$10.01` lines can therefore produce `$10.005`; the whole-cent order-line trigger correctly raises `INVALID_UNIT_PRICE_CENTS`, so the transaction safely rolls back but the valid draw cannot finish. The pending `20260813161614` candidate now rounds the weighted unit price to whole cents at the same source boundary where weighted cost is already settled, before line total/profit/commission math and insertion. The captured-ledger rebuild and draw-down contract test pass on that final body. The candidate is not applied, so production still has the blocking behavior until the governed migration rollout.
