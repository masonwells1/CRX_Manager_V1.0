...
**Last verified: 2026-07-20** (full document re-read plus live ledger/function refresh through `20260720181939`; older open/deferred claims retain their dated evidence below; owner-facing combined list: root `TODO.md`)
...
- **2026-07-20** — Governed split invoices can no longer pass through generic
  `save_invoice`, whose line replacement does not preserve source
  `order_item_id`; they must be voided/reissued. Singular grouped voids are
  refused and `void_invoice_group` retires every active private-provenance
  member atomically with actor/group/reason-bound replay. Applied live as
  ledger `20260720181939`; exact reviewers were CLEAN and the full H5 chain
  returned `SMOKE_PASS_ROLLBACK`.
