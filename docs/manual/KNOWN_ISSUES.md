...
**Last verified: 2026-08-09.** Live ledger high-water is `20260809130108` (`team_note_completion_rpc_and_assignment_notify`). The database half of the Team Board delegation fix is live and passed catalog/ACL checks, all 26 standing invariant predicates, and a genuine schema-registry refresh. A fresh RLS review found a live inactive-actor notification path; forward-only hardening `20260809154649_active_team_note_assignment_actor` is written but NOT LIVE. The compatible frontend remains pending in PR #351, and the revised rollback-only behavior smoke still needs its external `SMOKE_PASS_ROLLBACK` result. The earlier profile role-lock, CRM fact RPC, bulk-import lifecycle, idempotency, statement-disclosure, and historical AR report protections remain live as documented below.
...
## OPEN HIGH / PARTIALLY RESOLVED LIVE — Team Board delegated completion and assignment notifications

Migration `20260809130108_team_note_completion_rpc_and_assignment_notify` is applied live. It adds the governed `complete_team_note` RPC and the assignment-notification trigger without widening the existing `tnotes_update` policy. Live structure, grants, and the 26 standing invariant predicates passed. Fresh RLS review confirmed one HIGH: an inactive profile whose JWT remains valid can still satisfy the legacy `tnotes_insert` creator check, and the owner-run trigger does not independently require the actor to be active before inserting a notification. Forward-only migration `20260809154649_active_team_note_assignment_actor` closes both layers while leaving `tnotes_update` unchanged; it is parked pending review, proofs, and Mason's separate approval. The smoke was revised to use a rollback-only synthetic inactive auth/profile fixture instead of changing a real teammate's authentication state. PR #351 still owns the browser changes that call the RPC and open assignment notifications on Team Board, and an authorized external session must produce the exact `SMOKE_PASS_ROLLBACK` terminal before final closeout.
