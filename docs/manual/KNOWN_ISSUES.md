...
| Re-binding after a passing check (`p_performed_by := p_target_id;`), `EXECUTE … USING`, `INSERT … RETURNING … INTO`, temp-table round trips | **Not covered at write time, and not covered by the sweeps either.** The incidental `hasMutation` trigger that would catch `EXECUTE`/`INSERT` lives in **parked PR #449, not in the running hook** (213 lines, no such logic) — do not credit the active guard with it. **NARROWED 2026-09-02: the ASSIGNMENT form IS now covered by both sweep predicates** — each fails closed and scans the whole body instead of truncating at the refusal when the actor parameter is assigned to at statement position, matching both `:=` and bare `=`, pinned to statement position because named-argument syntax is lexically identical to assignment. Proved on real PostgreSQL 17; zero new findings against the live catalog. **The `INTO`-target form is proven STILL OPEN**: an exact-SHA `gpt-5.6-sol` proof ran `SELECT 1, p_target_id INTO v_dummy, p_performed_by` through the real hook and observed `allow` (the actor in the FIRST `INTO` target correctly denies), and both sweeps miss it because the rebinding rule reads assignment syntax, not `INTO` target lists. A temp-table round trip matches neither the `coalesce`/`auth.uid`/role proximity test in `actor-forgery.sql` nor the same-statement `financial_audit_log … <param>` test in `-fin-audit.sql`. At write time none of these are caught: the `hasMutation` trigger that would catch `EXECUTE`/`INSERT` lives in **parked PR #449, not in the running hook** — do not credit the active guard with it. |
