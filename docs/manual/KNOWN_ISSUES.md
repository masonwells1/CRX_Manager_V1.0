...
**Last verified: 2026-08-06.** Live ledger high-water is `20260806004644` (`bind_bulk_import_intent_and_profit`). The Section 4 bulk-order-import lifecycle gap is fixed live through five migrations: imports are confirmed-only, inventory-aware, activity-logged, actor/payload-bound for replay, non-finite-safe, Product-cost-authoritative, and create commissions from the trigger-canonical stored profit. Post-apply catalog/grant checks, fractional active-sales-rep rollback smoke, all 21 standing invariant predicates, schema-registry refresh, and zero-residue checks passed. The earlier idempotency, statement-disclosure, and historical AR report protections remain live as documented below.
