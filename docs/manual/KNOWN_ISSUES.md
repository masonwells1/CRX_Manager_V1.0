...
**Last verified: 2026-08-13 UTC, migration-ledger and registry re-read.** Live ledger high-water is `20260813011751` at 970 rows, carrying submitted name `20260813070000_pin_return_idempotency_helper_contract`; `.claude/schema-registry.json` is refreshed through the same high-water. Ledger versions are UTC and Supabase applies may assign a version different from the submitted filename, so match the recorded name when reconciling an apply. This re-read does not replace the historical Team Board, money, Customer 360, and commission-payout evidence below, which remains separately dated.

**Wave A — six parked migration candidates.** The local files `20260813010000` through `20260813060000` are **not applied**, confirmed absent from the live ledger by a read-only re-read on 2026-08-13. Nothing in this document describes state they created. They are parked drafts and require their normal review, fresh live preflight, and explicit live-apply authorization before any future apply.
