...
**Last verified: 2026-08-10.** Live ledger high-water is **`20260810025159`** (`backfill_stale_line_profit`), **954 ledger rows**, re-read live 2026-08-10. Three `20260810` rows are applied: `20260810000427` (the assigned version for merged file `20260809230500_single_canonical_line_profit.sql`, history row 862), plus **two whose files are NOT in `origin/main`** — `20260810010308_active_team_note_assignment_actor` (commit `c3f2a387`) and `20260810025159_backfill_stale_line_profit` (commit `a0a69a62`), both sitting on the unmerged branch `claude/session-orchestration-setup-d73e6c`. This is a milder form of the `20260809130108` problem below: the files exist in git, so a rebuild is recoverable, but **live is ahead of `main` and will stay ahead until that branch merges.** Whoever owns that branch should land it.

**2026-08-10 money re-measure (read-only, live).** Whole-cent conformance by column, which is what history rows 863–865 are scoped against: `orders.total_price` 0 dirty, `orders.total_cost` 0, `orders.total_profit` 0, `order_items.profit` 0, `quotes.total_price` 0, `quotes.total_profit` 0, `quote_items.total_price` 0, `quote_items.profit` 0 — and still dirty: **`order_items.total_price` 35/288, `quotes.total_cost` 2/4, `commissions.commission_amount` 3/35, `commissions.order_profit` 3/35.** The `order_items` figure moved 46 → 35 because `20260810025159` (above) backfilled stale line profit through the canonical trigger; the 3 fractional `commissions` rows are unchanged and still deliberately unrepaired. **43 dirty rows remain and repairing them rewrites stored money — still Mason's separate decision, still not done.**

**2026-08-10 commission-basis measurement, correctly characterized.** 10 of 35 order commissions have `order_profit ≠ orders.total_profit`. Do not read that as a live emergency: **9 are `pending` with a gap of exactly $0.01** (the disclosed backfill residual from `a0a69a62` on the branch above, which deliberately did not rewrite commission rows), and the **1 materially larger gap is on a `cancelled` row** (dollar figure deliberately withheld — this repository is public; it is in the access-controlled session record). The underlying mint-time code defect is real and confirmed from live function source — `_convert_quote_to_order_owner_impl` and `create_direct_order` mint from a cached/local profit after the item triggers already rewrote the canonical header — and history row 863 is the drafted fix. It is worth landing to stop future drift, not to repair present damage.
