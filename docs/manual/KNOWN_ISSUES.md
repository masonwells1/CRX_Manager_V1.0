...
**Last verified: 2026-07-17** (full document re-read and targeted money/inventory plus supplier-pricing refresh against current code and live DB through ledger `20260717112011`; older open/deferred claims retain their dated evidence below; owner-facing combined list: root `TODO.md`)
...
The additive pricing RPC/bootstrap and compatibility-safe zero-cost guard are live through ledger `20260717112011`; the governed calculator now rejects margin-driven zero cost while legacy Product-page editing remains available. The frontend, strict cutover, and `process-document` retirement are not live. The active production Edge Function is still v18 and retains the old `price_list` / price-bearing `product_list` OCR paths, and the deployed frontend still contains its legacy pricing UI. The repository rejects those document types before OCR, but production will not inherit that rule until a separately approved Edge Function deployment. Do not describe supplier-price OCR as retired live before that deployment is verified.
