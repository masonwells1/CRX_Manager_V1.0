...
separately as `20260729015706_application_service_cost_admin_only`, which revokes the table-level
...
**Residual after `20260729015706` — a SALES REP can still recover cost from the invoice side. Owner
decision, not a grant fix.** Two derived copies of cost live on invoices and are not touched by that
migration: `invoice_items.cost_cents`, which sits next to `acres` on a service line so cost/acre
divides straight back out, and `invoices.total_cost_cents`, the per-invoice roll-up. Both are reached
through `invoices_select` / `invoice_items_select`, verified live as
`is_admin() OR created_by = auth.uid() OR salesman_id = auth.uid()`. **Drivers cannot reach either** —
they neither create invoices nor are salesman on them — so the headline claim of that migration, that
drivers can no longer see internal cost, holds. What remains is that a rep sees cost on *their own*
invoices, which is unchanged by the migration rather than introduced by it, and which may well be
intended: a rep arguably needs margin on their own deals. Narrowing it is a product call for Mason and
would mean splitting the invoice read, not adjusting a grant. **Nothing is scheduled.** Worth noting
that the exposure is theoretical today for the application-services path specifically: 0 of 4 services
carry a non-zero `cost_per_acre_cents`, so there is no figure to derive yet.
