...
**`quote_items.cost_at_quote_cents` and the live schema registry are reconciled.** Added by submitted migration `20260812115236` (live ledger version `20260812151606`) and running in production since 2026-08-12. `src/types/index.ts` declares it as optional because partial projections may omit it; the column is trigger-stamped and the browser never writes it. The registry includes the pricing objects and matches live high-water `20260813011751`. Eleven migration files carry later filename stamps; `20260813070000` is already applied by name, while six Wave A files plus `20260813080000`, `20260813090000`, `20260813161614`, and `20260813170000` remain intentionally unapplied. `20260813030000`, `20260813060000`, and `20260813170000` would change the table/column/constraint surface represented by the schema registry, so the staleness warning is a pending-candidate signal rather than evidence that the current live schema snapshot is wrong.

**Wave A — six unapplied migration candidates.** This tree carries submitted names `20260813015000`, then `20260813020000` through `20260813060000`; all six remain absent from the fresh 970-row live ledger. Nothing in this document describes state they created. The first candidate was restamped from `20260813010000` after the live ledger advanced to `20260813011751`; revalidate the complete set and read the ledger again immediately before any apply is considered.
