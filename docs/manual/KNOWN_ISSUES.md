...
- **Fix pending apply:** `20260729151850_harden_profile_directory_followups.sql` replaces the duplicated active-profile predicate with `public.is_active_profile()`, reduces `service_role` on the directory table/view to SELECT-only, and removes direct application-role EXECUTE from the trigger-only synchronizer. Source and live-catalog review found no direct directory writers; profile writes continue through the owner-run trigger. Mark this resolved only after the migration applies and rolled-back role proof passes.
