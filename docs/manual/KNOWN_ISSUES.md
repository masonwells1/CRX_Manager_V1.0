...
| `supabase/migrations/20260718153736_per_line_split_billing_phase2_calculator.sql` | Phase 2 deterministic preview calculator and compatibility wrapper | Disposable PostgreSQL diagnostic is green, but it depends on the invalid/unapplied Phase 1 migration; independent Codex found no Phase 2 defect, and Claude Opus agreed on the calculator while returning NEEDS-WORK on Phase 1 plus a proof-integrity issue now fixed | Fix and prove Phase 1 first, then rerun Phase 2's guarded reviews; do not apply or enable the feature flag yet |
