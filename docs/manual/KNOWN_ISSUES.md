...
## 0d. RESOLVED 2026-07-29 — `profile_public_view` was an RLS bypass onto `profiles` for any signed-in user

**Status: CLOSED on live.** Fixed by `supabase/migrations/20260729125227_secure_profile_public_directory.sql` (authored `20260728185739`, first B7-renamed to `20260729043000` by PR #269, then to its server-assigned ledger version after apply), **applied live 2026-07-29 12:52 UTC**. Verified live afterwards: `has_table_privilege('authenticated','public.profile_public_view','INSERT')` is now **false** and the view is `security_invoker = true` over `profile_public_directory`, where `authenticated` holds SELECT only. The escalation chain below is recorded for its proofs; the record of what it took is worth more than the closed finding.
...
So the realistic threat was a new or low-activity account, acting deliberately through the API, escalating itself to admin. Not something a user trips into, and not reachable logged out — but a genuine escalation path, not merely a data-deletion nuisance. That is why this one was applied ahead of the queue rather than batched.
...
**Two review findings were baked into the applied file and are carried by a follow-up migration.** The file is applied and therefore immutable, so neither can be corrected in place. Neither is a live hole:

- The directory table's SELECT policy **inlines** the active-profile predicate rather than calling `public.is_active_profile()`. Same behavior today; the cost is that this one policy no longer tracks the shared helper, so a future change to the helper would silently skip it.
- The `REVOKE`s name `PUBLIC`, `anon` and `authenticated` but **not `service_role`**, and this project's `ALTER DEFAULT PRIVILEGES ... ON TABLES` grants `service_role` everything on new public objects — so `service_role` retains INSERT/UPDATE/DELETE/TRUNCATE on both the directory table and the view (verified live: all four true). `service_role` is a trusted server-side key that already holds `BYPASSRLS`, so no client capability follows from this; removing it is defence-in-depth.
