...
**Post-apply live proof, read read-only 2026-08-18:** `public.quote_versions` carries exactly one policy, `qversions_select`; `authenticated` holds **SELECT only** on the table; `anon` holds nothing; `qversions_insert` is gone. The live ledger row's stored statement text is 87,906 characters against the 88,098-byte UTF-8 disk file — the difference is multi-byte punctuation, so live and disk are the same migration, and the file is on `origin/main`. A live exploitation check over existing rows came back clean when the fix was written (2026-08-13); there are 3 `quote_versions` rows live.
