...
- Reserved instruction basenames and nested `.agents`, `.claude`, `.codex`, `.github`, or `.husky` segments are denied case-insensitively at every depth, closing the exact-head reviewer's nested agent-control finding.
- `npm run test:ci-scope`: 80 assertions passed. Real temporary repositories cover PR merge-base semantics, push ranges, safe and unsafe renames, deletion, symlink-shaped entries, changelog content/status, nested agent-control markers, and fallback errors.
