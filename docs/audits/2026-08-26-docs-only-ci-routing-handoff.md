...
- Correctly shaped `docs/changelog.d/` paths are checked with the trusted base's shared `entryContentVerdict`; malformed names, impossible dates, invalid candidate content, and the folder README do not receive the fast route.
- `npm run test:ci-scope`: 63 assertions passed. Real temporary repositories cover PR merge-base semantics, push ranges, safe and unsafe renames, deletion, symlink-shaped entries, changelog content, and fallback errors.
- Mutation proof: removing the lowercase `.md` requirement, replacing the trusted-base worktree invocation, routing the Phase 3C packet suite to the docs lane, and disabling changelog-content refusal each turned the owning test red; every guard was restored and rerun green.
