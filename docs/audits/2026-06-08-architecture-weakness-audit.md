...
- **Status: ACCEPTED (won't-fix) 2026-06-08.** During remediation the live bodies were re-read: `convert_quote_to_order` actually writes **`'quote_converted'`** (not `'order_created'`) to `financial_audit_log`, so the sibling originally named here was wrong. The **correct** comparable path is **`create_quick_delivery`**, which DOES write `'order_created'` to `financial_audit_log`. On review the gap is **marginal and accepted, not fixed**: (1) `create_direct_order` already logs creation to **`activity_feed`** (`'order_created'`) — it's only absent from `financial_audit_log`; (2) `create_quick_delivery` logs financially largely because it *also creates an invoice* (a financial artifact), whereas a plain order is arguably a financial event only at invoicing; (3) the fix would require reproducing the large `create_direct_order` body **plus** a dollars→cents conversion (it works in `v_total_price` numeric dollars; `total_impact_cents` is bigint cents) — a real transcription/units risk on a critical order-creation path, not worth a MED forensic nicety. Re-open only if a financial-audit completeness requirement makes order-creation auditing mandatory.
- **Pass 7.** `create_direct_order` creates an order but writes **no** `financial_audit_log` row (`audit=false`), while the comparable order-creation path **`create_quick_delivery`** writes an `order_created` row to `financial_audit_log`. **No trigger backfills it** — the only trigger on `financial_audit_log` is the immutability guard, so audit writes are explicit-in-body only.
...
- **Suggested hardening:** add the `order_created` `financial_audit_log` write to `create_direct_order` to match `create_quick_delivery`. *(Note: `save_invoice` is also `audit=false` but that's defensible — it handles **draft** invoices, which are pre-financial; `post_invoice` writes the financial event.)*
