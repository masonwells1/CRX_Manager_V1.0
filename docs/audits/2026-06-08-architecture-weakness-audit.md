...
- **Status: CONFIRMED REAL, DEFERRED 2026-06-08.** During remediation the live bodies were re-read: `convert_quote_to_order` actually writes **`'quote_converted'`** (not `'order_created'`) to `financial_audit_log`, so the sibling originally named here was wrong. The **correct** comparable path is **`create_quick_delivery`**, which DOES write `'order_created'` to `financial_audit_log`. The gap stands. Deferred (not applied) because the fix requires reproducing the large `create_direct_order` body — a transcription risk on a critical order-creation path not worth a MED forensic gain in the same session.
