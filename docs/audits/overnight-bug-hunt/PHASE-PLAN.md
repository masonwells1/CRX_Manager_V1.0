...
| `prepay-blend` | prepay apply/earmark + blend-ticket invoice/payment-status/actor | hunted ×3 (c5/c9/c11) — c5: 3 new + 2 refuted; c9: 1 new (cross-customer prepay). c11: 0 genuinely-new (a verifier tried to refute the parked blend double-billing HIGH but I VERIFIED that refutation WRONG — the multi-customer fan-out is real via derive_customer_shares_from_fields; HIGH stands). 1 dry pass; 1 more to drain |
| `splits-shares-allocation` | order_item_field_allocations, create_split_invoices_from_order, dormant shares subsystems, allocate_payment, write-offs | hunted ×3 (c5/c10/c11) — c5: 2 new + 2 refuted; c10: 0 new (re-confirms only). c11: 1 NEW (order_item_field_allocations no post-invoice edit-lock, LOW). NOT drained (c11 reset the dry streak); 1 more pass |
