...
| `commissions` | commission_split, per-order records, recompute-on-edit, commission_payments batch | **RE-OPENED** (was DRAINED c4+c7) — cycle-8 merge surfaced a real HIGH both prior passes MISSED: cancel/void an order → `post_commission_payment` resurrects the cancelled commission to 'paid' and pays the stale batch snapshot. Codex-confirmed HIGH, parked. Needs a fresh confirming hunt before re-draining |
