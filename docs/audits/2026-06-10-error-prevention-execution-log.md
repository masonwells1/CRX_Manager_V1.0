...
1. **plpgsql_check: 30 errors / 11 functions** (42703/42804/42P01/42883/0A000): `create_quick_delivery` (2× unknown column), `transfer_job_to_invoice` (4 + cascade), `create_invoice_from_blend_ticket` (1 + cascade ×10), `create_commission_payment` (FOR UPDATE with DISTINCT), `load_recipe_into_job`, `save_field_geometry`/`save_field_polygons` (search_path hides PostGIS schema), `get_gross_sales_report`, `get_inventory_cost_report`, `create_job_from_quote_section` (relation activity_log), `create_split_invoices_from_order` (jsonb/text 42804). — **DONE 2026-06-11 (all 11 functions fixed live):** `20260611000630` get_inventory_cost_report · `20260611001439` create_commission_payment · `20260611001615` create_split_invoices_from_order · `20260611001954` + `20260611203238` create_invoice_from_blend_ticket (42703 colfix + the post-smoke 55000 app-service guard it unmasked) · `20260611002043` get_gross_sales_report · `20260611002114` save_field_geometry/save_field_polygons · `20260611002226` create_job_from_quote_section · `20260611002255` transfer_job_to_invoice · `20260611201929` load_recipe_into_job · `20260611203302` create_quick_delivery. Final verify 2026-06-11: errors-only `plpgsql_check_function_tb` scan over all public non-trigger plpgsql functions = **0 rows**.
2. **`void_payment` prepay-credit reversal guaranteed-miss** (`source_reference` format mismatch) — strands `prepay_balance_cents` on every overpayment void. — **DONE 2026-06-11:** `20260611001904_void_payment_prepay_reversal` (chip task_d53a1704); `fin-prepay-balance` predicate 0 rows post-fix.
3. **`get_customer_statement` blind spots** — allocate_payment-path payments invisible; posted-only filter drops paid/overdue invoices; no `payments.deleted_at` filter; NULL-`order_id` payments excluded. — **DONE 2026-06-11:** `20260611131549_customer_statement_blind_spots` (chip task_25d25699); `fin-ar-statement-balance` transcription co-updated, post-fix predicate run 0 rows.
4. **22-RPC idempotency-scoping sweep** (operation-unscoped `WHERE idempotency_key =` lookups; fix the CLAUDE.md copy-paste snippet too). — **OPEN (staged, NOT applied):** CLAUDE.md snippet fixed (operation filter now in the canonical pattern); the 20-RPC sweep migration is drafted at `scripts/.staging-migrations/idempotency_operation_scope_sweep.sql` and the other 2 ride the planned-holds rebuild (`20260611132115_planned_holds_drawn_sync.sql` — on disk, NOT in live schema_migrations). Live probe 2026-06-11 final-verify: **still 22 unscoped**. Apply both via the APPLY role + review gate.
5. **REVOKE candidates** via the new grant gate: `generate_rup_sales_records`, `execute_sql_readonly` (+22 more annotated in caller-graph.json). — **PARTIAL:** `generate_rup_sales_records` DONE 2026-06-11 (`20260611001248`, live-verified authenticated/anon EXECUTE = false, service_role = true). `execute_sql_readonly` still authenticated-EXECUTE = true (live-verified 2026-06-11) — open.
