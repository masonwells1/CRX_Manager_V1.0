...
- The exact-head reviewer found that browser tab duplication copies `sessionStorage`, allowing two live pages to share one claimant ID. Claim IDs now add a collision-resistant per-page UUID, resolved tombstones are never deleted by definitive-failure cleanup, and a copied-tab regression proves one peer's late refusal cannot erase the other's committed result.
- CodeRabbit's incremental review through `6d5c4158` found that unmounted page claimants could strand an otherwise definitively rejected retry, and that reconciled receiving paths still emitted a contradictory normal-success toast. Per-page liveness markers now retire on unmount/page exit and are pruned atomically during coordination, while live peer claims remain protected. Receiving Hub and PO Detail now emit the normal stock-received success only for a newly committed receipt. The same follow-up reports vendor-payment preparation failures through the shared Sentry boundary and makes the layout-effect source assertion structural instead of newline-sensitive.
...
- `npm run test` after the copied-tab claimant correction — 342 files passed; 4,830 tests passed; 123 skipped.
...
- Copied-tab collision proof — hook and Section 9 contracts pass 35/35; the full suite passes 342 files and 4,830 tests with 123 skipped; typecheck, targeted lint, and diff hygiene pass.
- Remount-liveness and truthful receiving-toast follow-up — hook, Section 9, and Quick Receive contracts pass 39/39; the full suite passes 342 files and 4,831 tests with 123 skipped; typecheck, full lint, production build, documentation drift, and diff hygiene pass. Exact-head proof is pending this evidence update.
...
- Sol-high review of exact head `50f22a09` then found one HIGH: duplicated tabs inherit the same session-stored claimant ID, so a definitive response could erase a peer's resolved marker. The per-page claim suffix, resolved-tombstone preservation, and collision test close that finding locally; fresh exact-head review is required after commit.
- CodeRabbit's actual review body for `6d5c4158` (not its green status context) then reported four applicable follow-ups: stale unmounted claimants could preserve a lock after definitive rejection, reconciled receipts could still show a normal success toast, payment-preparation capture had been dropped during the lint-only correction, and the layout-effect guard depended on an exact newline. The liveness registry, dynamic success handling, shared Sentry capture, and structural guard described above close them locally; current-head full, exact-SHA, CI, and CodeRabbit proof remain required.
