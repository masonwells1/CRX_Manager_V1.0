...
- Typecheck, lint, production build, documentation drift, the focused static guard, drift slice (234 passed / 78 skipped), and the full Vitest suite (4,269 passed / 123 skipped) passed before PR review.
...
- PR #320 review expanded the rollback smoke so both public report RPCs must propagate the shared unsupported-history error and added a latest-replacement regression guard for role, row-scope, and reconstruction boundaries. CodeRabbit's proposed removal of the `invoice_voided` audit predicate was not applied: fresh live definitions prove the generic void writer atomically writes that event, `unpost_invoice` is covered by the separate `invoice_unposted` guard, and the status-only credit-memo unapply deliberately remains reconstructable from its preserved face amount and application ledger.
