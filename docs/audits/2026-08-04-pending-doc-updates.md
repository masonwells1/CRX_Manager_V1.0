...
git checkout -b docs/apply-pending-entries origin/main
```
...
git push -u origin docs/apply-pending-entries
```

The pre-commit hooks will run and must pass — do not use `--no-verify`. Then open
a PR and merge it as usual.
