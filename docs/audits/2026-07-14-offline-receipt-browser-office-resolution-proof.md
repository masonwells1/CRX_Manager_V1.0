...
- Any durable action missing its original queue-time target snapshot is staged without inventing a post-reconnect baseline. The server retains it as permanent `LEGACY_OUTCOME_UNKNOWN` office review work, preventing edits made during the offline window from being silently treated as the original state.
...
The first exact-commit Opus pass after the final `main` rebase returned `SHIP-WITH-FOLLOWUPS` and elevated one rollout-compatibility issue: offline work saved by the pre-receipt production build had no trustworthy queue-time snapshot. The final conservative behavior preserves that action as permanent office review rather than synthesizing a baseline after reconnect. The same pass identified the missing old-clock lower bound, which is now closed by the 14-day review boundary.

The next exact-commit Opus pass confirmed the legacy guard, concurrency behavior, interruption recovery, old-clock boundary, and migration ordering, then found the same missing-snapshot risk on newly saved native field-job work when the applicator had not expanded the job card before losing connectivity. The final implementation sends every durable action missing that original value to office review and has focused regression coverage for both legacy and native field paths. Remaining follow-ups are automatic device discovery of office resolutions, office visibility for old `received` receipts, pagination/count polish, and stale-tab release-note wording. The redundant partial-index note is non-blocking. The push guard separately records the required fresh final exact-commit Claude review in generated session state.

## Required release order

1. Obtain the final Claude Opus re-review and green PR checks.
2. Separately obtain Mason's explicit approval to apply the four live migrations in timestamp order.
...
Steps 2 and 3 completed on 2026-07-14. Supabase applied the migrations as live stamps `20260714171331`, `20260714171800`, `20260714172135`, and `20260714203709`; structural/RLS/grant checks passed, the final target-lock SQL matched the reviewed file hash, the full delivery + job + office-resolution chain ended in `SMOKE_PASS_ROLLBACK`, zero smoke rows persisted, and all 15 database invariant sweeps returned zero unallowlisted violations. The database-before-frontend gate is therefore cleared. Step 1 must be repeated against the final rebased commit before step 4.
