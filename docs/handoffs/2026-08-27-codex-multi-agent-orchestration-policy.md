...
- Checkout: isolated clean worktree for the branch below
...
- The original shared checkout had 60 uncommitted files and was 16 commits behind after the first inspection, so it was not used for edits.
...
- Latest-commit review showed that tool-specific Codex worker and model-routing rules do not belong in the shared `AGENTS.md`. The policy now retains only tool-neutral coordination and isolation invariants there; Codex model routing remains in Codex's global guidance.

## WRITTEN, NOT PROVEN

- `AGENTS.md` now assigns decomposition, worker contracts, isolation, integration, and Mason-facing status to the coordinating agent without overriding Claude or Codex tool-specific routing.
- Codex's existing global guidance continues to preserve Sol High ownership of critical architecture and final review while permitting lower-cost workers for bounded implementation and scans.
- No project-level promise is made about Spark or any other named worker that the current session may not expose.
...
- Current blocker: the final shared-policy cleanup requires focused checks and a fresh separate exact-head review before delivery.
...
Run the focused documentation and agent-workflow checks on the shared-policy cleanup, then obtain a fresh exact-head review before pushing it.
