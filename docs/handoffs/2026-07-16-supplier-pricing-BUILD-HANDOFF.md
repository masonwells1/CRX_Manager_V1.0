# BUILD HANDOFF — Supplier Pricing, Price History & Product Worksheet

**Date:** 2026-07-16
**From:** Claude (planning session, with Codex gpt-5.6 advisory + adversarial review already applied)
**To:** Codex ("Sol 5.6") — the builder for this feature
**Owner approval:** Mason approved the rev-5 plan on 2026-07-16 and directed that Codex do all of the build work.
**Canonical plan:** embedded in full at the bottom of this file (also at `docs/plans/2026-07-16-supplier-pricing-and-variants-plan.md` on branch `claude/supplier-pricing-strategy-9c6129`).

---

## Mission

Build the supplier-pricing system described in the embedded plan: multi-supplier price tracking with append-only observations, per-supplier price history, the owner-controlled product & pricing worksheet (.xlsx round-trip), and retirement of the unsafe legacy bulk-import write path. **Manual ingestion only — Mason has settled (2026-07-16) that there is NO AI/OCR extraction of supplier PDFs. Do not build or propose it.**

## Build order — strict

1. **Phase 1a only, first** (safety foundation). Open a PR when done. Do not start 1b in the same PR.
2. **Phase 1b** (supplier evidence) as its own PR after 1a merges.
3. **Phases 2 and 3 are NOT authorized yet** — stop after 1b and report to Mason.

Work on a feature branch created FROM `claude/supplier-pricing-strategy-9c6129` (e.g. `feat/supplier-pricing-phase1a`) so this handoff and the plan travel with the code. Never work directly on `main`.

## Hard rules (non-negotiable — these mirror AGENTS.md and the repo guards)

- **No pushes to `main`** — branch → PR → checks → merge, per the active branch-protection policy. PR merges follow the repo's Codex-gate rules.
- **No live migration applies.** Write migration files following the repo's staging-migrations DRAFT protocol (`scripts/.staging-migrations/` until an APPLY session moves them). Live applies happen in a separate, gated session (migration-review + apply-guard proof + Mason). Codex's Supabase MCP access here is read-only by design.
- **No destructive data operations, no secrets, no deploys.**
- **Money:** new columns are integer cents (`_cents bigint`). The preview/apply RPC is the ONLY cents↔dollars conversion point (legacy `products` columns are numeric dollars).
- **RLS on every new table**, following repo standards (see `docs/workflows/SAFE_DEVELOPMENT_RULES.md`, rls-security patterns). The observation tables are append-only: insert-only policies + trigger-rejected UPDATE/DELETE + constrained supersession.
- **Frozen keys:** never rename `unit_conversions` rows. `rate_unit` edits must validate against the allowed list.
- **Regulatory fields** (EPA registration, signal word, REI/PHI, max label rate) are READ-ONLY everywhere in this feature.
- **Sell-price safety:** nothing in this feature may change `products.current_cost` or tier prices except the worksheet apply RPC after an explicit approved change-set. Remember the live trigger `trigger_calculate_prices_from_margin` — ANY write to cost/margins rewrites sell prices.
- **Docs/ledger:** every commit that touches agent surface or adds a migration must carry its ledger update (CHANGELOG / manual doc) — the pre-commit guard enforces this. Update `docs/reference/` and `docs/manual/` files affected by shipped behavior in the same change.
- **Definition of done = ran and proven.** For each deliverable, actually exercise it (open the page, run the flow, upload a real test .xlsx, SELECT the resulting rows) and record the evidence in the PR description. Tests passing alone is not done.
...
2. **Product & pricing worksheet — pricing columns** (§6b of the plan):
   ...
   - Server-side **apply RPC**: validates all expected versions, applies an approved change-set atomically, exactly once (no replay). Sole cents↔dollars boundary.
   - UI: export button on Products page, upload → preview screen → approve → result summary.
3. **Single history writer:** `AFTER UPDATE` trigger on `products` writes `cost_history` (extended with actor/source/reason context); the frontend `cost_history` inserts in `ProductDetail.tsx` and `BulkPricingImport.tsx` are REMOVED in the same release (no double-logging).

Proof required: a real round-trip on dev — export the sheet, edit 3+ products in Excel-compatible form covering both pricing modes and one deliberate conflict, upload, show the preview catches the conflict and previews trigger-produced prices exactly, apply, show `cost_history` rows with correct old→new values, show a replayed change-set is rejected. Show `BulkPricingImport` can no longer write prices.
...
6. Product price-history view, **filterable by supplier**, charting observations + cost basis + actual paid.
7. Worksheet market-evidence columns go live from observations.
8. PO backfill through `legacy_vendor_resolution` only; unresolved lines display "supplier unknown".

Proof required: enter two suppliers' quotes for the same product (different pack sizes), show normalized comparison + one "cannot compare" case, show the supplier-filtered history chart, show observations reject UPDATE/DELETE at the DB level, show backfilled PO facts labeled `actual_purchase` with provenance.
...
Park the work cleanly (commit on the feature branch, note state in the PR/draft), and report what's blocked and why in plain English. Never bypass a guard (`--no-verify` is forbidden) — guard failures are signals, not obstacles.

---

# THE APPROVED PLAN (rev 5, verbatim)
