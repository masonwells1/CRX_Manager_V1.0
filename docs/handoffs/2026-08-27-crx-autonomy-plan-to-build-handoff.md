...
> **SUPERSEDED — historical record.** This document belongs to PR #513, which was **closed
> unmerged**; its autopilot-hook changes never landed. `AGENTS.md` and
> `docs/manual/DECISION_LOG.md` are authoritative for current autonomy policy. Preserved because it
> records the reasoning behind an approach that was considered and dropped.
...
- Exact-head local review of `13bcaf06` found that a permissive top-level GitHub CLI family list allowed destructive subcommands such as repository deletion, visibility changes, token output, secret writes, ruleset deletion, and workflow disabling. The classifier is now a small explicit action allowlist: read-only inspection, routine PR collaboration, CI recovery, and separately guarded delivery remain automatic; sensitive administration and new unclassified actions deny in autopilot plus both production guards.
