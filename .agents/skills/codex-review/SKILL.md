...
- (2) Money — binary-float arithmetic/rounding, cents-vs-dollars mixups, new money storage that is
  not bigint cents, or a documented legacy PostgreSQL numeric-dollar column that loses exact
  `numeric` arithmetic or its finite whole-cent constraint.
