...
- NEVER request a CodeRabbit review yourself — not with the `ready-for-coderabbit` label, and not by posting `@coderabbitai review`. The hourly `crx-hourly-coderabbit-slot` job owns that request; see the note near the top of this file
