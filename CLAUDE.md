...
- **Drive to completion, but stop at the production gates.** Don't stall on trivial questions or pause to ask "should I keep going?" mid-task — keep momentum. BUT never push, deploy, apply a live migration, delete data, or commit unrelated files without Mason's explicit OK in the current chat. Task momentum never overrides these gates — CRX runs a live business. **These gates are now harness-enforced** (`.claude/settings.json` permissions): reversible work (npm / build / test / reads / `git add`+`commit` / preview / DB reads + `execute_sql`) is auto-allowed so you run without interrupting Mason; **`git push` and edge-function deploy `ask` first**; `--force` push / `reset --hard` / `rm -rf` / `.env` writes / `deploy_to_vercel` are **hard-denied**; live migrations stay gated by `migration-apply-guard`. Never route around these.
