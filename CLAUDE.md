...
- 66 pages, **95** tables (+2 views; incl. `rebate_claim_counters`), **218** RPCs, **370 migrations**, 7 Edge Functions (+ `_shared` lib dir)
- **2026-06-07 (doc-verification pass — branch `claude/claude-md-docs-PnZ4Z`):** Reconciled CLAUDE.md against the live repo + live DB. **All counts confirmed accurate, no DB/code changes.** Verified locally: 66 `lazy(` pages in `App.tsx`, 370 `supabase/migrations/*.sql`, 7 Edge Functions + `_shared`, 130 unit-test files, 94 E2E spec files. Verified against live Supabase (`rhyzpcqhnizqbxphqdkr`) via MCP: **95** base tables + **2** views; Edge Function versions unchanged since 2026-05-29 (`create-user` v20, `send-email` v13, `setup-blend-tickets-storage` v15, `process-blend-ticket` v19, `process-document` v13, `reset-user-password` v12, `seed-admin` v15). Ran the full unit suite: **1,924 passed / 70 skipped (1,994 total)** — fixed the stale "1,918" figure below to match. Branch was even with `origin/main` at start (no other work). (`262` total functions exist in the `public` schema, which includes trigger + helper functions on top of the **218** curated callable RPCs.)
...
| `docs/reference/migration-history.md` | 370 migrations |
