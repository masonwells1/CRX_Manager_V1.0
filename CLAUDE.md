...
**Live counts — verify with `node scripts/check-doc-drift.mjs`, don't trust them blind:** 82 pages · 113 tables (+2 views) · 277 callable RPCs (+56 trigger fns) · **562 migrations** on disk · 6 Edge Functions · ~2,222 unit tests + 115 skipped / 94 E2E specs. *(Parity is now merged to live `main`. The +3 pages / +2 tables / +7 RPCs / +6 migrations over the post-parity base come from the in-progress `feat/fieldapp-beyond-parity` §1 Label-Data Backfill + §2 Watchdog Flags + §3 Office Cockpit + §4 Auto-Invoice on job completion (auto-DRAFT only, OFF by default) + §5 Label-Rate Guardrails (warn default, never blocks), applied LOCALLY only — not yet in live `schema_migrations`.)*
...
| `docs/reference/migration-history.md` | 562 migrations |
