...
- 66 pages, **95** tables (+2 views; incl. `rebate_claim_counters`), **218** RPCs, **371 migrations**, 7 Edge Functions (+ `_shared` lib dir)
- **2026-06-08 (architecture-weakness fixes — branch `fix/architecture-weakness-2026-06-08`):** Remediating the 2026-06-08 audit findings one migration at a time through the review gate. **AW-1 APPLIED LIVE** (`20260608144210_save_blend_ticket_idempotency`): `save_blend_ticket` declared `p_idempotency_key` but ignored it (UI passed it via `useIdempotencyKey`, RPC dropped it → a duplicate `activity_feed` row on double-click); wrapped with canonical `check_idempotency`/`save_idempotency` placed AFTER the auth check. Body verbatim from live otherwise; both reviewers (rls-security + migration-drift) clean; applied via MCP (stamp `20260608144210`, disk renamed per B7); rolled-back smoke test on a temp ticket confirmed `activity_delta=1` (no dup row) + cached return, overload=1, `uses_idem=true`. **Pending on this branch:** AW-2 (`create_direct_order` missing `order_created` audit row) + AW-3 (drop dead deprecated `record_payment`).
...
| `docs/reference/migration-history.md` | 371 migrations |
