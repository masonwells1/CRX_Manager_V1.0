...
### BlendTicket (4 independent status axes — OCR intake entity)
A blend ticket carries **four separate status columns**, each driven by a different actor — they do NOT form one linear lifecycle:
- **`status`** `pending → processing → completed → failed → needs_review` — set by the `process-blend-ticket` Edge Function (Google Vision OCR). No RPC transitions this.
- **`review_status`** `unreviewed → approved → rejected` — set by `batch_approve_blend_tickets` / `batch_reject_blend_tickets`; `reverse_blend_ticket_approval` flips approved → unreviewed.
- **`order_link_status`** `unlinked → linked` — set by `link_blend_ticket_to_order` / `unlink_blend_ticket_from_order`.
- **`payment_status`** `unbilled → billed → prepaid → no_charge` — set by `update_blend_ticket_billing_status`; `create_invoice_from_blend_ticket` sets `billed`; the `trg_sync_blend_ticket_payment` trigger auto-resets `billed → unbilled` when the linked invoice is voided/cancelled.
...
| `docs/reference/migration-history.md` | 374 migrations |
