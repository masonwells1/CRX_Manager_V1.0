...
- 68 pages (Field Mode +2: `/my-route` list + per-stop runner — branch `claude/recursing-cerf-6ae05f`, NOT pushed; see `docs/roadmap/field-mode-build-plan.md`), **96** tables (+2 views; incl. `quote_product_draws`), **219** RPCs (+= `assign_job_applicator`), **443 migrations** (all live; `create_direct_order_customer_po_param` `20260614142939` APPLIED LIVE 2026-06-14 — +`p_customer_po_number` so the SECDEF RPC sets the PO (sales_rep can't via the admin-only `orders_update` RLS); the 3 hold-resync migrations APPLIED LIVE 2026-06-13 in order — planned-holds re-home `20260613191120` → cancel_order draw-cancel resync `20260613191323` → void_order draw-void resync `20260613191516`; disk files B7-renamed to these MCP stamps), 7 Edge Functions (+ `_shared` lib dir)
