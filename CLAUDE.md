...
- 66 pages, **95** tables (+2 views; incl. `rebate_claim_counters`), **218** RPCs, **380 migrations**, 7 Edge Functions (+ `_shared` lib dir)
...
- **Codex cross-review pending — 2026-06-08 merge conflict resolution:** mid-merge on `main` after `git pull` produced 7 conflicted files (10 regions). Resolution plan drafted (mostly "take origin" for the money-formatter refactor overlap, "take HEAD" for two BlendTickets typed-`assertRpcResult` calls, "combine" for PaymentHistory). Codex must validate before any conflict marker is removed. See `docs/audits/2026-06-08-codex-merge-conflict-resolution-prompt.md`. Working tree is currently mid-merge — do NOT start unrelated work until this is resolved or aborted.
